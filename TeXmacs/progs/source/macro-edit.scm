...
(texmacs-module (source macro-edit)
  (:use (utils library cursor) (generic document-part))
) ;texmacs-module
...
(tm-define (get-definition l)
  ...
  (or (get-definition* l (buffer-tree)) (tree 'assign l (get-init-tree l)))
) ;tm-define
...
(tm-define (ext-edit-macro a)
  ...
  (let* ((c (tree-children a))
    ...
    `(with ,"par-first"
       ...
       ,"par-par-sep"
       ,"0.5em"
       (document (concat (inline-tag ,name* ,@args*) ," " (math "<assign>"))
         ,body*))
  ) ;let*
) ;tm-define
...
(define (tree-load-style* s)
  (with f
    (if (string-ends? s ".ts") s (string-append s ".ts"))
...
(tm-define (built-in-style? s)
  (with d
    (url-complete (url-append "$TEXMACS_PATH/styles" (url-any)) "dr")
    (with name
      (string-append s ".ts")
      (with f
        (url-complete (url-append (url-expand d) name) "fr")
...
(tm-define (get-public-style-list)
  (with st
    ...
    (if (null? st)
      st
      (with l
        (list-filter (cdr st) (negate hidden-package?))
        (if (built-in-style? (car st)) l (cons (car st) l))
      ) ;with
    ) ;if
  ) ;with
) ;tm-define
...
(define (search-style-definition-in done l name)
  ...
  (and (string? name)
    (not (in? name done))
    (with t (tree-load-style* name) (search-style-definition (cons name done) l t))
  ) ;and
) ;define

(define (search-style-definition-in-list done l packs)
  (first-match (reverse packs) (cut search-style-definition-in '() l <>))
) ;define
...
(define (search-style-package l)
  (with packs (get-style-list) (search-style-definition-in-list '() l packs))
) ;define
    ...
    (map car
      (sort (filter (lambda (p) (> (cdr p) 2)) matches)
        (lambda (a b)
          (with eq
            (= (cdr a) (cdr b))
            (if eq (string<? (car a) (car b)) (> (cdr a) (cdr b)))
          ) ;with
        ) ;lambda
      ) ;sort
    ) ;map
...
(define (edit-assign-macro def)
  (and-with m
    (tree-ref def 1)
    (if (tree-is? m 'macro) (tree-go-to m :last :start) (tree-go-to m :start))
    #t
  ) ;and-with
) ;define
  ...
  (with b
    ...
    (and-with def
      ...
      (when (tree-is? (tree-ref b 0) 'hide-preamble)
        ...
        (with other
          `(ignore (document ,@(cdr (tree-children b))))
...
(define (edit-macro-in-style-file l)
  (and-with name
    ...
    (let* ((style-name (string-append name ".ts"))
      ...
      (delayed (:idle 1)
        (and-with def (get-definition** l (buffer-tree)) (edit-assign-macro def))
      ) ;delayed
    ) ;let*
  ) ;and-with
) ;define
...
(define common-style-packages
  '(("strong" . "std-markup") ("item" . "std-list") ("list" . "std-list"))
) ;define
...
(tm-define (edit-macro-source l)
  (if (symbol? l) (set! l (symbol->string l)))
  (or (edit-macro-in-preamble l) (edit-macro-in-style-file l))
) ;tm-define
