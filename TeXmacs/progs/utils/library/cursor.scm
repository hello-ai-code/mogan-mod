...
(tm-define (go-to-repeat fun)
  (with p (cursor-path) (fun) (if (!= (cursor-path) p) (go-to-repeat fun)))
) ;tm-define
...
(tm-define (go-to-remain-inside fun . l)
  (with p
    (cursor-path)
    (fun)
    (let* ((q (cursor-path)) (pp (innermost-pattern p l)) (qq (innermost-pattern q l)))
      (if (!= pp qq) (go-to p))
    ) ;let*
  ) ;with
) ;tm-define
...
(tm-define (notify-cursor-moved status) (noop))
...
(tm-define (tree-at-start? t)
  (with p
    (cursor-path)
    (or (== p (tree->path t 0 :start)) (== p (tree->path t :start)))
  ) ;with
) ;tm-define
...
(tm-define (tree-at-end? t)
  (with p
    (cursor-path)
    (or (== p (tree->path t 1 :end)) (== p (tree->path t :end)))
  ) ;with
) ;tm-define
...
(define (history-get)
  (ahash-ref* cursor-history (window-get-serial) '())
) ;define

(define (history-set l)
  (ahash-set! cursor-history (window-get-serial) l)
) ;define

(define (future-get)
  (ahash-ref* cursor-future (window-get-serial) '())
) ;define

(define (future-set l)
  (ahash-set! cursor-future (window-get-serial) l)
) ;define
...
(define (position-valid? pos)
  (and-with t (path->tree (cDr (position-get pos))) (not (tm-func? t 'uninit)))
) ;define
...
(define-public-macro (with-buffer name . body)
  (let* ((old (gensym)) (new (gensym)) (res (gensym)))
    `(if (== ,name (current-buffer))
       ...
       (let* ((,old (current-buffer)) (,new ,name))
         (and (or (url? ,new) (string? ,new))
           (buffer-exists? ,new)
           (buffer-focus ,new ,#f)
           (with ,res (begin ,@body) (buffer-focus ,old ,#f) ,res))))
  ) ;let*
) ;define-public-macro
...
(define-public-macro (with-window name . body)
  (with buf
    (gensym)
    `(with ,buf
       (window-to-buffer ,name)
       (and (not (url-none? ,buf)) (with-buffer ,buf ,@body)))
  ) ;with
) ;define-public-macro
...
(tm-define-macro (keep-table-selection . body)
  `(with (sr1 sr2 sc1 sc2)
     (table-which-cells)
     (with ktres
       (begin ,@body)
       (if (or (!= sr2 sr1) (!= sc2 sc1))
         ;; FIXME: find a robust way to keep the selection
         (delayed (:pause 10) (table-select-cells sr1 sr2 sc1 sc2)))
       ktres))
) ;tm-define-macro
