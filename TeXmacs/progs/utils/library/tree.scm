...
(tm-define-macro (tree-assign! ref t)
  `(begin (set! ,ref (tree-assign ,ref ,t)) ,ref)
) ;tm-define-macro
...
(tm-define-macro (tree-insert-node! ref pos t)
  `(begin (set! ,ref (tree-insert-node ,ref ,pos ,t)) ,ref)
) ;tm-define-macro

(tm-define-macro (tree-remove-node! ref pos)
  `(begin (set! ,ref (tree-remove-node ,ref ,pos)) ,ref)
) ;tm-define-macro
...
(define (tree-focus-index ref l)
  (cond ((null? l) #f)
        ...
        (else (with r (tree-focus-index ref (cdr l)) (if r (+ r 1) #f)))
  ) ;cond
) ;define
...
(tm-define (tree-set-diff ref t)
  ...
  (let* ((p (tree->path ref))
    ...
    (cond ((not p) (texmacs-error "tree-set-diff" "~S isn't part of a document" ref))
          ((tm-equal? ref t) ref)
          ((tree-inside? t ref)
           (with q
             (tree->path t)
          ...
          ((and (tm-atomic? ref)
             (tm-atomic? t)
             (= (+ l r) (tm-length ref))
             (< (tm-length ref) (tm-length t))
           ) ;and
           (tree-insert! ref l (substring (tm->string t) l (- (tm-length t) r)))
          ) ;
          ((and (tm-atomic? ref)
             (tm-atomic? t)
             (= (+ l r) (tm-length t))
...
(tm-define-macro (tree-set-diff! ref t)
  (:synopsis "Assign @ref with @t")
  `(begin (set! ,ref (tree-set-diff ,ref ,t)) ,ref)
) ;tm-define-macro
...
(tm-define (tree-ref t . l)
  ...
  (cond ((not (tree? t)) #f)
        ...
        ((== (car l) :first) (apply tree-ref (cons t (cons 0 (cdr l)))))
        ((== (car l) :last)
         (and (tree-compound? t)
        ...
        ((symbol? (car l))
         (and (tree-compound? t)
           (with i
             (list-find-index (tree-children t) (cut tree-is? <> (car l)))
...
(define (tree-set-sub t l u)
  (cond ((not (tree? t)) (texmacs-error "tree-set-sub" "~S is not a tree" t))
        ...
        ((null? l) (if (tree-active? t) (tree-set-diff t u) (tree-assign t u)))
        ((integer? (car l))
         (with i
           (car l)
        ...
        ((== (car l) :first) (tree-set-sub t (cons 0 (cdr l)) u))
        ((== (car l) :last)
         (if (tree-compound? t)
        ...
        ((symbol? (car l))
         (with i
           ...
           (if i (tree-set-sub t (cons i (cdr l)) u) (tree-set-sub-error t l))
         ) ;with
        ) ;
...
(tm-define (tree-set t . args)
  ...
  (with r (reverse args) (tree-set-sub t (reverse (cdr r)) (car r)))
) ;tm-define

(tm-define-macro (tree-set! t . l)
  (:synopsis "Set a subtree of @t to a new value according to @l")
  (if (list-1? l)
    `(if (tree-active? ,t) (tree-set-diff! ,t ,@l) (tree-assign! ,t ,@l))
    `(tree-set ,t ,@l)
  ) ;if
) ;tm-define-macro
...
(tm-define (tree-innermost x . opt-flag)
  (:type (-> symbol tree) (-> (list symbol) tree) (-> (-> bool) tree))
  (:synopsis "Search upwards from the cursor position")
  (with p
   ((if (null? opt-flag) cDDr cDr) (cursor-path))
...
(tm-define (inside-which l)
  ...
  (with t (tree-innermost l) (and t (tree-label t)))
) ;tm-define

(tm-define-macro (with-innermost t x . body)
  `(let ((,t (tree-innermost ,x))) (if ,t (begin ,@body)))
) ;tm-define-macro
...
(tm-define (tree-is? t . l)
  (let* ((st (apply tree-ref (cons t (cDr l)))) (lab (cAr l)))
    (and st (== (tree-label st) lab))
  ) ;let*
) ;tm-define

(tm-define (tree-in? t . l)
  (let* ((st (apply tree-ref (cons t (cDr l)))) (ls (cAr l)))
  ...
  (if (null? l)
    ...
    (with i
      ...
      (if (or (== i :start) (== i :end) (integer? i))
        (let* ((u (apply tree-ref (cons t (cDr l)))) (p (and u (tree->path u))))
          (cond ((not p) #f)
        ...
        (with u (apply tree-ref (cons t l)) (and u (tree->path u)))
      ) ;if
    ) ;with
  ) ;if
) ;tm-define
...
(tm-define (tree-cursor-path t . l)
  ...
  (let* ((p (apply tree->path (cons t l))) (c (cursor-path)))
...
(tm-define (tree-focus t . l)
  ...
  (and-with t
    (apply tree-ref (cons t l))
    (and-with p (tree->path t) (set-manual-focus-path p))
  ) ;and-with
) ;tm-define
...
(tm-define-macro (conserve-focus . body)
  `(with-focus-after (focus-tree) ,@body)
) ;tm-define-macro

(tm-define (tree-correct-old t . l)
  (:synopsis "Deprecated old tree correction routine")
  (with p (apply tree->path (cons t l)) (if p (path-correct-old p)))
) ;tm-define
...
(tm-define-macro (try-modification . body)
  `(with mark-nr
     ...
     (with mark-ok
       (begin ,@body)
       (if mark-ok (mark-end mark-nr) (mark-cancel mark-nr))
       mark-ok))
) ;tm-define-macro
