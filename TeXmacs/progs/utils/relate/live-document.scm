...
(tm-define (live-create lid doc . opt-state)
  (with initial
    (if (null? opt-state) (create-unique-id) (car opt-state))
...
(tm-define (live-current-document lid) (ahash-ref live-documents lid))

(tm-define (live-current-state lid)
  (and (pair? (ahash-ref live-states lid)) (car (ahash-ref live-states lid)))
) ;tm-define

(tm-define (live-get-history lid)
  (and (pair? (ahash-ref live-states lid)) (ahash-ref live-states lid))
) ;tm-define
...
(tm-define (live-get-document lid state)
  (let* ((doc (live-current-document lid)) (p (live-get-patch lid state)))
...
(tm-define (live-get-state-list lid state)
  (with states (ahash-ref live-states lid) (live-state-list state states))
) ;tm-define

(define (live-patch-list state states changes)
  (if (== state (car states))
    (list)
    (cons (car changes) (live-patch-list state (cdr states) (cdr changes)))
  ) ;if
) ;define

(tm-define (live-get-patch-list lid state)
  (let* ((states (ahash-ref live-states lid)) (changes (ahash-ref live-changes lid)))
    (and states changes (in? state states) (live-patch-list state states changes))
  ) ;let*
) ;tm-define

(tm-define (live-get-patch lid state)
  (and-with l (live-get-patch-list lid state) (patch-compound l))
) ;tm-define

(tm-define (live-get-inverse-patch lid state)
  (and-with doc
    (live-current-document lid)
    (and-with p (live-get-patch lid state) (patch-invert p doc))
  ) ;and-with
) ;tm-define
...
(tm-define (live-forget-obsolete lid)
  (with oldest (live-oldest-state lid) (live-forget-prior lid oldest))
) ;tm-define
      ...
      (with (best p*)
        ...
        (with ok?
          ...
          (when (not ok?)
            ...
            (with iq
              (patch-invert (car changes) t)
              (display* "-- Patch 2: " (patch->scheme iq) "\n")
            ) ;with
            (display* "-- best  = " best "\n")
            (display* "-- states= " states "\n")
            (display* "-- can-pull? " (patch-can-pull? p* (car changes)) "\n")
          ) ;when
...
(tm-define (live-rewrite-history lid until new-states new-changes)
  ...
  (let* ((old-states (live-get-state-list lid until))
    ...
    (when (or (!= new-states old-states)
            (!= (map patch->scheme new-changes) (map patch->scheme old-changes))
          ) ;or
      (display* ">> until= " until "\n")
