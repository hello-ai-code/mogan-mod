...
(define (load-glyphs)
  (when (not glyphs-loaded?)
    (set! glyphs-loaded? #t)
    (with l
      (if (url-exists? "~/.TeXmacs/system/glyphs.scm")
        (load-object "~/.TeXmacs/system/glyphs.scm")
        '()
      ) ;if
      (set! glyph-table (list->ahash-table l))
      (for (x l)
        (let* ((key (car x)) (im (cdr x)))
          (for (y im) (glyph-register key y))
        ) ;let*
      ) ;for
    ) ;with
  ) ;when
...
(define (save-glyphs)
  (save-object "~/.TeXmacs/system/glyphs.scm" (ahash-table->list glyph-table))
) ;define
...
(tm-widget (glyph-widget)
  (padded (ink (set! last-glyph answer)))
  ---
  (hlist >>>
    (=> "Learn"
      (tile 8
...
(tm-define (learn-glyphs) (show glyph-widget))
