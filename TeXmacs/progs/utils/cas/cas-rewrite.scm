...
(define (cas-opposite x)
  (cond ((== x 0) 0)
        ...
        ((func? x '-)
         (cons* '+ (cas-opposite (cadr x)) (map cas-normal-opposites (cddr x)))
        ) ;
        (else (with y (cas-normal-opposites x) (if (func? y '- 1) (cadr y) (list '- y)))
        ) ;else
  ) ;cond
) ;define
...
(define (cas-inverse-pow x flag?)
  (let ((what (cas-normal-inverses (cadr x))) (pow (cas-normal-inverses (caddr x))))
    (when (func? what '/ 1)
      (set! what (cadr what))
      (set! flag? (not flag?))
    ) ;when
    ...
    (with y (if (== pow 1) what (list '^ what pow)) (if flag? (list '/ y) y))
  ) ;let
) ;define
...
(define (cas-inverse x)
  (cond ((== x 1) 1)
        ...
        ((func? x '/)
         (cons* '* (cas-inverse (cadr x)) (map cas-normal-inverses (cddr x)))
        ) ;
        ((func? x '^ 2) (cas-inverse-pow x #t))
        (else (with y (cas-normal-inverses x) (if (func? y '/ 1) (cadr y) (list '/ y))))
  ) ;cond
) ;define
...
(define (cas-degree-max l)
  (if (null? l)
    -1
    (let ((d1 (car l)) (d2 (cas-degree-max (cdr l))))
...
(tm-define (cas-term<=? x y vars)
  ...
  (if (null? vars)
    ...
    (let* ((dx (cas-degree x (car vars))) (dy (cas-degree y (car vars))))
      (if (== dx dy) (cas-term<=? x y (cdr vars)) (cas-default<=? dy dx))
    ) ;let*
  ) ;if
) ;tm-define
