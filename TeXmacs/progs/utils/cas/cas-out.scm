...
(define (cas-collect-members . grs)
  (append-map (lambda (x) (append-map tmsymbol->cas (math-group-members x))) grs)
) ;define

(define-macro (cas-define-test name . grs)
  (let* ((t (symbol-append name '-table)) (p (symbol-append name '?)))
    `(begin
...
(define cas-special-op-table
  (ahash-table-append cas-assign-op-table
    cas-flux-op-table
    cas-meta-op-table
...
(define (cas-out-infix x op? first second)
  (and (list-3? x)
    (op? (car x))
    `(concat ,(first (cadr x)) ,(cas->tmsymbol (car x)) ,(second (caddr x)))
  ) ;and
) ;define
...
(define (cas-out-assign x)
  (or (cas-out-infix x cas-assign-op? cas-out-flux cas-out-flux) (cas-out-flux x))
) ;define
...
(define (cas-out-implies x)
  (or (cas-out-infix x cas-implies-op? cas-out-or cas-out-or) (cas-out-or x))
) ;define

(define (cas-out-or x)
  (or (cas-out-associative x cas-or-op? cas-out-or cas-out-and) (cas-out-and x))
) ;define
...
(define (cas-out-arrow x)
  (or (cas-out-infix x cas-arrow-op? cas-out-plus cas-out-plus) (cas-out-plus x))
) ;define
...
(define (cas-out-times x)
  (or (and (func? x '/ 1) `(frac ,"1" ,(cas-out (cadr x))))
    ...
    (and (func? x 'mod 2)
      `(concat ,(cas-out-times (cadr x)) ," mod " ,(cas-out-prefix (caddr x)))
    ) ;and
...
(define (cas-is-root? x)
  (and (func? x '^ 2)
    (or (func? (caddr x) '/ 1) (and (func? (caddr x) '/ 2) (== (cadr (caddr x)) 1)))
  ) ;and
) ;define
...
(define (cas-out-decompose-prime x)
  (if (func? x '%prime 1)
    (receive (radical prime)
      (cas-out-decompose-prime (cadr x))
...
(define (cas-out-atom x)
  (cond ((null? x) "null")
        ...
        ((func? x 'eval 2) (plugin-eval (cadr x) "default" (tm->tree (caddr x))))
        ...
        ((== x '(tuple)) "()")
        ((func? x 'tuple)
         (with args (cas-out-several (cdr x)) `(concat (left "(")
                                                 ,@args
                                                 (right ")")))
        ) ;
        ((== x '(list)) "[]")
        ...
        ((== x '(comma)) "null")
        ((func? x 'comma) (with args (cas-out-several (cdr x)) `(concat ,@args)))
        ((cas-is-root? x)
         (with y
           (cAr (caddr x))
           (if (== y 2)
        ...
        ((func? x 'sqrt 1) `(sqrt ,(cas-out (cadr x))))
...
(define (cas-build-concat x)
  (cond ((func? x 'concat) (for-each cas-build-concat (cdr x)))
        (else (set! cas-concat-buffer (cons (cas-concat-simplify x) cas-concat-buffer)))
  ) ;cond
) ;define
