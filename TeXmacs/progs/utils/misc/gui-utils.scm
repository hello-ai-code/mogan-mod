...
(texmacs-module (utils misc gui-utils)
  (:use (utils misc tooltip) (utils library cursor))
) ;texmacs-module
...
(tm-define (gui-on-toggle type x y cmd)
  ...
  (let* ((val (tm-ref cmd :up 0))
    ...
    (and-let* ((cmd* (tm->stree cmd)) (ncmd (string-append "(with answer " new " " cmd* ")")))
...
(tm-define (emu-toggle-modifier t)
  (:secure #t)
  (ahash-set! emu-modifier-table t (not (ahash-ref emu-modifier-table t)))
  (refresh-now "custom-keyboard")
) ;tm-define

(tm-define (emu-active-modifier? t) (ahash-ref emu-modifier-table t))
...
(tm-define (emu-key t)
  ...
  (with s
    (tm->stree t)
    (when (string? s)
      (if (emu-active-modifier? "Control") (set! s (string-append "C-" s)))
      (if (emu-active-modifier? "Alt") (set! s (string-append "A-" s)))
      (if (emu-active-modifier? "Meta") (set! s (string-append "M-" s)))
      (key-press s)
...
(define (gui-hlist-pairs l pos)
  (if (null? l)
    l
    (let* ((head (gui-hlist-pair (car l) pos)) (tail (gui-hlist-pairs (cdr l) (+ pos 1))))
      (cons head tail)
    ) ;let*
  ) ;if
) ;define
...
(tm-define (gui-hlist-table* tag* t)
  (let* ((tag (as-symbol tag* 'stack)) (pairs (gui-hlist-pairs (tree-children t) 0)))
...
(define (gui-vlist-pairs l pos)
  (if (null? l)
    l
    (let* ((head (gui-vlist-pair (car l) pos)) (tail (gui-vlist-pairs (cdr l) (+ pos 1))))
      (cons head tail)
    ) ;let*
  ) ;if
) ;define
...
(tm-define (gui-vlist-table* tag* t)
  (:secure #t)
  (let* ((tag (as-symbol tag* 'stack)) (pairs (gui-vlist-pairs (tree-children t) 0)))
...
(define (gui-tiled-row l cols)
  (let* ((r (map (lambda (x) "") (.. (length l) cols))) (a (append l r)))
    ...
    (and-with u
      ...
      (when (tree-is? u tag)
        (let* ((val (tree->stree t))
               ...
               (new (if (== tag 'choice-list) val (check-toggle old val all)))
               (sel (if (func? new 'tuple) (cdr new) new))
               (c (as-string (tree-ref u 0) "(noop)"))
               (cmd (string-append "(with answer '" (object->string sel) " " c ")"))
              ) ;
          (focus-on-canvas)
          (close-tooltip)
...
(tm-define (gui-input-context? t) (tree-in? t '(input-field input-popup)))
...
(tm-define (keyboard-press key time)
  ...
  (with t
    (tree-innermost gui-input-context?)
    (when (!= key "return")
      (former key time)
    ) ;when
    (and-with t*
      ...
      (and-with nr
        ...
        (and-with names
          ...
          (and-with bodies
            ...
            (and-with activate
              (tm-ref bodies nr)
              (when (and (tm-is? passive 'passive-tab) (tm-in? bodies '(tabs-body switch)))
                (for (t (tm-children names)) (tree-assign-node t 'passive-tab))
                (tree-assign-node (tm-ref names nr) 'active-tab)
                (for (t (tm-children bodies)) (tree-assign-node t 'hidden))
