...
(tm-define (modify-comment-tooltip)
  (when (and (defined? 'update-comment-tooltip) (procedure? update-comment-tooltip))
    (update-comment-tooltip)
  ) ;when
) ;tm-define

(tm-define (modify-ref-tooltip)
  (when (and (defined? 'update-preview-tooltip) (procedure? update-preview-tooltip))
...
(tm-define (close-tooltip) (when tooltip-win (tooltip-unmap)))
