...
(tm-define (versioned? url)
  (or (nnot (version-tool url)) (and-with base (url-wrap url) (versioned? base)))
) ;tm-define
...
(tm-define (version-status name)
  ...
  ;; + modified
  ;; + unmodified
  (if (version-tool name) (version-status name) "unknown")
) ;tm-define
...
(tm-define (version-show-history url)
  ...
  (with s
    (url->tmfs-string url)
    (revert-buffer-revert (string->url (string-append "tmfs://history/" s)))
...
(define (string-repeat str n)
  (do ((i 1 (1+ i)) (ret "" (string-append ret str)))
    ((> i n) ret)
  ) ;do
) ;define
...
(define (get-row-from-x x maxs maxv)
  (define (get-length nr)
    (let* ((ret (if (== maxv 0) 0 (/ (* nr (min maxs maxv)) maxv))))
      (if (and (> ret 0) (< ret 1)) 1 ret)
    ) ;let*
  ) ;define
  `(row (cell ,(third x))
     (cell ,(number->string (+ (first x) (second x))))
     (cell (concat (with color
                     green
                     ,(string-repeat "+" (get-length (first x))))
             (with color red ,(string-repeat "-" (get-length (second x)))))))
) ;define
...
(tm-define (git-show-normal root rev)
  ...
  (define (length-of-2col x)
    (+ (string-length (number->string (sum2 x))) (fourth x))
  ) ;define
  ...
  (let* ((m (git-commit-message root rev))
    ...
    ($generic ($tmfs-title "Commit Message of " (version-beautify-revision root rev))
      (if (== rev p)
        "parent 0"
        `(concat ,"parent " ,($link (tmfs-url-commit root p) p))
      ) ;if
      (list 'new-line)
      ($for (x m) `(concat ,(utf8->cork x) ,(list 'new-line)))
      "-----"
      (list 'new-line)
      `(verbatim (tabular (tformat (cwith "1" "-1" "1" "-1" cell-lsep "0pt")
                            ,(cons 'table
...
(tmfs-load-handler (commit name)
  (let* ((root (tmfs-string->url (tmfs-cdr name)))
         (tool (version-tool root))
         ;; NOTE: forces lazy loading
         (rev (tmfs-car name))
        ) ;
...
(tm-define (version-revision? u)
  (and (url-rooted-tmfs? u)
    (with (class name) (tmfs-decompose-name u) (== class "revision"))
  ) ;and
) ;tm-define

(tm-define (version-get-revision u)
  (and (url-rooted-tmfs? u)
    (with (class name) (tmfs-decompose-name u) (tmfs-car name))
  ) ;and
) ;tm-define

(tm-define (version-get-revision* u)
  (if (version-revision? u) (version-get-revision u) "")
) ;tm-define

(tm-define (version-head u)
  (and (url-rooted-tmfs? u)
    (with (class name) (tmfs-decompose-name u) (tmfs-string->url (tmfs-cdr name)))
  ) ;and
) ;tm-define

(tmfs-format-handler (revision name)
  (with u (tmfs-string->url (tmfs-cdr name)) (url-format u))
) ;tmfs-format-handler

(tmfs-title-handler (revision name doc)
  (let* ((rev (tmfs-car name)) (u (tmfs-string->url (tmfs-cdr name))))
...
(tm-define (register-buffer name)
  (let* ((ret1 (version-register name)) (ret2 (string-replace ret1 "\n" "; ")))
...
(tm-define (commit-buffer-message name message)
  (let* ((ret1 (version-commit name message))
         (ret2 (string-replace ret1 "\n" "; "))
         (ret3 (if (!= ret2 "") ret2 "The current version has already been committed"))
        ) ;
...
(tm-define (version-head u) (:require (url-wrap u)) (version-head (url-wrap u)))
...
(tm-define (version-revision-url u rev)
  ...
  (let* ((name (part-open-name u)) (m (part-master name)) (f (part-file name)))
