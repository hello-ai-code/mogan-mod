...
(texmacs-module (version version-compare) (:use (version version-edit)))
...
(tm-define (version-test-grain? w) (== version-grain w))
...
(define-preferences ("versioning grain" "detailed" (lambda (var val) (set! version-grain val)))
) ;define-preferences
...
(define (denormalize-concat l)
  (cond ((null? l) l)
        ((string? (car l))
         (append (denormalize-string (car l)) (denormalize-concat (cdr l)))
        ) ;
        ((tm-is? (car l) 'concat) (denormalize-concat (append (cdar l) (cdr l))))
...
(define (normalize-list tag l)
  (cond ((null? l) l)
        ((tm-is? (car l) tag) (normalize-list tag (append (cdar l) (cdr l))))
...
(define (normalize-strings l)
  (with i
    (list-find-index l nstring?)
    (if (not i) (set! i (length l)))
    (if (== i 0)
      (if (null? l) l (cons (car l) (normalize-strings (cdr l))))
      (cons (apply string-append (sublist l 0 i)) (normalize-strings (list-tail l i)))
    ) ;if
  ) ;with
) ;define
...
(define (normalize t)
  (cond ((string? t) t)
        ((tm-is? t 'concat)
         (with l
           (normalize-strings (normalize-list 'concat (cdr t)))
           (cond ((null? l) "")
                 ((null? (cdr l)) (car l))
                 (else `(concat ,@l))
           ) ;cond
...
(define (common l1 l2)
  (if (or (null? l1) (null? l2) (!= (car l1) (car l2)))
    0
...
(define (longest-common-bis l1 l2)
  (let* ((i (list-find-index l2 (lambda (x) (== x (car l1)))))
    ...
    (if (or (>= n 25) (not (in? (car l1) (cdr r2))))
      ...
      (receive (i1 i2 nn)
        (longest-common-bis l1 (cdr r2))
        (if (>= n nn) (values 0 i n) (values i1 (+ i2 i 1) nn))
      ) ;receive
    ) ;if
  ) ;let*
) ;define
...
(define (longest-common l1 l2)
  ...
  (cond ((or (null? l1) (null? l2)) (values 0 0 0))
        ((and (in? (car l1) l2) (nin? (car l1) prohibited-breaks))
         ;; NOTE: don't consider breaks at spaces and empty lines
         (receive (i1 i2 n)
           (longest-common-bis l1 l2)
           (if (>= n 25)
             (values i1 i2 n)
             ;; NOTE: truncate for efficiency reasons
             (receive (j1 j2 m)
               (longest-common (cdr l1) l2)
               (if (>= n m) (values i1 i2 n) (values (+ j1 1) j2 m))
             ) ;receive
           ) ;if
         ) ;receive
        ) ;
        (else (receive (i1 i2 n) (longest-common (cdr l1) l2) (values (+ i1 1) i2 n)))
  ) ;cond
) ;define
...
(define (skeleton t)
  (cond ((string? t) t)
        ((tm-is? t 'concat)
         (with f (list-find (cdr t) (lambda (x) (!= x " "))) (if f (skeleton f) ""))
        ) ;
...
(tm-define (compare-versions t1 t2)
  ...
  (cond ((== t1 t2) t1)
        ...
        ((and (string? t1) (string? t2)) (compare-versions `(concat ,t1) `(concat ,t2)))
