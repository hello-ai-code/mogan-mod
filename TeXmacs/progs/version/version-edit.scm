...
(texmacs-module (version version-edit) (:use (version version-drd)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Version context
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(tm-define (version-context? t) (version-tag? (tree-label t)))

(tm-define (inside-version?) (not (not (tree-innermost version-context?))))

(texmacs-modes (in-versioning% (inside-version?) with-versioning-tool%))
...
(tm-define (standard-parameters l)
  ...
  (append (search-parameters "version-old") (search-parameters "version-new"))
) ;tm-define
...
(define (selection-subtrees t p1 p2)
  (cond ((or (null? p1) (null? p2)) '())
        ...
        ((== p2 (list 0)) '())
        ((== p1 (list 0)) (selection-subtrees t (list 0 0) p2))
        ((== p2 (list (tree-right-index t)))
         (with n
           (- (tree-arity t) 1)
           (with c (tree-ref t n) (selection-subtrees t p1 (list n (tree-right-index c))))
         ) ;with
        ) ;
        ((== (car p1) (car p2))
...
(tm-define (version-first-difference) (go-start) (version-next-difference))
...
(tm-define (version-last-difference) (go-end) (version-previous-difference))
...
(define (version-retain-version where which)
  (with p
    (if (version-context? where) (tree-up where) where)
    (tree-replace where
      version-context?
      (lambda (t)
        (cond ((number? which) (retain-version t (tree-ref t which)))
              ((tree-is? t 'version-old) (retain-version t (tree-ref t 0)))
              (else (retain-version t (tree-ref t 1)))
        ) ;cond
      ) ;lambda
    ) ;tree-replace
    (tree-normalize p)
