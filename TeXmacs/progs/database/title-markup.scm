...
(texmacs-module (database title-markup) (:use (database title-transform)))
...
(define (retrieve-note note h)
  (with n
    (tm->stree note)
    (with val (ahash-ref h n) (if (eq? (car val) note) (list (cons n val)) (list)))
  ) ;with
) ;define
...
(tm-define (add-annotations c notes)
  (if (null? notes)
    ...
    (with (n note sym id)
      (cAr notes)
      (if (eqv? (car sym) 'phantom)
        c
        (with c2
          (add-annotations c (cDr notes))
...
(tm-define (make-note l)
  (with (n note sym id) l `(doc-footnote-text ,sym ,id ,(tm-ref note 0)))
) ;tm-define
...
(tm-define (remove-annotations t)
  (cond ((tm-func? t 'doc-note-ref 4) (remove-annotations (tm-ref t 3)))
        ((tm-func? t 'new-line) " ")
        ((tm-func? t 'next-line) " ")
        ((tm-func? t 'concat) `(concat ,@(map remove-annotations
                                           (tm-children t))))
        (else t)
  ) ;cond
) ;tm-define
...
(tm-define (xdoc-data-sub t)
  `(surround ,(doc-data-hidden t)
     (concat)
     (document (doc-make-title ,(doc-data-main t))))
) ;tm-define
...
(tm-define (doc-data-sub t)
  ...
  `(doc-make-rich-title ,(doc-data-hidden t) (document ,(doc-data-main t)))
) ;tm-define

(tm-define (doc-data-impl t opts)
  ;; (display* "t1= " t "\n")
  (cond ((in? "abbreviate-authors" opts) (set! t (abbreviate-authors t)))
  ) ;cond
  (cond ((in? "cluster-all" opts) (set! t (single-author-list t)))
        ((in? "cluster-by-affiliation" opts) (set! t (factor-affiliation t)))
  ) ;cond
  ;; (display* "t2= " t "\n")
  (set! t (add-notes t))
  ;; (display* "t3= " t "\n")
  (cond ((in? "abbreviate-authors" opts) (set! t (abbreviate-authors-bis t)))
  ) ;cond
  ;; (display* "t4= " t "\n")
...
(define (plain-footnote t)
  `(render-plain-footnote ,t)
) ;define
  ...
  `(document ,@(select t '(doc-title))
     ...
     ,@(with authors
         ...
         (cond ((null? authors) ())
               ...
               ((null? (cddr authors))
                `((author-name (concat ,(car authors)
                                 ," "
                                 (localize "and")
                                 ," "
                                 ,(cadr authors)))))
               (else (with l
                       (list-intersperse authors ", ")
                       `((author-name (concat ,@(cDr l)
                                        (localize "and")
                                        ," "
                                        ,(cAr l))))))))
     ,@(select t '(doc-misc))
...
(tm-define (doc-data-impl t opts)
  ...
  `(doc-make-rich-title ,(ams-doc-data-hidden t) ,(ams-doc-data-main t))
) ;tm-define
