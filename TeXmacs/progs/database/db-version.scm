...
(texmacs-module (database db-version) (:use (database db-users)))
...
(tm-define (db-update-entry id new-l . opt-new-id)
  (with old-l
    (db-get-entry id)
    (if (db-same-entries? new-l old-l)
      id
      (let* ((new-h (cons id (or (assoc-ref new-l "newer") (list))))
        ...
        (with new-id
          (if (and (nnull? opt-new-id) (not (db-entry-exists? (car opt-new-id))))
            (begin
...
(define (assoc-ref-first l x)
  (with r (assoc-ref l x) (and (pair? r) (car r)))
) ;define
...
(tm-define (db-import-entry id l)
  (let* ((name (assoc-ref-first l "name"))
         ...
         (sim (if (and name contributor)
                (db-search (list (list "name" name) (list "contributor" contributor)))
                (list)
              ) ;if
         ) ;sim
    ...
    (cond ((nnull? (db-get-entry id))
           ;; new entries must have new identifiers
           (when (and db-duplicate-warning? (not (db-same-entries? l (db-get-entry id))))
             (db-warning "Ignored entry " name " with existing identifier")
           ) ;when
          ) ;
          ((nnull? (db-find-entry l))
           ;; find exact matches
           ;; (when (and db-duplicate-warning? (not exact?))
           ;; (db-warning "Existing entry " name))
           (for (xid (db-find-entry l)) (db-declare-supersedes xid (list id)))
          ) ;
          ...
          ((begin (for (oid his) (db-declare-superseded oid)) #f) (noop))
          ((nnull? sim)
           (let* ((xid (car sim))
                  (xl (db-get-entry xid))
                  (date (assoc-ref-first l "date"))
                  (xdate (assoc-ref-first xl "date"))
                  (modus (or (assoc-ref-first l "modus") (assoc-ref-first db-extra-fields "modus"))
                  ) ;modus
             ...
             (cond ((and (== xmodus "manual") (!= modus "manual"))
                   ...
                   ((and (== modus "manual") (!= xmodus "manual"))
                    (with newer
                      (list-remove-duplicates (cons xid his))
                   ...
                   ((and (string? xdate)
                      ...
                      (or (not (string? date))
                        ...
                        (>= (string->number xdate) (string->number date))
                      ) ;or
                    ) ;and
                    (db-declare-supersedes xid (cons id his))
                    (when db-duplicate-warning?
