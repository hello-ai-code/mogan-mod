...
(tm-define (db-entry? t)
  (and (tm-func? t 'db-entry 5) (tm-func? (tm-ref t :last) 'document))
) ;tm-define
...
(tm-define (inside-db-field? which)
  (and-with t
    (tree-search-upwards (cursor-tree) db-field-any?)
    (and (tm-equal? (tm-ref t 0) which) (cursor-inside? (tm-ref t 1)))
  ) ;and-with
) ;tm-define
...
(tm-define (db-field-attr f)
  (and (db-field-any? f) (tm-atomic? (tm-ref f 0)) (tm->string (tm-ref f 0)))
) ;tm-define
...
(tm-define (db-entry-remove t attr)
  (and (db-entry-any? t)
    (if (in? attr (list "id" "type" "name"))
      t
      (let* ((lab (tm-label t)) (r (db-field-remove (tm-children (tm-ref t :last)) attr)))
        `(,lab
          ,(tm-ref t 0)
          ,(tm-ref t 1)
          ,(tm-ref t 2)
  ...
  (and (db-field-any? f)
    (or (null? l)
      (and (field-not-among? f (cdr l))
        (or (not (db-field-any? (car l))) (!= (tm-ref f 0) (tm-ref (car l) 0)))
      ) ;and
    ) ;or
  ) ;and
) ;define
...
(define (db-complete-field? t)
  (and (db-field-any? t)
    (or (db-field-optional? t) (not (tree-empty? (tm->tree (tm-ref t 1)))))
  ) ;and
) ;define
...
(define (db-next-field t)
  (with u
    (tm-ref t :next)
    (and u (or (and (db-field-any? u) u) (db-next-field u)))
  ) ;with
) ;define
...
(define (db-next-empty-field t)
  (with u
    (tm-ref t :next)
    (and u
      (or (and (db-field-any? u) (tree-empty? (tree-ref u 1)) u)
...
(define (db-previous-field t)
  (with u
    (tm-ref t :previous)
    (and u (or (and (db-field-any? u) u) (db-previous-field u)))
  ) ;with
) ;define
  ...
  (and-with res
    ...
    (if all?
      ...
      (and (> (tree-arity (tree-ref res :last)) 0)
        (with e
          (tree-ref res :last 0)
          (with f (if (db-field-any? e) e (db-next-field e)) (and f (tree-ref f 1)))
        ) ;with
      ) ;and
    ) ;if
  ) ;and-with
...
(tm-define (db-format-alternatives type attr)
  ...
  (and-with l (smart-ref db-format-table type) (get-alternatives l attr))
) ;tm-define
...
(tm-define (db-alternative-fields t)
  (and (db-field-alternative? t)
    (and-with res
      (tree-search-upwards t db-entry-any?)
      (and-with l
        (db-format-alternatives (tree-ref res 1) (tree-ref t 0))
        (with r (map (cut db-entry-ref res <>) l) (and (list-and r) (map tree-up r)))
      ) ;and-with
    ) ;and-with
  ) ;and
) ;tm-define
...
(define (selected-entries)
  (and (selection-active-any?) (append-map find-db-entries (selection-trees)))
) ;define
...
(tm-define (make-db-field attr plus)
  (with-innermost t
    db-entry?
    (let* ((ins `(db-field ,(locase-all attr) ,"")) (pos 0))
...
(tm-define (db-field-possible-attributes u)
  (with t
    (tree-search-upwards u db-entry?)
    (if (not t)
      (list)
...
(define (db-this-field-possible-attributes)
  (with-innermost t db-entry? (cons "" (db-field-possible-attributes t)))
) ;define
  ...
  (with-innermost t
    ...
    (when (and t
      ...
      (let* ((doc (tree-ref t :up))
        ...
        (cond ((and (not forwards?) (> i 0)) (tree-remove doc (- i 1) 1))
              ((not forwards?) (tree-go-to doc :up 2 :end))
              ((and (>= j 0) (db-field? (tree-ref doc j)))
               (tree-go-to doc j 1 :start)
               (tree-remove doc i 1)
              ) ;
              ((>= j 0) (tree-go-to doc j :start) (tree-remove doc i 1))
              (else (tree-go-to doc :up 2 :end) (tree-remove doc i 1))
        ) ;cond
      ) ;let*
    ) ;when
  ) ;with-innermost
      ...
      (with l
        ...
        (when (> (length l) 1)
          (with-database (user-database)
            (let* ((old (db-get-entry id)) (pred? (cut db-matches-entry? <> old)))
              (receive (l1 l2)
                (list-partition l pred?)
                (with r
                  (cdr (append l1 l2))
                  (for (t r) (with id (db-create-id) (tree-set (tree-ref t 0) id)))
                ) ;with
              ) ;receive
            ) ;let*
          ) ;with-database
...
(tm-define (detach-buffer-entries)
  (with ts (find-db-entries (buffer-tree)) (detach-entries ts))
) ;tm-define
    ...
    (with-database (user-database)
      (let* ((old-id (tm->string (db-entry-ref t "id")))
        ...
        (with-extra-fields (list (list "contributor" (get-default-user))
          ...
          (with-time-stamp #t
            (with new-id
              (db-update-entry old-id new)
              (if (== new-id old-id)
                (begin
                  (set-message "Entry up to date in database" "save entry")
                  :up-to-date
                ) ;begin
                (with new-t
                  (db-load-entry new-id)
                  (tree-set (tree-ref t 0) new-id)
                  (tree-set (tree-ref t 3) (tm-ref new-t 3))
                  (set-message "Saved modifications in database" "save entry")
                  :saved
                ) ;with
              ) ;if
            ) ;with
...
(define (keep-completing t opt?)
  ...
  (cond ((db-first-empty-field t #t) (tree-go-to (db-first-empty-field t #t) :end))
        ((not (db-complete-fields? t))
         (and-with res
           (tree-search-upwards t db-entry-any?)
           (tree-assign-node! res 'db-entry)
           (db-complete-fields res opt?)
           (and-with f (db-first-empty-field res #t) (tree-go-to f :end))
         ) ;and-with
        ) ;
...
(define (confirm-entries l)
  (detach-buffer-entries)
  (receive (l1 l2)
    (list-partition l db-complete-fields?)
    (with r
      (map-in-order confirm-entry l1)
    ...
    (when (nnull? l2)
      ...
      (set-message "Incomplete entries found to be filled out" "save entries")
    ) ;when
    (null? l2)
  ) ;receive
) ;define
...
(tm-define (db-confirm-entries-in t)
  (and-with l (find-db-entries t) (confirm-entries l))
) ;tm-define
...
(tm-define (kbd-enter t shift?)
  ...
  (with u
    (tree-ref t :down)
    (cond ((tree-search-upwards t 'inactive) (former t shift?))
          ((and u (= (tree-index u) 2))
           (if (tree-empty? u)
             (set-message "Error: should fill out name for referencing" "db-entry")
             (with d
               (tree-ref u :up :last)
...
(tm-define (kbd-enter t shift?)
  ...
  (cond ((tree-empty? (tree-ref t 1))
        ...
        ((db-next-field t) (tree-go-to (db-next-field t) 1 :end))
        (else (keep-completing t #t))
  ) ;cond
) ;tm-define
...
(tm-define (kbd-enter t shift?)
  ...
  (if (tree-empty? (tree-ref t 1))
    (let* ((u t) (next (db-next-field t)) (prev (db-previous-field t)))
      (when (or next prev)
