...
(tm-define-macro (with-database* db . body)
  `(with-global current-database
     ,db
     (tmdb-keep-history current-database #f)
     ,@body)
) ;tm-define-macro

(tm-define (db-reset) (set! current-database (url-none)))
...
(tm-define-macro (with-time t . body) `(with-global db-time ,t ,@body))
...
(define (db-get-time*)
  (cond ((== db-time :now) (current-time))
        ...
        (else (display* "db-time= " db-time "\n")
          (texmacs-error "db-get-time" "invalid time")
        ) ;else
  ) ;cond
) ;define

(tm-define (db-get-time) (exact->inexact (db-get-time*)))
...
(tm-define (db-get-field-first id attr default)
  (with l (db-get-field id attr) (if (null? l) default (car l)))
) ;tm-define
...
(tm-define (db-entry-exists? id) (nnull? (db-get-field id "name")))

(tm-define (db-get-entry id) (tmdb-get-entry (db-get-db) id (db-get-time)))
...
(tm-define (db-set-entry id l)
  ...
  (let* ((old-l (db-get-entry id)) (new-l (assoc-add db-extra-fields l)))
...
(tm-define (db-create-id)
  (if (url-none? current-database)
    ...
    (with id
      (create-unique-id)
      (while (nnull? (db-get-attributes id)) (set! id (create-unique-id)))
      id
    ) ;with
  ) ;if
) ;tm-define
...
(tm-define (db-search-name name) (db-search (list (list "name" name))))

(tm-define (db-search-owner owner) (db-search (list (list "owner" owner))))

(tm-define (db-inspect-history name) (tmdb-inspect-history (db-get-db) name))
...
(tm-define (prefix->queries q) (list (list :completes q)))
