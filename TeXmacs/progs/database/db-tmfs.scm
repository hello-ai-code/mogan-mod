...
(texmacs-module (database db-tmfs) (:use (database db-version)))
...
(define (db-current-search file)
  (let* ((search (db-get-query-preference file "search" ""))
    ...
    (if (== (string-length search) 0) e-search search)
  ) ;let*
) ;define
...
(define (name->query name)
  (let* ((h (tmfs-car name)) (t (tmfs-cdr name)) (i (string-search-forwards "=" 0 h)))
    (if (>= i 0)
      (let* ((l (name->query t))
             (var (substring h 0 i))
...
(tm-define (db-get-style kind) (string-append "database-" kind))
...
(tmfs-permission-handler (db name type) (in? type (list "read" "write")))

(tmfs-save-handler (db name doc) (db-confirm-entries-in (tm->tree doc)))
