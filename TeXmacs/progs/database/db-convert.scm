...
(texmacs-module (database db-convert) (:use (database db-edit)))
...
(tm-define (db-load-post t)
  (db-entry-rename t
    (list (cons "id*" "id") (cons "type*" "type") (cons "name*" "name"))
  ) ;db-entry-rename
) ;tm-define
...
(tm-define (db-pretty l kind fm) l)
...
(define (db-meta-field? f)
  (and (func? f 'db-field 2) (in? (cadr f) (db-meta-attributes)))
) ;define

(tm-define (assoc-list->entry id l)
  (let* ((type (assoc-ref l "type")) (name (assoc-ref l "name")))
    ;; (display* "Load " id " -> " l "\n")
...
(tm-define (db-load-entry id)
  (with l (db-get-entry id) (assoc-list->entry id l))
) ;tm-define

(tm-define (db-load-types types)
  (let* ((l (db-search (list (cons "type" types)))) (i (map db-load-entry l)))
    `(document ,@i)
  ) ;let*
) ;tm-define

(tm-define (db-load)
  (let* ((l (db-search (list))) (i (map db-load-entry l)))
    `(document ,@i)
  ) ;let*
) ;tm-define
...
(define (db-save-field t)
  (if (tm-func? t 'db-field 2) (list (tm-children t)) (list))
) ;define
...
(define (db-duplicate? t)
  (and (db-entry-any? t)
    (with id (tm->string (tm-ref t 0)) (db-entry-exists? id))
  ) ;and
) ;define
...
(tm-define (db-save-selected t pred?)
  (cond ((tm-func? t 'document)
        ...
        ((and (tree? t) (tm-compound? t))
         (for-each (cut db-save-selected <> pred?) (tree-accessible-children t))
        ) ;
  ) ;cond
) ;tm-define
...
(tm-define (db-save t) (db-save-selected t (lambda (x) #t)))
...
(tm-define (db-url? u)
  (with s
    (url->string u)
    (or (string-starts? s "tmfs://db/") (string-ends? s ".tmdb"))
  ) ;with
) ;tm-define

(tm-define (db-confirm-imported name)
  (with key
    (string-append "recent-import," (url->system (current-buffer)))
...
(tm-define (db-confirm-exported name)
  (with key
    (string-append "recent-export," (url->system (current-buffer)))
    (learn-interactive key (list (cons "name" (url->system name))))
  ) ;with
) ;tm-define
...
(tm-define (db-import-select) (:interactive #t) (noop))

(tm-define (db-export-select) (:interactive #t) (noop))
  ...
  (let* ((types (or (smart-ref db-kind-table kind) #t)) (ids '()))
    ...
    (with-user #t
      (with get
        (lambda (id)
          (list id
            (with-time :always (db-get-field-first id "name" #f))
            (with-time :now (db-get-entry id))
          ) ;list
        ) ;lambda
        (with r
          (map get ids)
