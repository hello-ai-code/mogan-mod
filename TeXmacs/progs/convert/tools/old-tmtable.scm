...
(tm-define (tmtable formats cells)
  ...
  (tmtable-record (length cells) (list-max (map length cells)) cells formats)
) ;tm-define
...
(tm-define (tmformat-cell i j name value) `(cwith ,i ,i ,j ,j ,name ,value))

(define (tmformat-frame? f)
  (func? f 'twith)
) ;define

(define (tmformat-cell? f)
  (func? f 'cwith)
) ;define

(define (tmformat-cell-name f)
  (sixth f)
) ;define

(define (tmformat-cell-value f)
  (if (seventh f) (seventh f) '())
) ;define

(define (tmtable-format-on-cell? t f i j)
  (and (tmformat-cell? f)
    (with (sym I1 I2 J1 J2 var val)
      f
  ...
  (with nc
    ...
    (and (tmformat-cell? f)
      (with (sym I1 I2 J1 J2 var val)
        ...
        (and (== 1 J1)
          (or (== -1 J2) (== nc J2))
          (let ((i1 (decode-row t I1)) (i2 (decode-row t I2)))
            (and (>= i i1) (<= i i2))
          ) ;let
        ) ;and
      ) ;with
  ...
  (with nr
    ...
    (and (tmformat-cell? f)
      (with (sym I1 I2 J1 J2 var val)
        ...
        (and (== 1 I1)
          (or (== -1 I2) (== nr I2))
          (let ((j1 (decode-column t J1)) (j2 (decode-column t J2)))
            (and (>= j j1) (<= j j2))
          ) ;let
        ) ;and
      ) ;with
...
(define (decode-row t i)
  (decode i (tmtable-nrows t))
) ;define

(define (decode-column t j)
  (decode j (tmtable-ncols t))
) ;define
...
(define (tmtable-format-partition-by-column t fs)
  (map (lambda (j) (list-filter fs (cut tmtable-format-on-column? t <> j)))
    (iota (tmtable-ncols t))
  ) ;map
) ;define

(define (tmtable-format-partition-by-row t fs)
  (map (lambda (j) (list-filter fs (cut tmtable-format-on-row? t <> j)))
...
(define (stm-table-decode-format name value)
 ((cond ((stm-table-length-name? name) string->tmlength)
        ((stm-table-number-name? name) string->number)
        ((stm-table-color-name? name) stm->tmcolor)
        (else noop)
  ) ;cond
  (force-string value)
 ) ;
...
(define (stm->tmtable x)
  (tmtable (reverse (stm-table-formats x)) (stm-table-cells x))
) ;define
...
(define (stm-table-encode-format name value)
 ((cond ((stm-table-length-name? name) tmlength->string)
        ((stm-table-number-name? name) number->string)
        ((stm-table-color-name? name) tmcolor->stm)
        (else noop)
  ) ;cond
  value
 ) ;
...
(define (make-block-cell x)
  ...
  (if (func? x 'document) `(cell ,x) `(cell (document ,x)))
) ;define

(define (tmtable-cells->stm t)
  (cons 'table
    (map (lambda (r) (cons 'row (map make-block-cell r))) (tmtable-cells t))
  ) ;cons
) ;define
