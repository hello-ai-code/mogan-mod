...
(define tmcolor-type
  (make-record-type "tmcolor" '(name-promise red green blue))
) ;define
...
(define (tmcolor-name c)
  (force (tmcolor-name-promise c))
) ;define
...
(tm-define (tmcolor red green blue)
  ...
  (tmcolor-record (delay (tmcolor-closest-name (tmcolor-record #f red green blue)))
    red
    green
    blue
  ) ;tmcolor-record
...
(tm-define (rgb255->tmcolor rgb255) (apply tmcolor (map (cut / <> 255) rgb255)))
...
(tm-define (stm->tmcolor name)
  (let ((c (list-any (lambda (c) (== name (tmcolor-name c))) texmacs-colors)))
...
(define texmacs-colors
  (map (cut apply named-rgb255->tmcolor <>)
    ...
    `(("black" (0 0 0))
      ...
      (,"pastel red" (,255 ,pastel ,pastel))
      (,"pastel blue" (,pastel ,pastel ,255))
      (,"pastel yellow" (,255 ,255 ,pastel))
      (,"pastel green" (,pastel ,255 ,pastel))
      (,"pastel magenta" (,255 ,pastel ,255))
      (,"pastel cyan" (,pastel ,255 ,255))
...
(define (cmy255->cmyk255 col)
  (let ((c (car col)) (m (cadr col)) (y (caddr col)) (k 255))
    (if (< c k) (set! k c))
    (if (< m k) (set! k m))
    (if (< y k) (set! k y))
    (if (== k 255)
      '(0 0 0 255)
      (let* ((k* (- 1 (/ k 255))) (col* (map (lambda (x) (round (/ (- x k) k*))) col)))
        `(,@col* ,k)
      ) ;let*
    ) ;if
  ) ;let
