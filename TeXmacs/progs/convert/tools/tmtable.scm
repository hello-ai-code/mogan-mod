...
(define (tmindex-get x last)
  (with n (string->number x) (if (< n 0) (+ last 1 n) n))
) ;define
...
(define (tmtformat-formats l nrrows nrcols)
  ...
  (receive (table-l table-r)
    (tmtformat-split l table-cwith?)
    (receive (column-l column-r)
      (tmtformat-split table-r column-cwith?)
      (receive (row-l row-r)
        (tmtformat-split column-r row-cwith?)
...
(define (tmtformat-props-list l start end)
  (if (> start end)
    '()
    (cons (tmtformat-props l start end) (tmtformat-props-list l (+ start 1) end))
  ) ;if
) ;define
...
(define (tmtformat-cell-props-sub l nr-cols)
  (with r (map (lambda (x) (cons 'cwith x)) l) (tmtformat-props-list r 1 nr-cols))
) ;define
...
(tm-define (tmtable-properties* t)
  ...
  (receive (tablef colf rowf cellf)
    (tmtable-formats t)
    (receive (t1 t2)
      (list-partition tablef (lambda (x) (func? x 'twith)))
      (set! tablef t1)
