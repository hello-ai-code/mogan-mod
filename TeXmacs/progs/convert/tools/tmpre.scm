...
(define (tmpre-non-isolated? l)
  (not (and (list? l) (= (length l) 2) (logic-in? (car l) tmpre-sectional%)))
) ;define
...
(define (tmpre-glueable? l)
  (or (func? l 'assign 2)
    ...
    (and (func? l 'mtm 2) (tmpre-glueable? (caddr l)))
  ) ;or
) ;define
...
(define (tmpre-document l)
  (if (null? l)
    l
    (let ((h (tmpre (car l))) (r (tmpre-document (cdr l))))
...
(define (tmpre-var-document l)
  (if (and (tmpre-empty? (car l)) (tmpre-glueable? (cadr l)))
    (cons (list 'concat (tmpre (car l)) (list 'tmdummy)) (tmpre-document (cdr l)))
    (tmpre-document l)
  ) ;if
) ;define
...
(tm-define (tmpre-produce l) (tmpre l))
