...
(define (tmlength-unit? x)
  (and (symbol? x)
    (or (string-locase-alpha? (symbol->string x))
...
(define (tmlength-check-value x)
  (if (not (tmlength-value? x)) (error "bad length value:" x))
) ;define

(define (tmlength-check-unit x)
  (if (not (tmlength-unit? x)) (error "bad length unit:" x))
) ;define
...
(tm-define tmlength
  ...
  (case-lambda
   (() '())
   ((n unit) (tmlength-check-value n) (tmlength-check-unit unit) (list n unit))
  ) ;case-lambda
) ;tm-define
...
(tm-define (tmlength? x)
  ...
  (or (tmlength-null? x)
    (and (list? x)
      (= 2 (length x))
      (tmlength-value? (first x))
      (tmlength-unit? (second x))
    ) ;and
  ) ;or
) ;tm-define
...
(tm-define (tmlength-value+unit tmlen)
  ...
  (:returns (1 "value part of @tmlen") (2 "unit part of @tmlen"))
  (if (tmlength-null? tmlen) (values 0 #f) (values (first tmlen) (second tmlen)))
) ;tm-define
...
(define (char-decimal? c)
  (or (char-numeric? c) (char-in-string? c "-."))
) ;define

(define (string-span s pred)
  (receive (head tail)
    (list-break (string->list s) (compose not pred))
