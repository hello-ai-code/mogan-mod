...
(define-public (delq x l)
  (if (pair? l)
    (if (eq? x (car l)) (delq x (cdr l)) (cons (car l) (delq x (cdr l))))
    ()
  ) ;if
) ;define-public
...
(define-public (list-copy lst) (copy lst))
;; S7 has generic functions. copy do a shallow copy

(define-public (copy-tree tree)
  (let loop
    ((tree tree))
    (if (pair? tree) (cons (loop (car tree)) (loop (cdr tree))) tree)
  ) ;let
) ;define-public
...
(define-public (assoc-ref l what) (let ((b (assoc what l))) (if b (cdr b) #f)))
...
(define-public (make-promise done? proc) (list (cons done? proc)))

(define-public-macro (delay-force expr) `(make-promise ,#f (lambda ,() ,expr)))

(define-public-macro (delay expr)
...
(define-public-macro (while test . body)
  `(call-with-exit (lambda (break)
                     (let continue
                       ,()
                       (if (let ,() ,test)
                         (begin (let ,() ,@body) (continue))
                         (break)))))
) ;define-public-macro
...
(define-public (char-set-adjoin cs . l) (lambda (ch) (or (cs ch) (memq ch l))))

(define-public (char-set-complement cs) (lambda (ch) (not (cs ch))))

(define-public (char-set:whitespace ch) (memq ch '(#\space #\tab #\newline)))
...
(define-public (string-index str cs)
  (let ((chr (if (char? cs) (lambda (c) (char=? c cs)) cs)))
    ...
    (do ((pos 0 (+ 1 pos)))
      ((or (>= pos len) (chr (string-ref str pos))) (and (< pos len) pos))
    ) ;do
  ) ;let
) ;define-public
