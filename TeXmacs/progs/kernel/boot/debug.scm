...
(define-public (regression-test-nequal group test res-in result exp-in expected)
...
(define-public-macro (regression-test-group group-desc group-id result-cmd expected-cmd . body)
  (let* ((make-command (lambda (cmd) (if (equal? cmd ':none) (lambda (x) x) (lambda (x) (list cmd x))))
         ) ;make-command
         ...
         (tests (let rec
                  ...
                  (define (check-test)
                    (let ((t (first l)))
                      (if (null? (cdr t)) (error "empty test in group " group-id))
                      (let ((test-desc (second t)))
                        (check-arg-type string? test-desc group-id)
                        (check-arg-number (lambda (x) (equal? 4 x))
                          (length t)
                  ...
                  (cond ((null? l) `(,(1- n)))
                        ...
                        ((not (pair? l)) l)
                        ((not (pair? (car l))) (cons (car l) (rec n (cdr l))))
                        ;; Test case.
