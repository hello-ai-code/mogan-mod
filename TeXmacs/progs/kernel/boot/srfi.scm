...
(define-public-macro (and-let* claws . body)
  (let* ((new-vars '()) (result (cons 'and '())) (growth-point result))
    ...
    (if (not (null? body))
      (if (null? (cdr body)) (andjoin! (car body)) (andjoin! `(begin ,@body)))
    ) ;if
    result
  ) ;let*
) ;define-public-macro
...
(define-public-macro (receive vars vals . body) `((lambda ,vars ,@body) ,vals))
...
(define-public-macro (cut slot . slots)
  (let loop
    ((slots (cons slot slots)) (params '()) (args '()))
    (if (null? slots)
      `(lambda ,(reverse! params) ,(reverse! args))
      (let ((s (car slots)) (rest (cdr slots)))
        (case s
         ((<>) (let ((var (gensym))) (loop rest (cons var params) (cons var args))))
         ((<...>)
          (if (pair? rest) (error "<...> not on the end of cut expression"))
          (let ((var (gensym)))
            `(lambda ,(append! (reverse! params) var)
               (apply ,@(reverse! (cons var args))))
          ) ;let
         ) ;
         (else (loop rest params (cons s args)))
        ) ;case
      ) ;let
    ) ;if
  ) ;let
