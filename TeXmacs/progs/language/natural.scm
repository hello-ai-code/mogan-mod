...
(tm-define (tr-file language)
  ...
  (url-concretize (string-append "$TEXMACS_PATH/langs/natural/dic/english-" language ".scm")
  ) ;url-concretize
) ;tm-define
...
(define (tr-hash language)
  ...
  (with h
    (ahash-ref tr-hashes language)
    (if h h (ahash-set! tr-hashes language (make-ahash-table)))
  ) ;with
) ;define
...
(define (tr-parse language)
  ...
  (let* ((f (tr-file language))
    ...
    (close-input-port p)
    (if r
      (display* (replace "Read %1 items from %2" (ahash-size h) f))
...
(define (tr-match language str)
  ...
  (let* ((lstr (locase-all str))
         (trans (or (ahash-ref (tr-hash language) str) (ahash-ref (tr-hash language) lstr))
         ) ;trans
        ) ;
