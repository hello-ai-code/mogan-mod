...
(texmacs-module (language std-math) (:use (language std-symbols)))
...
(define-language std-math-operators
  ...
  (define Infix (:operator) (Infix Post) (Pre Infix) Infix-symbol)

  (define Prefix (:operator) (Prefix Post) Prefix-symbol)

  (define Postfix (:operator) (Pre Postfix) Postfix-symbol)
  ...
  (define Assign-prefix (:operator) (Assign-prefix Post) Assign-symbol)
  ...
  (define Models-prefix (:operator) (Models-prefix Post) Models-symbol)
  ...
  (define Not-prefix (:operator) (Not-prefix Post) Not-symbol)
  ...
  (define Times-prefix (:operator) (Times-prefix Post) Times-symbol)

  (define Over-infix (:operator) (Over-infix Post) (Pre Over-infix) Over-symbol)

  (define Over-prefix (:operator) (Over-prefix Post) Over-symbol)
  ...
  (define Power-prefix (:operator) (Power-prefix Post) Power-symbol)

  (define Space-infix (:operator associative) Spacing-symbol)

  (define Ponctuation-infix (:operator associative) Ponctuation-symbol ".")
  ...
  (define Open (:operator) (Open Post) Open-symbol (:<left :args :>))

  (define Close (:operator) (Pre Close) Close-symbol (:<right :args :>))
  ...
  (define Comma (:operator associative) Ponctuation-symbol)
  ...
  (define Big-or (:operator) (Big-or Post) (:<big Big-or-symbol :>))

  (define Big-and (:operator) (Big-and Post) (:<big Big-and-symbol :>))

  (define Big-union (:operator) (Big-union Post) (:<big Big-union-symbol :>))
  ...
  (define Big-sum (:operator) (Big-sum Post) (:<big Big-sum-symbol :>))
...
(define-language std-math-grammar
  ...
  (define Main (Main Separator) (Main ".") (Main "\n") Relaxed-expressions)
  ...
  (define Expressions (Expressions Middle Expression-list) Expression-list)
  ...
  (define Union-list (Union-list Comma Union) (Union Comma Union))

  (define Assignment (Modeling Assign-infix Assignment) Modeling)
  ...
  (define Quantifier-prefix-symbol (:operator) Quantifier-symbol)

  (define Quantifier-prefix (Quantifier-prefix-symbol Relation))

  (define Quantifier-prefixes (+ Quantifier-prefix))
  ...
  (define Implication (Implication Imply-infix Disjunction) Disjunction)

  (define Disjunction (Disjunction Or-infix Conjunction) Conjunction)

  (define Conjunction (Conjunction And-infix Negation) Negation)

  (define Negation ((+ Not-prefix) Prefixed) Relation)

  (define Relation (Relation Relation-infix Union) Union)
  ...
  (define Intersection (Intersection Intersection-infix Sum) Sum)

  (define Sum (Sum Plus-infix Product) (Sum Minus-infix Product) Sum-prefix)

  (define Sum-prefix (Plus-prefix Sum-prefix) (Minus-prefix Sum-prefix) Product)
  ...
  (define Fenced Fenced-postfix)

  (define Restrict
    (:focus disallow)
    ((or "|" (:<mid "|" :>)) (+ (or (:<rsub Script :>) (:<rsup Script :>))))
  ) ;define
