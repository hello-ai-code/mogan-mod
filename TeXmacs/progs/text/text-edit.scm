...
(tm-define (style-category p)
  (:require (in? p (list "modern-program" "centered-program" "framed-program")))
  :program-theme
) ;tm-define
...
(tm-define (document-propose-title?)
  (with bt
    (buffer-tree)
    (with brothers
      (map tree-label (tree-children bt))
      (and-with t
        (tree-ref bt :down)
...
(tm-define (doc-author-context? t)
  (and (tree-search-upwards t 'doc-data) (tree-in? t (author-data-tag-list)))
) ;tm-define

(define doc-data-inactive-tags (doc-title-inactive-tag-list))
...
(tm-define (abstract-data-context? t) (tree-in? t (abstract-data-tag-list)))
...
(tm-define (kbd-space-bar t shift?)
  (:require (and (tree-is-buffer? t) (in-text?) (!= (get-env "language") "verbatim"))
  ) ;:require
  (let* ((b (before-cursor)) (p (get-preference "text spacebar")))
    (cond ((== p "allow multiple spaces") (insert " "))
...
(tm-define (kbd-enter t shift?)
  (:require (or (tree-is? t 'abstract-arxiv)
...
(tm-define (doc-data-clean t)
  (for (c (reverse (tree-children t)))
    (cond ((tree-empty? c) (tree-remove t (tree-index c) 1))
          ((and (tree-func? c 'doc-author 1) (tree-empty? (tree-ref c 0)))
           (tree-remove t (tree-index c) 1)
          ) ;
          ((and (tree-func? c 'doc-author 1) (tm-is? (tree-ref c 0) 'author-data))
...
(tm-define (kbd-remove t forwards?)
  ...
  (with d
    (tree-search-upwards t 'doc-data)
    (former t forwards?)
...
(tm-define (test-doc-title-clustering? mode)
  (with cl
    (list "cluster-all" "cluster-by-affiliation")
    (with old
      (get-doc-title-options)
...
(tm-define (set-doc-title-clustering mode)
  ...
  (with cl
    (list "cluster-all" "cluster-by-affiliation")
    (with old
      (list-difference (get-doc-title-options) cl)
...
(define (doc-data-go-to-active t i)
  (cond ((< i 0) (tree-go-to t :end))
        ((tree-in? t i (doc-title-inactive-tag-list)) (doc-data-go-to-active t (- i 1)))
        ((not (cursor-inside? (tree-ref t i))) (tree-go-to t i :end))
  ) ;cond
) ;define
...
(tm-define (doc-data-has-hidden?)
  (with-innermost t
    ...
    (with l
      ...
      (with fun
        (lambda (t)
          (or (tree-in? t (doc-title-inactive-tag-list)) (tree-is? t 'doc-inactive))
        ) ;lambda
        (list-or (map fun l))
      ) ;with
    ) ;with
...
(tm-define (doc-data-activate-all)
  (with-innermost t
    ...
    (with i
      (tree-down-index t)
      (with l (cdr (tree->list t)) (for-each doc-data-activate-one l))
...
(tm-define (doc-data-deactivate-all)
  (with-innermost t
    'doc-data
    (with l (cdr (tree->list t)) (for-each doc-data-deactivate-one l))
  ) ;with-innermost
) ;tm-define

(tm-define (doc-data-activate-toggle)
  (if (doc-data-deactivated?) (doc-data-activate-all) (doc-data-deactivate-all))
) ;tm-define
...
(tm-define (previous-section)
  (with bt
    ...
    (and (cursor-inside? bt)
      (with bp
        (list-drop (cursor-path) (length (tree->path bt)))
        (with sp
          (path-previous-section bt bp)
...
(define (make-section-aux l flag)
  (if (selection-active-any?)
    ...
    (if (not (make-return-after)) (begin (make l) (if flag (make-return-before))))
  ) ;if
) ;define
...
(tm-define (list-context? t) (tree-in? t (list-tag-list)))

(tm-define (itemize-context? t) (tree-in? t (itemize-tag-list)))

(tm-define (enumerate-context? t) (tree-in? t (enumerate-tag-list)))

(tm-define (itemize-enumerate-context? t)
  (or (tree-in? t (itemize-tag-list)) (tree-in? t (enumerate-tag-list)))
) ;tm-define

(tm-define (make-tmlist l)
  (with flag?
    (and (selection-active-non-small?) (in? l (description-tag-list)))
    (wrap-selection-any (make l) (if flag? (insert '(item* "")) (make-item)))
  ) ;with
) ;tm-define

(define (blank-text? t)
  (and (tree-atomic? t) (== (string-trim-spaces (tree->string t)) ""))
) ;define
...
(define (blank-tree? t)
  (cond ((blank-text? t) #t)
        ((tree-is? t 'concat)
         (let loop
           ((i 0))
           (or (>= i (tree-arity t)) (and (blank-tree? (tree-ref t i)) (loop (+ i 1))))
         ) ;let
        ) ;
        (else (tree-empty? t))
  ) ;cond
...
(define (left-siblings-blank-until? t stop)
  (or (not t)
    (== t stop)
    (let* ((parent (tree-up t)) (index (and parent (tree-index t))))
      (and parent
        index
        (let loop
          ((i 0))
          (or (>= i index) (and (blank-tree? (tree-ref parent i)) (loop (+ i 1))))
        ) ;let
...
(tm-define (line-start-empty-text?)
  (let* ((t (cursor-tree)) (i (cAr (cursor-path))) (p (and t (paragraph-tree))))
    (and t
      (tree-atomic? t)
...
(tm-define (make-tmlist-if-line-start l fallback)
  (if (line-start-empty-text?) (make-tmlist l) (insert fallback))
) ;tm-define
...
(define (list-item-logical-tree t)
  (if (and (list-item-marker? t) (list-item-wrapper? (tree-up t))) (tree-up t) t)
) ;define

(define (current-list-item-tree)
  (and-with item
    (tree-search-upwards (cursor-tree)
      (lambda (t) (or (list-item-marker? t) (list-item-wrapper? t)))
    ) ;tree-search-upwards
...
(tm-define (kbd-enter t shift?)
  (:require (and (list-context? t) (current-list-item-tree) (not (in-prog?))))
...
(tm-define (focus-label t)
  ...
  (and-with doc
    ...
    (and (tree-is? doc 'document)
      (and-with par (tree-down doc) (focus-search-label par))
    ) ;and
  ) ;and-with
) ;tm-define
...
(tm-define (numbered-numbered? t) (:require (enumerate-context? t)) #t)
...
(tm-define (hyphenate-selection-as hyph)
  ...
  (with ins
    `(hyphenate-as ,hyph ,(selection-tree))
    (clipboard-cut "null")
    (insert ins)
  ) ;with
) ;tm-define
...
(tm-define (focus-label t)
  ...
  (and-with row (down-to-row (tree-down t)) (focus-search-label row))
) ;tm-define
...
(define (automatic-section-name)
  (with-innermost t
    ...
    (let* ((var (automatic-name-var t)) (val (if var (get-env-tree var) "")))
...
(tm-define (automatic-section-rename new-name)
  ...
  (with-innermost t
    automatic-section-context?
    (when t
      (let* ((l (tree-label t)) (l* (symbol-append l '*)))
        (tree-set t `(,l* ,@(cDr (tm-children t)) ,new-name ,(tm-ref t :last)))
      ) ;let*
    ) ;when
  ) ;with-innermost
) ;tm-define
...
(tm-define (enunciation-context? t) (tree-in? t (enunciation-tag-list)))
...
(tm-define (dueto-added? t) (tm-find t (lambda (x) (tm-is? x 'dueto))))

(tm-define (dueto-add t) (tree-go-to t :last :start) (make 'dueto))

(tm-define (focus-label t)
  (:require (enunciation-context? t))
  (and (== (tree-arity t) 1) (focus-search-label (tree-ref t 0)))
) ;tm-define
...
(tm-define (algorithm-context? t) (tree-in? t (algorithm-tag-list)))

(tm-define (algorithm-root s)
  (cond ((symbol-ends? s '*) (algorithm-root (symbol-drop-right s 1)))
        ((symbol-starts? s 'specified-) (algorithm-root (symbol-drop s 10)))
        ((symbol-starts? s 'named-) (algorithm-root (symbol-drop s 6)))
        (else s)
  ) ;cond
) ;tm-define

(tm-define (algorithm-numbered? t)
  (let* ((l (tree-label t)) (r (algorithm-root l)))
    (in? l (list r (symbol-append 'specified- r)))
  ) ;let*
) ;tm-define

(tm-define (algorithm-named? t)
  (with l (tree-label t) (symbol-starts? l 'named-))
) ;tm-define

(tm-define (algorithm-specified? t)
  (with l
    (tree-label t)
    (or (symbol-starts? l 'named-specified-) (symbol-starts? l 'specified-))
  ) ;with
) ;tm-define
...
(tm-define (algorithm-toggle-specification t)
  (let* ((l (tree-label t)) (r (algorithm-root l)))
    (if (algorithm-specified? t)
      (begin
        (cond ((algorithm-named? t) (tree-assign-node! t (symbol-append 'named- r)))
              ((algorithm-numbered? t) (tree-assign-node! t r))
              (else (tree-assign-node! t (symbol-append r '*)))
        ) ;cond
        (tree-remove! t (- (tree-arity t) 2) 1)
      ) ;begin
      (begin
        (cond ((algorithm-named? t) (tree-assign-node! t (symbol-append 'named-specified- r)))
              ((algorithm-numbered? t) (tree-assign-node! t (symbol-append 'specified- r)))
              (else (tree-assign-node! t (symbol-append 'specified- r '*)))
        ) ;cond
...
(tm-define (detached-note-context? t) (tree-in? t (detached-note-tag-list)))

(tm-define (auto-note-context? t) (tree-in? t (auto-note-tag-list)))

(tm-define (custom-note-context? t) (tree-in? t (custom-note-tag-list)))

(tm-define (note-toggle-custom t)
  (let* ((l (symbol-toggle-number (tree-label t))) (c (tree-children t)))
...
(tm-define (frame-context? t) (tree-in? t (frame-tag-list)))

(tm-define (frame-titled-context? t) (tree-in? t (frame-titled-tag-list)))
...
(tm-define (customizable-parameters t)
  ...
  (list (list "padding-above" "Above") (list "padding-below" "Below"))
) ;tm-define
...
(tm-define (customizable-parameters t)
  ...
  (append (list (list "ornament-shape" "Shape::ornament") (list "ornament-color" "Color"))
    (if (== (tree-arity t) 1)
      (list)
...
(tm-define (float-context? t) (tree-in? t '(float wide-float)))

(tm-define (footnote-context? t) (tree-in? t '(footnote wide-footnote)))

(tm-define (figure-context? t) (tree-in? t (figure-tag-list)))

(tm-define (table-context? t) (tree-in? t (table-tag-list)))

(tm-define (float-or-footnote-context? t)
  (tree-in? t '(float wide-float footnote wide-footnote))
) ;tm-define

(tm-define (phantom-float-context? t) (tree-is? t 'phantom-float))
...
(tm-define (floatable-context? t)
  (and (tree-in? t '(big-figure big-figure*
    ...
    (or (tree-is? t :up 'document)
      (and (tree-is? t :up 'with) (tree-is? t :up :up 'document))
    ) ;or
  ) ;and
) ;tm-define
...
(tm-define (float-wide? t)
  (and-with f
    (tree-search-upwards t float-or-footnote-context?)
    (tree-in? f '(wide-float wide-footnote))
  ) ;and-with
) ;tm-define

(tm-define (test-float-wide? . args) (float-wide? (focus-tree)))
...
(tm-define (floatable-wide? t) (and-with w (tree-up t) (with-wide? w)))

(tm-define (test-floatable-wide? . args) (floatable-wide? (focus-tree)))
...
(tm-define (cursor-at-anchor?)
  (with t (cursor-tree) (float-or-footnote-context? t))
) ;tm-define

(tm-define (go-to-anchor)
  (cond ((or (inside? 'float) (inside? 'wide-float))
         (with-innermost t float-context? (tree-go-to t :end))
        ) ;
        ((or (inside? 'footnote) (inside? 'wide-footnote))
         (with-innermost t footnote-context? (tree-go-to t :end))
        ) ;
  ) ;cond
) ;tm-define
...
(tm-define (go-to-float)
  (with t
    (cursor-tree)
    (cond ((float-context? t) (tree-go-to t 2 :start))
          ((footnote-context? t) (tree-go-to t 0 :start))
    ) ;cond
  ) ;with
) ;tm-define
...
(tm-define (cursor-toggle-anchor)
  ...
  (if (cursor-at-anchor?) (go-to-float) (go-to-anchor))
) ;tm-define
