...
(texmacs-module (text text-speech) (:use (text text-kbd) (math math-speech)))
...
(define (speech-accepts*? lan type s)
  ...
  (or (speech-accepts? lan type s) (string->number s))
) ;define
...
(define (speech-start-accepts*? lan type s)
  ...
  (or (speech-border-accepts? lan type s) (string->number s))
) ;define
...
(define (accept-start? lan l r)
  (cond ((< (length r) 1) #f)
        ...
        ((and (nnull? l)
           ...
           (in? (car r) (list "a" (car l)))
         ) ;and
         #t
        ) ;
        ((and (null? (cdr r))
           (> (string-length (car r)) 1)
           (nin? (string-append "<" (car r) ">") greek-letters)
         ) ;and
         #f
        ) ;
  ...
  (let* ((s (string-recompose l " "))
    ...
    (cond ((or (null? l) (null? r)) (text-speech* lan (post-punctuate pre punc) post))
          ((speech-has? lan 'math-mode (car l))
           (set! l (cdr l))
           (when (speech-has? lan 'text-mode (cAr l))
             (set! l (cDr l))
           ) ;when
           (text-math-speech* lan pre l post*)
          ) ;
          ((not (accept-middle? lan l r)) (text-speech* lan (append pre l*) post))
...
(define (longest-math-prefix* lan l)
  (cond ((null? l) l)
        ((letterized? (car l)) (cons (car l) (longest-math-prefix* lan (cdr l))))
...
(tm-define (kbd-speech S)
  ...
  (let* ((prev1 (before-cursor))
    ...
    (cond ((== S "") (noop))
          ...
          ((speech-make S) (noop))
          ((in? prev (list "." "!" "?")) (when spc? (kbd-space)) (text-speech S))
          ((in? prev (list "," ":" ";"))
           (when spc?
             (kbd-space)
           ) ;when
...
(tm-define (speech-proof)
  (with-innermost t enunciation-context? (tree-go-to t :end))
  (make 'proof)
) ;tm-define
