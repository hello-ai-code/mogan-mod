...
(texmacs-module (math math-speech)
  (:use (math math-kbd) (math math-stats) (utils library cursor))
) ;texmacs-module
...
(tm-define (speech-current-mode) (:mode in-math?) 'math)

(define (stats-has-operator? s)
  (and (<= (string-length s) 10)
    (>= (string-length s) 2)
    (string-alpha? s)
    (let* ((ss (locase-all s)) (Ss (upcase-first ss)) (SS (upcase-all s)))
...
(tm-define (speech-exec-hook s)
  ...
  (and (nin? s standard-operators)
    (and-with op (stats-has-operator? s) (speech-insert-operator op) #t)
  ) ;and
) ;tm-define
  ...
  (with l
    ...
    (cond ((< (length l) 2) s)
          ...
          ((and (== lan 'french)
           ...
           (with de
             (if (== (string-ref s 1) #\d) " d/de " " t/de ")
             (when (== (string-length s) 2)
               (set! de (string-drop-right de 1))
             ) ;when
             (string-append (string-take s 1) de (letterize-one lan (string-drop s 2)))
           ) ;with
          ) ;
...
(tm-define (letterized? s*) (with s (locase-all s*) (!= (letterize s) s)))

(tm-define (letterized-list s*)
  (with s (locase-all s*) (string-decompose (letterize (locase-all s)) " "))
) ;tm-define
...
(define (speech-exit-from op)
  (and-with t (tree-innermost op) (tree-go-to t :end))
  (set! speech-state (cdr speech-state))
) ;define

(define (speech-exit-innermost)
  (with type
    (and (nnull? speech-state) (car speech-state))
    (cond ((not type) (noop))
...
(define (speech-exit-all)
  (while (nnull? speech-state) (speech-exit-innermost))
) ;define
...
(tm-define (speech-leave)
  (:mode in-math?)
  (if (nnull? speech-state) (speech-exit-innermost) (structured-exit-right))
) ;tm-define

(define short-list (list :short-over :short-subscript :short-superscript))
...
(tm-define (speech-end tag)
  (while (and (inside? tag) (nnull? speech-state)) (speech-exit-innermost))
  (and-with t (tree-innermost tag) (tree-go-to t :end))
) ;tm-define
...
(define (update-mode mods type)
  (cond ((in? type mods) mods)
        ((and (in? type (list :cal :bbb)) (nin? :small mods) (nin? :big mods))
...
(define (root-letter s)
  (cond ((not s) #f)
        ...
        ((and (string? s) (string-occurs? "-" s) (string-ends? s ">"))
         (let* ((n (string-length s)) (i (string-search-backwards "-" n s)))
...
(tm-define (speech-alter-letter* type)
  ...
  (set! speech-letter-mode* (update-mode speech-letter-mode* type))
  (and-with prev
    (root-letter (expr-before-cursor))
    (cut-before-cursor)
...
(define (add-variant x mods)
  (set! current-variants (cons (modified-letter x mods) current-variants))
) ;define

(define (add-variants* x mods)
  (when (and (nin? :up mods)
          (nin? :it mods)
          (nin? :cal mods)
          (nin? :frak mods)
          (nin? :bbb mods)
...
(define (add-variants x mods)
  (when (== mods (list))
    (when (== x "e")
      (add-variant "<mathe>" mods)
    ) ;when
    (when (== x "i")
      (add-variant "<mathi>" mods)
    ) ;when
    (when (== x "<pi>")
      (add-variant "<mathpi>" mods)
    ) ;when
  ) ;when
  (when (and (in? x (list "<theta>" "<kappa>" "<pi>" "<rho>" "<sigma>"))
    ...
    (with y
      (string-append "<var" (substring x 1 (string-length x)))
      (add-greek-variants y (cons :small mods))
    ) ;with
  ) ;when
  (if (in? x greek-letters) (add-greek-variants x mods) (add-variants* x mods))
  (when (and (in? x (list "<epsilon>" "<phi>")) (nin? :big mods) (nin? :up mods))
    (with y
      (string-append "<var" (substring x 1 (string-length x)))
      (add-greek-variants y (cons :small mods))
    ) ;with
...
(tm-define (letter-variants x mods)
  (let* ((key (list x mods)) (im (ahash-ref letter-variants-table key)))
    (when (not im)
...
(tm-define (stats-has-variant? x)
  (with l (letter-variants x speech-letter-mode) (nnull? (stats-filter l)))
) ;tm-define
...
(define (best-variant x mods)
  ...
  (with l (letter-variants x mods) (apply stats-best l))
) ;define

(tm-define (best-letter-variant x) (best-variant x speech-letter-mode))
...
(define (find-best i v o)
  (if (null? (cdr i))
    ...
    (with (bi bv bo)
      (find-best (cdr i) (cdr v) (cdr o))
      (if (> bo (car o)) (list bi bv bo) (list (car i) (car v) (car o)))
    ) ;with
  ) ;if
) ;define
  ...
  (if (not prev)
    ...
    (let* ((a (letter-variants x speech-letter-mode))
      ...
      (if (or (null? l) (== (apply max o) 0) (not (exists? (cut != <> :none) i)))
        ...
        (with (bi bv bo) (find-best i l o) (list bi bv))
      ) ;if
    ) ;let*
  ) ;if
) ;tm-define
...
(tm-define (stats-preferred l mode)
  (and (nnull? l)
    (with best
      (stats-preferred (cdr l) mode)
      (if (and best (stats-prefer? best (car l) mode))
        best
...
(tm-define (speech-insert-number x)
  (:mode in-math?)
  (let* ((prev (root-before-cursor)) (impl (best-implicit prev x)))
...
(tm-define (speech-insert-operator x)
  (set! speech-operator-mode :off)
  (let* ((prev (root-before-cursor)) (impl (best-implicit prev x)))
    (if (!= impl :none) (speech-insert-implicit impl x) (insert x))
  ) ;let*
) ;tm-define
...
(tm-define (speech-insert-d x) (speech-insert-operator "<mathd>") (insert x))
...
(tm-define (speech-operator) (set! speech-operator-mode :start))
...
(tm-define (speech-best-letter . l)
  (with sym (stats-preferred l :normal) (speech-insert-symbol (or sym (cAr l))))
) ;tm-define

(tm-define (speech-insert-best . l)
  (with sym (stats-preferred l :strong) (speech-insert-symbol (or sym (car l))))
) ;tm-define
...
(define (complete-letter-variants l mode)
  (if (null? l)
    l
    (with v
      (letter-variants (car l) mode)
      (append (stats-filter v) (complete-letter-variants (cdr l) mode))
    ) ;with
  ) ;if
) ;define
...
(tm-define (speech-best-combination l1 l2)
  ...
  (let* ((l1* (list-remove-duplicates (complete-letter-variants l1 speech-letter-mode)))
         (l2* (list-remove-duplicates (complete-letter-variants l2 (list))))
         (p (stats-preferred-combination l1* l2*))
        ) ;
    ;; (display* "best combination " l1* ", " l2* " ~> " p "\n")
    (if p
      (with (x y) p (speech-insert-symbol x) (speech-insert-symbol y))
      (begin
...
(tm-define (speech-wide acc) (make-wide acc) (speech-enter :wide))

(tm-define (speech-under)
  (cond ((inside? 'wide) (with t (tree-innermost 'wide) (tree-assign-node! t 'wide*)))
        ((tm-is? (before-cursor) 'wide)
         (with t (before-cursor) (tree-assign-node! t 'wide*))
        ) ;
  ) ;cond
) ;tm-define
...
(define (extend-script? x)
  (with v
    (letter-variants x speech-letter-mode)
...
(define (extend-script op x)
  (with expr
    ...
    (and (tm-in? expr '(rsub rsup))
      (with-cursor (tree->path expr 0 :end) (insert op) (speech-insert-symbol x))
    ) ;and
  ) ;with
) ;define
  ...
  (with v
    ...
    (and speech-can-extend-script?
      (or (exists? must-continue-script? v)
        (and (not (exists? dont-continue-script? v)) (exists? do-continue-script? v))
      ) ;or
    ) ;and
  ) ;with
) ;define
...
(define (continue-script x)
  (with op (cut-before-cursor) (extend-script op x))
) ;define

(tm-define (speech-insert-symbol x)
  (:require (or (string-number? x) (in? x roman-letters) (in? x greek-letters)))
  (with extend?
    (extend-script? x)
...
(define (speech-start-2d)
  (when (not (selection-active-any?))
    (let* ((prev (root-before-cursor)) (impl (best-implicit prev "<nosymbol>")))
...
(tm-define (speech-sqrt) (speech-start-2d) (make 'sqrt))

(tm-define (speech-sqrt-of) (speech-sqrt) (speech-enter :sqrt))

(tm-define (speech-fraction) (speech-start-2d) (make 'frac))
...
(tm-define (speech-open open close) (math-bracket-open open close 'default))

(tm-define (speech-close)
  (and-with t (tree-innermost around-context?) (tree-go-to t :end))
) ;tm-define
...
(define (brackets-before-cursor)
  (let* ((expr (expr-before-cursor)) (prev (expr-before-before-cursor)))
...
(define (root-before-brackets)
  (and-let* ((expr (brackets-before-cursor))
    ...
    (with-cursor (tree->path prev :end) (root-before-cursor))
  ) ;and-let*
) ;define

(define (dont-extend-brackets? x)
  (and-with root
    (root-before-brackets)
    (or (stats-role? (tmconcat root "*" x))
      (stats-role? (tmconcat root " " x))
      (stats-role? (tmconcat root "," x))
      (stats-role? (tmconcat root ";" x))
...
(define (extend-brackets op x)
  (with expr
    ...
    (and (tm-in? expr '(around around*))
      (with-cursor (tree->path expr 1 :end) (insert op) (speech-insert-symbol x))
    ) ;and
  ) ;with
) ;define
...
(define (continue-brackets x)
  (with op (cut-before-cursor) (extend-brackets op x))
) ;define
...
(tm-define (editing-big-operator?)
  (and-with t
    (tree-innermost script-context?)
    (while (and t (script-context? t)) (set! t (tree-ref t :previous)))
    (and t (tree-is? t 'big))
  ) ;and-with
) ;tm-define

(tm-define (speech-for) (make 'rsub))
  ...
  (if (inside? 'rsub)
    (with-innermost t
      ...
      (let* ((prev (tree-ref t :previous))
        ...
        (cond ((not big?) (noop))
              ((exists? math-weak-infix? l) (tree-go-to t :end) (make 'rsup))
              ((== op "sum") (big->dots prev t "+" "<cdots>"))
              ((== op "prod") (big->dots prev t "*" "<cdots>"))
              (else (tree-go-to t :end) (make 'rsup))
        ) ;cond
      ) ;let*
    ) ;with-innermost
    (speech-dots "," "<ldots>")
...
(tm-define (speech-to) (when (inside? 'rsub) (speech-until)))
