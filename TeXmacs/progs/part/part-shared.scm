...
(texmacs-module (part part-shared) (:use (utils plugins plugin-eval)))
...
(tm-define (make-mirror name)
  ...
  (let* ((l (id->trees name)) (id (create-unique-id)))
    (if (and (nnull? l) (mirror-body? (car l)))
      (let* ((doc (tm->stree (car l))) (p (path-start doc '())))
...
(define (mirror-invalidate t)
  ...
  (when (and (tree->path t) (mirror-body? t) (mirror-unique-id t))
    (when (> (ahash-size mirror-black-list) 0)
      (delayed (:idle 1) (mirror-separate) (delayed (:idle 1) (mirror-synchronize)))
    ) ;when
      ...
      (with failed
        ...
        (for (uid (ahash-set->list mirror-black-list))
          (for (t (id->trees uid))
            (when (not (mirror-up-to-date? t))
              (with l
                (list-filter (mirror-list t) mirror-white-listed?)
                (when (null? l)
        ...
        (for (id (ahash-set->list failed))
          ...
          (with l
            (id->trees id)
            (when (>= (length l) 2)
              (for (t (cdr l)) (tree-set! t (tree-copy (car l))))
            ) ;when
          ) ;with
        ) ;for
      ) ;with
...
(tm-define (mirror-initialize t)
  ...
  (when (mirror-body? t)
    (with key
      (cons (mirror-id t) (mirror-unique-id t))
  ...
  (for (m l)
    (with uid
      ...
      (when uid
        ...
        (with old
          (or (ahash-ref mirror-pending uid) (list))
          (ahash-set! mirror-pending uid (cons (modification-copy mod) old))
        ) ;with
      ) ;when
    ) ;with
  ) ;for
...
(tm-define (mirror-notify event t mod)
  ...
  (when (and mirror-idle?
    ...
    (with l (mirror-list t) (mirror-list-notify l mod))
  ) ;when
) ;tm-define
...
(tm-define (buffer-initialize id t buf) (ahash-set! buffer-shared-list id #t))
...
(tm-define (buffer-notify event t mod)
  (when (and mirror-idle?
    ...
    (with ids
      (list-filter (tree->ids t) (cut ahash-ref buffer-shared-list <>))
      (when (== (length ids) 1)
...
(tm-define (buffer-notify-shared id mod)
  (when (ahash-ref buffer-shared-list id)
    ...
    (with old
      (or (ahash-ref buffer-pending id) (list))
      (ahash-set! buffer-pending id (cons (modification-copy mod) old))
    ) ;with
  ) ;when
) ;tm-define
