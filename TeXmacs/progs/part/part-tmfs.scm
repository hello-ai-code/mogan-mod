...
(texmacs-module (part part-tmfs)
  (:use (part part-shared) (texmacs texmacs tm-files))
) ;texmacs-module
...
(tm-define (part-master name)
  (with pos
    (string-search-forwards ".tm/" 0 name)
    (if (<= pos 0)
  ...
  (with pos
    ...
    (if (<= pos 0)
      ...
      (let* ((m (tmfs-string->url (substring name 0 (+ pos 3))))
        ...
        (if (string-starts? s "here/") (url-relative m f) f)
      ) ;let*
    ) ;if
  ) ;with
) ;tm-define
...
(tm-define (part-url master file)
  (cond ((== master file) (string-append "tmfs://part/" (url->tmfs-string master)))
        ((url-descends? file (url-head master))
         (string-append "tmfs://part/"
           (url->tmfs-string master)
           "/"
           (url->tmfs-string (url-delta master file))
         ) ;string-append
        ) ;
        (else (string-append "tmfs://part/"
                (url->tmfs-string master)
                "/"
...
(define (get-extra-init t delta)
  (cons '(associate "part-flag" "true")
    (if (and (tm-func? t 'tuple) (tm-equal? (tm-ref t 0) delta))
...
(define (exclude-from-inherit)
  (list "preamble" "mode" "page-medium" "page-printed" "page-first")
) ;define
...
(tmfs-title-handler (part name doc)
  (let* ((m (part-master name)) (f (part-file name)))
    (if (== m f)
...
(define (make-shared u body)
  (let* ((id (create-unique-id)) (fid (url->unix u)))
...
(define (part-expand doc mas u m)
  (cond ((tm-atomic? doc) doc)
        ((tm-func? doc 'body 1)
         (with body
           (part-expand-body (tm-ref doc 0) u)
        ...
        ((tm-in? doc '(style references auxiliary))
         (with val
           (tmfile-extract mas (tm-label doc))
           (if val `(,(tm-label doc) ,val) doc)
         ) ;with
        ) ;
        ((tm-is? doc 'initial)
         (let* ((mt (master-inits mas u m)) (ft (tm-ref doc 0)) (t (collection-append mt ft)))
           `(initial ,t)
         ) ;let*
        ) ;
        ((tm-is? doc 'document)
         (when (and (tmfile-extract doc 'body) (not (tmfile-extract doc 'initial)))
           (set! doc (tmfile-assign doc 'initial (assoc->collection (list))))
         ) ;when
         (cons 'document (map (cut part-expand <> mas u m) (tm-children doc)))
        ) ;
        (else doc)
  ) ;cond
) ;define
...
(tmfs-master-handler (part name) (part-file name))
  ...
  (cond ((tm-atomic? doc) doc)
        ((tm-func? doc 'body 1)
         (with body
           ...
           (when (and (!= u m) (tm-func? body 'document 1) (tm-func? (tm-ref body 0) 'shared))
        ...
        ((tm-is? doc 'initial)
         (let* ((mt (master-inits mas u m)) (ft (tm-ref doc 0)) (t (collection-delta mt ft)))
           `(initial ,t)
         ) ;let*
        ) ;
        ((tm-is? doc 'document)
         (cons 'document (map (cut part-compress <> ori mas u m) (tm-children doc)))
        ) ;
        (else doc)
  ) ;cond
) ;define
...
(tmfs-wrap-handler (part name) (part-file name))
