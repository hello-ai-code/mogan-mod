...
(tm-widget (texmacs-bottom-toolbars)
  (if toolbar-search-active? (link search-toolbar))
  (if (and toolbar-replace-active? (not toolbar-search-active?))
...
(tm-define (test-bottom-bar? which)
  (cond ((== which "search") toolbar-search-active?)
        ((== which "replace")
         (and toolbar-replace-active? (not toolbar-search-active?))
        ) ;
        ((== which "spell")
...
(menu-bind view-menu
  ...
  (assuming (os-macos?) ("Retina settings" (open-retina-settings)))
  (assuming (not (os-macos?)) ("High resolution settings" (open-retina-settings)))
  ---
