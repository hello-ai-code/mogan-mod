...
(texmacs-module (server server-chat) (:use (server server-tmfs)))
...
(tm-service (remote-chat-room-create name)
  ...
  (with uid
    ...
    (with crid (chat-room-create uid name) (server-return envelope crid))
  ) ;with
) ;tm-service

(define (chat-room-id name)
  (with l
    (db-search `(("type" "chat-room") (,"name" ,name)))
    (and (nnull? l) (car l))
  ) ;with
) ;define
...
(tm-service (remote-list-chat-rooms)
  ...
  (with (client msg-id)
    ...
    (let* ((uid (server-get-user envelope))
           (l (db-search `(("type" "chat-room") (,"owner" ,uid))))
           (get-name (lambda (id) (db-get-field-first id "name" #f)))
           (r (list-filter (map get-name l) (lambda (s) (not (string-starts? s "mail-")))))
          ) ;
...
(define (chat-message-retrieve mid)
  (let* ((action (db-get-field-first mid "action" "unknown"))
    ...
    (when (== action "send")
      (with doc
        (string-load (repository-get msg))
...
(define (chat-room-retrieve crid)
  (with l
    (db-search `(("type" "chat-message") (,"to" ,crid) (:order "date" #t)))
...
(tm-service (remote-chat-room-open name)
  ...
  (with (client msg-id)
    envelope
    (let* ((uid (server-get-user envelope)) (crid (chat-room-id name)))
      (cond ((not crid) (server-error envelope "Error: unknown chat room"))
            ...
            (else (chat-room-initialize crid)
              (with l
                (ahash-ref chat-room-present crid)
                (when (nin? client l)
                  (ahash-set! chat-room-present crid (cons client l))
                ) ;when
              ) ;with
              (let* ((ms (ahash-ref chat-room-messages crid)) (w? (db-allow? crid uid "writable")))
...
(define (remote-create-message uid msg)
  (let* ((rid (with-time-stamp #t (db-create-entry `(("type" "message")
                                                     (,"owner" ,uid))))
         ) ;rid
...
(define (remote-send uid dest action msg)
  (cond ((== action "send-document")
         (and-with msg*
           (remote-create-message uid msg)
           (remote-send uid dest "send" msg*)
         ) ;and-with
        ) ;
        ((list? dest) (for (did dest) (remote-send uid did action msg)))
