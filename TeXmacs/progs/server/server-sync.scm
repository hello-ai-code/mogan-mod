...
(texmacs-module (server server-sync) (:use (server server-db-sync)))
...
(define (server-sync-list rid uid)
  ...
  (if (not (db-allow? rid uid "readable"))
    (list)
    ...
    (with uid
      ...
      (if (not uid)
        ...
        (with rid
          ...
          (cond ((not rid) (server-return envelope (list)))
                ((not (db-allow? rid uid "readable"))
                 (server-error envelope "Error: read access required")
                ) ;
                (else (with l (server-sync-list rid uid) (server-return envelope l)))
          ) ;cond
        ) ;with
      ) ;if
    ) ;with
  ...
  (with (cmd dir? local-name local-id remote-name remote-id doc)
    ...
    (let* ((rname (remote-file-name remote-name))
      ...
      (cond ((and dir? (not rid))
             (with r (server-dir-create uid rname) (and (== (car r) :created) (cadr r)))
            ) ;
            ((and dir? rid) rid)
            ((not rid)
             (with r
               (server-file-create uid rname doc msg)
               (and (== (car r) :created) (cadr r))
             ) ;with
            ) ;
            (else (with r
                    (server-file-save uid rname doc msg)
                    (and (== (car r) :created) (cadr r))
                  ) ;with
            ) ;else
      ) ;cond
  ...
  (with (cmd dir? local-name local-id remote-name remote-id)
    ...
    (let* ((rname (remote-file-name remote-name))
      ...
      (if dir?
        (list remote-id "")
        (with r
          (server-file-load uid rname)
          (and (== (car r) :loaded) (list remote-id (cadr r)))
        ) ;with
      ) ;if
    ) ;let*
  ) ;with
    ...
    (with rname
      ...
      (with-remote-context rname
        (with uid
          ...
          (if (not uid)
            (server-error envelope "Error: not logged in")
            (with r (map (cut remote-download-one uid <>) l) (server-return envelope r))
          ) ;if
        ) ;with
      ) ;with-remote-context
    ) ;with
...
(define (remote-remove-one uid line)
  ...
  (with (cmd dir? local-name local-id remote-name remote-id)
    ...
    (let* ((rname (remote-file-name remote-name))
      ...
      (if dir?
        (with r
          (server-dir-remove uid rname #f)
          (and (== (car r) :removed) (list remote-id :removed))
        ) ;with
        (with r
    ...
    (with rname
      ...
      (with-remote-context rname
        (with uid
          ...
          (if (not uid)
            (server-error envelope "Error: not logged in")
            (with r (map (cut remote-remove-one uid <>) l) (server-return envelope r))
          ) ;if
        ) ;with
      ) ;with-remote-context
    ) ;with
