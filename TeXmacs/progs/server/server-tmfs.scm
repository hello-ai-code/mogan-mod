...
(texmacs-module (server server-tmfs) (:use (server server-base)))
...
(define (repository-add-into dir name)
  ...
  (with rdir
    ...
    (if (not (url-exists? rdir))
      ...
      (with sub
        (number->string (random 10 repo-seed))
...
(tm-define (repository-add rid suffix)
  (let* ((name (if (== suffix "") rid (string-append rid "." suffix)))
         ...
         (tail (substring full (+ (string-length repo) 1) (string-length full)))
        ) ;
...
(tm-define-macro (with-remote-context rname . body)
  `(let* ((path (tmfs->list ,rname))
     ...
     (with-global db-time time (with-global ,rname next ,@body)))
) ;tm-define-macro
...
(define (search-file l . where)
  (if (null? l)
    where
    (let* ((q (if (null? where) (list "type" "dir") (list "dir" (car where))))
...
(define (dir-contents dir)
  (db-search `((,"dir" ,dir) (:order "name" #t)))
) ;define
...
(define (copy-properties base-rid derived-rid pred?)
  (let* ((props1 (db-get-entry base-rid)) (props2 (list-filter props1 pred?)))
    (for (prop props2) (db-set-field derived-rid (car prop) (cdr prop)))
  ) ;let*
) ;define
...
(tm-service (remote-identifier rname)
  ...
  (with-remote-context rname
    (let* ((uid (server-get-user envelope)) (rid (search-remote-identifier rname)))
      (cond ((not uid) (server-error envelope "Error: not logged in"))
            ...
            ((!= db-time :now) (server-return envelope (list rid db-time)))
...
(smart-table db-encoding-table (("version-by" * :pseudos) :users))

(define (version-first uid name)
  (with vname
    (string-append name "-versions")
...
(define (version-next vid)
  (let* ((cur (db-get-field-first vid "version-current" "0"))
    ...
    (with-user #t (db-set-field vid "version-current" (list next)) next)
  ) ;let*
) ;define

(define (version-get-list rid)
  (with-user #t (db-get-field-first rid "version-list" #f))
) ;define

(define (version-get-number rid)
  (with-user #t (db-get-field-first rid "version-nr" "0"))
) ;define
  ...
  (with-user #t
    (with-time :always
      (let* ((date (db-get-field-first rid "date" #f))
             ...
             (by (with-encoding :pseudos (db-get-field-first rid "version-by" #f)))
            ) ;
...
(define (version->file-name rid)
  (with (rid* date name by msg)
    (version-get-info rid)
    (if (not date) name (string-append "time=" date "/" name))
  ) ;with
) ;define

(define (version-get-current vid)
  (with-user #t (db-get-field-first vid "version-current" "0"))
) ;define

(define (version-get-versions vid)
  (with-user #t
    (with-time :always
      (db-search (list (list "version-list" vid) (list :order "version-nr" #t)))
    ) ;with-time
  ) ;with-user
) ;define
...
(define ((readable-by? uid) info)
  (with (rid date name by msg)
    info
    (with-time (+ (string->number date) 5) (db-allow? rid uid "readable"))
  ) ;with
) ;define
...
(tm-service (remote-get-versions rname)
  ...
  (with-remote-context rname
    (let* ((uid (server-get-user envelope))
      ...
      (cond ((not uid)
             ;; FIXME: anonymous access
             (server-error envelope "Error: not logged in")
            ) ;
            ((npair? vl) (server-error envelope "Error: file does not exist"))
            (else (let* ((info (map version-get-info vl)) (filt (list-filter info (readable-by? uid))))
...
(tm-define (server-file-create uid rname doc msg)
  (let* ((fid (file-name->resource (tmfs-cdr rname)))
    ...
    (cond ((not uid) (list :error "Error: not logged in"))
          (fid (list :error "Error: file already exists"))
          ((not did) (list :error "Error: directory does not exist"))
          ((not (db-allow? did uid "writable"))
           (list :error "Error: directory write access required")
          ) ;
          (else (let* ((vid (version-first uid (cAr l)))
...
(tm-service (remote-file-create rname doc msg)
  ...
  (with-remote-context rname
    (if past?
      ...
      (let* ((uid (server-get-user envelope)) (r (server-file-create uid rname doc msg)))
        (if (== (car r) :error)
...
(tm-define (server-file-load uid rname)
  (let* ((rid (file-name->resource (tmfs-cdr rname))) (fname (repository-get rid)))
    (cond ((not uid)
           ;; FIXME: anonymous access
           (list :error "Error: not logged in")
          ) ;
          ((not rid) (list :error "Error: file does not exist"))
          ((not (db-allow? rid uid "readable")) (list :error "Error: read access denied"))
          ((not (url-exists? fname)) (list :error "Error: file not found"))
          (else (with doc (string-load fname) (list :loaded doc)))
    ) ;cond
  ) ;let*
) ;tm-define
...
(tm-service (remote-file-load rname)
  ...
  (with-remote-context rname
    (let* ((uid (server-get-user envelope)) (r (server-file-load uid rname)))
      (if (== (car r) :error)
        (server-error envelope (cadr r))
...
(tm-define (server-file-save uid rname doc msg)
  (let* ((fid (file-name->resource (tmfs-cdr rname)))
    ...
    (cond ((not uid) (list :error "Error: not logged in"))
          ((not fid) (list :error "Error: file does not exist"))
          ...
          ((== (string-load fname) doc)
           ;; no changes need to be saved
           (list :unchanged fid)
          ) ;
          (else (let* ((nr (version-next vid)) (rid (remote-create uid rname vid nr doc msg)))
...
(tm-service (remote-file-save rname doc msg)
  ...
  (with-remote-context rname
    (if past?
      (server-error envelope "Error: cannot modify past")
      (let* ((uid (server-get-user envelope)) (r (server-file-save uid rname doc msg)))
        (if (== (car r) :error)
          (server-error envelope (cadr r))
          (server-return envelope doc)
        ) ;if
...
(tm-define (server-file-remove uid rname)
  ...
  (with rid
    ...
    (cond ((not uid) (list :error "Error: not logged in"))
          ((not rid)
           (list :error (string-append "Error: file '" rname "' does not exist"))
          ) ;
          ((not (db-allow? rid uid "writable"))
           (list :error (string-append "Error: write access denied for '" rname "'"))
          ) ;
          (else (db-remove-entry rid) (list :removed rid))
    ) ;cond
  ) ;with
) ;tm-define
...
(tm-service (remote-file-remove rname)
  ...
  (with-remote-context rname
    (if past?
      (server-error envelope "Error: cannot modify past")
      (let* ((uid (server-get-user envelope)) (r (server-file-remove uid rname)))
        (if (== (car r) :error)
          (server-error envelope (cadr r))
...
(tm-define (server-dir-create uid rname)
  (let* ((fid (file-name->resource (tmfs-cdr rname)))
    ...
    (cond ((not uid) (list :error "Error: not logged in"))
          (fid (list :error "Error: directory already exists"))
          ((not did) (list :error "Error: directory does not exist"))
          ((not (db-allow? did uid "writable"))
           (list :error "Error: directory write access required")
          ) ;
          (else (with rid
                  (db-create-entry (list (list "type" "dir") (list "name" (cAr l)) (list "owner" uid))
                  ) ;db-create-entry
                  (copy-properties did rid inherit-property?)
                  (db-set-field rid "dir" (list did))
                  (list :created rid)
...
(tm-service (remote-dir-create rname)
  ...
  (with-remote-context rname
    (if past?
      (server-error envelope "Error: cannot modify past")
      (let* ((uid (server-get-user envelope)) (r (server-dir-create uid rname)))
        (if (== (car r) :error)
          (server-error envelope (cadr r))
...
(tm-define (server-dir-load uid rname)
  (if (not uid)
    ...
    (let* ((server (car (tmfs->list rname)))
      ...
      (cond ((not rid) (list :error "Error: directory does not exist"))
            ((not (db-allow? rid uid "readable"))
             (list :error "Error: read access required")
            ) ;
            (else (let* ((matches (dir-contents rid))
...
(tm-service (remote-dir-load rname)
  ...
  (with-remote-context rname
    (let* ((uid (server-get-user envelope)) (r (server-dir-load uid rname)))
      (if (== (car r) :error)
        (server-error envelope (cadr r))
        (server-return envelope (cadr r))
      ) ;if
...
(define (server-files-remove uid prefix fids)
  ...
  (if (null? fids)
    (list :removed)
    (with r
      (server-files-remove uid prefix (cdr fids))
      (if (== (car r) :error)
        r
        (with rname
          (string-append prefix "/" (resource->file-name (car fids)))
...
(tm-define (server-dir-remove uid rname recurse?)
  ...
  (with rid
    ...
    (cond ((not uid) (list :error "Error: not logged in"))
          ((not rid)
           (list :error (string-append "Error: directory '" rname "' does not exist"))
          ) ;
          ((not (db-allow? rid uid "writable"))
           (list :error (string-append "Error: write access denied for '" rname "'"))
          ) ;
          (else (with fids
                  (db-search `((,"dir" ,rid) (:order "name" #t)))
                  (cond (recurse? (with r
                                    (server-files-remove uid (tmfs-car rname) fids)
                                    (if (== (car r) :error) r (begin (db-remove-entry rid) (list :removed rid)))
                                  ) ;with
                        ) ;recurse?
                        ((nnull? fids)
                         (list :error (string-append "Error: directory '" rname "' is non empty"))
                        ) ;
                        (else (db-remove-entry rid) (list :removed rid))
                  ) ;cond
                ) ;with
...
(tm-service (remote-dir-remove rname)
  ...
  (with-remote-context rname
    (if past?
      (server-error envelope "Error: cannot modify past")
      (let* ((uid (server-get-user envelope)) (r (server-dir-remove uid rname #t)))
        (if (== (car r) :error)
          (server-error envelope (cadr r))
          (server-return envelope "removed")
        ) ;if
