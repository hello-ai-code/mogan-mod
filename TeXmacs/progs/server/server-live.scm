...
(texmacs-module (server server-live)
  (:use (utils relate live-connection) (server server-tmfs))
) ;texmacs-module
...
(define (live-find lid)
  (with l
    (db-search `(("type" "live") (,"name" ,(live-name lid))))
    (and (nnull? l) (car l))
  ) ;with
) ;define
...
(define (live-load lid)
  (and-let* ((rid (live-find lid))
    ...
    (if (== doc "") '(document "") (convert doc "texmacs-snippet" "texmacs-stree"))
  ) ;and-let*
) ;define
...
(tm-service (remote-list-live)
  ...
  (with (client msg-id)
    ...
    (let* ((uid (server-get-user envelope))
           (l (db-search `(("type" "live") (,"owner" ,uid))))
...
(define (live-applicable? lid client p old-state)
  ...
  (when (debug-get "live")
    (cond ((!= (live-get-remote-state lid client) old-state)
           (display* "  ** Bad remote state "
             (live-get-remote-state lid client)
             " instead of "
             old-state
             "\n"
           ) ;display*
          ...
          ((!= (live-current-state lid) old-state)
           (display* "  ** Bad state "
             (live-current-state lid)
             " instead of "
             old-state
             "\n"
           ) ;display*
          ) ;
          ((not (with doc (live-current-document lid) (patch-applicable? p doc)))
  ...
  (and (== (live-get-remote-state lid client) old-state)
    (== (live-current-state lid) old-state)
    (with doc (live-current-document lid) (patch-applicable? p doc))
  ) ;and
) ;define
  ...
  (with key
    ...
    (when (not (ahash-ref live-waiting key))
      ...
      (let* ((p (live-get-inverse-patch lid state))
        ...
        (when (debug-get "live")
          (display* "Send " client ": " `(live-modify ,lid
                                           ,mods
                                           ,state
                                           ,new-state) "\n")
        ) ;when
...
(define (live-broadcast-one lid client)
  (with state
    (live-get-remote-state lid client)
    (when (and (!= state (live-current-state lid)) (active-client? client))
...
(define (live-broadcast lid)
  (for (client (live-get-connections lid)) (live-broadcast-one lid client))
) ;define
...
(tm-service (live-exists? lid) (server-return envelope (nnot (live-find lid))))
  ...
  (with uid
    ...
    (when (not (live-current-document lid))
      (when (not (live-find lid))
        (let* ((rid (live-new uid lid)) (doc '(document "")))
          (live-save lid doc)
        ) ;let*
      ) ;when
      (with doc (live-load lid) (live-create lid doc))
    ) ;when
    (if (and-with rid (live-find lid) (db-allow? rid uid "readable"))
      (with (client msg-id)
        envelope
        (live-connect lid client)
        (let* ((doc (live-current-document lid)) (state (live-get-remote-state lid client)))
...
(tm-service (live-modify lid mods old-state new-state)
  ...
  (if (and-let* ((rid (live-find lid)) (uid (server-get-user envelope)))
    ...
    (with (client msg-id)
      ...
      (with ok?
        (live-applicable? lid client mods old-state)
        (when (debug-get "live")
          (when (not ok?)
            (display* ">> refuse "
              client
