...
(texmacs-module (server server-db-sync)
  (:use (server server-tmfs) (database db-convert))
) ;texmacs-module
...
(tm-service (remote-db-changes kinds t)
  ...
  (with uid
    ...
    (if (not uid)
      ...
      (with l
        (map (cut db-change-list uid <> t) kinds)
...
(tm-service (remote-db-sync l kinds rtime)
  ...
  (with uid
    ...
    (if (not uid)
      (server-error envelope "Error: not logged in")
      (with changes
        (append-map (cut db-change-list uid <> rtime) kinds)
        (with ok?
          (and (null? changes) (list-and (map (cut db-remote-sync-one uid <>) l)))
