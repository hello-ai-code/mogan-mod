...
(tm-define (style-get-documentation style)
  (with doc (ahash-ref style-synopsis style) (and doc (nnull? doc) (car doc)))
) ;tm-define

(tm-define (style-get-menu-name style)
  (with doc
    (ahash-ref style-menu-name style)
    (if (and doc (nnull? doc)) (car doc) (upcase-first style))
  ) ;with
) ;tm-define
...
(tm-define (embedded-style-list . xpacks)
  ...
  (with l (get-style-list) (list-remove-duplicates (append l xpacks)))
) ;tm-define
...
(define (normalize-style-list l2)
  (with l
    ...
    (if (null? l)
      l
      (cons (car l)
        (normalize-style-list** (normalize-style-list* (cdr l)) (list (car l)))
      ) ;cons
    ) ;if
  ) ;with
) ;define
...
(tm-define (has-no-style?) (null? (get-style-list)))

(tm-define (set-no-style) (:check-mark "v" has-no-style?) (set-style-list '()))

(tm-define (document-empty?)
  (:synopsis "Check if current document is empty")
  (with t (buffer-tree) (tree-empty? t))
) ;tm-define

(tm-define (has-main-style? style)
  (with l (get-style-list) (and (nnull? l) (== (car l) style)))
) ;tm-define

(tm-define (notify-new-style style) (noop))
...
(tm-define (set-main-style style)
  ...
  (if (and (== style "beamer") (not (document-empty?)))
    ...
    (begin
      ...
      (delayed (:idle 1) (notify-new-style style))
    ) ;begin
  ) ;if
) ;tm-define
...
(tm-define (not-has-style-package? pack) (not (has-style-package? pack)))
...
(tm-define (make* l name)
  (if (or (has-style-package? name) (style-has? (string-append name "-package")))
    (make l)
    (begin
      (add-style-package name)
      (delayed (:idle 1) (make l))
    ) ;begin
  ) ;if
) ;tm-define
