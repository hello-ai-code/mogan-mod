...
(texmacs-module (generic generic-doc) (:use (generic generic-menu)))
...
(define (avoid-conflict name prev-names i)
  (with s
    (if (== i 1) name (string-append name (number->string i)))
    (if (nin? s prev-names) s (avoid-conflict name prev-names (+ i 1)))
  ) ;with
) ;define
...
(tm-define (focus-doc-arg-names t i prev-names)
  (if (>= i (tree-arity t))
    '()
    (with s
      (focus-doc-arg-name t i prev-names)
...
(tm-generate (focus-doc-source t)
  ($with s
    ...
    ($with def (get-env-tree s) ($tm-fragment `(inactive* (assign ,s ,def))) $lf)
  ) ;$with
) ;tm-generate
...
(tm-generate (focus-doc-variants t)
  ($let* ((lab (tree-label t)) (l (focus-variants-of t)))
    ($para "The "
      ($markup lab)
      " tag has several "
      ($tmdoc-link "main/editing/man-structured-variants" "structured variants")
      ": "
      ($markup (car l))
      ($for (x (cdr l)) ", " ($markup x))
      ". "
      "You may circle among these variants using the keyboard shortcuts "
      ($shortcut (variant-circulate (focus-tree) #t))
      " and "
      ($shortcut (variant-circulate (focus-tree) #f))
      ". "
      "You may also pick a specific variant from the "
      ($menu "Focus" (focus-tag-name lab))
      " menu or the "
      ($menu (focus-tag-name lab))
      " menu on the focus toolbar."
    ) ;$para
  ) ;$let*
) ;tm-generate

(tm-generate (focus-doc-numbered t)
  ($let* ((lab (tree-label t)) (lab* (symbol-toggle-number lab)))
    ($para ($when (numbered-numbered? t)
             "For most style files, the "
             ($markup lab)
             " environment is numbered. "
             "The environment admits an unnumbered variant "
             ($markup lab*)
             ". "
           ) ;$when
      ($when (not (numbered-numbered? t))
        ...
        "The environment admits a numbered variant "
        ($markup lab*)
        ". "
      ) ;$when
      "You may toggle the numbering using the keyboard shortcut "
      ($shortcut (numbered-toggle (focus-tree)))
      ", the menu entry "
      ($menu "Focus" "Numbered")
      ", or by pressing the "
      ($tmdoc-icon "tm_numbered.xpm")
  ...
  ($let* ((lab (tree-label t)) (lab* (symbol-toggle-alternate lab)))
    ($para ($when (alternate-first? t)
      ...
      ($when (alternate-second? t)
        "The "
        ($markup lab)
        " environment is \x10;unfolded\x11; "
        "and has a folded variant "
        ($markup lab*)
        ". "
        "You may fold the environment using the keyboard shortcut "
        ($shortcut (alternate-toggle (focus-tree)))
        ", the menu entry "
        ($menu "Focus" (alternate-second-name t))
        ", or by pressing the "
      ...
      ($tmdoc-link "main/beamer/man-beamer" "presentation mode")
      "."
    ) ;$para
  ) ;$let*
) ;tm-generate
...
(tm-generate (focus-doc-toggles t)
  ($when (numbered-context? t) (focus-doc-numbered t))
  ($when (alternate-context? t) (focus-doc-alternate t))
) ;tm-generate

(tm-generate (focus-doc-toggles-variants t)
  ($when (focus-has-variants? t) (focus-doc-variants t))
  ($when (focus-has-toggles? t) (focus-doc-toggles t))
) ;tm-generate
...
(tm-generate (focus-doc-parameter par)
  ($with par-doc
    ...
    ($when (not par-doc)
      ($explain `(src-var ,par)
        "A parameter of type "
        (tree-label-type (string->symbol par))
        "."
      ) ;$explain
    ) ;$when
  ) ;$with
...
(tm-generate (focus-doc-move t)
  ($with lab
    (tree-label t)
    ($para "It is sometimes useful to quickly go through all "
      ($markup lab)
      " tags"
    ...
    ($description-long ($describe-item ($inline ($shortcut (kbd-select-if-active traverse-first))
      ...
      ($describe-item ($inline ($shortcut (traverse-next))
                        ", "
      ...
      ($describe-item ($inline ($shortcut (kbd-select-if-active traverse-last))
                        ", "
    ...
    ($para "For more information and further useful shortcuts, "
      "we refer to the section on "
      ($tmdoc-link "main/editing/man-structured-move" "structured cursor movements")
      "."
    ) ;$para
  ) ;$with
) ;tm-generate
...
(tm-generate (focus-doc-insert t)
  ($with lab
    ...
    ($para "The "
      ($markup lab)
      " tag has a variable number of arguments. "
...
(tm-generate (focus-doc-geometry t) "Not yet documented.")
...
(tm-define (focus-doc t)
  ($tmdoc ($tmdoc-title "Contextual help on the \x10;"
            (symbol->string (tree-label t))
            "\x11; tag"
          ) ;$tmdoc-title
    ($when #t ($unfolded-documentation "Usage" (focus-doc-usage t)))
    ($with tagdoc
      (tmdoc-search-tag (tree-label t))
      ($when tagdoc ($unfolded-documentation "Description" tagdoc))
    ) ;$with
    ($when (tree-label-extension? (tree-label t))
      ($unfolded-documentation "Current definition" (focus-doc-source t))
    ) ;$when
    ($when (or (focus-has-variants? t) (focus-has-toggles? t))
      ($unfolded-documentation "Structured variants" (focus-doc-toggles-variants t))
    ) ;$when
    ($when (focus-has-preferences? t)
      ($unfolded-documentation "Style preferences" (focus-doc-preferences t))
    ) ;$when
    ($when (focus-can-move? t)
      ($unfolded-documentation "Structured navigation" (focus-doc-move t))
    ) ;$when
    ($when (focus-can-insert-remove? t)
      ($unfolded-documentation "Structured insert and delete" (focus-doc-insert t))
    ) ;$when
    ($when (< (length (tree-accessible-children t)) (length (tree-children t)))
      ($unfolded-documentation "Hidden arguments" (focus-doc-hidden t))
    ) ;$when
    ($when (focus-has-geometry? t)
      ($unfolded-documentation "Structured geometry" (focus-doc-geometry t))
    ) ;$when
  ) ;$tmdoc
) ;tm-define
