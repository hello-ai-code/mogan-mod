...
(define (with-ref* t var i)
  (and (<= i (- (tree-arity t) 3))
    (or (and (tm-equal? (tree-ref t i) var) (tree-ref t (+ i 1)))
      (with-ref* t var (+ i 2))
    ) ;or
  ) ;and
) ;define

(tm-define (with-ref t var) (and (tm-is? t 'with) (with-ref* t var 0)))
...
(tm-define (with-merge t)
  (when (and (tree-is? t 'with) (tree-is? t :up 'with))
    (let* ((p (tree-up t)) (c (map tree-copy (cDr (tree-children t)))))
...
(tm-define (test-env? var val) (== (get-env var) val))
...
(define (add-with l t)
  (if (tm-is? t 'with)
    (with c (tm-children t) `(with ,@(cDr c) ,(add-with l (cAr c))))
    `(with ,@l ,t)
  ) ;if
) ;define
...
(define (add-with-path l t)
  (if (tm-is? t 'with)
    (with c (tm-children t) (cons (length (cDr c)) (add-with-path l (cAr c))))
    (cons (length l) (path-end t '()))
  ) ;if
) ;define

(define (get-cars l)
  (if (or (null? l) (null? (cdr l))) (list) (cons (car l) (get-cars (cddr l))))
) ;define

(define (get-cadrs l)
  (if (or (null? l) (null? (cdr l))) (list) (cons (cadr l) (get-cadrs (cddr l))))
) ;define
...
(tm-define (make-line-with var val)
  ...
  (if (and (selection-active-table?) #f)
    ;; FIXME: does not work yet
    (make-with var val)
    (begin
      (if (not (selection-active-normal?)) (select-line))
      (make-with var val)
      (insert-return)
      (remove-text #f)
    ) ;begin
...
(tm-define (make-multi-line-with l)
  (when (nnull? l)
    (cond ((selection-active-table?) (make-multi-with l))
          (else (when (not (selection-active-normal?))
                  (select-line)
                ) ;when
            (make-multi-with l)
            (insert-return)
...
(define (with-like-search t)
  (if (with-like? t)
    ...
    (and (or (tree-atomic? t) (tree-in? t '(concat document)))
      (and-with p (tree-ref t :up) (with-like-search p))
    ) ;and
  ) ;if
) ;define
...
(tm-define (make-with-like w)
  (cond ((func? w 'with 3) (make-with (cadr w) (caddr w)))
...
(tm-define (with-like-selection-parent-target t u w)
  (and u (with-like? u) (with-same-type? u w) (tm-equal? (tree-ref u :last) t) u)
) ;tm-define

(tm-define (with-like-selection-target t w)
  (cond ((and t (with-like? t) (with-same-type? t w)) t)
...
(define (with-like-attrs t)
  (let loop
    ...
    (if (>= i (- (tree-arity t) 1))
      (reverse r)
      (loop (+ i 2) (cons (tree-ref t (+ i 1)) (cons (tree-ref t i) r)))
    ) ;if
  ) ;let
) ;define

(define (with-like-build-body attrs body)
  (if (null? attrs) body `(with ,@attrs ,body))
) ;define

(define (with-like-string-offset s n)
  (let loop
    ((i 0) (pos 0))
    (if (>= i n) pos (loop (+ i 1) (string-next s pos)))
  ) ;let
) ;define

(define (with-like-compact l)
  (cond ((null? l) '())
        ((or (not (car l)) (== (car l) "")) (with-like-compact (cdr l)))
...
(tm-define (with-like-partial-toggle-result t var start end)
  (with data
    (with-like-partial-toggle-data t var start end)
    (with parts*
      (car data)
      (cond ((null? parts*) "")
  ...
  (cond ((== p1 p2) '())
        ...
        ((< (car p1) (car p2))
         (let* ((i1 (car p1))
                ...
                (t1 (tm-ref t i1))
                (t2 (tm-ref t i2))
                (l1 (selection-subtrees t1 (cdr p1) (list (tree-right-index t1))))
...
(tm-define (with-like-selection-trees)
  (let* ((p1 (selection-get-start)) (p2 (selection-get-end)))
    (and p1 p2 (selection-subtrees (root-tree) p1 p2))
  ) ;let*
) ;tm-define

(define (with-like-wrap-node w t)
  (if (func? w 'with 3) `(with ,(cadr w) ,(caddr w) ,t) `(,(car w) ,t))
) ;define
...
(define (with-like-strip-target-from-node t w)
  (if (with-like-node-has-target? t w) (with-like-node-without t w) t)
) ;define

(define (with-like-uniform-selection-tree? t)
  (and t (tree-compound? t) (tree-in? t '(document concat)) (> (tree-arity t) 1))
) ;define
  ...
  (with t
    ...
    (when t
      (let* ((replacement (with-like-uniform-toggle-result t w))
        ...
        (clipboard-cut "nowhere")
        (insert-go-to replacement (with-like-uniform-toggle-path replacement*))
      ) ;let*
    ) ;when
  ) ;with
) ;tm-define
...
(tm-define (toggle-with-like-partial-remove t w)
  (let* ((start (selection-get-start))
    ...
    (if (null? (cdr parts*)) (tree-go-to t :end) (tree-go-to t mid-index :end))
  ) ;let*
) ;tm-define
...
(tm-define (toggle-underlined) (toggle-with-like '(underline "") #f))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Predicates for toolbar button state synchronization
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(tm-define (inside-bold?) (== (get-env "font-series") "bold"))
...
(tm-define (customizable-context? t) (nnull? (customizable-parameters t)))

(tm-define (customizable-parameters t) (list))
  ...
  (and (tree? t)
    (with hit
      ...
      (cond (hit (tree-ref t (+ hit 1)))
            ((and (tree-up t) (tree-up (tree-up t))) (tree-with-get (tree-up t) var))
            (else #f)
      ) ;cond
    ) ;with
  ) ;and
...
(tm-define (make-page-break) (make 'page-break) (insert-return))

(tm-define (make-new-page) (make 'new-page) (insert-return))

(tm-define (make-new-dpage) (make 'new-dpage) (insert-return))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Multiple text flows
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(tm-define (focus-has-preferences? t) (:require (tree-in? t '(float))) #t)
...
(tm-define (customizable-parameters t)
  ...
  (list (list "gnawed-strength" "Strength") (list "gnawed-frequency" "Frequency"))
) ;tm-define
...
(tm-define (customizable-parameters t)
  ...
  (list (list "blur-pen-dx" "Pen dx") (list "blur-pen-dy" "Pen dy"))
) ;tm-define
...
(tm-define (customizable-parameters t)
  ...
  (list (list "outline-pen-dx" "Pen dx") (list "outline-pen-dy" "Pen dy"))
) ;tm-define
...
(tm-define (customizable-parameters t)
  ...
  (list (list "thicken-pen-dx" "Pen dx") (list "thicken-pen-dy" "Pen dy"))
) ;tm-define
...
(tm-define (customizable-parameters t)
  ...
  (list (list "erode-pen-dx" "Pen dx") (list "erode-pen-dy" "Pen dy"))
) ;tm-define
...
(tm-define (parameter-choice-list var)
  (:require (or (string-ends? var "-wavelen-x") (string-ends? var "-wavelen-y")))
...
(tm-define (parameter-choice-list var)
  (:require (in? var
              (list "distort-radius-x" "distort-radius-y" "gnaw-radius-x" "gnaw-radius-y")
            ) ;in?
  ) ;:require
...
(tm-define (pen-effect-context? t) (tree-in? t (pen-effect-tag-list)))
