...
(texmacs-module (generic format-tools)
  (:use (generic format-widgets) (utils library cursor))
) ;texmacs-module
...
(tm-define (window-get-init win var) (window-get-env win var :global))

(tm-define (window-set-init win var val) (window-set-env win var val :global))
  ...
  (refreshable "paragraph tool"
    (aligned (item (text "Alignment:")
               (enum (window-set-env win "par-mode" answer mode)
                 ...
                 (window-get-env win "par-mode" mode)
                 "10em"
               ) ;enum
             ) ;item
      ...
      (item (text "Interparagraph space:")
        (enum (window-set-env win "par-par-sep" answer mode)
          ...
          (window-get-env win "par-par-sep" mode)
          "10em"
        ) ;enum
      ) ;item
      (item ====== ======)
      (item (text "Number of columns:")
        (enum (begin
          ...
          '("1" "2" "3" "4" "5" "6")
          (window-get-env win "par-columns" mode)
          "10em"
        ) ;enum
      ) ;item
      ...
      (assuming (global-ref win mode :advanced)
        ...
        (item (text "Line breaking:")
          (enum (window-set-env win "par-hyphen" answer mode)
            '("normal" "professional")
            (window-get-env win "par-hyphen" mode)
            "10em"
          ) ;enum
        ) ;item
        (item ====== ======)
        (item (text "Extra interline space:")
          (enum (window-set-env win "par-line-sep" answer mode)
            ...
            (window-get-env win "par-line-sep" mode)
            "10em"
          ) ;enum
        ) ;item
        (item (text "Minimal line separation:")
          (enum (window-set-env win "par-ver-sep" answer mode)
            ...
            (window-get-env win "par-ver-sep" mode)
            "10em"
          ) ;enum
        ) ;item
        (item (text "Horizontal collapse distance:")
          (enum (window-set-env win "par-hor-sep" answer mode)
            ...
            (window-get-env win "par-hor-sep" mode)
            "10em"
          ) ;enum
        ) ;item
        (item ====== ======)
        ...
        (item (text "CJK spacing:")
          (enum (window-set-env win "par-spacing" answer mode)
            ...
            (window-get-env win "par-spacing" mode)
            "10em"
          ) ;enum
        ) ;item
        (item ====== ======)
        (item (text "Intercharacter stretching:")
          (enum (window-set-env win "par-kerning-stretch" answer mode)
            ...
            (window-get-env win "par-kerning-stretch" mode)
            "10em"
          ) ;enum
        ) ;item
        (item (text "Intercharacter compression:")
          (enum (window-set-env win "par-kerning-reduce" answer mode)
            ...
            (window-get-env win "par-kerning-reduce" mode)
            "10em"
          ) ;enum
        ) ;item
        (item (text "Character expansion:")
          (enum (window-set-env win "par-expansion" answer mode)
            ...
            (window-get-env win "par-expansion" mode)
            "10em"
          ) ;enum
        ) ;item
        (item (text "Character contraction:")
          (enum (window-set-env win "par-contraction" answer mode)
            ...
            (window-get-env win "par-contraction" mode)
            "10em"
          ) ;enum
        ) ;item
        (item ====== ======)
        (item (text "Use margin kerning:")
          (toggle (window-set-env win "par-kerning-margin" (if answer "true" "false") mode)
            (== (window-get-env win "par-kerning-margin" mode) "true")
          ) ;toggle
        ) ;item
      ) ;assuming
    ) ;aligned
...
(tm-tool* (format-page-tool win u st t)
  ...
  (dynamic ((page-formatter u st t #f) (tool-quit 'format-page-tool #f win)))
) ;tm-tool*
...
(define (window-get-page-rendering win)
  (or (with-window win (get-init-page-rendering)) "?")
) ;define

(define (window-set-page-rendering win s)
  (with-window win (init-page-rendering s))
) ;define
    ...
    (aligned (item (text "Page rendering:")
      ...
      (item (text "Page type:")
        (enum (begin
          ...
          (page-type-pretty (window-get-init win "page-type"))
          "10em"
        ) ;enum
      ) ;item
      (item (when (window-user-page-size? win)
              (text "Page width:")
            ) ;when
        (when (window-user-page-size? win)
          (enum (window-set-init win "page-width" answer)
            (list (window-get-init win "page-width") "")
            (window-get-init win "page-width")
            "10em"
          ) ;enum
        ) ;when
      ) ;item
      (item (when (window-user-page-size? win)
        ...
        (when (window-user-page-size? win)
          (enum (window-set-init win "page-height" answer)
            ...
            (window-get-init win "page-height")
            "10em"
          ) ;enum
        ) ;when
      ) ;item
      (item (text "Orientation:")
        (enum (window-set-init win "page-orientation" answer)
          '("portrait" "landscape")
          (window-get-init win "page-orientation")
          "10em"
        ) ;enum
      ) ;item
      (item (text "First page:")
        (enum (window-set-init win "page-first" answer)
          ...
          (window-get-init win "page-first")
          "10em"
        ) ;enum
      ) ;item
      (item (text "Crop marks:")
        (enum (window-set-init win "page-crop-marks" (encode-crop-marks answer))
          '("none" "a3" "a4" "letter")
        ...
        (aligned (item (text "Text width:")
          ...
          (item (text "Odd page shift:")
            (input (window-set-init win "page-odd-shift" answer)
              "string"
              (list (window-get-init win "page-odd-shift"))
              "6em"
            ) ;input
          ) ;item
          (item (text "Even page shift:")
            (input (window-set-init win "page-even-shift" answer)
              "string"
              (list (window-get-init win "page-even-shift"))
              "6em"
            ) ;input
          ) ;item
          (item (text "Top:")
  ...
  (refreshable "page-breaking-settings"
    ...
    (aligned (item (text "Page breaking algorithm:")
               (enum (window-set-init win "page-breaking" answer)
                 '("sloppy" "professional")
                 (window-get-init win "page-breaking")
                 "10em"
               ) ;enum
             ) ;item
      ...
      (item (text "Vertical space stretchability:")
        (enum (window-set-init win "page-flexibility" answer)
          ...
          (window-get-init win "page-flexibility")
          "10em"
        ) ;enum
      ) ;item
    ) ;aligned
...
(define (apply-headers-settings win u)
  (with l
    ...
    (for (var header-parameters)
      (and-with doc
        (get-field-contents (header-buffer win var))
...
(define (editing-headers? win)
  (in? (current-buffer)
    (map (lambda (var) (header-buffer win var)) header-parameters)
  ) ;in?
) ;define
...
(define (synchronize win var)
  (let* ((key (list "header" win var))
    ...
    (when (and old (!= val old))
      (delayed (:idle 1) (buffer-set-body (header-buffer win var) `(document ,val)))
    ) ;when
    (when (!= val old)
...
(tm-widget (page-headers-tool win)
  (let* ((u (window->buffer win)) (style (embedded-style-list "macro-editor")))
    (for (var header-parameters)
      ======
      (text (eval (parameter-name var)))
      ===
      (cached (string-append "edit-" var)
        (synchronize win var)
        (resize "330px"
          "60px"
  ...
  ===
  (division "plain"
    (hlist ("Tab" (when (editing-headers? win) (make-htab "5mm")))
...
(tm-tool* (document-page-tool win)
  ...
  (section-tabs "document-page-tabs"
    win
    (section-tab "Format" === (centered (dynamic (page-format-tool win))))
    (section-tab "Breaking" === (centered (dynamic (page-breaking-tool win))))
    (section-tab "Margins" === (centered (dynamic (page-margins-tool win))))
    (section-tab "Headers" === (centered (dynamic (page-headers-tool win))))
  ) ;section-tabs
) ;tm-tool*
...
(tm-widget (texmacs-side-tool win tool . opts)
  ...
  (division "title" (text "Page headers and footers"))
