...
(texmacs-module (generic document-doc)
  (:use (generic generic-doc) (generic document-menu))
) ;texmacs-module
...
(tm-define (focus-doc-top-generalities t)
  (let* ((st (get-style-list)) (style (if (null? st) "none" (car st))))
    ($block ($para "The "
              ($tmdoc-link "main/text/man-structure" "current focus")
              ...
              (get-init "page-type")
              ") and font size ("
              (get-init "font-base-size")
              "pt). "
              "The toolbar also indicates the current section "
              "at the position of your cursor."
            ) ;$para

      ($para "The above properties and the current section can also "
        ...
        "By clicking on the "
        ($tmdoc-icon "tm_add.xpm")
        " icon "
        "after the document style, you may select additional style packages."
      ) ;$para

      ($para "Similarly, when clicking on the current section, "
...
(tm-define (focus-doc t)
  ...
  (let* ((style (get-style-list))
    ...
    ($tmdoc ($tmdoc-title "Top level contextual help")
      ($when #t
        ($unfolded-documentation "Top level editing" (focus-doc-top-generalities t))
      ) ;$when
      ($when #t
        ...
        ($unfolded-documentation "Other active style customizations"
          ($for (pack (cdr style))
            ($with packdoc
              ...
              ($when (not packdoc)
                ($explain `(tmpackage ,pack) "No documentation available.")
              ) ;$when
            ) ;$with
          ) ;$for
        ) ;$unfolded-documentation
