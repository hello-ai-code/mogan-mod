...
(tm-define (style-category-precedes? x y)
  (:require (and (== x :beamer-theme)
              (in? y (list :beamer-title-theme :theorem-decorations))
            ) ;and
  ) ;:require
  #t
) ;tm-define
...
(tm-define (style-includes? x y)
  (:require (and (== x "beamer") (in? y (list "title-bar" "bluish" "framed-session")))
  ) ;:require
  #t
) ;tm-define
...
(tm-define (screens-buffer?)
  (with t (buffer-tree) (and (tree-is? t 'document) (tree-is? t :last 'screens)))
) ;tm-define

(tm-define (slideshow-buffer?)
  (with t
    (buffer-tree)
    (and (tree-is? t 'document) (tree-is? t :last 'slideshow))
  ) ;with
) ;tm-define
...
(tm-define (dynamic-extremal t forwards?)
  (and-with p (tree-outer t) (dynamic-extremal p forwards?))
) ;tm-define

(tm-define (dynamic-incremental t forwards?)
  (and-with p (tree-outer t) (dynamic-incremental p forwards?))
) ;tm-define

(tm-define (dynamic-first) (dynamic-extremal (focus-tree) #f))
(tm-define (dynamic-last) (dynamic-extremal (focus-tree) #t))
(tm-define (dynamic-previous) (dynamic-incremental (focus-tree) #f))
(tm-define (dynamic-next) (dynamic-incremental (focus-tree) #t))
...
(tm-define (toggle-context? t) (toggle-tag? (tree-label t)))

(tm-define (toggle-first-context? t) (toggle-first-tag? (tree-label t)))

(tm-define (toggle-second-context? t) (toggle-second-tag? (tree-label t)))
...
(tm-define (dynamic-extremal t forwards?)
  (:require (toggle-context? t))
  (with action (if forwards? alternate-unfold alternate-fold) (action t))
) ;tm-define
...
(tm-define (get-hidden-tag t) 'hidden)

(tm-define (get-hidden-tag t) (:require (unroll-context? t)) 'hidden*)

(define (switch-ref t i)
  (and t (>= i 0) (< i (tree-arity t)) (not (hidden-context? (tree-ref t i))))
) ;define
...
(define (switch-last-visible t)
  (with v
    (- (tree-arity t) 1)
    (while (and (>= v 0) (not (switch-ref t v))) (set! v (- v 1)))
    v
  ) ;with
) ;define
...
(tm-define (switch-context? t) (switch-tag? (tree-label t)))
...
(tm-define (switch-remove-at t i)
  ...
  (when (and (>= i 0) (< i (tree-arity t)) (> (tree-arity t) 1))
    (let* ((v (switch-index t :visible)) (l (- (tree-arity t) 2)))
...
(tm-define (alternative-context? t) (alternative-tag? (tree-label t)))
...
(tm-define (dynamic-extremal t forwards?)
  ...
  (if forwards? (switch-to t :last :end) (switch-to t :first :start))
) ;tm-define

(tm-define (dynamic-incremental t forwards?)
  (:require (switch-context? t))
  (if forwards? (switch-to t :next :start) (switch-to t :previous :end))
) ;tm-define
...
(tm-define (structured-remove-horizontal t forwards?)
  (:require (switch-context? t))
  (with i
    (if forwards? :current :var-previous)
    (set! i (switch-index t i))
...
(tm-define (tree-show-hidden t)
  ...
  (with i
    (tree-down-index t)
    (if (hidden-context? (tree-ref t i)) (switch-select t i))
  ) ;with
) ;tm-define
...
(tm-define (overlays-context? t)
  (and (overlays-tag? (tree-label t)) (== (tree-arity t) 3))
) ;tm-define

(tm-define (overlay-context? t) (overlay-tag? (tree-label t)))
...
(define (overlays-animate t old new)
  (when (tree-compound? t)
    (when (and (overlay-visible? t new) (not (overlay-visible? t old)) (> new old))
      (players-set-elapsed t 0.0)
    ) ;when
    (for-each (cut overlays-animate <> old new) (tree-children t))
  ) ;when
) ;define
...
(tm-define (overlays-switch-to t i)
  (when (overlays-context? t)
    (let* ((tot (tree->number (tree-ref t 1))) (nxt (min (max i 1) tot)))
      (overlays-set t nxt)
    ) ;let*
  ) ;when
) ;tm-define
...
(tm-define (overlay-current t)
  (and-with p (tree-search-upwards t overlays-context?) (overlays-current p))
) ;tm-define

(tm-define (overlay-arity t)
  (and-with p (tree-search-upwards t overlays-context?) (overlays-arity p))
) ;tm-define
...
(define (overlay-visible-sub? t i)
  (cond ((tree-atomic? t) #t)
        ((or (overlay-context? t) (tree-in? t '(with anim-static anim-morph)))
         (overlay-visible? t i)
        ) ;
        (else (forall? (cut overlay-visible-sub? <> i) (tree-children t)))
  ) ;cond
) ;define

(define (overlay-frame-visible? f i)
  (and (tree-func? f 'tuple 2) (overlay-visible-sub? (tree-ref f 1) i))
) ;define
...
(tm-define (overlay-visible? t i)
  (or (and (overlay-context? t)
    ...
    (and (tree-in? t '(anim-static anim-dynamic))
      (tree-is? t 0 'morph)
      (list-and (map (cut overlay-frame-visible? <> i) (tree-children (tree-ref t 0)))
      ) ;list-and
    ) ;and
  ) ;or
) ;tm-define
...
(tm-define (dynamic-extremal t forwards?)
  ...
  (let* ((tot (tree->number (tree-ref t 1))) (nxt (if forwards? tot 1)))
    (overlays-set t nxt)
...
(define (overlay-renumber-sub t p cur tot delta)
  (let* ((old (tree->number t))
    ...
    (cond ((or (>= new cur) (> delta 0)) (tree-set t (number->string new)))
          ((tree-in? p '(overlay-from alternate-from)) (tree-set t (number->string cur)))
          ((tree-in? p '(overlay-until alternate-until))
           (tree-set t (number->string new))
          ) ;
          ((tree-in? p '(overlay-this alternate-this)) (tree-set t "0"))
          ((tree-in? p '(overlay-other alternate-other)) (tree-set t "0"))
          (else
...
(define (overlay-renumber t cur tot delta)
  (cond ((tree-atomic? t) (noop))
        ...
        ((overlay-context? t)
         (for (c (cdr (tree-children t))) (overlay-renumber c cur tot delta))
         (overlay-renumber-sub (tree-ref t 0) t cur tot delta)
        ) ;
        (else (for-each (cut overlay-renumber <> cur tot delta) (tree-children t)))
  ) ;cond
) ;define

(tm-define (structured-horizontal? t) (:require (overlays-context? t)) #t)
...
(define (fold-get-environments-sub t)
  (if (tree-compound? t) (for-each fold-get-environments-sub (tree-children t)))
  (if (toggle-context? t) (fold-add-environment t (toggle-first-context? t)))
) ;define
...
(tm-define (fold-get-environments-in-buffer)
  ...
  (with envl
    (map car (ahash-table->list fold-environments))
    (values (list-sort (map symbol->string envl) string<=?)
...
(define (fold-matching-env? t tag)
  (cond ((tree-in? t '(summarized-algorithm detailed-algorithm)) (== tag 'algorithm))
        ((toggle-first-context? t) (fold-matching-env? (tree-ref t 1) tag))
        ((toggle-second-context? t) (fold-matching-env? (tree-ref t 1) tag))
        ((tm-func? t 'document 1) (fold-matching-env? (tree-ref t 0) tag))
        ((or (tree-atomic? t) (tree-in? t '(document concat))) (== tag 'text))
        ((== tag 'proof) (tree-in? t '(proof render-proof)))
        (else (tree-is? t tag))
  ) ;cond
) ;define
...
(define (dynamic-operate-sub t mode)
  ...
  (cond ((and (converter-context? t) (toggle-first-context? t))
        ...
        ((toggle-first-context? t)
         (cond ((== mode :var-last) (tree-insert-node! t 0 '(traversed)))
               ((in? mode '(:unfold :expand :var-expand :last)) (alternate-toggle t))
               ((and (pair? mode) (== (car mode) :unfold) (fold-matching-env? t (cadr mode)))
                (alternate-toggle t)
               ) ;
         ) ;cond
        ) ;
        ((toggle-second-context? t)
         (cond ((== mode :var-last) (alternate-toggle t) (tree-insert-node! t 0 '(traversed)))
               ((in? mode '(:fold :compress :var-compress :first)) (alternate-toggle t))
               ((and (pair? mode) (== (car mode) :fold) (fold-matching-env? t (cadr mode)))
                (alternate-toggle t)
               ) ;
         ) ;cond
        ) ;
        ((and (== mode :expand) (switch-context? t)) (switch-set-range t 0 :last #t))
        ((and (== mode :compress) (switch-context? t))
         (switch-set t 0 #t)
         (switch-set-range t 1 :last #f)
        ) ;
        ((alternative-context? t)
         (cond ((== mode :first) (switch-set-range t 1 :last #f) (switch-set t 0 #t))
               ((== mode :last) (switch-set-range t 0 :last #f) (switch-set t :last #t))
               ((== mode :var-expand)
        ...
        ((unroll-context? t)
         (cond ((== mode :var-last)
               ...
               ((== mode :first) (switch-set-range t 1 :last #f) (switch-set t 0 #t))
               ((== mode :last) (switch-set-range t 0 :last #t))
               ((== mode :var-expand) (switch-set-range t 0 :last #t))
               ((== mode :var-compress) (switch-set t 0 #t) (switch-set-range t 1 :last #f))
         ) ;cond
        ) ;
        ((expanded-context? t)
        ...
        ((overlays-context? t)
         (cond ((== mode :first) (dynamic-extremal t #f))
               ((== mode :last) (dynamic-extremal t #t))
         ) ;cond
        ) ;
  ) ;cond
) ;define
...
(define (dynamic-first-alternative-list l)
  (if (null? l)
    #f
    (with r
      (dynamic-first-alternative (car l))
...
(define (dynamic-alternative-keep-other slide)
  (and-with t
    (dynamic-first-alternative slide)
    (cond ((alternative-context? t) (tree-remove t 0 1))
          ((tree-is? t 'overlays-range)
           (with nr (tree->number (tree-ref t 0)) (tree-set t 0 (number->string (+ nr 1))))
          ) ;
    ) ;cond
  ) ;and-with
) ;define
...
(define (dynamic-make-slide t)
  ...
  (if (and (tm-func? t 'slide 1) (dynamic-first-alternative t))
    (let* ((p (tree-up t)) (i (tree-index t)))
      ...
      (let* ((v (tree-ref p i)) (w (tree-ref p (+ i 1))))
...
(define (keep-shown! t)
  (if (alternative-context? t)
    (map
      ...
      (lambda (i) (if (tm-func? (tree-ref t i) 'hidden) (tree-remove! t i 1)))
...
(define (create-slide scr)
  (if (not (tree? scr))
    (tree 'slide '(document ""))
    ;; just in case
    (with t (tree-copy scr) (keep-shown! t) (tree-assign-node! t 'slide) t)
  ) ;if
) ;define
...
(define (process-screen scr)
  (cons (create-slide scr)
    (begin
      ...
      (if (tm-func? scr 'hidden) '() (process-screen scr))
    ) ;begin
  ) ;cons
) ;define
...
(define (screens->slides t)
  (if (not (tm-func? t 'screens))
    (tree 'document "")
    (with f
      (lambda (scr) (list->tree 'document (process-screen scr)))
      ;; (system-wait "Generating slides" "please wait") ;crashes if printing
      ;; Insert fake screen at the end
      (tree-insert! t (tree-arity t) (list (tree 'hidden '(document ""))))
...
(tm-define (dynamic-filter-buffer mode) (dynamic-filter (buffer-tree) mode))
...
(define (dynamic-traverse-unfolded t mode)
  (cond ((== mode :var-next)
        ...
        ((in? mode '(:previous :var-previous))
         (with last-mode
           (if (== mode :previous) :last :var-last)
           (alternate-toggle t)
...
(define (dynamic-traverse-list l mode)
  (and (nnull? l)
    (or (dynamic-traverse (car l) mode) (dynamic-traverse-list (cdr l) mode))
  ) ;and
) ;define

(tm-define (dynamic-traverse t mode)
  (cond ((tree-atomic? t) #f)
        ((tree-is? t 'traversed) (dynamic-traverse-traversed t mode))
        ((tree-is? t 'fold-back)
        ...
        ((toggle-first-context? t)
         (or (dynamic-traverse (tree-ref t 0) mode) (dynamic-traverse-folded t mode))
        ) ;
        ((toggle-second-context? t)
         (or (dynamic-traverse (tree-ref t 1) mode) (dynamic-traverse-unfolded t mode))
        ) ;
        ((or (alternative-context? t) (unroll-context? t))
         (let* ((i (switch-last-visible t)) (l (- (tree-arity t) 1)))
           (or (dynamic-traverse (tree-ref t i) mode) (dynamic-traverse-switch t i l mode))
         ) ;let*
        ) ;
...
(tm-define (dynamic-traverse-buffer mode) (dynamic-traverse (buffer-tree) mode))
...
(tm-define (make-switch-list switch-tag list-tag)
  (with flag?
    (and (selection-active-non-small?) (in? list-tag (description-tag-list)))
    (wrap-selection-any (make list-tag)
      (make-switch switch-tag)
...
(tm-define (screens-context? t)
  (or (tree-is? t 'screens) (slideshow-context? t))
) ;tm-define

(tm-define (nr-pages)
  (:require (tree-innermost screens-context?))
  (with t*
    (tree-innermost screens-context?)
    (with t (if (slideshow-context? t*) (tree-ref t* 0) t*) (tree-arity t))
  ) ;with
) ;tm-define

(tm-define (screens-switch-to which)
  (:secure #t)
  (and-with t (tree-innermost 'screens) (switch-to t which :start))
) ;tm-define

(tm-define (screens-show-all)
  (and-with t
    (tree-innermost 'screens)
...
(tm-define (slide-get-switch t) (if (slideshow-context? t) (tree-ref t 0) t))

(tm-define (slide-get-document t)
  (cond ((not (tree? t)) #f)
        ((slideshow-context? t) (slide-get-document (tree-ref t 0 :down :down)))
        ((screens-context? t) (slide-get-document (tree-ref t :down :down)))
        ((tree-in? t '(shown hidden screen slide)) (slide-get-document (tree-ref t 0)))
        ((tree-in? t '(with with-screen-color)) (slide-get-document (tree-ref t :last)))
...
(define (slide-get-bg-color-bis t)
  (cond ((tree-is? t :up 'with) (slide-get-bg-color-bis (tree-up t)))
        ((tree-is? t :up 'with-screen-color) (tree-ref t :up 0))
        (else "")
  ) ;cond
) ;define
...
(tm-define (slide-get-bg-color)
  (and-with t
    (tree-innermost screens-context?)
    (with u (slide-get-document t) (slide-get-bg-color-bis u))
  ) ;and-with
) ;tm-define

(define (slide-set-bg-color-bis t col)
  (cond ((tree-is? t :up 'with) (slide-set-bg-color-bis (tree-up t) col))
        ((tree-is? t :up 'with-screen-color)
         (if (== col "") (tree-set t :up t) (tree-set t :up 0 col))
        ) ;
        ((!= col "") (tree-set t `(with-screen-color ,col ,t)))
  ) ;cond
) ;define
...
(tm-define (slide-set-bg-color col)
  (and-with t
    (tree-innermost screens-context?)
    (with u (slide-get-document t) (slide-set-bg-color-bis u col))
  ) ;and-with
) ;tm-define
...
(tm-define (structured-horizontal? t) (:require (slideshow-context? t)) #t)

(tm-define (focus-can-insert? t) (:require (slideshow-context? t)) #t)

(tm-define (focus-can-remove? t) (:require (slideshow-context? t)) #t)

(tm-define (structured-insert-horizontal t forwards?)
  (:require (slideshow-context? t))
  (set! t (tree-ref t 0))
  (with i
    (+ (tree-index (tree-down t)) (if forwards? 1 0))
...
(tm-define (structured-remove-horizontal t forwards?)
  (:require (slideshow-context? t))
  (set! t (tree-ref t 0))
  (let* ((i (- (tree-index (tree-down t)) (if forwards? 0 1))) (n (tree-arity t)))
    (when (and (>= i 0) (> n 1))
      (tree-remove! t i 1)
...
(tm-define (clipboard-cut which)
  (:require (and (inside? 'slideshow) (slide-range)))
  (with (i1 i2)
    (slide-range)
    (with t
      (tree-innermost slideshow-context?)
...
(tm-define (clipboard-paste which)
  (:require (and (inside? 'slideshow) (slides? (clipboard-get which))))
...
(tm-define (in-screens-graphics?) (and (inside? 'gr-screen) (in-graphics?)))

(define (innermost-gr-screen)
  (and-with t
    (tree-innermost 'screens)
    (and-with s
      (slide-get-document t)
...
(define (innermost-graphics-screen)
  (and-with t
    (innermost-gr-screen)
    (set! t (tree-ref t 0))
    (while (or (tree-func? t 'document 1) (tree-func? t 'gr-overlays 3) (tree-is? t 'with))
      (set! t (tm-ref t :last))
    ) ;while
...
(tm-define (make-gr-overlays forwards?)
  (and-with t
    (tree-innermost 'graphics #t)
    (while (tree-is? t :up 'with) (set! t (tree-up t)))
...
(tm-define (move-slide-up)
  ...
  (with-innermost s
    ...
    (if (tree-ref s :previous)
      (let* ((p (tree->path s))
             ;; path to the current slide
             (i (last p))
             ;; index of the slide
             (t (tree-ref s :up))
            ) ;
        ;; parent node of the slide
...
(tm-define (move-slide-down) (noop))
