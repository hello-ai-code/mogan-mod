...
(tm-define (script-evaluable?)
  (or (selection-active-any?) (nnot (tree-innermost formula-context? #t)))
) ;tm-define
...
(tm-define (script-feed lan ses in out opts)
  ...
  (with ok?
    (or (scripts-defined? lan) (session-defined? lan))
    (when (not ok?)
      (with m
        `(concat ,"Error: " (verbatim ,lan) ," is not a scripting language")
              ...
              (when (== check out)
                (if (in? :replace opts)
                  ...
                  (with-cursor (tree->path check :end)
                    ...
                    (if (in? :declaration opts) (set! r in))
                    (insert r)
                  ) ;with-cursor
                ) ;if
              ) ;when
...
(tm-define (kbd-enter t forwards?)
  (:require (and (tree-is? t 'script-eval) (xor (not forwards?) (tree-is? t 1 'document)))
  ) ;:require
...
(tm-define (script-eval) (script-modified-eval noop))

(tm-define (script-approx)
  (and-with cmd
    (plugin-approx-command-ref (get-env "prog-scripts"))
    (with fun (lambda () (insert-function cmd)) (script-modified-eval fun :approx))
  ) ;and-with
) ;tm-define
...
(tm-define (widget->script cas-var id)
  (with cmd
    `(concat ,cas-var ,":" ,(tree->stree (widget-ref id)))
...
(define (script-widget-eval id in . opts)
  (let* ((lan (get-env "prog-scripts"))
    ...
    (when (scripts-defined? lan)
      (with return
        (lambda (r) (widget-with prefix (widget-set! id r)))
...
(tm-define (alternate-toggle t) (:require (plot-context? t)) (activate-plot t))
...
(tm-define (notify-graphics-extents id x1 y1 x2 y2)
  (when (mathemagix-alive?)
    (with msg
      (string-append "notify_graphics_extents (\""
        id
        "\", "
        (number->string x1)
        ", "
...
(tm-define (kbd-enter t forwards?)
  ...
  (let* ((format (string-append (tree->string (tree-ref t 0)) "-snippet"))
    ...
    (if (and (== format "latex-snippet") (tm-equal? mode "math"))
      (with c
        (convert (string-append "$" in "$") format "texmacs-tree")
        (if (tm-func? c 'math 1)
          (insert (tm-ref c 0))
...
(tm-define (kbd-remove t forwards?)
  (:require (and (or (tree-is? t 'converter-input) (tree-is? t 'converter-eval))
              (tm-ref t 1)
