...
(texmacs-module (dynamic animate-menu) (:use (dynamic animate-edit)))
...
(tm-menu (focus-toggle-menu t) (:require (anim-get-accelerate t)))

(tm-menu (focus-toggle-icons t) (:require (anim-get-accelerate t)))
...
(tm-menu (anim-fixed-menu t)
  (with setter
    (lambda (portion) (accelerate-set-type* t `(tuple ,"fixed" ,portion)))
    ("Start" (setter "0"))
    ("10%" (setter "0.1"))
    ("20%" (setter "0.2"))
    ("30%" (setter "0.3"))
...
(tm-menu (focus-animate-menu t)
  ...
  (with type
    ...
    (-> "Time evolution" (dynamic (anim-acceleration-menu t)))
  ) ;with
) ;tm-menu
...
(tm-menu (anim-progressive-menu t inv?)
  (assuming (not inv?) ("Outwards" (progressive-set t 0.5 0.5 0.5 0.5 #f)) ---)
  (assuming inv? ("Inwards" (progressive-set t 0.5 0.5 0.5 0.5 #t)) ---)
...
(tm-menu (focus-toggle-menu t)
  (:require (tree-func? t 'gr-screen 1) (user-anim-context? (tree-ref t 0)))
  (dynamic (focus-toggle-menu (tree-ref t 0)))
) ;tm-menu

(tm-menu (focus-hidden-menu t) (:require (user-anim-context? t)))

(tm-menu (focus-toggle-icons t)
  (:require (tree-is? t 'anim-edit))
  ((balloon (icon "tm_search_next.xpm") "Play animation") (current-anim-commit))
) ;tm-menu

(tm-menu (focus-toggle-icons t)
  (:require (tree-in? t '(anim-static anim-dynamic)))
  ((balloon (icon "tm_show_hidden.xpm") "Edit animation") (current-anim-checkout))
) ;tm-menu
...
(tm-menu (focus-hidden-icons t) (:require (tree-is? t 'anim-edit)))
    ...
    (hlist (assuming (not t)
      ...
      (assuming (tree-in? t '(anim-edit))
       ...
       (assuming (current-anim-can-copy?)
         ...
         //
         //
         ((balloon (icon "tm_delete_before.xpm") "Remove from previous frames")
          (current-anim-delete-before)
         ) ;
         ((balloon (icon "tm_delete_after.xpm") "Remove from subsequent frames")
          (current-anim-delete-after)
         ) ;
       ) ;assuming
       (assuming (and (not (current-anim-can-copy?)) (anim-show-new-mode?))
         //
         //
         //
         ((check (balloon (icon "tm_copy_before.xpm") "Copy new objects to previous frames")
            "v"
            (anim-test-new-mode? "before")
          ) ;check
          (anim-set-new-mode "before")
         ) ;
         ((check (balloon (icon "tm_copy_both.xpm") "Copy new objects to all frames")
            "v"
            (anim-test-new-mode? "all")
          ) ;check
          (anim-set-new-mode "all")
         ) ;
         ((check (balloon (icon "tm_copy_after.xpm") "Copy new objects to subsequent frames")
            "v"
            (anim-test-new-mode? "after")
          ) ;check
