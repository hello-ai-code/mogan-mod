...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; MODULE      : program-doc.scm
;; DESCRIPTION : documentation of program tags
...
(texmacs-module (dynamic program-doc)
  (:use (generic generic-doc) (dynamic program-menu))
) ;texmacs-module
...
(tm-generate (focus-doc-insert t)
  ...
  ($with lab
    ...
    ($para "The "
      ($markup lab)
      " tag is used inside interactive programs. "
    ...
    ($para "Existing input or input/output fields can be removed as follows:")
    ($description-long ($describe-item ($inline ($menu "Focus" "Remove above") ", " ($tmdoc-icon "tm_delete_up.xpm"))
      ...
      ($describe-item ($inline ($menu "Focus" "Remove banner"))
        "Remove the start-up banner of the program."
      ) ;$describe-item
      ($describe-item ($inline ($menu "Focus" "Remove last field"))
        "Remove the last field of the program."
      ) ;$describe-item
    ) ;$description-long
  ) ;$with
) ;tm-generate
