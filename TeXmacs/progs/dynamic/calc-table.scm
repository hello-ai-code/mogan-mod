...
(texmacs-module (dynamic calc-table) (:use (dynamic calc-edit)))
...
(tm-define (cell-column cell)
  (and (tree-is? cell 'cell) (+ (tree-index cell) 1))
) ;tm-define
...
(tm-define (cell-ref-decode s)
  (if (string? s)
    ...
    (and (tree-is? s 'cell-ref) (cell-ref-decode (texmacs->string (tree-ref s 0))))
  ) ;if
) ;tm-define

(define ((cell-ref-range-sub c1 c2) r)
  (map (lambda (c) (cell-ref-encode (list r c))) (... c1 c2))
) ;define
...
(tm-define (cell-ref-range x1 x2)
  (with (r1 c1)
    ...
    (with (r2 c2)
      (cell-ref-decode x2)
      (with rows (map (cell-ref-range-sub c1 c2) (... r1 r2)) (apply append rows))
    ) ;with
  ) ;with
) ;tm-define
...
(tm-define (calc-table-collect t r c dic)
  ...
  (cond ((tree-atomic? t) t)
         ...
         (let* ((body (tree-ref t 0))
           ...
           (set! body
             (if (tree-in? body '(cell-inert cell-input cell-output))
               (let* ((old (tree->string (tree-ref body 0))) (l (tree-children body)))
        ...
        (else (with fun (lambda (u) (calc-table-collect u r c dic)) (tree-map-children fun t))
        ) ;else
  ) ;cond
) ;tm-define
...
(tm-define (calc-table-renumber-sub t dic)
  ...
  (cond ((tree-atomic? t) t)
        ...
        ((tree-is? t 'cell-ref)
         (let* ((old (texmacs->string (tree-ref t 0))) (new (ahash-ref dic old)))
...
(tm-define (calc-table-renumber t x y)
  (let* ((dic (make-ahash-table)) (u (calc-table-collect t x y dic)))
...
(define (tree-replace-cells t r)
  (cond ((== t r) (noop))
        ...
        (else (for-each tree-replace-cells (tree-children t) (tree-children r)))
  ) ;cond
) ;define

(tm-define (calc-table-update-table t)
  (with r (calc-table-renumber t 1 1) (tree-replace-cells t r))
) ;tm-define

(tm-define (calc-table-update)
  (with-innermost t 'calc-table (calc-table-update-table t))
) ;tm-define
...
(define (starts-with-equal? t)
  (cond ((tree-atomic? t) (string-starts? (tree->string t) "="))
        ((tree-in? t '(math concat document)) (starts-with-equal? (tree-ref t 0)))
        ((tree-in? t '(with)) (starts-with-equal? (tree-ref t :last)))
...
(define (remove-equal t)
  (if (tree-atomic? t)
    (if (not (string-starts? (tree->string t) "="))
      t
...
(tm-define (calc-eat-cell-name cs num?)
  (cond ((null? cs) (and num? 0))
        ((and (not num?) (char>=? (car cs) #\a) (char<=? (car cs) #\z))
         (with i (calc-eat-cell-name (cdr cs) #f) (and i (+ i 1)))
        ) ;
        ((and (char>=? (car cs) #\0) (char<=? (car cs) #\9))
         (with i (calc-eat-cell-name (cdr cs) #t) (and i (+ i 1)))
        ) ;
        (else (and num? 0))
  ) ;cond
) ;tm-define
...
(tm-define (cell-input-decode t)
  (cond ((tree-atomic? t) t)
        ((tree-is? t 'concat)
         (with l (map cell-input-decode (tree-children t)) (tm->tree (apply tmconcat l)))
        ) ;
...
(tm-define (cell-input-expand t)
  (cond ((tree-atomic? t) t)
        ((tree-is? t 'concat)
         (let* ((x (cell-input-expand-ranges (tree-children t))) (l (map cell-input-expand x)))
           (tm->tree (apply tmconcat l))
         ) ;let*
        ) ;
        ((tree-in? t '(cell-range cell-sum))
         (let* ((c1 (tree-ref t 0))
                (c2 (tree-ref t 1))
