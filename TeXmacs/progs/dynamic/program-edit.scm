...
(tm-define (program-math-input?) (ahash-ref program-math-input (program-key)))
...
(tm-define (toggle-program-math-input)
  ...
  (with-innermost t prog-field-context? (prog-field-update-math t))
) ;tm-define
...
(tm-define (toggle-program-multiline-input)
  ...
  (ahash-set! program-multiline-input
    (program-key)
...
(tm-define (toggle-program-output-timings)
  ...
  (ahash-set! program-output-timings
    (program-key)
...
(tm-define (toggle-session-program)
  ...
  (with t
    (tree-innermost '(program session))
    (when t
      (if (program-session-mode?) (program->session t) (session->program t))
    ) ;when
  ) ;with
) ;tm-define
...
(tm-define (program-scheme-trees?) program-scheme-trees)
...
(tm-define (program-scheme-strees?) program-scheme-strees)
...
(tm-define (program-scheme-math?) program-scheme-math)
...
(tm-define (program-scheme-eval t mode)
  ...
  (let* ((s (texmacs->code t "iso-8859-1")) (r (eval-string-with-catch s)))
    (cond ((and (tree? r) (error-tree? r) (program-scheme-trees?)) (tree-copy r))
          ((and (tree? r) (program-scheme-trees?)) (tree 'text (tree-copy r)))
          ((and (tm? r)
             (== mode :silent)
          ...
          ((and (tm? r) (program-scheme-strees?)) (tree 'text (tree-copy (tm->tree r))))
          ((program-scheme-math?)
           (with m
             (cas->stree r)
...
(define (program-do lan ses)
  (with l
    ...
    (with (in out next opts)
      (program-decode (car l))
      ;; (display* "Program do " lan ", " ses ", " in "\n")
      (if (or (and (tree-empty? in) (!= lan "r")) (not (program-coherent? out next)))
        (plugin-next lan ses)
        (begin
...
(define (program-next lan ses)
  ...
  (with l
    ...
    (with (in out next opts)
      ...
      (when (and (program-coherent? out next) (tree-empty? out))
  ...
  (with l
    ...
    (with (in out next opts)
      ...
      (when (program-coherent? out next)
        (cond ((== ch "output") (program-output out t))
              ((== ch "error") (program-errput out t))
              ((== ch "prompt")
...
(tm-define (program-feed lan ses in out next opts)
  ...
  (with x
    (program-encode in out next opts)
...
(tm-define (program-document-context? t)
  (and (tm-func? t 'document) (tm-func? (tree-ref t :up) 'program))
) ;tm-define

(tm-define (subprogram-document-context? t)
  (or (and (tm-func? t 'document) (tm-func? (tree-ref t :up) 'program))
...
(tm-define (prog-field-input-context? t)
  (and (prog-field-context? t) (== (tree-down-index t) 1))
) ;tm-define
...
(tm-define (program-alive?) (> (program-status) 1))

(tm-define (program-supports-completions?)
  (and (program-alive?) (plugin-supports-completions? (get-env "prog-language")))
) ;tm-define

(tm-define (program-supports-input-done?)
  (and (program-alive?) (plugin-supports-input-done? (get-env "prog-language")))
) ;tm-define
...
(define (prog-field-extreme t last?)
  (with u
    ...
    (if (prog-field-context? u) u (prog-field-next u (not last?)))
  ) ;with
) ;define
...
(define (program-forall-sub fun t)
  (for (u (tree-children t))
    ...
    (when (and (tm-func? u 'unfolded-subprogram) (tm-func? (tree-ref u 1) 'document))
...
(define (program-forall fun)
  (with-innermost t subprogram-document-context? (program-forall-sub fun t))
) ;define
...
(define (prog-field-process-input t)
  (when (program-ready? #t)
    ...
    (cond ((tm-func? t 'folded-prog-io) (tree-assign-node! t 'unfolded-prog-io))
          ((tm-func? t 'folded-prog-io-math) (tree-assign-node! t 'unfolded-prog-io-math))
    ) ;cond
    (let* ((lan (get-env "prog-language"))
      ...
      (with u
        (or (prog-field-next* t #t) (prog-field-create t p #t))
...
(define (kbd-enter-sub t done?)
  (if (in? done? (list #f "#f"))
    (insert-return)
    (delayed (:idle 1) (program-evaluate))
  ) ;if
) ;define

(tm-define (kbd-enter t shift?)
  (:require (prog-field-input-context? t))
  (cond ((xor (program-multiline-input?) shift?) (insert-return))
...
(tm-define (program-evaluate)
  (with-innermost t prog-field-input-context? (prog-field-process-input t))
) ;tm-define
...
(tm-define (document-context? t)
  (:require (and (tree-is? t 'document) (prog-field-input-context? (tree-ref t :up)))
  ) ;:require
  #f
) ;tm-define
...
(tm-define (traverse-extremal t forwards?)
  (:require (prog-field-input-context? t))
  (with move
    (if forwards? prog-field-go-down prog-field-go-up)
    (go-to-repeat move)
  ) ;with
) ;tm-define
...
(tm-define (prog-field-insert t* forwards?)
  (and-with t
    (tree-search-upwards t* prog-field-input-context?)
    (let* ((lan (get-env "prog-language"))
           (ses (get-env "prog-program"))
           (p (plugin-prompt lan ses))
...
(tm-define (prog-field-remove-banner t*)
  (and-with t
    (tree-search-upwards t* program-document-context?)
...
(tm-define (prog-field-remove-extreme t* last?)
  (and-with t
    (tree-search-upwards t* prog-field-input-context?)
    (with u
      (prog-field-extreme t last?)
      (with v
        (prog-field-next t (not last?))
...
(tm-define (program-clear-all) (program-forall prog-field-remove-output))

(tm-define (program-fold-all) (program-forall prog-field-fold))

(tm-define (program-unfold-all) (program-forall prog-field-unfold))
