...
(texmacs-module (client client-db-sync)
  (:use (client client-tmfs) (database db-convert))
) ;texmacs-module
...
(tm-define (db-sync-kinds server)
  (list-filter (list "bib") (cut db-sync-kind? server <>))
) ;tm-define
...
(tm-define (db-sync-kind? server kind)
  (with-database (user-database "sync")
    (let* ((server-name (client-find-server-name server))
      ...
      (or (null? ids) (!= (db-get-field-first (car ids) "enabled" "yes") "no"))
    ) ;let*
  ) ;with-database
) ;tm-define
...
(define (db-last-sync server)
  (with-database (user-database "sync")
    (let* ((server-name (client-find-server-name server))
           (ids (db-search `(("type" "db-sync") (,"server" ,server-name))))
          ) ;
      (if (null? ids)
        (list "0" "0")
...
(define (db-equivalent? e1 e2)
  ...
  (with leq?
    (lambda (f1 f2) (string<=? (car f1) (car f2)))
    ...
    (for (name names)
      (let* ((local-pair (ahash-ref local-t name))
        ...
        (cond ((and (null? local-val) (not remote-val))
               (ahash-set! status-t name (list "remote-delete" kind))
              ) ;
              ((and (null? remote-val) (not local-val))
               (ahash-set! status-t name (list "local-delete" kind))
              ) ;
              ((and local-val (not remote-val))
               (ahash-set! status-t name (list "upload" kind local-id local-val))
              ) ;
              ((and remote-val (not local-val))
               (ahash-set! status-t name (list "download" kind remote-id remote-val))
              ) ;
              ((and local-val remote-val (db-equivalent? local-val remote-val)) (noop))
              (else (ahash-set! status-t
                      name
                      (list "conflict" kind local-id remote-id local-val remote-val)
                    ) ;ahash-set!
              ) ;else
        ) ;cond
      ) ;let*
...
(tm-define (db-requalify-conflicting line t)
  (if (!= (cadr line) "conflict")
    line
    (with (name cmd kind local-id remote-id local-val remote-val)
      line
      (let* ((key (string-append kind " - " name)) (action (or (ahash-ref t key) "Remote")))
        (cond ((and (== action "Local") (null? local-val)) (list name "remote-delete" kind))
              ((and (== action "Local") (nnull? local-val))
               (list name "upload" kind local-id local-val)
              ) ;
              ((and (== action "Remote") (null? remote-val)) (list name "local-delete" kind))
...
(define (local-db-changes kinds t)
  ...
  (with l
    (map (lambda (kind) (with-database (user-database kind) (db-change-list #t #t t)))
      kinds
    ) ;map
    (list l (current-time))
  ) ;with
...
(tm-define (db-client-sync-status server cont)
  (let* ((kinds (db-sync-kinds server)) (timep (db-last-sync server)))
    (with (ltime rtime)
      timep
      (with local-p
        (local-db-changes kinds ltime)
        (with (local-l ltime*)
          local-p
          ;; (for (ll local-l)
          ;;  (for (x ll)
          ...
          (client-remote-eval server
            ...
            (lambda (remote-p)
              (with (remote-l rtime*)
                ...
                ;;    (display* "Remote: " x "\n")))
                (with status-l
                  (append-map db-change-status local-l remote-l kinds)
                  ;; (for (x status-l)
                  ;;  (display* "Status: " x "\n"))
                  (when (and (null? status-l) (nnull? (apply append (append local-l remote-l))))
...
(tm-define (db-client-sync-proceed server l ltime rtime cont)
  (with kinds
    (db-sync-kinds server)
    (if (nnull? (apply append (car (local-db-changes kinds ltime))))
      (cont #f)
      (client-remote-eval server
        `(remote-db-sync ,l ,kinds ,rtime)
        (lambda (rtime*)
          (if (not rtime*)
            (cont #f)
            (with ltime*
              (db-local-sync l kinds ltime)
              (db-dub-in-sync server ltime* rtime*)
              (cont #t)
            ) ;with
          ) ;if
