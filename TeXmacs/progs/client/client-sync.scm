...
(texmacs-module (client client-sync) (:use (client client-db-sync)))
...
(tm-define (client-sync-list u)
  ...
  (cond ((dont-sync? u) (list))
        ...
        (else (let* ((dirl (url-append u (url-wildcard "*")))
                     (l (url->list (url-expand (url-complete dirl "r"))))
                    ) ;
                (cons (list #t u) (append-map client-sync-list l))
              ) ;let*
        ) ;else
  ) ;cond
) ;tm-define
...
(define (first-string-leq? l1 l2)
  (and (pair? l1) (pair? l2) (string<=? (car l1) (car l2)))
) ;define
      ...
      (lambda (remote-l)
        ...
        (for (local-e local-l)
          (with (dir? name)
            local-e
            (with d
              (url->string (url-subtract name local-base))
        ...
        (with l
          (sort (ahash-table->list t) first-string-leq?)
          ;; (for (x l)
...
(define (get-url-sync-info dir? local-exists? local-name remote-name)
  (with-database* (user-database "sync")
    (let* ((ids (db-search `((,"name" ,(url->system local-name))
           ...
           (date (and local-exists? (number->string (url-last-modified local-name))))
          ) ;
      (if (nnull? ids)
...
(define (get-sync-status info remote-name remote-id)
  (with (dir? local-name local-id date date* remote-id*)
    info
    ...
    (let* ((local-info (list (url->system local-name) local-id))
      ...
      (cond ((and date remote-id (== date date*) (== remote-id remote-id*)) #f)
            ...
            ((and (!= remote-id remote-id*) (== date date*)) (cons "download" all-info))
            ((and (!= date date*) (== remote-id remote-id*)) (cons "upload" all-info))
            ((and remote-id (not date) (not date*)) (cons "download" all-info))
            ((and date (not remote-id) (not remote-id*)) (cons "upload" all-info))
            (else (let* ((suf1 (if date "*" "-")) (suf2 (if remote-id "*" "-")))
...
(define (conflicting-local-delete? line ref)
  (and (!= (car line) "local-delete")
    (third line)
    (url-descends? (system->url (third line)) (system->url (third ref)))
  ) ;and
) ;define
...
(define (conflicting-remote-delete? line ref)
  (and (!= (car line) "remote-delete")
    (fifth line)
    (url-descends? (system->url (fifth line)) (system->url (fifth ref)))
  ) ;and
) ;define
...
(tm-define (requalify-conflicting line t)
  (with (cmd dir? local-name local-id remote-name remote-id*)
    line
    (let* ((action (or (ahash-ref t local-name) "Keep")) (n (string-length cmd)))
      (cond ((not (string-starts? cmd "conflict")) line)
            ((== action "Keep") line)
            ((and (== action "Local") (== (substring cmd (- n 2) (- n 1)) "*"))
             (cons "upload" (cdr line))
            ) ;
            ((and (== action "Local") (== (substring cmd (- n 2) (- n 1)) "-"))
             (cons "remote-delete" (cdr line))
            ) ;
            ((and (== action "Remote") (== (substring cmd (- n 1) n) "*"))
...
(tm-define (client-sync-status local-base remote-base cont)
  ...
  (compute-sync-list local-base
    remote-base
    (lambda (l)
      (with r
        (list)
        (for (x l)
          (with (rname dir? local? remote-id)
            x
...
(tm-define (sync-test)
  (client-sync-status (string->url "~/test/sync-test")
    (current-buffer)
    (lambda (l) (display* "----- result -----\n") (for (x l) (display* x "\n")))
  ) ;client-sync-status
) ;tm-define
...
(tm-define (sync-repair)
  (with-database* (user-database "sync")
    (with ids (db-search '(("type" "sync"))) (for-each sync-repair-one ids))
  ) ;with-database*
) ;tm-define
...
(define (status-line->server line)
  (with sname
    (tmfs-car (remote-file-name (fifth line)))
...
(define (append-doc line)
  (with (cmd dir? local-name local-id remote-name remote-id*)
    line
    (with doc (if dir? "" (string-load (system->url local-name))) (rcons line doc))
  ) ;with
) ;define
...
(define (post-upload line uploaded)
  ...
  (and uploaded
    (with-database* (user-database "sync")
      (with (cmd dir? local-name local-id remote-name remote-id* doc)
        line
        (with remote-id
          uploaded
...
(define (client-upload uploads* msg cont)
  (if (null? uploads*)
    (cont #t)
    (with uploads
      (map append-doc uploads*)
      (with server
        (status-line->server (car uploads))
        (client-remote-eval server
          `(remote-upload ,uploads ,msg)
          (lambda (r)
            (when (and (list? r) (== (length r) (length uploads)))
              (with success? (list-and (map post-upload uploads r)) (cont success?))
            ) ;when
          ) ;lambda
        ) ;client-remote-eval
      ) ;with
    ...
    (with-database* (user-database "sync")
      (with (cmd dir? local-name local-id remote-name remote-id*)
        ...
        (with (remote-id doc)
          downloaded
          (with u
            (system->url local-name)
            (if dir? (when (not (url-exists? u)) (system-mkdir u)) (string-save doc u))
...
(define (client-download downloads cont)
  (if (null? downloads)
    (cont #t)
    (with server
      (status-line->server (car downloads))
      (client-remote-eval server
        `(remote-download ,downloads)
        (lambda (r)
          (when (and (list? r) (== (length r) (length downloads)))
            (with success? (list-and (map post-download downloads r)) (cont success?))
          ) ;when
        ) ;lambda
      ) ;client-remote-eval
    ) ;with
...
(define (remove-local-one line)
  ...
  (with-database* (user-database "sync")
    (with (cmd dir? local-name local-id remote-name remote-id*)
      line
      (with u (system->url local-name) (if dir? (system-rmdir u) (system-remove u)))
...
(define (remove-local remove-l cont)
  ...
  (for (line remove-l) (remove-local-one line))
  (cont #t)
) ;define
...
(define (remove-remote remove-l cont)
  ...
  (if (null? remove-l)
    (cont #t)
    (with server
      (status-line->server (car remove-l))
      (client-remote-eval server
        `(remote-remove-several ,remove-l)
        (lambda (r)
          (when (and (list? r) (== (length r) (length remove-l)))
            (with success? (list-and (map post-remove-remote remove-l r)) (cont success?))
          ) ;when
        ) ;lambda
      ) ;client-remote-eval
    ) ;with
...
(tm-define (remote-upload local-name remote-name msg . cont)
  (client-sync-status local-name
    remote-name
    (lambda (l)
      (client-upload (append (filter-status-list l "upload") (filter-status-list l "conflict"))
        msg
...
(tm-define (remote-download local-name remote-name . cont)
  (client-sync-status local-name
    remote-name
    (lambda (l)
      (client-download (append (filter-status-list l "download") (filter-status-list l "conflict"))
        (if (null? cont) ignore (car cont))
      ) ;client-download
    ) ;lambda
  ) ;client-sync-status
...
(tm-define (client-auto-sync-list server)
  (with-database* (user-database "sync")
    (let* ((server-name (client-find-server-name server))
           (ids (db-search `(("type" "auto-sync") (,"server" ,server-name))))
          ) ;
      (with build
        (lambda (id)
