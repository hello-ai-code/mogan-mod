...
(tm-define (live-remote-context? t)
  (and (live-context? t) (url-rooted-tmfs? (live-id t)))
) ;tm-define
...
(define (live-terminate-retrieval lid)
  (with t
    (ahash-ref live-remote-initializing lid)
    (when t
...
(tm-define (live-apply-patch lid p . opt-state)
  (let* ((old-state (live-current-state lid))
    ...
    (if (or (not server) following-server-instruction?)
      new-state
      (and old-state
        new-state
        (begin
          (when (debug-get "live")
            (display* "Send " (patch->modlist p) ", " old-state ", " new-state "\n")
          ) ;when
...
(define (patch-pull-list p l)
  (if (null? l)
    (list p l)
    (with (p* l*)
      (patch-pull-list p (cdr l))
      (list (patch-pull p* (car l)) (cons (patch-co-pull p* (car l)) l*))
    ) ;with
  ) ;if
) ;define
...
(define (live-resend-local-changes lid old-state)
  (let* ((new-state (live-current-state lid))
    ...
    (when (!= new-state old-state)
      (when (debug-get "live")
        (display* "Resend " (patch->modlist p) ", " old-state ", " new-state "\n")
      ) ;when
      (live-remote-modify lid p old-state new-state)
    ) ;when
  ) ;let*
...
(tm-call-back (live-modify lid mods old-state new-state)
  ...
  (with (server msg-id)
    envelope
    (let* ((old-t (live-get-document lid old-state))
           ;; TODO: check old-t != #f
      ...
      (when (debug-get "live")
        (when (!= ok-state (live-current-state lid))
          ...
          (display* "-- mods= " mods "\n")
          (display* "-- oldp= "
            (patch->modlist (live-get-inverse-patch lid old-state))
            "\n"
          ) ;display*
        ) ;when
      ) ;when
      ...
      (let* ((rev (live-get-patch-list lid old-state))
        ...
        (with-global following-server-instruction?
          #t
...
(tmfs-permission-handler (live-list name type) (in? type (list "read")))
    ...
    (client-remote-eval server
      ...
      (lambda (l)
        (with hyp
          ...
          (with doc
            `(document (section* "Live documents") ,@(map hyp l))
      ...
      (lambda (err) (set-message err "list of live documents"))
    ) ;client-remote-eval
...
(define (get-live-vid lid)
  (or (ahash-ref live-vid-table lid)
    (with vid (create-unique-id) (ahash-set! live-vid-table lid vid) vid)
  ) ;or
) ;define

(tmfs-load-handler (live fname)
  (with lid
    (string-append "tmfs://live/" fname)
    `(document (TeXmacs ,(texmacs-version))
       (style (tuple "generic" "live-document"))
       (body (document (live-io* ,(get-live-vid lid) ,lid (document "")))))
  ) ;with
) ;tmfs-load-handler
...
(tm-define (live-create-interactive server)
  ...
  (and-with sname
    ...
    (interactive (lambda (name)
                   (with lid
                     (string-append "tmfs://live/" sname "/" name)
                     (client-remote-eval server
                       `(live-exists? ,lid)
                       (lambda (e?)
                         (if (not e?)
                           (load-document lid)
                           (set-message "live document already exists" "create live document")
                         ) ;if
                       ) ;lambda
                     ) ;client-remote-eval
                   ) ;with
                   ...
                   (with lid
                     ...
                     (client-remote-eval server
                       ...
                       (lambda (e?)
                         (if e?
                           (load-document lid)
                           (set-message "live document not found" "open live document")
                         ) ;if
                       ) ;lambda
                     ) ;client-remote-eval
                   ) ;with
