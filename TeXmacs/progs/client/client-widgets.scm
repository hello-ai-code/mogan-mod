...
(tm-widget ((error-widget s) quit)
  (padded (centered (text s)) ====== (bottom-buttons >> ("Ok" (quit)) >>))
) ;tm-widget
...
(tm-widget (remote-account-widget quit)
  (let* ((use-password? #t)
         (use-key? #f)
         (set-password? (lambda (flag?) (set! use-password? flag?) (refresh-now "password-info"))
         ) ;set-password?
         (set-key? (lambda (flag?) (set! use-key? flag?) (refresh-now "key-info")))
        ) ;
    (padded (form "account-info"
              (aligned (item (text "Server:")
                         (form-enum "server" (rcons (tm-servers) "") (car (tm-servers)) "280px")
                       ) ;item
                (item (text "Pseudo:")
                  (form-input "pseudo" "string" (list (get-user-info "pseudo")) "280px")
                ) ;item
                (item (text "Full name:")
                  (form-input "name" "string" (list (get-user-info "name")) "280px")
                ) ;item
                (item (text "Email:")
                  (form-input "email" "string" (list (get-user-info "email")) "280px")
                ) ;item
              ) ;aligned
              ======
              ======
              (aligned (item (toggle (set-password? answer) use-password?)
                         (hlist // (text "Allow authentification through password"))
                       ) ;item
              ) ;aligned
              ===
              (refreshable "password-info"
                (when use-password?
                  (aligned (item (text "Password:") (form-input "password" "password" (list "") "280px"))
                    (item (text "Repeat:") (form-input "repeat" "password" (list "") "280px"))
                  ) ;aligned
                ) ;when
              ) ;refreshable
              ======
              ======
              (when #f
                (aligned (item (toggle (set-key? answer) use-key?)
                           (hlist // (text "Allow authentification through cryptographic key"))
                         ) ;item
                ) ;aligned
              ) ;when
              ===
              (refreshable "key-info" (when use-key? (text "Not yet implemented")))
              ======
              (bottom-buttons >>
               ("Proceed"
                 (with t
                   ...
                   (cond ((not (or use-password? use-key?))
                          (open-error "Please enable at least one authentification method")
                         ) ;
                         ((and use-password? (!= (ahash-ref t "password") (ahash-ref t "repeat")))
                          (open-error "Passwords do not match")
                         ) ;
                         ((first-incomplete-field t use-password? use-key?)
                          =>
                          (lambda (s) (open-error (string-append "Missing '" s "'")))
                         ) ;
                         (else (client-create-account (ahash-ref t "server")
                                 (ahash-ref t "pseudo")
                                 (ahash-ref t "name")
...
(tm-tool* (remote-account-tool win)
  ...
  (with quit
    (lambda () (tool-close :any 'remote-account-tool noop win))
...
(define ((save-licence-as doc) f)
  (with s (convert doc "texmacs-stree" "texmacs-document") (string-save s f))
) ;define
...
(tm-widget ((accept-licence-widget doc) cmd)
  (let* ((ok? #f)
         ...
         (msg (string-append "You must agree with the licence " "in order to proceed"))
        ) ;
    (padded ======
      (resize "720px"
        "480px"
      ...
      ======
      (hlist (toggle (set! ok? answer) ok?) // // // (text decl) >>)
      ======
      (bottom-buttons >>
...
(tm-define (client-create-account server-name pseudo name passwd email)
  (with cmd
    (lambda () (client-new-account server-name pseudo name passwd email ""))
    (with server
...
(tm-define (client-login-home server-name pseudo passwd)
  (client-login-then server-name
    ...
    (lambda (ret)
      (when (== ret "ready")
        (with-wallet (wallet-set (list "remote" server-name pseudo) passwd))
        (and-let* ((server (client-find-server server-name)) (dir (remote-home-directory server)))
          (load-document dir)
        ) ;and-let*
      ) ;when
      (when (== ret "invalid password")
        (with server (client-find-server server-name) (client-logout server))
...
(tm-widget ((remote-login-widget server-name pseudo retry?) quit)
  (padded (form "login-info"
            (if retry? === (centered (bold (text "Incorrect password; please try again."))))
            ======
            (aligned (item (text "Server:")
                       (verb (form-enum "server"
                               (if (== server-name "")
                                 (rcons (tm-servers) "")
                                 (cons server-name (rcons (tm-servers) ""))
                               ) ;if
                               (if (== server-name "") (car (tm-servers)) server-name)
                               "280px"
                             ) ;form-enum
                       ) ;verb
                     ) ;item
              (item (text "Pseudo:") (form-input "pseudo" "string" (list pseudo) "280px"))
              (item (text "Password:") (form-input "name" "password" (list "") "280px"))
            ) ;aligned
            ======
            (bottom-buttons >>
              (assuming (not (side-tools?)) ("Cancel" (quit)) // //)
...
(tm-tool* (remote-login-tool win server-name pseudo retry?)
  (:name "Remote login")
  (with quit
    (lambda () (tool-close :any 'remote-login-tool noop win))
...
(tm-define (open-remote-login* server-name pseudo retry?)
  (if (side-tools?)
    (tool-select :right (list 'remote-login-tool server-name pseudo retry?))
...
(tm-define (open-remote-login server-name pseudo)
  (:interactive #t)
  (with-wallet (with passwd
                 (wallet-get (list "remote" server-name pseudo))
                 (if passwd
...
(define (get-ancestors dir)
  (let* ((parent (remote-parent dir))
    ...
    (if (and (not (remote-home-directory? dir)) (not (remote-root-directory? dir)))
...
(tm-widget ((remote-file-browser server initial type) quit)
  (let* ((save-flag? (or (func? type :save-file 1) (func? type :save-directory 1)))
         (dir-flag? (or (== type :directory) (func? type :save-directory 1)))
         (dir (if (and save-flag? (not (remote-home-directory? initial)))
                (remote-parent initial)
                initial
              ) ;if
         ) ;dir
         ...
         (select-entry (lambda (e)
                         (with (full-name dir? props)
                           (assoc-ref entries e)
                           (with name
                             (remote-relative dir full-name dir?)
                             (cond (dir? (select-dir name))
                                   (save-flag? (set! file (url->string (url-tail name)))
         ...
         (list-entry? (lambda (e) (with (short-name full-name dir? props) e (if dir-flag? dir? #t)))
         ) ;list-entry?
         (dummy (select-dir dir))
        ) ;
    (padded (refreshable "remote-file-browser"
              (hlist (explicit-buttons (for (a (get-ancestors dir)) ((eval (car a)) (select-dir (cdr a))) //)
                       >>
                     ) ;explicit-buttons
              ) ;hlist
              ===
              (resize "600px"
                "400px"
                (choice (select-entry answer) (map car (list-filter entries list-entry?)) "")
              ) ;resize
              (assuming save-flag?
                ===
                (hlist (text (cadr type))
                  //
                  (refreshable "remote-save-as"
                    (input (when answer (quit (url-append dir answer))) "string" (list file) "1w")
                  ) ;refreshable
                  //
                  //
                  //
                  (explicit-buttons ("Ok" (quit (url-append dir file))))
                ) ;hlist
              ) ;assuming
              (assuming (and dir-flag? (not save-flag?))
                (bottom-buttons >> ("Ok" (quit (url->url dir))))
              ) ;assuming
            ) ;refreshable
    ) ;padded
  ) ;let*
...
(tm-define (remote-rename-interactive server)
  ...
  (with dir?
    ...
    (open-remote-file-browser server
      (current-buffer)
...
(tm-define (remote-remove-interactive server)
  (:interactive #t)
  (with msg
    (if (remote-directory? (current-buffer))
      "Really remove directory and its recursive contents?"
      "Really remove file?"
    ) ;if
    (user-confirm msg
      #f
      (lambda (answ) (when answ (remote-remove (current-buffer))))
    ) ;user-confirm
  ) ;with
) ;tm-define
...
(define (set-permission-all perms server id attr on?)
  (with old-vals
    ...
    (with new-vals
      (append (if on? (list "all") (list)) (list-difference old-vals (list "all")))
...
(define (get-permissions perms dec server id attr)
  (with vals
    (list-union (ahash-ref perms "owner") (ahash-ref perms attr))
    (with vals* (map (cut ahash-ref dec <>) vals) (sort vals* string<=?))
  ) ;with
) ;define

(tm-widget ((entry-permissions-editor server id attrs users enc dec perms) quit)
  (padded (tabs (loop (attr attrs)
                  (tab (text (upcase-first attr))
                    (padded (hlist (toggle (begin
...
(tm-define (open-entry-permissions-editor server id attrs)
  (:interactive #t)
  (with-remote-search-user users
    server
    (list)
    (let* ((perms (make-ahash-table)) (enc (make-ahash-table)) (dec (make-ahash-table)))
      (with-remote-get-entry entry
        server
        id
        (for (attr attrs)
          (with vals
            (or (assoc-ref entry attr) (list))
            (ahash-set! perms attr vals)
            (set! users (list-union users vals))
          ) ;with
        ) ;for
        (with-remote-get-user-pseudo pseudos
          server
          users
          (with-remote-get-user-name names
            server
            users
            ...
            (dialogue-window (entry-permissions-editor server id attrs users enc dec perms)
              noop
              "Change permissions"
            ) ;dialogue-window
          ) ;with-remote-get-user-name
        ) ;with-remote-get-user-pseudo
      ) ;with-remote-get-entry
    ) ;let*
  ) ;with-remote-search-user
) ;tm-define

(tm-define (open-permissions-editor server u)
  (:interactive #t)
  (with-remote-identifier rid
    server
    u
    (when rid
      (with attrs
        (list "readable" "writable" "owner")
...
(tm-widget ((simple-list-widget l) quit)
  (padded (resize "600px" "400px" (scrollable (choice (noop) l ""))))
) ;tm-widget

(define (list-item line)
  (with (cmd dir? local-name local-id remote-name remote-id*)
    line
...
(define (show-list l title)
  (with fl (map list-item l) (dialogue-window (simple-list-widget fl) noop title))
) ;define

(define (db-list-item line)
  (with (name cmd kind . args) line (string-append kind " - " name))
) ;define
...
(tm-widget ((conflicts-list-widget l t can-skip?) quit)
  (let* ((set-all (lambda (action)
                    (for (f l) (ahash-set! t f action))
                    (refresh-now "selected-actions")
                  ) ;lambda
         ) ;set-all
        ) ;
    (padded (hlist (text "Select the action to be undertaken for each conflict") >>)
      ======
      (resize "600px"
        "400px"
        (scrollable (padded (refreshable "selected-actions"
                              (for (f l)
                                (hlist (enum (ahash-set! t f answer)
                                         (if can-skip? '("Skip"
                                                         "Local"
                                                         "Remote") '("Local"
                                                                     "Remote"))
                                         (or (ahash-ref t f) (if can-skip? "Skip" "Remote"))
                                         "6em"
                                       ) ;enum
                                  //
                                  //
      ...
      ======
      (hlist (explicit-buttons (if can-skip? ("Skip all" (set-all "Skip")) // //)
...
(tm-widget ((client-sync-widget server l dbl ltime rtime) quit)
  (let* ((upl (filter-status-list l "upload"))
    ...
    (padded (form "sync-form"
              (explicit-buttons ===
                (with msg
                  (items-msg upl "" "to be uploaded")
                  (if (nnull? upl) (hlist (text msg) // >> ("Details" (show-list upl msg))))
                ) ;with
                ===
                (with msg
                  (items-msg dol "" "to be downloaded")
                  (if (nnull? dol) (hlist (text msg) // >> ("Details" (show-list dol msg))))
                ) ;with
                ===
                (with msg
                  (items-msg ldl " local" "to be deleted")
                  (if (nnull? ldl) (hlist (text msg) // >> ("Details" (show-list ldl msg))))
                ) ;with
                ===
                (with msg
                  (items-msg rdl " remote" "to be deleted")
                  (if (nnull? rdl) (hlist (text msg) // >> ("Details" (show-list rdl msg))))
                ) ;with
                ===
                (with msg
                  (items-msg cfl "" "with conflicts")
                  (if (nnull? cfl)
                    (hlist (text msg) // >> ("Details" (show-conflicts cfl t msg)))
                  ) ;if
                ) ;with
                ===
                (with msg
                  (db-msg dbupl "" "to be uploaded")
                  (if (nnull? dbupl)
                ...
                ===
                (with msg
                  (db-msg dbldl " local" "to be deleted")
                  (if (nnull? dbldl)
                ...
                ===
                (with msg
                  (db-msg dbrdl " remote" "to be deleted")
                  (if (nnull? dbrdl)
                ...
                ===
                (with msg
                  (db-msg dbcfl "" "with conflicts")
                  (if (nnull? dbcfl)
                    (hlist (text msg) // >> ("Details" (show-db-conflicts dbcfl dbt msg)))
                  ) ;if
                ) ;with
              ) ;explicit-buttons
              (if (nnull? upl)
               ...
               ("Synchronize"
                 (with msg
                   ...
                   (let* ((r (map (cut requalify-conflicting <> t) l))
                     ...
                     ;; (for (x dbr)
                     ;; (display* "Requalified: " x "\n"))
                     (client-sync-proceed r
                       msg
                       (lambda ()
                         (db-client-sync-proceed server
                           dbr
                           ltime
                           rtime
                           (lambda (ok?)
                             (when (not ok?)
                               (show-message "Extra modifications occurred in the meantime"
                                 "Synchronize with remote server"
                               ) ;show-message
                             ) ;when
                             (quit)
                           ) ;lambda
                         ) ;db-client-sync-proceed
                       ) ;lambda
                     ) ;client-sync-proceed
                   ) ;let*
                 ) ;with
...
(tm-define (open-sync-widget server l dbl ltime rtime)
  ...
  (if (and (null? l) (null? dbl))
    ...
    (dialogue-window (client-sync-widget server l dbl ltime rtime)
      noop
...
(define (client-auto-sync-status l cont)
  (if (null? l)
    (cont (list))
    (client-sync-status (caar l)
      (cdar l)
      (lambda (r1)
        (client-auto-sync-status (cdr l) (lambda (r2) (cont (append r1 r2))))
      ) ;lambda
    ) ;client-sync-status
  ) ;if
) ;define
...
(tm-define (client-auto-sync server)
  (client-auto-sync-status (client-auto-sync-list server)
    ...
    (lambda (l)
      ...
      ) ;db-client-sync-status
    ) ;lambda
  ) ;client-auto-sync-status
) ;tm-define
  ...
  (hlist (refreshable "local-name"
    ...
    (explicit-buttons ("Browse"
                        (choose-file (lambda (name)
                          ...
                          (with name
                            (form-ref field-name)
                            (if (url-rooted? name) (system->url name) (system->url "~"))
                          ) ;with
                        ) ;choose-file
                      ) ;
    ) ;explicit-buttons
...
(tm-widget (form-remote-widget server form-name field-name title prompt width)
  (hlist (refreshable "remote-name"
           (form-input field-name "string" (list (form-ref field-name)) width)
         ) ;refreshable
    //
    //
    (explicit-buttons (when (or (remote-file-name (form-ref field-name)) (remote-home-directory server))
                        ("Browse"
                          (open-remote-file-browser server
                            (with name
                              (form-ref field-name)
...
(tm-widget (client-auto-sync-data-widget server)
  (padded (aligned (item (toggle (db-sync-kind server "bib" answer) (db-sync-kind? server "bib"))
...
(define (consistent? l)
  (or (null? l) (and (consistent-with? (car l) (cdr l)) (consistent? (cdr l))))
) ;define
...
(tm-widget ((client-auto-sync-modify-widget server lname) quit)
  (form "auto-sync"
    (let* ((l (client-auto-sync-list server))
           ...
           (dummy (begin (form-set "local-name" lname) (form-set "remote-name" rname)))
          ) ;
      (padded (aligned (item (text "Local:")
                         (dynamic (form-local-widget "auto-sync"
                                    "local-name"
                ...
                (item (text "Remote:")
                  (dynamic (form-remote-widget server
                             "auto-sync"
                             "remote-name"
        ...
        (bottom-buttons >>
         ...
         ("Ok"
           (let* ((lname* (form-ref "local-name"))
                  ...
                  (new-l (assoc-set! (list-copy (assoc-remove! l lname)) lname* rname*))
                 ) ;
             (if (and (consistent? (map system->url (map car new-l)))
               ...
               (show-message "Synchronized files and directories should be disjoint"
...
(tm-define (client-auto-sync-modify server lname quit)
  (dialogue-window (client-auto-sync-modify-widget server lname)
    quit
    "Synchronize file or directory"
  ) ;dialogue-window
) ;tm-define
...
(tm-widget (client-auto-sync-files-widget server)
  (let* ((selected #f)
    ...
    (padded (refreshable "sync-file-pairs"
              (resize "450px"
                "250px"
                (scrollable (choice (select answer) (map car (client-auto-sync-list server)) "")
                ) ;scrollable
              ) ;resize
            ) ;refreshable
      ======
      (refreshable "sync-file-buttons"
        (hlist (explicit-buttons ("Add"
                                   (client-auto-sync-modify server
                                     ""
                                     (lambda (new-lname) (refresh-now "sync-file-pairs"))
                                   ) ;client-auto-sync-modify
                                 ) ;
                 (when selected
                   //
                   //
...
(tm-widget ((client-auto-sync-widget server) quit)
  (form "auto-sync"
    (padded (tabs (tab (text "Files") (dynamic (client-auto-sync-files-widget server)))
              (tab (text "Data") (dynamic (client-auto-sync-data-widget server)))
            ) ;tabs
      ======
      (hlist >> (explicit-buttons ("Synchronize" (client-auto-sync server))))
    ) ;padded
  ) ;form
) ;tm-widget
...
(tm-define (remote-interactive-sync server)
  (:interactive #t)
  (dialogue-window (client-auto-sync-widget server)
    noop
    "Synchronize with remote server"
  ) ;dialogue-window
) ;tm-define
...
(tm-widget ((upload-widget server src dest) quit)
  (padded (form "upload-form"
            ======
            (aligned (item (text "Local source:")
                       (with dummy
                         (form-set "local-name" src)
                         (dynamic (form-local-widget "upload-form"
                                    "local-name"
                                    "Local file or directory"
                                    ""
                                    "300px"
            ...
            (bottom-buttons >>
             ...
             ("Upload"
               (let* ((local-name (system->url (form-ref "local-name")))
                 ...
                 (when (and (url-exists? local-name) (remote-file-name remote-name))
                   (when (and (url-regular? local-name) (remote-directory? remote-name))
                     (set! remote-name (url-append remote-name (url-tail local-name)))
                   ) ;when
                   (if (and (url-directory? local-name) (remote-file? remote-name))
                     (set-message "cannot upload directory to file" "upload")
                     (begin
                       ;; (display* "Local  : " local-name "\n")
                       ;; (display* "Remote : " remote-name "\n")
                       ;; (display* "Message: " message "\n")
                       (remote-upload local-name remote-name message (lambda x (revert-buffer-revert)))
                     ) ;begin
                   ) ;if
                   (quit)
                 ) ;when
...
(tm-define (remote-interactive-upload server)
  (:interactive #t)
  (let* ((src (if (remote-file-name (current-buffer)) "" (url->system (current-buffer))))
         (dest (if (remote-file-name (current-buffer)) (url->system (current-buffer)) "")
         ) ;dest
        ) ;
    (dialogue-window (upload-widget server src dest)
      noop
...
(tm-widget ((download-widget server) quit)
  (padded (form "download-form"
            ======
            (aligned (item (text "Remote source:")
                       (with dummy
                         (form-set "remote-name"
                           (if (remote-file-name (current-buffer)) (url->system (current-buffer)) "")
                         ) ;form-set
                         (dynamic (form-remote-widget server
                                    "download-form"
                                    "remote-name"
              ...
              (item (text "Local destination:")
                (with dummy
                  ...
                  (dynamic (form-local-widget "download-form"
                             "local-name"
                             "Local file or directory"
                             "Download as:"
                             "300px"
             ...
             ("Download"
               (let* ((remote-name (system->url (form-ref "remote-name")))
                 ...
                 (when (and (remote-file-name remote-name)
                         (or (url-exists? local-name) (url-exists? (url-head local-name)))
                       ) ;and
                   (when (and (remote-file? remote-name) (url-directory? local-name))
                     (set! local-name (url-append local-name (url-tail remote-name)))
                   ) ;when
                   (if (and (remote-directory? remote-name) (url-regular? local-name))
...
(tm-define (remote-interactive-download server)
  (:interactive #t)
  (dialogue-window (download-widget server) noop "Download file or directory")
) ;tm-define
...
(define (simple-upload server local-name remote-dir)
  (when (and (url-exists? local-name)
    ...
    (with remote-name
      (url-append remote-dir (url-tail local-name))
      (remote-upload local-name
        remote-name
        "uploaded"
        (lambda x (revert-buffer-revert) (set-message "upload completed" "upload"))
      ) ;remote-upload
    ) ;with
  ) ;when
) ;define
...
(tm-define (simple-interactive-upload server)
  (:interactive #t)
  (choose-file (lambda (local-name) (simple-upload server local-name (current-buffer)))
    "Upload file or directory"
    "generic"
    ""
    (system->url "$PWD")
...
(define (simple-download server remote-name local-name)
  (when (and (remote-file-name remote-name)
    ...
    (remote-download local-name
      remote-name
      (lambda x (set-message "download completed" "download"))
    ) ;remote-download
  ) ;when
) ;define
...
(tm-define (simple-interactive-download server)
  ...
  (let* ((name (current-buffer))
    ...
    (choose-file (lambda (local-name) (simple-download server name local-name))
...
(define (simple-synchronize server remote-name local-name)
  (when (and (remote-file-name remote-name)
    ...
    (client-auto-sync-add server (url->system local-name) (url->system remote-name))
...
(tm-define (simple-interactive-synchronize server)
  ...
  (let* ((name (current-buffer))
    ...
    (choose-file (lambda (local-name) (simple-synchronize server name local-name))
      "Synchronize file or directory"
      type
      "Synchronize"
      prop
  ...
  (and-let* ((doc (buffer-get-body u))
    ...
    (client-remote-eval server
      ...
      (lambda x
        ...
        (show-message "Your message has been sent." "Send instant message")
      ) ;lambda
    ) ;client-remote-eval
  ) ;and-let*
) ;define
...
(tm-widget ((message-editor server u users to) quit)
  (padded (horizontal (vertical (bold (text "To"))
                        ===
                        ===
                        (resize "250px" "350px" (choices (set! to answer) (sort users string<=?) to))
                      ) ;vertical
            ///
            (vertical (bold (text "Message"))
              ===
              ===
              (resize "500px"
                "350px"
    ...
    ======
    (hlist >> (explicit-buttons ("Send" (message-send server u to) (quit))))
  ) ;padded
) ;tm-widget
...
(tm-define (open-message-editor server)
  ...
  (with-remote-search-user users
    server
    (list)
    (let* ((b (current-buffer-url)) (u (string->url "tmfs://aux/message-editor")))
...
(define (share-document server u to)
  (and-let* ((msg (url->string u))
    ...
    (client-remote-eval server
      cmd
      (lambda x
        (set-message (if (chat-room-url? u) "invitation sent" "document shared")
          "instant message"
        ) ;set-message
...
(tm-widget ((share-document-widget server u users to) quit)
  (padded (with s (if (chat-room-url? u) "Invite" "Share with") (bold (text s)))
    ===
    ===
    (resize "250px" "350px" (choices (set! to answer) (sort users string<=?) to))
    ======
    (hlist >> (explicit-buttons ("Send" (share-document server u to) (quit))))
  ) ;padded
) ;tm-widget
    ...
    (with-remote-identifier rid
      ...
      (when rid
        (with-remote-get-entry entry
          ...
          rid
          (with to
            (list-union (or (assoc-ref entry "readable") (list))
