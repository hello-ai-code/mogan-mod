...
(texmacs-module (client client-tmfs)
  (:use (client client-base) (client client-db) (version version-tmfs))
) ;texmacs-module
...
(define (generic-document doc)
  `(document (TeXmacs ,(texmacs-version)) (style (tuple "generic")) (body ,doc))
) ;define
...
(define (buffer-set-stm u doc)
  (let* ((s (object->tmstring doc)) (t (tree-import-loaded s u "stm")))
    (buffer-set u t)
...
(define (remote-file-set name doc)
  (with fname
    (string-append "tmfs://remote-file/" name)
...
(define (remote-dir-set name doc)
  (with fname
    (string-append "tmfs://remote-dir/" name)
    (buffer-set-stm fname doc)
  ) ;with
) ;define
...
(tm-define-macro (with-remote-identifier r server u . body)
  `(remote-identifier ,server ,u (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define (remote-get-time fname)
  (and-with rname
    (remote-file-name fname)
    (let* ((name (tmfs-cdr rname)) (head (tmfs-car name)))
      (if (string-starts? head "time=") (string->number (string-drop head 5)) :now)
    ) ;let*
  ) ;and-with
) ;tm-define
  ...
  (and-with rname
    ...
    (let* ((name (tmfs-cdr rname)) (head (tmfs-car name)))
      (if (string-starts? head "time=")
        (string-append "tmfs://remote-file/" (tmfs-car rname) "/" (tmfs-cdr name))
        fname
      ) ;if
    ) ;let*
  ) ;and-with
...
(tm-define (remote-create-file server fname doc)
  ...
  (let* ((sname (client-find-server-name server))
    ...
    (client-remote-eval server
      `(remote-file-create ,name ,tm ,msg)
      (lambda (msg) (load-document fname))
      (lambda (err) (set-message err "create remote file"))
    ) ;client-remote-eval
  ) ;let*
) ;tm-define
...
(tm-define (remote-create-file-interactive server)
  ...
  (interactive (lambda (name)
                 (and-with fname
                   (prepend-dir server name "remote-file")
...
(tmfs-title-handler (remote-file name doc)
  (let* ((sname (tmfs-car name))
    ...
    (with old-title
      (or (ahash-ref remote-title-table fname) (append-time def-title t))
      (when server
        (with-remote-identifier rid
          server
          fname
          (when rid
            (with-remote-get-field new-title
              server
              rid
              "title"
              (set! new-title
                (if (pair? new-title) (append-time (car new-title) t) old-title)
              ) ;set!
...
(tmfs-load-handler (remote-file name)
  (let* ((sname (tmfs-car name))
    ...
    (if (not server)
      ...
      (begin
        (client-remote-eval server
          `(remote-file-load ,name)
          (lambda (tm)
            (with doc
              (convert tm "texmacs-document" "texmacs-stree")
              ;; (display* "LOAD ") (write doc) (display* "\n")
              (remote-file-set name doc)
            ) ;with
            (set-message "retrieved contents" "load remote file")
          ) ;lambda
          (lambda (err) (set-message err "load remote file"))
        ) ;client-remote-eval
...
(tmfs-save-handler (remote-file name doc)
  ...
  (let* ((sname (tmfs-car name))
    ...
    (if (not server)
      (texmacs-error "remote-file" "invalid server")
      (with tm
        (convert doc "texmacs-stree" "texmacs-document")
        (client-remote-eval server
          `(remote-file-save ,name ,tm ,msg)
          (lambda (saved) (set-message "file saved" "save remote file"))
          (lambda (err) (set-message err "save remote file"))
        ) ;client-remote-eval
      ) ;with
    ) ;if
  ) ;let*
...
(tm-define (remote-create-dir server fname)
  ...
  (let* ((sname (client-find-server-name server))
    ...
    (client-remote-eval server
      `(remote-dir-create ,name)
      (lambda (entries) (load-document fname))
      (lambda (err) (set-message err "create remote directory"))
    ) ;client-remote-eval
  ) ;let*
) ;tm-define
...
(tm-define (remote-create-dir-interactive server)
  ...
  (interactive (lambda (name)
                 (and-with fname
                   (prepend-dir server name "remote-dir")
...
(define (dir-line sname entry)
  (with (short-name full-name dir? props)
    entry
    ...
    (if (not server)
      ...
      (begin
        (client-remote-eval server
          ...
          (lambda (err) (set-message err "remote directory"))
        ) ;client-remote-eval
...
(define (ancestor? d f)
  (or (== d f) (and (!= (remote-parent f) f) (ancestor? d (remote-parent f))))
) ;define
...
(tm-define (remote-rename src dest)
  (let* ((src* (remote-file-name src))
    ...
    (with-remote-identifier rid
      server
      src
      (with-remote-identifier did
        server
        dir
        (with-remote-identifier oid
          server
          dest
          (cond ((== dest src) (set-message "same names" action))
                ((!= dest-sv src-sv)
                 (set-message "cannot rename across different servers" action)
                ) ;
                ((not server) (set-message "not connected to server" action))
                ((ancestor? src dest) (set-message "invalid renaming" action))
                ((not rid) "source file not found" action)
                ((not rid) "destination directory not found" action)
                (oid "destination already exists" action)
                (else (remote-set-field server rid "dir" (list did))
                  (remote-set-field server rid "name" (list name))
                  (buffer-rename src dest)
                  (set-message (string-append "renamed as " (url->string dest)) action)
                ) ;else
          ) ;cond
        ) ;with-remote-identifier
      ) ;with-remote-identifier
  ...
  (let* ((dir? (remote-directory? what))
    ...
    (if (remote-home-directory? what)
      ...
      (client-remote-eval server
        ...
        (lambda (err) (set-message err action))
      ) ;client-remote-eval
    ) ;if
  ) ;let*
) ;tm-define
...
(tm-define (versioned? name) (:require (remote-file? name)) #t)
...
(define (build-version-page name prefix l)
  (with u
    ...
    ($generic ($tmfs-title "History of "
                ($link (url->unix u) ($verbatim (url->system (url-tail u))))
              ) ;$tmfs-title
      ($description-long ($for (i (.. 0 (length l)))
                           ($with info
                             (list-ref l i)
                             ($with (rid date vname by msg)
                               info
                               ($let* ((dest (string-append prefix "time=" date "/" vname))
                                 ...
                                 ($when (and msg (!= msg "")) ($describe-item vs msg))
                               ) ;$let*
                             ) ;$with
                           ) ;$with
                         ) ;$for
  ...
  (let* ((name (remote-file-name rname))
    ...
    (and server
      (client-remote-eval server
        ...
        (lambda (err) (set-message err "get list with remote versions"))
      ) ;client-remote-eval
    ) ;and
  ) ;let*
) ;define
...
(tm-define (commit-buffer-message name msg)
  (:require (remote-file? name))
  (with-global remote-commit-message msg (save-buffer name))
) ;tm-define
