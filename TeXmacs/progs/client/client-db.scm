...
(texmacs-module (client client-db) (:use (client client-base)))
...
(tm-define-macro (with-remote-get-field r server id attr . body)
  `(remote-get-field ,server ,id ,attr (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-get-attributes r server id . body)
  `(remote-get-attributes ,server ,id (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-get-entry r server id . body)
  `(remote-get-entry ,server ,id (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-create-entry r server l . body)
  `(remote-create-entry ,server ,l (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-search r server q . body)
  `(remote-search ,server ,q (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-search-user r server q . body)
  `(remote-search-user ,server ,q (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-get-user-pseudo r server uid . body)
  `(remote-get-user-pseudo ,server ,uid (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
...
(tm-define-macro (with-remote-get-user-name r server uid . body)
  `(remote-get-user-name ,server ,uid (lambda (msg) (with ,r msg ,@body)))
) ;tm-define-macro
