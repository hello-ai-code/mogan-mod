...
(texmacs-module (education edu-markup)
  (:use (database title-markup) (education edu-edit))
) ;texmacs-module
...
(define (mc-popup-first l)
  (and (nnull? l)
    (with (h . t) l (if (tm-func? h 'mc-field 2) (tm-ref h 1) (mc-popup-first t)))
  ) ;and
) ;define

(define (mc-popup-selected l)
  (and (nnull? l)
    (with (h . t) l (if (mc-field-active? h) (tm-ref h 1) (mc-popup-selected t)))
  ) ;and
) ;define
...
(tm-define (ext-mc-popup t)
  ...
  (let* ((ch (tm-children t))
    ...
    `(button-popup ,text
       ,t
       (with ,"button-nr" ,"0" (vertical-items* ,@ch))
       ,"left"
       ,"Bottom"
       ,"default")
  ) ;let*
