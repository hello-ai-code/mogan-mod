...
(define (output-commands-folded-section handle)
  ($folded-documentation (car handle)
    ($para ($for (cmd (cdr handle)) (links-to-modules-of cmd)))
...
(define ($doc-all-symbols-buffer)
  ...
  (with sorted
    (sort (reverse (ahash-table->list indexed-commands)) sort-by-car)
    ($tmapidoc ($tmdoc-title (replace "All TeXmacs commands"))
      ($para "This is an alphabetical list of TeXmacs's public interface. "
      ...
      ($para ($for (entry sorted) (output-commands-folded-section entry)))
    ) ;$tmapidoc
  ) ;with
) ;define

(define ($doc-symbol-buffer ssym)
  ($tmapidoc ($tmdoc-title (replace "Documentation for %1" ssym))
...
(define ($doc-module-buffer smod)
  (with m
    (string->module smod)
    (if (is-real-module? m)
      ($tmapidoc ($tmdoc-title (replace "Documentation for %1" smod))
        `(document ,($doc-module-header m) ,($doc-module-exported m))
      ) ;$tmapidoc
      ($tmapidoc ($tmdoc-title (replace "Module family %1" smod))
...
(define ($query-not-implemented query)
  ($tmdoc ($tmdoc-title (translate "Request unknown or not implemented"))
    ($para query)
  ) ;$tmdoc
) ;define
...
(tmfs-title-handler (apidoc query doc)
  (let* ((type (query-ref query "type")) (what (query-ref query "what")))
    (cond ((== type "symbol")
           (if (== what "")
             (replace "Help - Browse all symbols")
             (replace "Help - Documentation for %1" what)
           ) ;if
          ) ;
          ((== type "module")
           (if (== what "")
             (replace "Help - Browse all modules")
