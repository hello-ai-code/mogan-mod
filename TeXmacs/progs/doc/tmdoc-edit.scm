...
(tm-define (tmdoc-insert-copyright)
  (with buf
    ...
    (when (tree-is? buf 'document)
      (with n
        (- (tree-arity buf)
          (if (tree-in? buf :last '(tmdoc-license tmweb-license)) 1 0)
        ) ;-
...
(tm-define (tmweb-insert-license)
  (with buf
    (buffer-tree)
    (when (tree-is? buf 'document)
      (with n (tree-arity buf) (tree-insert! buf n '((tmweb-license))))
    ) ;when
  ) ;with
) ;tm-define
...
(tm-define (tmweb-insert-classifiers main sub)
  ...
  (and-with tit
    (tree-ref (buffer-tree) 0)
    (when (tree-is? tit 'tmweb-title)
      (and-with links
        (tree-ref tit 1)
        (with s
          (string-append "tmweb-" (locase-all main) "-links")
