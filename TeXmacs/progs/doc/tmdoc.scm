...
(define (tmdoc-relative cur name)
  ...
  (with rel
    (url-relative cur name)
    (if (url-regular? rel)
      rel
      (let* ((nname (string-append name "." (ext-language-suffix) ".tm"))
             (nfile (url-search-upwards (url-head cur) nname (list "doc" "web" "texmacs")))
            ) ;
        (if (not (url-none? nfile))
          nfile
          (let* ((ename (string-append name ".en.tm"))
                 (efile (url-search-upwards (url-head cur) ename (list "doc" "web" "texmacs")))
                ) ;
...
(define (tmdoc-branch x root cur level done)
  (let* ((name (caddr x)) (rel-name (tmdoc-relative cur name)))
    ...
    (if done?
      ...
      (with t
        ...
        (if (not (and (pair? t) (list? (cdr t)) (forall? pair? (cdr t))))
          ...
          (with u
            (cadr (assoc 'body (cdr t)))
            (cons 'document (tmdoc-rewrite (cdr u) root cur level done))
          ) ;with
        ) ;if
      ) ;with
    ) ;if
...
(define (tmdoc-label-table doc t)
  (cond ((tm-atomic? doc) (noop))
        ((tm-func? doc 'label 1) (ahash-set! t (tm->stree (tm-ref doc 0)) #t))
...
(define (tmdoc-internal-label dest)
  (and (string? dest)
    (with pos
      (string-search-backwards "#" (string-length dest) dest)
      (if (>= pos 0)
        (substring dest (+ pos 1) (string-length dest))
        (with name
          (url-basename (url-basename dest))
...
(define (tmdoc-internal-link dest t)
  (and-with lab
    (tmdoc-internal-label dest)
    (and (ahash-ref t lab) (string-append "#" lab))
  ) ;and-with
) ;define
...
(define (tmdoc-internalize* doc t)
  (cond ((tm-atomic? doc) doc)
        ((or (tm-func? doc 'hlink 2) (tm-func? doc 'hyper-link 2))
         (let* ((text (tm-ref doc 0)) (dest (tmdoc-internal-link (tm-ref doc 1) t)))
           (if dest `(hlink ,text ,dest) text)
         ) ;let*
        ) ;
        ((tm-compound? doc)
         (cons (tm-label doc) (map (cut tmdoc-internalize* <> t) (tm-children doc)))
        ) ;
        (else "")
  ) ;cond
) ;define
...
(define (tmdoc-internalize doc)
  (with t (make-ahash-table) (tmdoc-label-table doc t) (tmdoc-internalize* doc t))
) ;define
...
(define (tmdoc-language file-name)
  (with t (tree-import file-name "texmacs") (tmfile-language t))
) ;define
...
(define-preferences ("manual style" "tmmanual" (lambda args (noop))))

(tmfs-permission-handler (help name type)
  (and (== type "read")
    (let* ((file (or (tmfs-cdr name) "")) (root (tmfs-string->url file)))
...
(define (tmdoc-find-title-list l)
  (and (nnull? l) (or (tmdoc-find-title (car l)) (tmdoc-find-title-list (cdr l))))
) ;define
...
(tm-define (tmdoc-find-title doc)
  (cond ((tm-atomic? doc) #f)
        ((tm-in? doc '(title doc-title tmdoc-title tmdoc-title* tmweb-title))
         (with title
           (cpp-texmacs->verbatim (tm-ref doc 0) #f "default")
...
(tmfs-title-handler (help name doc)
  (or (tmdoc-find-title doc) (string-append "tmfs://help/" (url->unix name)))
) ;tmfs-title-handler
...
(tm-define (tmdoc-expand-help-manual root)
  (tmdoc-expand-help-manual* root
    (lambda () (system-wait "Finishing manual" "(soon ready)"))
...
(define (tmdoc-remove-hyper-links l)
  (cond ((npair? l) l)
        ...
        (else (cons (tmdoc-remove-hyper-links (car l)) (tmdoc-remove-hyper-links (cdr l)))
        ) ;else
  ) ;cond
) ;define
