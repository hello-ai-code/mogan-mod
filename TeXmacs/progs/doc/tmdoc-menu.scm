...
(texmacs-module (doc tmdoc-menu)
  (:use (text text-menu) (doc tmdoc-edit) (doc apidoc-menu))
) ;texmacs-module
...
(menu-bind tmdoc-menu
  (when (in-text?)
    (if (not (in-tmweb?)) (-> "Meta data" (link tmdoc-meta-menu)))
    (if (in-tmweb?) (-> "Meta data" (link tmweb-meta-menu)))
  ...
  (if developer-mode? (-> "Scheme" (link apidoc-manual-menu)))
) ;menu-bind
...
(tm-menu (focus-document-extra-icons t)
  ...
  (minibar ((balloon "Title" "Insert title") (tmdoc-insert-title)))
) ;tm-menu

(tm-menu (focus-document-extra-icons t)
  (:require (tmdoc-propose-copyright-and-license?))
  (minibar ((balloon "Copyright" "Insert copyright and license information")
...
(tm-menu (focus-document-extra-icons t)
  ...
  (minibar ((balloon "Title" "Insert title") (tmweb-insert-title)))
) ;tm-menu
