...
(tm-define (inside-program?)
  ...
  (let* ((ct (cursor-tree)) (dt (tree-ref ct :up)))
...
(tm-define (program-row row)
  ...
  (and-with doc
    (program-tree)
    (and-with par (tree-ref doc row) (and (tree-atomic? par) (tree->string par)))
  ) ;and-with
) ;tm-define
...
(tm-define (program-go-to row col)
  ...
  (and-with doc (program-tree) (tree-go-to doc row col))
) ;tm-define


(tm-define (program-character path)
  (let ((s (tree->string (path->tree (cDr path)))) (pos (cAr path)))
...
(define-preferences ("prog:automatic brackets" "off" notify-auto-close-brackets)
...
(define (string-bracket-find s pos inc br ibr level)
  (with r (string-bracket-find* s pos inc br ibr level) (and (>= r 0) r))
) ;define

(tm-define (string-bracket-level s pos inc br ibr)
  (with ret
    (string-bracket-find* s pos inc br ibr 0)
    (if (< ret 0) (+ ret 1000) (string-bracket-level s (+ ret inc) inc br ibr))
  ) ;with
) ;tm-define
...
(define (program-bracket-find row col inc br ibr level)
  (and-with s
    (program-row row)
    (with ret
      (string-bracket-find* s col inc br ibr level)
      (if (>= ret 0)
        (cons row ret)
        (with level*
          (+ ret 1000)
          (and-with s*
            (program-row (+ row inc))
            (with col*
              (if (> inc 0) 0 (- (string-length s*) 1))
              (program-bracket-find (+ row inc) col* inc br ibr level*)
            ) ;with
          ) ;and-with
        ) ;with
      ) ;if
...
(tm-define (program-previous-match row br ibr)
  ...
  (let* ((s (program-row row)) (last (- (string-length s) 1)))
    (if (not s)
      row
      (with ret
        (string-bracket-level s last -1 br ibr)
        (if (== ret 0)
          row
          (with pos
            (program-bracket-find row last -1 br ibr -1)
            (if (not pos) row (car pos))
          ) ;with
        ) ;if
      ) ;with
    ) ;if
...
(tm-define (select-brackets path lb rb)
  ...
  (let ((prev (find-left-bracket path lb rb)) (next (find-right-bracket path lb rb)))
    (if (or (null? prev) (null? next))
      (if (nnull? (get-alt-selection "brackets")) (cancel-alt-selection "brackets"))
      (set-alt-selection "brackets" (list prev (path++ prev) next (path++ next)))
    ) ;if
  ) ;let
) ;tm-define
...
(tm-define (select-brackets-after-movement lbs rbs esc)
  ...
  (let* ((p (cursor-path))
    ...
    (cond (i1 (select-brackets p (string-ref* lbs i1) (string-ref* rbs i1)))
          ...
          ((nnull? (get-alt-selection "brackets")) (cancel-alt-selection "brackets"))
    ) ;cond
  ) ;let*
) ;tm-define
...
(tm-define (bracket-open lb rb esc)
  (if prog-auto-close-brackets?
    (if (selection-active-normal?)
      ...
      (with ch
        (or (before-cursor) "")
        (if (== ch esc) (insert lb) (insert-go-to (string-append lb rb) '(1)))
      ) ;with
    ) ;if
    (insert lb)
  ) ;if
...
(define-preferences ("editor:verbatim:tabstop" 4 (lambda (pref val) (noop))))
...
(tm-define (remove-tabstop)
  (with w
    (get-tabstop)
    (with c (program-get-indent) (if (>= c w) (program-set-indent (- c w))))
  ) ;with
) ;tm-define
...
(tm-define (string-get-indent s)
  ...
  (with pos
    (list-find-index (string->list s) char-non-space?)
...
(tm-define (program-get-indent)
  ...
  (and (inside-program?) (string-get-indent (program-row (program-row-number))))
) ;tm-define
...
(tm-define (program-set-indent i)
  ...
  (when (inside-program?)
    (with t (cursor-tree) (tree-set t (string-set-indent (tree->string t) i)))
  ) ;when
) ;tm-define
...
(tm-define (program-indent-line doc row unindent?)
  (let* ((i (program-compute-indentation doc row -1)) (t (tree-ref doc row)))
...
(tm-define (program-indent unindent?)
  (and-with doc
    (program-tree)
    (let* ((r (program-row-number)) (c (program-indent-line doc r unindent?)))
      (program-go-to r c)
    ) ;let*
  ) ;and-with
) ;tm-define
