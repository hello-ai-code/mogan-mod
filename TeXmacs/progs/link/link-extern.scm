...
(texmacs-module (link link-extern) (:use (link link-edit) (link link-navigate)))
...
(define (load-registry-sub x)
  (with (id . l) x (for-each (cut ahash-set! name-to-id-table <> id) l))
) ;define

(define (load-registry)
  (with name
    "$TEXMACS_HOME_PATH/system/registry.scm"
...
(define (id->locations id)
  ...
  (with name
    ...
    (if name
      (map unix->url name)
      (with l (ahash-ref* link-locator-table id '()) (append-map id->locations l))
    ) ;if
  ) ;with
) ;define
...
(define (resolve-id-sub id l)
  (cond ((null? l) #f)
        ...
        (else (ahash-set! already-loaded-table (car l) #t) (load-buffer (car l)))
  ) ;cond
) ;define
...
(define (register-unique-id here t)
  (with (dummy id) t (registry-add id (url->unix here)))
) ;define
...
(define (register-locator here t)
  (with (dummy id id2)
    t
    (ahash-set! link-locator-table
      id
      (list-union (list id2) (ahash-ref* link-locator-table id '()))
    ) ;ahash-set!
  ) ;with
) ;define
...
(tm-define (register-link-locations here t)
  ...
  (with l (cdr (tree->stree t)) (for-each (cut register-link-location here <>) l))
) ;tm-define
