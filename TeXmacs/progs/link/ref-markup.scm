...
(texmacs-module (link ref-markup) (:use (link ref-edit)))
...
(define (smart-extract* tp c tps)
  (if (null? c)
    (list)
    (with r
      (smart-extract* tp (cdr c) (cdr tps))
...
(define (smart-extract tp c tps)
  `(,(string->symbol (string-append tp "-ref")) ,@(smart-extract* tp c tps))
) ;define
...
(define (smart-list l)
  (cond ((<= (length l) 1) l)
        ((== (length l) 2) (list (car l) " " '(localize "and") '(nbsp) (cadr l)))
        (else (smart-list* l))
  ) ;cond
) ;define
