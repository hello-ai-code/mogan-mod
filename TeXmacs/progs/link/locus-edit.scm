...
(define (base62 x)
  (if (== x 0) '() (append (base62 (quotient x 62)) (list (remainder x 62))))
) ;define
...
(tm-define (create-unique-id)
  ...
  (string-append "+"
    (number->base62 texmacs-serial-pre)
...
(tm-define (locus-remove-link ln) (tree-remove (tree-up ln) (tree-index ln) 1))
...
(tm-define (locus-remove-all-links t ln)
  (for-each (cut locus-remove-match <> ln) (reverse (cDr (tree-children t))))
) ;tm-define
...
(define (tree->locus t)
  (and-with p (tree-up t) (and (tm-func? p 'locus) p))
) ;define

(tm-define (id->loci id) (filter-map tree->locus (id->trees id)))
