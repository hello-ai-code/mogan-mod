...
(define (navigation-bidirectional?)
...
(tm-define (navigation-toggle-external)
  ...
  (set-boolean-preference "external navigation" (not navigation-external-links?))
) ;tm-define

(define (navigation-build-link-pages?)
  navigation-link-pages?
) ;define
(tm-define (navigation-toggle-build-link-pages)
  ...
  (set-boolean-preference "link pages" (not navigation-link-pages?))
) ;tm-define
...
(tm-define (navigation-toggle-type type)
  ...
  (ahash-set! navigation-blocked-types
    type
    (not (ahash-ref navigation-blocked-types type))
  ) ;ahash-set!
) ;tm-define
...
(define (navigation-allow-no-types?)
  (with l
    (ahash-table->list navigation-blocked-types)
    (null? (list-difference (current-link-types) (map car (list-filter l cdr))))
  ) ;with
) ;define
(tm-define (navigation-allow-no-types)
  ...
  (for-each (cut ahash-set! navigation-blocked-types <> #t) (current-link-types))
) ;tm-define
...
(define-preferences ("bidirectional navigation" "off" notify-bidirectional-navigation)
...
(define (id->link-list id)
  (let* ((lns (vertex->links `(id ,id))) (sts (map link-flatten lns)))
...
(define (filter-on-bidirectional item)
  (== (link-item-id item) (vertex->id (car (link-item-vertices item))))
) ;define
...
(define (filter-link-list l event)
  (let* ((f1 (if (navigation-bidirectional?) l (list-filter l filter-on-bidirectional)))
...
(tm-define (exact-link-list t filter?)
  ...
  (with l
    (if (and filter? (not (navigation-external?)))
...
(tm-define (upward-link-list t filter?)
  ...
  (if (or (not t) (null? (tree->path t)))
    '()
    (with l
      (exact-link-list t filter?)
      (if (root? t) l (append l (upward-link-list (tree-up t) filter?)))
    ) ;with
  ) ;if
) ;tm-define
...
(tm-define (complete-link-list t filter?)
  ...
  (with l
    ...
    (if (tree-atomic? t)
      l
      (with ls
        (map (cut complete-link-list <> filter?) (tree-children t))
...
(define (link-active-upwards-sub t active-ids)
  (if (or (not t) (null? (tree->path t)))
    ...
    (let* ((ids (tree->ids t))
      ...
      (if (root? t) r (append r (link-active-upwards-sub (tree-up t) active-ids)))
    ) ;let*
  ) ;if
) ;define
...
(tm-define (link-active-ids l)
  ...
  (with r
    (filter-link-list (ids->link-list l) "hover")
...
(tm-define (link-mouse-ids l)
  ...
  (with r
    (exclude-link-list (ids->link-list l) "focus")
    (list-remove-duplicates (map link-item-id r))
  ) ;with
) ;tm-define
...
(define (navigation-list-sub type attrs nr source l)
  (if (null? l)
    ...
    (let* ((head (list type attrs nr source (car l)))
      ...
      (if (== source (vertex->id (car l))) tail (cons head tail))
    ) ;let*
  ) ;if
) ;define
...
(tm-define (navigation-list-xtypes l)
  (let* ((direct (navigation-list-filter l #t 1 #t))
         ...
         (itypes (map (cut string-append <> "*") (map navigation-type inverse)))
        ) ;
...
(define (resolve-navigation-list l fun)
  (if (null? l)
    ...
    (let* ((id (vertex->id (navigation-target (car l))))
      ...
      (if ok
        (resolve-navigation-list (cdr l) fun)
        (begin
          (resolve-id id)
...
(tm-define (build-enumeration l)
  (if (<= (length l) 1)
    l
    `((enumerate (document ,@(map (lambda (x) `(surround (item) ,"" ,x)) l))))
  ) ;if
) ;tm-define
...
(define (navigation-list-by-type->document type l)
  (cons `(strong ,type) (build-enumeration (map navigation-item->document l)))
) ;define
...
(define (navigation-list->document style l)
  (let* ((direct (navigation-list-filter l #t 1 #t))
         ...
         (direct-by-type (map (cut navigation-list-filter direct <> #t #f) direct-types))
         (inverse-by-type (map (cut navigation-list-filter inverse <> #t #f) inverse-types)
         ) ;inverse-by-type
        ) ;
    `(document (style ,style)
       (body (document (strong "Source")
               ,(automatic-link `(id ,(navigation-source (car l)))
                  "Unaccessible")
               ,@(append-map navigation-list-by-type->document
...
(define (build-navigation-page-sub style l)
  (with doc (navigation-list->document style l) (open-auxiliary "Link page" doc))
) ;define
...
(define (url->item u)
  (with (base qry)
    ...
    (let* ((file (url->system u))
           (help? (and (== "texmacs-file" (file-format u)) (url-exists-in-help? file)))
...
(define (default-filter-url u)
  (with (base qry)
    (process-url u)
    (or (== base "") (or (!= (url-root base) "default") (url-exists? base)))
  ) ;with
) ;define
  ...
  (with (base qry)
    ...
    (with fm
      ...
      (cond ((== qry "") (noop))
            ...
            (else (display* "Unhandled format for default queries: " fm "\n"))
      ) ;cond
    ) ;with
  ) ;with
) ;define
...
(define (url-handlers u)
  (with root
    (or (and (url-rooted? u) (url-root u)) "default")
    (cond ((== root "default") (list default-root-handler default-post-handler))
          ((== root "tmfs") (list tmfs-root-handler (lambda (x) (noop))))
          ((== root "file")
           ;; TODO: to be refined
          ...
          (else (list default-root-handler default-post-handler))
    ) ;cond
  ) ;with
) ;define
...
(tm-define (go-to-url u . opt-from)
  ...
  (with (action post) (url-handlers u) (action u) (post u))
  (if (nnull? opt-from) (cursor-history-add (cursor-path)))
) ;tm-define

(define (execute-at cmd opt-location)
  (if (null? opt-location)
    (exec-delayed cmd)
...
(define (navigation-item-follow hit)
  (let* ((source (navigation-source hit)) (target (navigation-target hit)))
    (id-set-visited source)
    (and-with target-id (vertex->id target) (id-set-visited target-id))
    (and-with target-url (vertex->url target) (url-set-visited target-url))
...
(tm-define (navigation-list-follow-xtyped xtype)
  ...
  (and-with hit
    (navigation-list-first-xtype the-navigation-list xtype)
...
(tm-define (navigation-list-follow nl)
  ...
  (with types
    (navigation-list-types nl)
    (if (and (>= (length types) 2) (in? "automatic" types))
      (with auto-nl
        (navigation-list-filter nl "automatic" #t #f)
        (set! nl (list-difference nl auto-nl))
      ) ;with
    ) ;if
    (with xtypes
      (navigation-list-xtypes nl)
      (cond ((null? xtypes) (noop))
            ...
            (else (set! the-navigation-list nl) (interactive navigation-list-follow-xtyped))
      ) ;cond
    ) ;with
  ) ;with
) ;tm-define
...
(tm-define (link-follow-ids ids event)
  ...
  (navigation-list-follow (link-list->navigation-list (filter-link-list (ids->link-list ids) event))
  ) ;navigation-list-follow
) ;tm-define

(tm-define (locus-link-follow)
  (:synopsis "Follow one of the links in the current locus")
  (let* ((ts (link-active-upwards (cursor-tree))) (ids (append-map tree->ids ts)))
...
(tm-define (show-hlink-tooltip ids)
  ...
  (when (nnull? ids)
    (let* ((items (ids->link-list ids))
      ...
      (close-tooltip)
      (delayed (:idle 100)
        (show-tooltip "link-tooltip" anchor-tree tip "Top" "Top" "default" 2.2)
      ) ;delayed
    ) ;let*
  ) ;when
) ;tm-define
