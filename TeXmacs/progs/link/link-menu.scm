...
(tm-define (toggle-locus-preserve-on-paper)
  ...
  (with val
    (if (test-locus-preserve-on-paper?) "change" "preserve")
...
(tm-define (interactive-change-locus-rendering var)
  ...
  (interactive (lambda (val) (change-locus-rendering var val))
    (list (logic-ref env-var-description% var) "string" (get-locus-rendering var))
  ) ;interactive
) ;tm-define
...
(tm-menu (link-delete-menu)
  (let* ((l1 (locus-link-types #t)) (l2 (list-sort l1 string<=?)))
...
(tm-menu (navigation-type-menu)
  (let* ((l1 (current-link-types)) (l2 (list-sort l1 string<=?)))
    (for (name l2) ((eval name) (navigation-toggle-type name)))
  ) ;let*
) ;tm-menu
...
(menu-bind link-menu
 ...
 (-> "Target"
   ...
   ((check "Url" "o" (link-target-is-url?)) (interactive link-set-target-url))
 ...
 (if (null? (locus-link-types #t)) (when #f ("Delete link" (noop))))
 (if (nnull? (locus-link-types #t)) (-> "Delete link" (link link-delete-menu)))
 ---
 (-> "Locus rendering"
 ...
 (-> "Active link types"
  ...
  (if (nnull? (current-link-types)) --- (link navigation-type-menu))
 ) ;->
 (-> "Extract"
