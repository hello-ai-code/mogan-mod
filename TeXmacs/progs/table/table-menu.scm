...
(texmacs-module (table table-menu)
  (:use (table table-edit) (generic generic-menu))
) ;texmacs-module
...
(menu-bind insert-table-menu
  (if (and (style-has? "std-dtd") (in-text?) (style-has? "env-float-dtd"))
    (when (not (selection-active-non-small?))
      ("Small table"
        (wrap-selection-small (insert-go-to '(small-table "" "") '(0 0))
          (make 'tabular)
        ) ;wrap-selection-small
      ) ;
      ("Big table"
        (wrap-selection-small (insert-go-to '(big-table "" (document "")) '(0 0))
          (make 'tabular)
        ) ;wrap-selection-small
      ) ;
      ---
  ...
  (if (and (style-has? "calc-dtd") (calc-ready?)) (link calc-table-menu) ---)
  ("Three line table" (make 'three-line-table))
  (if (not (in-math?)) ("Wide tabular" (make 'wide-tabular)))
  (when (not (selection-active-non-small?))
    ("Plain tabular" (make 'tabular))
    ("Centered tabular" (make 'tabular*))
  ) ;when
  (if (not (in-math?)) ("Wide block" (make 'wide-block)))
  (when (not (selection-active-non-small?))
...
(menu-bind cell-border-icons-menu
  (with width
    ...
    ((icon "tm_border_d.xpm") (cell-set-format* "cell-dborder" width))
    ((icon "tm_border_a.xpm") (cell-set-format* "cell-aborder" width))
  ) ;with
) ;menu-bind
...
(menu-bind cell-color-menu
 ...
 ---
 ("Palette"
   (interactive-background (lambda (col) (cell-set-background col)) '())
 ) ;
...
(menu-bind cell-split-join-menu
  (when (and (== (get-cell-mode) "cell") (not (selection-active-table?)))
    ("Insert subtable" (make-subtable))
  ) ;when
  (when (selection-active-table?)
    ("Join selected cells" (cell-set-span-selection))
  ) ;when
  (when (and (== (get-cell-mode) "cell") (cell-spans-more?))
...
(menu-bind table-menu
  ...
  (-> "Border color" (link table-border-color-menu))
...
(menu-bind cell-menu
  ...
  (-> "Border color" (link cell-border-color-menu))
...
(menu-bind table-insert-icons
 ((balloon (icon "tm_table_arow.xpm") "Insert row above") (table-insert-row #f))
 ((balloon (icon "tm_table_brow.xpm") "Insert row below") (table-insert-row #t))
 ((balloon (icon "tm_table_lcol.xpm") "Insert column to the left")
  (table-insert-column #f)
 ) ;
 ((balloon (icon "tm_table_rcol.xpm") "Insert column to the right")
  (table-insert-column #t)
 ) ;
 ---
 ((balloon (icon "tm_table_drow.xpm") "Delete row") (table-remove-row #t))
 ((balloon (icon "tm_table_dcol.xpm") "Delete column") (table-remove-column #t))
) ;menu-bind

(menu-bind table-hpos-icons
 ((balloon (icon "tm_table_left.xpm") "Align table at the left")
  (table-set-halign "l")
 ) ;
 ((balloon (icon "tm_table_hcenter.xpm") "Align table at the horizontal center")
  (table-set-halign "c")
 ) ;
 ((balloon (icon "tm_table_right.xpm") "Align table to the right")
  (table-set-halign "r")
 ) ;
 ((icon "tm_empty.xpm") (noop))
 ((balloon (icon "tm_table_bleft.xpm") "Align table at base of left column")
  (table-set-halign "L")
 ) ;
 ((balloon (icon "tm_table_bhcenter.xpm") "Align table at base of middle column")
  (table-set-halign "C")
 ) ;
 ((balloon (icon "tm_table_bright.xpm") "Align table at base of right column")
  (table-set-halign "R")
 ) ;
 ((balloon (icon "tm_table_hbase.xpm") "Align table at base of origin column")
  (table-set-halign "O")
 ) ;
) ;menu-bind

(menu-bind table-vpos-icons
 ((balloon (icon "tm_table_bottom.xpm") "Align table at the bottom")
  (table-set-valign "b")
 ) ;
 ((balloon (icon "tm_table_vcenter.xpm") "Align table at the center")
  (table-set-valign "c")
 ) ;
 ((balloon (icon "tm_table_top.xpm") "Align table at the top")
  (table-set-valign "t")
 ) ;
 ((balloon (icon "tm_table_vfrac.xpm") "Align table at axis")
  (table-set-valign "f")
 ) ;
 ((balloon (icon "tm_table_bbottom.xpm") "Align table at the base of bottom row")
  (table-set-valign "B")
 ) ;
 ((balloon (icon "tm_table_bvcenter.xpm") "Align table at the base of middle row")
  (table-set-valign "C")
 ) ;
 ((balloon (icon "tm_table_btop.xpm") "Align table at the base of top row")
  (table-set-valign "T")
 ) ;
 ((balloon (icon "tm_table_vbase.xpm") "Align table at the base of origin row")
  (table-set-valign "O")
 ) ;
) ;menu-bind

(menu-bind cell-mode-icons
 ((balloon (icon "tm_cell_by_cell.xpm") "Perform operations on cells")
  (set-cell-mode "cell")
 ) ;
 ((balloon (icon "tm_cell_by_row.xpm") "Perform operations on rows")
  (set-cell-mode "row")
 ) ;
 ((balloon (icon "tm_cell_by_column.xpm") "Perform operations on columns")
  (set-cell-mode "column")
 ) ;
 ((balloon (icon "tm_cell_by_table.xpm") "Perform operations on entire table")
  (set-cell-mode "table")
 ) ;
) ;menu-bind

(menu-bind cell-pos-icons
 ((balloon (icon "tm_cell_left.xpm") "Align cell to the left")
  (cell-set-halign "l")
 ) ;
 ((balloon (icon "tm_cell_hcenter.xpm") "Horizontally center the cell")
  (cell-set-halign "c")
 ) ;
 ((balloon (icon "tm_cell_right.xpm") "Align cell to the right")
  (cell-set-halign "r")
 ) ;
 ((balloon (icon "tm_cell_hbase.xpm") "Horizontally align cell to the base")
  (cell-set-halign "R")
 ) ;
 ((balloon (icon "tm_cell_hdot.xpm") "Align cell to the decimal dot")
  (cell-set-halign "L.")
 ) ;
 ((balloon (icon "tm_cell_hcomma.xpm") "Align cell to the decimal comma")
  (cell-set-halign "L,")
 ) ;
 ((balloon (icon "tm_cell_bottom.xpm") "Align cell to the bottom")
  (cell-set-valign "b")
 ) ;
 ((balloon (icon "tm_cell_vcenter.xpm") "Vertically center the cell")
  (cell-set-valign "c")
 ) ;
 ((balloon (icon "tm_cell_top.xpm") "Align cell to the top")
  (cell-set-valign "t")
 ) ;
 ((balloon (icon "tm_cell_vbase.xpm") "Vertically align cell to the base")
  (cell-set-valign "B")
 ) ;
) ;menu-bind

(menu-bind cell-size-icons
 ((balloon (icon "tm_cell_width.xpm") "Set width of cell")
 ...
 ((balloon (icon "tm_cell_hexact.xpm") "Width is exactly the specified width")
 ...
 ((balloon (icon "tm_cell_height.xpm") "Set height of cell")
 ...
 ((balloon (icon "tm_cell_vexact.xpm") "Height is exactly the specified height")
...
(menu-bind cell-decoration-icons
 ((balloon (icon "tm_decorate_left.xpm") "Use left hand column as border")
  (table-column-decoration #f)
 ) ;
 ((balloon (icon "tm_decorate_right.xpm") "Use right hand column as border")
  (table-column-decoration #t)
 ) ;
 ((balloon (icon "tm_decorate_up.xpm") "Use row above as border")
  (table-row-decoration #f)
 ) ;
 ((balloon (icon "tm_decorate_down.xpm") "Use row below as border")
...
(define (document-like? t)
  (or (tree-is? t 'document)
    (and (tree-is? t 'with) (document-like? (tree-up t)))
  ) ;or
) ;define
...
(tm-menu (focus-extra-icons t)
  ...
  (minibar (if (== (get-cell-mode) "cell")
    ...
    (if (and (cell-spans-more?) (not (selection-active-table?)))
     ...
     ((balloon (icon "tm_cell_split.xpm") "Dissociate joined cells")
      (cell-reset-span)
     ) ;
    ) ;if
    (if (selection-active-table?)
     ((balloon (icon "tm_cell_join.xpm") "Join selected cells")
