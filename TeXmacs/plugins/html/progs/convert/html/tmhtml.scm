...
(define tmhtml-image-progress 0)
...
(tm-define (tmhtml-initialize opts)
  ...
  (set! tmhtml-css? (== (assoc-ref opts "texmacs->html:css") "on"))
  (set! tmhtml-mathjax? (== (assoc-ref opts "texmacs->html:mathjax") "on"))
  (set! tmhtml-mathml? (== (assoc-ref opts "texmacs->html:mathml") "on"))
  (set! tmhtml-images? (== (assoc-ref opts "texmacs->html:images") "on"))
  (set! tmhtml-base64? (not (== (get-preference "texmacs->html:base64") "off")))
  (set! tmhtml-image-cache (make-ahash-table))
  (set! tmhtml-image-progress 0)
  (let* ((suffix (url-suffix current-save-target)) (n (+ (string-length suffix) 1)))
    (if (in? suffix '("html" "xhtml"))
      (begin
        (set! tmhtml-image-serial 0)
        (set! tmhtml-image-root-url (url-unglue current-save-target n))
        (set! tmhtml-image-root-string (url->unix (url-tail tmhtml-image-root-url)))
      ) ;begin
      (begin
        (set! tmhtml-image-serial 0)
        (set! tmhtml-image-root-url (url-temp))
        (set! tmhtml-image-root-string (url->unix tmhtml-image-root-url))
      ) ;begin
    ) ;if
  ) ;let*
) ;tm-define
...
(define (tmhtml-sub-token s pos)
  (with ss
    ...
    (if (= (string-length ss) 1) ss (tmhtml-math-token (string-append "<" ss ">")))
  ) ;with
) ;define
...
(define (tmhtml-string s)
  (if (ahash-ref tmhtml-env :math)
    (tmhtml-post-simplify-nodes (map tmhtml-math-token (tmconcat-tokenize-math s)))
    (list (cork->html s))
  ) ;if
) ;define
...
(define (make-ligatures s)
  ...
  (string-replace (string-replace (string-replace s "--" cork-endash) "``" cork-ldquo)
    "''"
    cork-rdquo
  ) ;string-replace
) ;define
...
(define (with-extract* w var)
  (with post
    (lambda (x) (if (tm-func? x 'quote 1) (cadr x) x))
...
(define (tmhtml-file l)
  ...
  (let* ((doc (car l))
    ...
    (set! tmhtml-site-version (with-extract doc "html-site-version"))

    (set! title
      (cond ((with-extract doc "html-title") (with-extract doc "html-title"))
            ((and (not title) (with-extract doc "html-doc-title"))
             (with-extract doc "html-doc-title")
            ) ;
            ((not title) "No title")
            ((or (in? "tmdoc" styles) (in? "tmweb" styles) (in? "tmweb2" styles))
             `(concat ,(tmhtml-force-string title) ," (FSF GNU project)")
            ) ;
    ...
    (if (and (not (func? css 'h:link))
          (string-ends? (get-preference "texmacs->html:css-stylesheet") ".css")
        ) ;and
      (with src
        (get-preference "texmacs->html:css-stylesheet")
        (with link-css
          `(h:link (@ (rel "stylesheet") (href ,src) (type "text/css")))
    ...
    (if (tm-func? (with-extract* doc "html-extra-javascript-src") 'tuple)
      (for (src (cdr (with-extract* doc "html-extra-javascript-src")))
        (with script
          `(h:script (@ (language "javascript") (src ,src) (defer "<implicit>")))
          (set! xhead (append xhead (list script)))
        ) ;with
      ) ;for
    ) ;if
    (if (tm-func? (with-extract* doc "html-extra-javascript") 'tuple)
      (for (code (cdr (with-extract* doc "html-extra-javascript")))
        (with script
          `(h:script (@ (language "javascript") (defer "<implicit>")) ,code)
          (set! xhead (append xhead (list script)))
        ) ;with
      ) ;for
    ) ;if
    ...
    (if tmhtml-css? (set! body (tmhtml-css-post body)))
    `(h:html (h:head (h:title ,@(tmhtml title))
               (h:meta (@ (name "generator")
                         (content ,(string-append "MoganSTEMSuite "
                                     (xmacs-version)))))
...
(define (tmhtml-compute-max-vspace l after?)
  (and (nnull? l)
    (with s1
      (tmhtml-compute-vspace (car l) after?)
      (with s2
...
(define (xhtml-block? x)
  (tm-in? x
    '(h:p h:div h:pre h:h1 h:h2 h:h3 h:h4 h:h5 h:h6 h:ol h:ul h:dl h:table)
  ) ;tm-in?
) ;define
...
(define (as-block x)
  ...
  (cond ((func? x 'h:div) (tmhtml-simplify-div x))
        ((and (func? x 'h:p)
         ...
         (let* ((a (sxml-attr-list x)) (m (mixed-block (sxml-content x))))
...
(define (as-blocks l)
  (cond ((null? l) l)
        ...
        (else (cons (as-block (car l)) (as-blocks (cdr l))))
  ) ;cond
) ;define

(define (add-style-attr x s1 s2)
  (with s
    (or (and s1 s2 (string-append s1 "; " s2)) s1 s2)
    (cond ((not s) x)
...
(define (tmhtml-paragraph l)
  (let rec
    ...
    (if (null? l)
      '()
      (let ((h (tmhtml (car l))) (r (rec (cdr l))))
        (cond ((null? h) r)
              ((null? r) h)
              (else `(,@h (h:br) ,@r))
        ) ;cond
...
(define (serialize-paragraph x)
  (serialize-concat x)
  (with l
    (tmconcat-simplify (reverse concat-done))
...
(define (blockify x)
  (cond ((func? x 'document) x)
        ((or (func? x 'surround 3) (func? x 'with)) (rcons (cDr x) (blockify (cAr x))))
        (else `(document ,x))
  ) ;cond
) ;define
...
(define (tmhtml-post-heading l)
  ...
  (receive (labels-before rest)
    (list-span l sxhtml-label?)
    (receive (heading rest)
      (car+cdr rest)
      (if (sxml-attr heading 'id)
        `(,heading ,@labels-before ,@rest)
        (receive (labels-after rest)
          (list-partition rest sxhtml-label?)
          (let ((labels (append labels-before labels-after)))
            (if (null? labels)
              l
              (cons (sxml-prepend (sxhtml-glue-label heading (car labels)) (cdr labels)) rest)
            ) ;if
          ) ;let
        ) ;receive
      ) ;if
  ...
  (define (glue-label-to-table x knil)
    (cond ((null? knil) (list x))
          ((and (sxhtml-label? x)
           ...
           (cons (sxhtml-glue-label (car knil) x) (cdr knil))
          ) ;
          ((and (sxhtml-table? x) (not (sxml-attr x 'id)) (sxhtml-label? (car knil)))
           (cons (sxhtml-glue-label x (car knil)) (cdr knil))
          ) ;
          (else (cons x knil))
    ) ;cond
  ) ;define
...
(define (tmhtml-align-left l)
  (with r
    (tmhtml-concat l)
    (if (in? r '(() (""))) '() `((h:div (@ (class "left-tab")) ,@r)))
  ) ;with
) ;define
...
(define (tmhtml-align-middle l)
  (with r
    (tmhtml-concat l)
    (if (in? r '(() (""))) '() `((h:div (@ (class "center-tab")) ,@r)))
  ) ;with
) ;define
...
(define (tmhtml-align-right l)
  (with r
    (tmhtml-concat l)
    (if (in? r '(() (""))) '() `((h:div (@ (class "right-tab")) ,@r)))
  ) ;with
) ;define
...
(define (tmhtml-post-simplify-nodes l)
  ...
  (let rec
    ((l l))
    (cond ((null? l) '())
          ((and (string? (car l)) (string-null? (car l))) (rec (cdr l)))
          ((null? (cdr l)) l)
...
(define (tmhtml-post-simplify-element e)
  ...
  (list (append (sxml-element-head e) (tmhtml-post-simplify-nodes (sxml-content e)))
  ) ;list
) ;define
...
(define (tmhtml-hspace l)
  (with len
    (tmlength->htmllength (if (list-1? l) (car l) (cadr l)) #t)
    (if (not len) '() `((h:span (@ (style ,(string-append "margin-left: " len))))))
  ) ;with
) ;define
...
(define (tmhtml-with-math-display val arg)
  (ahash-with tmhtml-env :math-display (== val "true") (tmhtml arg))
) ;define
...
(define (font-size-string->number str)
  (if (string=? str "") 0 (string->number str))
) ;define
...
(define (tmhtml-with-font-size val arg)
  (let* (
          ...
          (font-base-size (if font-base-size-str (font-size-string->number font-base-size-str) 10)
          ) ;font-base-size
...
(define (tmhtml-with-par-left val arg)
  (with x
    ...
    (if (not x)
      (tmhtml arg)
      (with d
        (- x (ahash-ref tmhtml-env :left-margin))
        (with s
          (string-append "margin-left: " (number->htmlstring d) "px")
          (ahash-with tmhtml-env :left-margin x (tmhtml-with-block s arg))
        ) ;with
      ) ;with
    ) ;if
  ) ;with
...
(define (tmhtml-with-par-right val arg)
  (with x
    ...
    (if (not x)
      (tmhtml arg)
      (with d
        (- x (ahash-ref tmhtml-env :right-margin))
        (with s
          (string-append "margin-right: " (number->htmlstring d) "px")
          (ahash-with tmhtml-env :right-margin x (tmhtml-with-block s arg))
        ) ;with
      ) ;with
    ) ;if
  ) ;with
...
(define (tmhtml-with-par-first val arg)
  (with x
    (tmlength->htmllength val #t)
    (if (not x)
      (tmhtml arg)
      (with s (string-append "text-indent: " x) (tmhtml-with-block s arg))
    ) ;if
  ) ;with
) ;define
...
(define (tmhtml-force-string x)
  (cond ((string? x) x)
        ...
        ((func? x 'tmlen 1) (string-append (tmhtml-force-string (cadr x)) "tmpt"))
        ((func? x 'tmlen 3) (string-append (tmhtml-force-string (caddr x)) "tmpt"))
        ((func? x 'tuple)
         (apply string-append (list-intersperse (map tmhtml-force-string (cdr x)) ";"))
        ) ;
        ;; (else (force-string x))))
...
(define (tmhtml-include l)
  (if (or (npair? l) (nstring? (car l)))
    '()
    (with u
      (url-relative current-save-source (unix->url (car l)))
      (if (not (url-exists? u))
        '()
        (with-global current-save-source
          u
          (with t (tree-inclusion u) (tmhtml (tm->stree t)))
        ) ;with-global
      ) ;if
    ) ;with
  ) ;if
...
(define (tmhtml-src-args l)
  (if (null? l) l `(,(blue "|") ,@(tmhtml (car l)) ,@(tmhtml-src-args (cdr l))))
) ;define
...
(define (tmhtml-open-tag l)
  `(,(blue "&lt;\\") ,@(tmhtml (car l)) ,@(tmhtml-src-args (cdr l)) ,(blue "|"))
) ;define

(define (tmhtml-middle-tag l)
  `(,@(tmhtml-src-args (cdr l)) ,(blue "|"))
) ;define
...
(define (tmhtml-suffix s)
  ...
  (let* ((sdir (string-rindex s #\/)) (sep (string-rindex s #\#)))
    (cond ((or (string-starts? s "http:")
          ...
          ((and sep (or (not sdir) (< sdir sep)))
           (string-append (tmhtml-suffix (substring s 0 sep)) (string-drop s sep))
          ) ;
          ((string-ends? s ".tm") (string-append (string-drop-right s 3) ".html"))
          ((string-ends? s ".texmacs") (string-append (string-drop-right s 8) ".tm"))
          (else s)
    ) ;cond
  ) ;let*
) ;define
...
(define (tmhtml-hyperlink l)
  ...
  (let* ((body (tmhtml (first l))) (to (cork->html (force-string (second l)))))
    (if (string-starts? to "$")
      body
      ;; temporary fix for URLs like $TEXMACS_PATH/...
...
(define (tmhtml-action l)
  (if tmhtml-css? (tmhtml (car l)) `((h:u ,@(tmhtml (car l)))))
) ;define
...
(define (html-css-attrs l)
  ...
  (if (null? l)
    '()
    (receive (css html)
      (list-partition l string?)
      (if (nnull? css)
        (with style
          (apply string-append (list-intersperse css "; "))
...
(define (tmhtml-width-replace attrl sum)
  (with part
    (tmhtml-width-part attrl)
    (if (== part 0)
      attrl
      (with l
        (list-filter attrl (lambda (x) (!= (car x) "cell-width")))
        (with w
          (number->htmlstring (/ part sum))
...
(define (tmhtml-make-cells l cellf)
  (let* ((partl (map tmhtml-width-part cellf)) (sum (apply + partl)))
...
(define (tmhtml-make-column-group colf)
  (if (list-every null? colf) '() `((h:colgroup ,@(map tmhtml-make-col colf))))
) ;define
...
(define (tmhtml-make-table t tablef colf rowf cellf)
  (let* ((attrs (map* tmhtml-make-table-attr tablef)) (va "vertical-align: middle"))
...
(define (tmhtml-image-names ext)
  ...
  (let* ((postfix (string-append "-" (number->string tmhtml-image-serial) "." ext))
...
(define (tmhtml-png y)
  (let* ((mag (ahash-ref tmhtml-env :mag))
    ...
    (with cached
      (ahash-ref tmhtml-image-cache x)
      (if (not cached)
        (receive (name-url name-string)
          (tmhtml-image-names "png")
          (set! tmhtml-image-progress (+ tmhtml-image-progress 1))
          (set-message (string-append "Exporting inline image "
                         (number->string tmhtml-image-progress)
                         " ..."
                       ) ;string-append
            "HTML Export"
          ) ;set-message
          (when (and (qt-gui?) (> tmhtml-image-total 0))
            (html-progress-update tmhtml-image-progress)
          ) ;when
...
(define (tmhtml-image-name name)
  ...
  (with u
    (url-relative current-save-target (unix->url name))
...
(define (hex-char->int c)
  (let ((val (char->integer c)))
    (cond ((and (>= val 48) (<= val 57)) (- val 48))
          ((and (>= val 97) (<= val 102)) (- val 87))
          ((and (>= val 65) (<= val 70)) (- val 55))
          (else 0)
    ) ;cond
  ) ;let
) ;define

(define (hex-string->byte-vector s)
  (let* ((len (string-length s))
         (out-len (quotient len 2))
         (bv (make-byte-vector out-len))
        ) ;
    (if (odd? len)
      #u8()
      (begin
...
(define (tmhtml-decode-base64-string s)
  (import (liii base64))
  (bytevector-base64-decode (string->utf8 s))
) ;define

(tm-define (tmhtml-extract-embedded name)
...
(define (tmhtml-post-item args)
  (let* ((i (car args)) (r (tmhtml (cadr args))))
    (if (or (tm-is? i 'item) (null? (cdr i)))
      `((h:li ,@r))
      `((h:dt ,@(tmhtml (cadr i))) (h:dd ,@r))
    ) ;if
  ) ;let*
) ;define
  ...
  (with body
    ...
    (with pre?
      ...
      (if (or (stm-block-structure? body) pre?)
        (verbatim-pre (ahash-with tmhtml-env :preformatted #t (tmhtml body)))
...
(define (tmhtml-doc-title-block l)
  `((h:table (@ (class "title-block")) (h:tr (h:td ,@(tmhtml (car l))))))
) ;define

(define (tmhtml-equation* l)
  (with first
    (simplify-document (car l))
    (with x
      `(with ,"mode" ,"math" (with ,"math-display" ,"true" ,first))
      (ahash-with tmhtml-env :math-display #t `((h:center ,@(tmhtml x))))
    ) ;with
  ) ;with
) ;define
...
(define (tmhtml-mathjax-formula x)
  (cond ((tm-func? x 'with 1) (tmhtml-mathjax-formula (cadr x)))
        ((and (tm-func? x 'with 3) (== (cadr x) "math-display") (== (caddr x) "true"))
         (ahash-with tmhtml-env :math-display #t (tmhtml-mathjax-formula (cadddr x)))
        ) ;
...
(define (tmhtml-tmdoc-post body)
  (with r
    (append-map tmhtml-tmdoc-post-sub body)
...
(define (tmhtml-breaks-post* l)
  (let* ((n (length l)) (h (quotient n 2)))
    (if (<= n 10000)
      (tmhtml-breaks-post l)
...
(define (tmhtml-implicit-compound l)
  (or (tmhtml-dispatch 'tmhtml-stdmarkup% l)
    (and (!= tmhtml-site-version "2") (tmhtml-dispatch 'tmhtml-tmdocmarkup% l))
    '()
  ) ;or
) ;define
...
(tm-define (tmhtml x)
  ...
  (cond ((and tmhtml-mathjax? (ahash-ref tmhtml-env :math)) (tmhtml-mathjax-formula x))
...
(logic-dispatcher tmhtml-primitives%
  ...
  ((:or old-matrix old-table old-mosaic old-mosaic-item) tmhtml-noop)
  (table tmhtml-table)
...
(logic-table tmhtml-with-cmd%
 ...
 (("ornament-hpadding")
  (:ornament-hpadding "padding-left" "padding-right" ,tmlength->htmllength)
 ) ;
 (("ornament-vpadding")
  (:ornament-vpadding "padding-top" "padding-bottom" ,tmlength->htmllength)
 ) ;
 (("ornament-border") (:ornament-border "border-width" ,tmlength->htmllength))
 (("ornament-shape") (:ornament-shape "border-radius" ,tmshape->htmllength))
 (("ornament-color") (:ornament-color "background-color" ,tmcolor->htmlcolor))
 (("ornament-shadow-color")
  (:ornament-shadow-color "border-bottom-color"
    "border-right-color"
    ,tmcolor->htmlcolor
  ) ;:ornament-shadow-color
 ) ;
 (("ornament-sunny-color")
  (:ornament-sunny-color "border-left-color"
    "border-top-color"
    ,tmcolor->htmlcolor
  ) ;:ornament-sunny-color
 ) ;
 ...
 (("font-shape" "small-caps") (h:class (@ (style "font-variant: small-caps"))))
) ;logic-table
...
(define (strip-latex-spacing-artifacts x)
  (cond ((not (pair? x)) x)
        ...
        (else (map strip-latex-spacing-artifacts x))
  ) ;cond
) ;define
  ...
  (let ((x* (strip-latex-spacing-artifacts x)))
    (if (tmfile? x*)
      ...
      (begin
        ...
        (with res
         ((if (func? x* '!file) tmhtml-finalize-document tmhtml-finalize-selection)
          (tmhtml-root x*)
         ) ;
         (when (and (qt-gui?) (> tmhtml-image-total 0))
           (html-progress-end)
