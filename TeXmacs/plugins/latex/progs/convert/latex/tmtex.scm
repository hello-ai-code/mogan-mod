...
(define (tmtex-wide l)
  (let ((wide? (tmtex-wide? (car l)))
    ...
    (cond ((nstring? acc) arg)
          ((in? acc '("<hat>" "^")) (list (if text? '^ (if wide? 'widehat 'hat)) arg))
          ((in? acc '("<tilde>" "~"))
           (list (if text? '~ (if wide? 'widetilde 'tilde)) arg)
          ) ;
          ((== (cadr l) "<wide-bar>") (list (if text? '= 'overline) arg))
          ((== acc "<bar>") (list (if text? '= (if wide? 'overline 'bar)) arg))
          ...
          ((== acc "<acute>") (list (if text? (string->symbol "'") 'acute) arg))
          ((== acc "<grave>") (list (if text? (string->symbol "`") 'grave) arg))
          ((== acc "<dot>") (list (if text? (string->symbol ".") 'dot) arg))
          ((== acc "<ddot>") (list (if text? (string->symbol "\"") 'ddot) arg))
...
(define (unescape-angles l)
  (cond ((string? l) (string-replace (string-replace l "<less>" "<") "<gtr>" ">"))
        ((symbol? l) l)
