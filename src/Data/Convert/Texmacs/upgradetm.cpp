  ...
  if (is_atomic (t)) {
    ...
    for (i= 0; i < n;) {
      if (t->label[i] == '<') {
        ...
        int start= i++;
        while ((i < n) && (t->label[i] == t->label[i - 1]))
          i++;
        if (s != "") {
          r << s;
          s= "";
    ...
    for (i= 0; i < n; i++) {
      ...
      if (is_func (u, SET)) {
        ...
        if (u == tree (RESET, "mode"))
          if (!is_nil (mode_stack)) mode_stack= mode_stack->next;
      }
      else if (is_func (u, BEGIN, 1)) {
        if ((u[0] == "equation") || (u[0] == "equation*") ||
            (u[0] == "eqnarray*") || (u[0] == "leqnarray*"))
...
static charp        apply_expand_value_strings[]= {
    "part", "part*", "chapter", "chapter*", "appendix", "section", "section*",
    "subsection", "subsection*", "subsubsection", "subsubsection*", "paragraph",
    "paragraph*", "subparagraph", "subparagraph*", "footnote", "item*",
    "overline", "underline", "mathord", "mathbin", "mathopen", "mathpunct",
    "mathop", "mathrel", "mathclose", "mathalpha", "op", "strong", "em", "tt",
    "name", "samp", "abbr", "dfn", "kbd", "var", "acronym", "person", "menu",
    "submenu", "subsubmenu", "tmdef", "tmref", "key", "skey", "ckey", "akey",
    "mkey", "hkey", "include-document", "include-project", "globalize-variable",
    "localize-variable", "assign-variable", "gb", "cgb", "gbt", "cgbt", "head",
    "tail", "hm", "tm", "binom",
...
matching (tree open, tree close) {
  ...
  if (is_func (open, BEGIN)) {
    if (is_func (close, END, 1)) return open[0] == close[0];
...
concat_search (tree t, int& i, tree open= "") {
  ...
  while (i < n) {
    ...
    if (begin_end && is_func (t[i], END, 2) && t[i][0] == "algo-repeat") return;
    i++;
  }
}
...
concat_replace (tree t, int i1, int i2) {
  ...
  if (is_func (t[i1], SET)) return with_replace (t[i1][0], t[i1][1], v);
...
document_search (tree t, int& i, int& j, tree open= "") {
  ...
  while (i < n) {
    ...
    while (j < k) {
      ...
      if (begin_end && is_func (t[i][j], END, 2) && t[i][j][0] == "algo-repeat")
        return;
      j++;
    }
    i++;
...
static void
document_merge (tree& doc, tree& con, tree doc_t, int doc_1, int con_1,
                int doc_2, int con_2) {
...
document_replace (tree doc_t, int doc_1, int con_1, int doc_2, int con_2) {
  ...
  bool flag=
      (doc_1 != doc_2) || ((con_1 == 0) && (con_2 == N (doc_t[doc_2]) - 1));
  /*
  ...
  */
  ...
  if (is_func (doc_t[doc_1][con_1], SET))
    return with_replace (doc_t[doc_1][con_1][0], doc_t[doc_1][con_1][1], doc_b);
  else return expand_replace (doc_t[doc_1][con_1], doc_t[doc_2][con_2], doc_b);
}
...
upgrade_set_begin_concat (tree t) {
  ...
  do {
    t= u;
    u= upgrade_set_begin_concat_once (t);
  } while (u != t);
  ...
  while ((doc_i < N (doc_t)) && (con_i < N (doc_t[doc_i]))) {
    ...
    if ((doc_2 < N (doc_t)) &&
      ...
      if (is_func (ins, EXPAND, 2)) {
        ...
        else if (ins[0] == "abstract") upgrade_abstract_expand (doc, con, ins);
        else con << ins;
      }
      else if (is_func (ins, WITH) && is_func (ins[N (ins) - 1], DOCUMENT, 1)) {
...
upgrade_set_begin_surround (tree t, tree search, bool& found) {
  if (t == search) {
    found= true;
    if (upgrade_tex_flag) return tree (DOCUMENT, copy (t));
    return copy (t);
  }
...
upgrade_env_args (tree t, tree env) {
  if (is_atomic (t)) return t;
    ...
    int i, k= N (env);
    for (i= 0; i < k - 2; i++)
      if (t[0] == env[i]) return tree (ARG, t[0]);
    return t;
  }
  else {
    int  i, n= N (t);
...
eliminate_set_begin (tree t) {
  if (is_atomic (t)) return t;
  if (is_func (t, SET) || is_func (t, RESET) || is_func (t, BEGIN) ||
      is_func (t, END) || is_func (t, ENV))
    return "";
...
with_needs_surrounding (string s) {
  return (s == "paragraph mode") || (s == "paragraph hyphenation") ||
...
needs_surrounding (tree t) {
  ...
  if (is_func (t, WITH)) {
    int i, n= N (t) - 1;
    for (i= 0; i < n; i+= 2)
      if (is_atomic (t[i]) && with_needs_surrounding (t[i]->label)) return true;
  }
  return false;
}
...
handle_mosaic_format (tree& fm, tree t, int i, int j) {
  ...
  tree w (CWITH);
  w << as_string (i + 1) << as_string (j + 1) << as_string (i + 1)
    << as_string (j + 1);
...
upgrade_table (tree t) {
  if (is_atomic (t)) return t;
  else if (is_func (t, OLD_MATRIX) || is_func (t, OLD_TABLE) ||
...
upgrade_split (tree t, bool eq= false) {
  ...
  if (is_atomic (t)) return t;
    ...
    r= tree (SURROUND, upgrade_split (r), upgrade_split (t[1]),
             upgrade_split (u));
    return r;
  }
  else if (is_concat (t)) {
    ...
    if (split != "") {
      ...
      if ((split[N (split) - 1] == "left") ||
          (split[N (split) - 1] == "justify"))
        tf << tree (CWITH, "1", "-1", "-1", "-1", "cell hpart", "1");
      for (i= 0; i < N (split); i++) {
        tree with (CWITH);
        int  j= (i == N (split) - 1) ? -1 : i + 1;
        with << "1"
             << "-1" << as_string (j) << as_string (j) << "cell halign";
    ...
    if (n == 1 || is_func (t, EXPAND, 2)) {
      ...
      if (s == "eqnarray" || s == "align" || s == "multline" || s == "gather" ||
          s == "eqsplit") {
        tree arg= t[n - 1];
...
upgrade_title (tree t, tree& tit, tree& auth, tree& meta) {
  if (is_atomic (t)) return t;
  else if (is_func (t, APPLY, 2) || is_func (t, EXPAND, 2)) {
    if (t[0] == "title") {
      tit << tree (EXPAND, "title", t[1]);
      return "";
    }
    if (t[0] == "author") {
      auth << tree (EXPAND, "author", t[1]);
      return "";
    }
    if (t[0] == "address") {
      auth << tree (EXPAND, "address", t[1]);
      return "";
    }
    if (t[0] == "urladdr") {
      auth << tree (EXPAND, "title-web", t[1]);
      return "";
    }
    if (t[0] == "title-email") {
      auth << tree (EXPAND, "title-email", t[1]);
      return "";
    }
    if (t[0] == "title-thanks") {
      meta << tree (EXPAND, "title-thanks", t[1]);
      return "";
    }
    if (t[0] == "keywords") {
      meta << tree (EXPAND, "title-keywords", t[1]);
      return "";
    }
    if (t[0] == "subjclass") {
      meta << tree (EXPAND, "title-ams-class", t[1]);
      return "";
    }
  ...
  else if (is_func (t, APPLY, 3) || is_func (t, EXPAND, 3)) {
    if (t[0] == "subjclass*") {
      meta << tree (EXPAND, "title-ams-class", t[2]);
      return "";
    }
  }
...
upgrade_expand (tree t, tree_label WHICH_EXPAND) {
  ...
  else if (is_func (t, ASSIGN, 2) && (t[0] == "quote") &&
           is_func (t[1], MACRO)) {
...
upgrade_apply (tree t) {
  ...
  */
  ...
  int  i, n= N (t);
  tree r (t, n);
  if (is_func (t, APPLY)) r= tree (COMPOUND, n);
  for (i= 0; i < n; i++)
...
static charp var_rename[]= {"shrinking factor",
                            "sfactor",
                            "info flag",
                            "info-flag",
    ...
    for (i= 0; i < n; i++) {
      ...
      if (is_atomic (u) && H->contains (u->label))
        if (((L (t) == WITH) && ((i % 2) == 0) && (i < n - 1)) ||
            ((L (t) == ASSIGN) && (i == 0)) || ((L (t) == VALUE) && (i == 0)) ||
            ((L (t) == CWITH) && (i == 4)) || ((L (t) == TWITH) && (i == 0)) ||
            ((L (t) == ASSOCIATE) && (i == 0)) ||
            ((L (t) == MARKUP) && (i == 0)))
          u= copy (H[u->label]);
      r[i]= u;
...
upgrade_use_package (tree t) {
  ...
  int  i, n= N (init);
  for (i= 0; i < n; i++)
    if (init[i] == tree (ASSOCIATE, PREAMBLE, "true")) preamble= true;
...
upgrade_style_rename (tree t) {
  ...
  return rename_vars (t, cached_renamer (style_rename, style_rename_table),
                      true);
}
...
search_abstract_tag (tree t, tree& data, string tag) {
  if (is_atomic (t)) return;
    ...
    int i, n= N (t);
    for (i= 0; i < n; i++)
      if (is_compound (t[i], tag, 1)) abstract_add (data, t[i][0]);
  }
}
...
static void
add_info (tree& data, tree t, string ntag, string otag, bool flag1= false,
          bool flag2= false) {
  ...
  if (is_atomic (t)) return t;
    ...
    for (int i= 0; i < N (u); i++) {
      ...
      if (N (search_title_tag (v, "author")) != 0) {
        if (N (author_data) != 0 && is_compound (author_data[0], "author-name"))
...
upgrade_switch (tree t) {
  if (is_atomic (t)) return t;
    ...
    if (is_compound (r, "fold", 2)) return compound ("folded", r[0], r[1]);
    if (is_compound (r, "unfold", 2)) return compound ("unfolded", r[0], r[1]);
    if (is_compound (r, "fold-bpr", 2) || is_compound (r, "fold-text", 2) ||
        is_compound (r, "fold-proof", 2) || is_compound (r, "fold-exercise", 2))
      return compound ("summarized", r[0], r[1]);
    if (is_compound (r, "unfold-bpr", 2) || is_compound (r, "unfold-text", 2) ||
...
upgrade_fill (tree t) {
  ...
  if (is_compound (t, "with")) {
    ...
    if (fm == "none") t= remove_attr (t, "fill-color");
    if (fm == "inside") t= set_attr (t, "color", tree ("none"));
  }
  int  n= N (t);
  tree r (t, n);
  for (i= 0; i < n; i++)
...
upgrade_graphics (tree t) {
  ...
  if (is_compound (t, "with") &&
    ...
    tree fr  = get_attr (t, "gr-frame",
                         tuple ("scale", "1cm", tree (TUPLE, "0.5par", "0cm")));
    tree clip= get_attr (
        t, "gr-clip",
        tuple ("clip", tuple ("0par", "-0.3par"), tuple ("1par", "0.3par")));
    ...
    if (length_add_error) fr[2][0]= tree (PLUS, length_abs (cg), ox);
    fr[2][1]= tree (length_add (length_abs (cb), oy));
    if (length_add_error) fr[2][1]= tree (PLUS, length_abs (cb), oy);
...
rename_primitive (tree t, string which, string by) {
  ...
  int  n= N (t);
  tree r (t, n);
  if (is_compound (t, which)) r= tree (make_tree_label (by), n);
  for (i= 0; i < n; i++)
...
upgrade_scheme_doc (tree t) {
  int i;
  if (is_atomic (t)) return t;
  else if (is_compound (t, "scm-fun", 1) || is_compound (t, "scm-macro", 1))
...
upgrade_session (tree t, tree lan, tree ses) {
  ...
  else if (is_func (t, WITH, 5) && t[0] == PROG_LANGUAGE &&
           t[2] == PROG_SESSION)
...
declare_style (url u) {
  if (is_or (u)) {
    ...
    if (ends (s, ".ts") && !starts (s, "source")) {
      ...
      if (starts (s, "old-")) existing_styles->insert (s (4, N (s) - 3));
      if (starts (s, "old2-")) existing_styles->insert (s (5, N (s) - 3));
    }
  }
}
...
upgrade_resize_arg (tree t) {
  ...
  if (is_func (t, ARG, 1) || is_func (t, PLUS, 2) || is_func (t, MINUS, 2) ||
...
upgrade_resize_clipped (tree t) {
  if (is_atomic (t)) return t;
    ...
    if (is_func (t, CLIPPED)) t= tree (CLIPPED, t[4], t[0], t[1], t[2], t[3]);
    int  i, n= 5;
    tree r (t, n);
...
upgrade_image (tree t) {
  if (is_atomic (t)) return t;
    ...
    return tree (IMAGE, t[0], upgrade_image_length (t[1], "w"),
...
upgrade_root_switch (tree t, bool top= true) {
  ...
  else if (is_compound (t, "switch")) return compound ("screens", A (t));
  else return t;
}
...
upgrade_algorithm (tree t, bool flag= true) {
  if (is_atomic (t)) return t;
    ...
    return compound ("named-algorithm-old", upgrade_algorithm (t[0]),
                     upgrade_algorithm (t[1]));
    ...
    if (is_document (t))
      for (i= 0; i < n; i++)
        if (is_compound (t[i], "TeXmacs", 1) || is_compound (t[i], "style") ||
            is_compound (t[i], "initial") || is_compound (t[i], "references"))
          flag= false;
    for (i= 0; i < n; i++)
...
upgrade_math_ops (tree t) {
  ...
  if (is_func (t, WITH, 3) && is_atomic (t[2]) && is_alpha (t[2]->label) &&
...
eat_spaces (tree t, bool after) {
  ...
  return is_compound (t, "hide-preamble") || is_compound (t, "doc-data") ||
         is_compound (t, "abstract") || is_compound (t, "bibliography") ||
         is_compound (t, "bibitem") || is_compound (t, "bibitem*");
}
...
encode_arrow (tree t) {
  if (is_func (t, WITH, 3) && t[0] == "dash-style") {
    if (t[2] == tree (LINE, tuple ("10ln", "6ln"), tuple ("0ln", "0ln"),
                      tuple ("10ln", "-6ln")))
      return "<less>";
    if (t[2] == tree (LINE, tuple ("-10ln", "6ln"), tuple ("0ln", "0ln"),
                      tuple ("-10ln", "-6ln")))
...
replace_line_arrows (tree& t, string var, string begin, string end) {
  if (find_attr (t, var)) {
    ...
    if (is_func (val, TUPLE, 1)) t= set_attr (t, end, encode_arrow (val[0]));
...
upgrade_gr_attributes (tree t) {
  ...
  if (is_func (t, WITH)) {
    ...
    replace_line_arrows (t, "gr-line-arrows", "gr-arrow-begin", "gr-arrow-end");
...
correct_metadata (tree t) {
  ...
  if (is_compound (t, "author-misc")) r= compound ("author-note");
  else r= compound (as_string (L (t)));
...
add_abstract_data (tree t, array<tree> abstract_data, bool& stop) {
  ...
  for (int i= 0; i < N (t); i++) {
    ...
    else r << add_abstract_data (t[i], abstract_data, stop);
  }
  return r;
}
...
upgrade_style (tree t, bool flag) {
  ...
  if (is_atomic (t)) return t;
    ...
    for (i= 0; i < n; i++)
      ...
      else if (t[i] == "tmdoc-keyboard") {
        if (!doc) r << "doc";
        doc= true;
      }
      else if (t[i] == "tmdoc-markup") {
        if (!doc) r << "doc";
        doc= true;
      }
      else if (t[i] == "tmdoc-traversal") {
        if (!doc) r << "doc";
        doc= true;
      }
...
upgrade_doc_language (tree t) {
  ...
  if (new_style != style) t= change_doc_attr (t, "style", new_style);
  if (new_init != init) t= change_doc_attr (t, "initial", new_init);
  return t;
}
...
upgrade_subsession (tree t, bool in_session= false) {
  ...
  if (is_atomic (t)) return t;
    ...
    bool flag=
        (is_func (t, DOCUMENT) && in_session) || is_compound (t, "session");
    int  n= N (t);
    tree r (t, n);
    for (i= 0; i < n; i++)
...
upgrade_quotes (tree t) {
  if (is_atomic (t)) return upgrade_quotes (t->label);
  else if (is_concat (t)) {
    int  i, n= N (t);
    tree r (t, n);
    bool adjust= false;
  ...
  else if (is_compound (t, "emdash", 0)) return "\26";
  else if (is_compound (t, "body", 1))
    ...
    tree r (t, n);
    for (i= 0; i < n; i++) {
      if (is_std_textual_env (as_string (L (t)))) r[i]= upgrade_quotes (t[i]);
...
upgrade_draw_over_under (tree t) {
  if (is_atomic (t)) return t;
    ...
    return compound ("draw-over", upgrade_ancient (t[0]),
                     upgrade_ancient (t[1]), "0cm");
  else if (is_compound (t, "draw-under", 2))
    return compound ("draw-under", upgrade_ancient (t[0]),
                     upgrade_ancient (t[1]), "0cm");
  else {
    int  i, n= N (t);
    tree r (t, n);
...
upgrade_qed (tree t) {
  if (is_atomic (t)) return t;
  else if (t == tree (VALUE, "qed")) return compound ("qed");
  else {
    int  i, n= N (t);
    tree r (t, n);
    for (i= 0; i < n; i++)
...
upgrade (tree t, string version) {
  ...
  if (version_inf (version, "0.3.3.1")) t= upgrade_apply_expand_value (t);
  if (version_inf (version, "0.3.3.20")) t= upgrade_new_environments (t);
  if (version_inf (version, "0.3.3.24")) t= upgrade_items (t);
  if (version_inf (version, "0.3.4.4")) t= upgrade_resize (t);
  if (version_inf_eq (version, "0.3.4.7")) t= upgrade_table (t);
  if (version_inf_eq (version, "0.3.4.8")) t= upgrade_split (t, false);
  if (version_inf_eq (version, "0.3.5.6")) t= upgrade_project (t);
  if (version_inf_eq (version, "0.3.5.10")) t= upgrade_title (t);
  if (version_inf_eq (version, "1.0.0.1")) t= upgrade_cas (t);
  if (version_inf_eq (version, "1.0.0.8"))
    t= simplify_correct (upgrade_mod_symbols (t));
  if (version_inf_eq (version, "1.0.0.11")) t= upgrade_menus_in_help (t);
  if (version_inf_eq (version, "1.0.0.13")) t= upgrade_capitalize_menus (t);
  if (version_inf_eq (version, "1.0.0.19")) t= upgrade_traverse_branch (t);
  if (version_inf_eq (version, "1.0.1.20")) t= upgrade_session (t);
  if (version_inf_eq (version, "1.0.2.0")) t= upgrade_formatting (t);
  if (version_inf_eq (version, "1.0.2.3")) t= upgrade_expand (t, EXPAND);
  if (version_inf_eq (version, "1.0.2.4")) t= upgrade_expand (t, HIDE_EXPAND);
  ...
  if (version_inf_eq (version, "1.0.2.8")) t= upgrade_env_vars (t);
  if (version_inf_eq (version, "1.0.3.3")) t= upgrade_use_package (t);
  if (version_inf_eq (version, "1.0.3.4")) t= upgrade_style_rename (t);
  if (version_inf_eq (version, "1.0.3.4")) t= upgrade_item_punct (t);
  if (version_inf_eq (version, "1.0.3.7")) t= upgrade_page_pars (t);
  if (version_inf_eq (version, "1.0.4")) {
    t= substitute (t, tree (VALUE, "hrule"), compound ("hrule"));
    t= upgrade_doc_info (t);
  }
  if (version_inf_eq (version, "1.0.4.6")) t= upgrade_bibliography (t);
  if (version_inf_eq (version, "1.0.5.4")) t= upgrade_switch (t);
  if (version_inf_eq (version, "1.0.5.7")) t= upgrade_fill (t);
  if (version_inf_eq (version, "1.0.5.8")) t= upgrade_graphics (t);
  if (version_inf_eq (version, "1.0.5.11")) t= upgrade_textat (t);
  if (version_inf_eq (version, "1.0.6.1")) t= upgrade_cell_alignment (t);
  if (version_inf_eq (version, "1.0.6.2"))
    t= rename_primitive (t, "hyper-link", "hlink");
  if (version_inf_eq (version, "1.0.6.2")) t= upgrade_label_assignment (t);
  if (version_inf_eq (version, "1.0.6.10")) t= upgrade_scheme_doc (t);
  if (version_inf_eq (version, "1.0.6.14")) t= upgrade_mmx (t);
  if (version_inf_eq (version, "1.0.7.1"))
    t= upgrade_session (t, "scheme", "default");
  if (version_inf_eq (version, "1.0.7.6")) t= upgrade_presentation (t);
  if (version_inf_eq (version, "1.0.7.6") && is_non_style_document (t))
    t= upgrade_math (t);
  if (version_inf_eq (version, "1.0.7.7")) t= upgrade_resize_clipped (t);
  if (version_inf_eq (version, "1.0.7.7")) t= upgrade_image (t);
  if (version_inf_eq (version, "1.0.7.7")) t= upgrade_root_switch (t);
  if (version_inf_eq (version, "1.0.7.8")) t= upgrade_hyphenation (t);
  if (DEBUG_CORRECT)
    if (is_non_style_document (t)) math_status_cumul (t);
  ...
  if (version_inf_eq (version, "1.0.7.9")) {
    t= move_brackets (t);
    if (is_non_style_document (t)) t= upgrade_algorithm (t, false);
    t= upgrade_math_ops (t);
  }
  if (version_inf_eq (version, "1.0.7.10")) t= downgrade_big (t);
  if (version_inf_eq (version, "1.0.7.13")) t= upgrade_gr_attributes (t);
  if (version_inf_eq (version, "1.0.7.14")) t= upgrade_cursor (t);
  if (version_inf_eq (version, "1.0.7.15")) t= upgrade_cyrillic (t);
  ...
  if (version_inf_eq (version, "1.99.4")) t= upgrade_draw_over_under (t);
  if (version_inf_eq (version, "1.99.6")) {
    t= upgrade_qed (t);
    if (is_non_style_document (t)) t= preserve_spacing (t);
  }
  ...
  if (version_inf_eq (version, "1.99.11"))
    if (is_non_style_document (t)) t= preserve_dots (t);
  ...
  if (version_inf_eq (version, "1.99.13")) t= preserve_lengths (t);
  if (version_inf_eq (version, "2.1.2")) {
    t= rename_primitive (t, "mouse-over-balloon", "hover-balloon");
    t= rename_primitive (t, "mouse-over-balloon*", "hover-balloon*");
  }

  if (is_non_style_document (t)) t= automatic_correct (t, version);
  return t;
}
