...
texmacs_input_rep::texmacs_input_rep (string type2)
    : type (type2), status (STATUS_NORMAL), buf (""), format ("verbatim"),
      mode (get_mode (format)), channel (type), stack (""), ignore_verb (false),
      docs (tree (DOCUMENT, "")) {
  bof ();
}
...
texmacs_input_rep::channel_flush (bool force) {
  if (force) {
    if ((buf == "prompt") || (buf == "input")) docs (buf)= tree (DOCUMENT, "");
    stack[1]= buf;
    buf     = "";
  }
}
...
texmacs_input_rep::image_flush (string image_data, string type, string w_unit,
  ...
  if (width == 0 && height == 0) {
    ...
    t << tree (as_string (width) * w_unit)
      << tree (as_string (height) * h_unit);
  }
  t << tree ("") << tree ("");
  write (t);
}
...
void
parse_url (string buf, string& path_file, string& w_unit, string& h_unit,
...
bool
validate_h_unit (string unit) {
  return unit == "pt" || unit == "px" || unit == "pag";
}

bool
validate_w_unit (string unit) {
