...
public:
  ...
  virtual box  left_contract_kerning (double factor);
  virtual box  right_contract_kerning (double factor);
...
inline box_rep::box_rep (path ip2)
    : x0 (0), y0 (0), x1 (0), y1 (0), x2 (0), y2 (0), x3 (0), y3 (0), x4 (0),
      y4 (0), ip (ip2) {
  TM_DEBUG (box_count++);
}
inline box_rep::~box_rep () { TM_DEBUG (box_count--); }
inline bool
box_rep::test_in (SI x, SI y) {
...
#define attach_left(t, ip)                                                     \
  attach_dip (t, decorate_left (ip)), decorate_left (ip)
#define attach_middle(t, ip)                                                   \
  attach_dip (t, decorate_middle (ip)), decorate_middle (ip)
#define attach_right(t, ip)                                                    \
  attach_dip (t, decorate_right (ip)), decorate_right (ip)
