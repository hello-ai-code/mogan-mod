...
page_item_rep::page_item_rep (box b2, array<lazy> f2, int n2)
    : type (PAGE_LINE_ITEM), b (b2), spc (0), penalty (0), fl (f2),
      nr_cols (n2) {}
...
page_item_rep::page_item_rep (int tp2, box b2, space spc2, int pen2,
    ...
    : type (tp2), b (b2), spc (spc2), penalty (pen2), fl (fl2),
      nr_cols (nr_cols2), t (t2) {}
...
page_item::page_item (int type, box b, space spc, int penalty, array<lazy> fl,
                      int nr_cols, tree t)
    : rep (tm_new<page_item_rep> (type, b, spc, penalty, fl, nr_cols, t)) {}
bool
page_item::operator== (page_item item2) {
...
page_item
copy (page_item l) {
  return page_item (l->type, l->b, l->spc, l->penalty, l->fl, l->nr_cols, l->t);
}
