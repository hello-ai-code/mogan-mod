...
concater_rep::typeset_large (tree t, path ip, int tp, int otp, string prefix) {
  ...
  if (N (t) < 1 || !is_atomic (t[0])) typeset_error (t, ip);
  else {
    string br= t[0]->label;
...
concater_rep::typeset_bigop (tree t, path ip) {
  if ((N (t) == 1) && is_atomic (t[0])) {
    ...
    if (env->fn->type == FONT_TYPE_UNICODE) {
      ...
      b= big_operator_box (ip, s, mfn, env->pen, env->display_style ? 2 : 1);
    }
    else
      b= big_operator_box (ip, s, env->fn, env->pen,
...
concater_rep::typeset_lprime (tree t, path ip) {
  if ((N (t) == 1) && is_atomic (t[0])) {
    ...
    if (flag || env->fn->math_type != MATH_TYPE_TEX_GYRE)
      b2= move_box (sip, b2, flag ? 0 : env->as_length (string ("-0.05fn")),
                    flag ? env->as_length ("-0.75ex") : 0, false, true);
...
concater_rep::typeset_long_arrow (tree t, path ip) {
  ...
  space spc= env->fn->spc;
  if (env->math_condensed)
    spc= space (spc->min >> 3, spc->def >> 3, spc->max >> 2);
...
concater_rep::typeset_script (tree t, path ip, bool right) {
  ...
  a << line_item (type, OP_SKIP, script_box (ip, b1, b2, env->fn),
                  HYPH_INVALID);
...
concater_rep::typeset_wide_frac (tree t, path ip) {
  ...
  typeset_large (tree (MID, "/"), decorate_middle (ip), MIDDLE_BRACKET_ITEM,
                 OP_MIDDLE_BRACKET, "<mid-");
  if (denb)
...
concater_rep::typeset_wide_sqrt (tree t, path ip) {
  ...
  if (disp) env->local_end (MATH_DISPLAY, old);
  penalty_max (HYPH_INVALID);
  a << line_item (RSUP_ITEM, OP_SKIP, script_box (ip, box (), fr, env->fn),
                  HYPH_INVALID);
  ...
  if (colored) {
    ...
    case VAR_AROUND:
      if (N (t) == 3) {
        ...
        env->fn= new_fn;
        typeset (make_large (LEFT, t[0]), decorate_middle (descend (ip, 0)));
        env->fn= old_fn;
        typeset (t[1], descend (ip, 1));
        env->fn= new_fn;
        typeset (make_large (RIGHT, t[2]), decorate_middle (descend (ip, 2)));
        env->fn= old_fn;
      }
      else typeset_error (t, ip);
      break;
    case BIG_AROUND:
      if (N (t) == 2) {
        typeset (make_large (BIG, t[0]), decorate_middle (descend (ip, 0)));
        typeset (t[1], descend (ip, 1));
      }
      else typeset_error (t, ip);
      break;
...
concater_rep::typeset_table (tree t, path ip) {
  ...
  for (int i= 0; i < N (t1); i++) {
    ...
    for (int j= 0; j < N (t2); j++) {
      tree t3= t2[j];
      if (!is_func (t3, CELL, 1)) {
        print (b);
        return;
      }
      if (is_func (t3[0], DOCUMENT)) {
        print (b);
        return;
      }
    }
