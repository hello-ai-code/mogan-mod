...
void
concater_rep::typeset_math_macro (string s, tree_label m, path ip, int p1,
                                  int p2, int otype) {
...
concater_rep::typeset_math_string (tree t, path ip, int pos, int end) {
  ...
  do {
    ...
    if ((pos > start) && (s[start] == ' ')) { // spaces
      ...
      else typeset_math_macro (s, tp->macro, ip, start, pos, tp->op_type);
      penalty_min (tp->pen_after);
...
concater_rep::typeset_hspace (tree t, path ip) {
  ...
  if (N (t) == 1) print (env->as_hspace (t[0]));
    ...
    print (space (env->as_length (t[0]), env->as_length (t[1]),
                  env->as_length (t[2])));
  control (t, ip);
}
...
concater_rep::typeset_if_page_break (tree t, path ip) {
  ...
  tree  sep= tree (TMLEN, as_string (spc->min), as_string (spc->def),
                   as_string (spc->max));
