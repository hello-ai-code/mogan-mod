...
inherited_format (hashmap<string, tree> cvar) {
  ...
  if (cvar->contains (CELL_LSEP))
    inh << tree (CWITH, "1", "-1", "1", "1", CELL_LSEP, cvar[CELL_LSEP]);
  if (cvar->contains (CELL_RSEP))
    inh << tree (CWITH, "1", "-1", "-1", "-1", CELL_RSEP, cvar[CELL_RSEP]);
  if (cvar->contains (CELL_BSEP))
    inh << tree (CWITH, "-1", "-1", "1", "-1", CELL_BSEP, cvar[CELL_BSEP]);
  if (cvar->contains (CELL_TSEP))
    inh << tree (CWITH, "1", "1", "1", "-1", CELL_TSEP, cvar[CELL_TSEP]);
  if (cvar->contains (CELL_VCORRECT))
    inh << tree (CWITH, "1", "-1", "1", "-1", CELL_VCORRECT,
                 cvar[CELL_VCORRECT]);
  if (cvar->contains (CELL_HYPHEN))
    inh << tree (CWITH, "1", "-1", "1", "-1", CELL_HYPHEN, cvar[CELL_HYPHEN]);
  if (cvar->contains (CELL_BLOCK))
    inh << tree (CWITH, "1", "-1", "1", "-1", CELL_BLOCK, cvar[CELL_BLOCK]);
  if (cvar->contains (CELL_SWELL))
    inh << tree (CWITH, "1", "-1", "1", "-1", CELL_SWELL, cvar[CELL_SWELL]);
...
table_rep::typeset_subtable (tree t, path iq, hashmap<string, tree> cvar) {
  ...
  if (is_func (t, TABLE, 1) && is_func (t[0], ROW, 1) &&
...
static void
blow_up (SI* w, SI* l, SI* r, SI* W, SI* L, SI* R, SI room, double* part, int n,
         bool vert) {
  int i;
  for (i= 0; i < n; i++)
    if (w[i] >= W[i] || part[i] < 0) part[i]= 0;
...
table_rep::position_columns (bool large) {
  // cout << HRULE << LF;
  // cout << "Position columns " << (width >> 8) << ", " << large << LF <<
  // INDENT;
...
table_rep::finish_horizontal () {
  ...
  for (j= 0; j < nr_cols; j++) {
    for (i= 0; i < nr_rows; i++) {
      cell C= T[i][j];
      if (!is_nil (C)) {
        if (C->col_span > 1) C->width+= sum (mw + j + 1, C->col_span - 1);
        if (!is_nil (C->T)) C->T->finish_horizontal ();
        else if (C->hyphen != "n") C->finish_horizontal ();
        SI tot= sum (mw + j, C->col_span);
