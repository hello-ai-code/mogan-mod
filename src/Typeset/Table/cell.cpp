...
cell_rep::typeset (tree fm, tree t, path iq) {
  ...
  if (decoration != "") {
    ...
    tree dt= decoration;
    while (is_func (dt, TFORMAT))
      dt= dt[N (dt) - 1];
    for (i= 0; i < N (dt); i++) {
      tree dr= dt[i];
      while (is_func (dr, TFORMAT))
        dr= dr[N (dr) - 1];
      for (j= 0; j < N (dr); j++) {
        tree dc= dr[j];
        while (is_func (dc, TFORMAT))
          dc= dc[N (dc) - 1];
        if (dc == tree (TMARKER)) {
          or_row= i;
          or_col= j;
...
cell_rep::cell_local_begin (tree fm) {
  ...
  for (i= 0; i < l; i++) {
    tree with= fm[i];
    if (is_func (with, CWITH, 2) && is_atomic (with[0]) &&
...
extract_format (tree fm, tree* r, int n) {
  ...
  for (i= 0; i < N (fm); i++)
    if (is_func (fm[i], CWITH))
      if ((N (fm[i]) >= 2) && (is_int (fm[i][0])) && (is_int (fm[i][1]))) {
        int  k;
...
cell_rep::format_item (tree with) {
  if (is_func (with, CWITH, 2)) var (as_string (with[0]))= with[1];
}
...
cell_rep::compute_width (SI& mw, SI& lw, SI& rw, bool large) {
  // cout << "  large= " << large << LF;
  char align_c= '\0';
  if (N (halign) != 0) align_c= halign[0];
...
cell_rep::compute_height (SI& mh, SI& bh, SI& th, SI xh) {
  char align_c= '\0';
  if (N (valign) != 0) align_c= valign[0];
  if (is_nil (T)) {
    bh= th= mh= 0;
