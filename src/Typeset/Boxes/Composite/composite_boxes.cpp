...
composite_box_rep::composite_box_rep (path ip, array<box> B, bool init_sx_sy)
    : box_rep (ip) {
  bs= B;
  if (init_sx_sy) {
    int i, n= N (bs);
...
composite_box_rep::composite_box_rep (path ip, array<box> B, array<SI> x,
                                      array<SI> y)
    : box_rep (ip) {
  bs= B;
  int i, n= subnr ();
...
void
composite_box_rep::loci (SI x, SI y, SI delta, list<string>& ids,
                         rectangles& rs) {
  int m= find_child (x, y, delta, true);
  if (m == -1) box_rep::loci (x, y, delta, ids, rs);
  else {
    bs[m]->loci (x - sx (m), y - sy (m), delta + get_delta (x, x1, x2), ids,
                 rs);
  ...
  for (i= 0; i < n; i++) {
    ...
    */
    if (is_accessible (l) && is_accessible (r)) {
      if (is_decoration (lip) || path_less (reverse (l), reverse (lip))) lip= l;
      if (is_decoration (rip) || path_less (reverse (rip), reverse (r))) rip= r;
    }
  }
  /*
...
composite_box_rep::find_box_path (path p, bool& found) {
  ...
  while (step > 0) {
    ...
    if (path_less (reverse (sr), p)) {
      ...
      acc  = start;
      while ((acc > old_start) && (!is_accessible (bs[acc]->find_rip ())))
        acc--;
...
composite_box_rep::find_selection (path lbp, path rbp) {
  if ((!is_atom (lbp)) && (!is_atom (rbp)) && (lbp->item == rbp->item)) {
    ...
    return selection (translate (sel->rs, sx (i), sy (i)), sel->start,
                      sel->end);
  }
