...
void
stack_box_rep::clear_incomplete (rectangles& rs, SI pixel, int which, int i1,
                                 int i2) {
  if (N (bs) == 0) return;
  if ((i1 <= i2) && (!is_nil (rs))) {
    ...
    // cout << "  ys : " << Min_y/256 << ", " << min_y/256 << ", " << max_y/256
    << ", " << Max_y/256 << "\n";
    */
...
stack_box_rep::find_selection (path lbp, path rbp) {
  ...
  if (i1 == i2) {
    ...
    int j1= i1, j2= i2;
    while (j1 < i2 && bs[j1]->h () == 0)
      j1++;
    while (j2 > j1 && bs[j2]->h () == 0)
      j2--;
