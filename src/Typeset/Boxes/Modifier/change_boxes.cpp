...
change_box_rep::graphical_select (SI x, SI y, SI dist) {
  ...
  if (child_flag) return composite_box_rep::graphical_select (x, y, dist);
  else return bs[0]->graphical_select (x - sx (0), y - sy (0), dist);
}

gr_selections
change_box_rep::graphical_select (SI x1, SI y1, SI x2, SI y2) {
  // TODO : Check if it is correct
  if (child_flag) return composite_box_rep::graphical_select (x1, y1, x2, y2);
  else
    return bs[0]->graphical_select (x1 - sx (0), y1 - sy (0), x2 - sx (0),
                                    y2 - sy (0));
}
...
struct move_delimiter_box_rep : public move_box_rep {
  ...
  operator tree () { return tree (TUPLE, "move_delimiter", (tree) bs[0]); }
...
struct resize_box_rep : public change_box_rep {
  resize_box_rep (path ip, box b, SI x1, SI y1, SI x2, SI y2, bool child_flag,
                  bool adjust);
...
resize_box_rep::resize_box_rep (path ip, box b, SI X1, SI Y1, SI X2, SI Y2,
...
vcorrect_box_rep::vcorrect_box_rep (path ip, box b, SI top_cor2, SI bot_cor2)
    : change_box_rep (ip, false, false), top_cor (top_cor2),
      bot_cor (bot_cor2) {
  insert (b, 0, -top_cor);
  position ();
  y1-= bot_cor;
...
public:
  clip_box_rep (path ip, box b, SI x1, SI y1, SI x2, SI y2, tree xt, tree yt,
                SI scx, SI scy);
  operator tree () { return tree (TUPLE, "clip", (tree) bs[0]); }
  int get_type () {
    return xt != UNINIT || yt != UNINIT ? SCROLL_BOX
                                        : change_box_rep::get_type ();
  }
...
clip_box_rep::find_selection (path lbp, path rbp) {
  ...
  return selection (sel->rs & rectangle (x1, y1, x2, y2), sel->start, sel->end,
                    sel->valid);
}
...
repeat_box_rep::repeat_box_rep (path ip, box b, box r2, SI xoff2, bool u2)
  ...
  if (width >= PIXEL) {
    ...
    while (i1 * width < (xoff + b->x1))
      i1++;
    while (i2 * width > (xoff + b->x2))
      i2--;
    for (i= i1; i < i2; i++) {
...
tag_box_rep::find_tag (string search) {
  for (int i= 0; i < N (keys); i++)
    if (keys[i]->label == search) return reverse (descend_decode (tip, 1));
  return path ();
}
...
text_at_box_rep::text_at_box_rep (path ip, box b, SI x, SI y, SI hx2, SI hy2,
                                  SI a2, SI p2)
    : move_box_rep (ip, b, x, y, false, false), hx (hx2), hy (hy2), axis (a2),
      pad (p2) {}

gr_selections
text_at_box_rep::graphical_select (SI x, SI y, SI dist) {
  array<point> special;
  special << point (x1 - pad, y1 - pad) << point (x1 - pad, y2 + pad)
          << point (x2 + pad, y2 + pad) << point (x2 + pad, y1 - pad)
          << point ((x1 + x2) >> 1, y1 - pad)
          << point ((x1 + x2) >> 1, y2 + pad) << point (x1 - pad, y1 + axis)
...
relay_box_rep::message (tree type, SI x, SI y, rectangles& rs) {
  ...
  if (!is_bool (r) || as_bool (r)) rs << rectangle (x3, y3, x4, y4);
...
box
resize_box (path ip, box b, SI x1, SI y1, SI x2, SI y2, bool child_flag,
            bool adjust) {
...
box
clip_box (path ip, box b, SI x1, SI y1, SI x2, SI y2, tree xt, tree yt, SI scx,
          SI scy) {
