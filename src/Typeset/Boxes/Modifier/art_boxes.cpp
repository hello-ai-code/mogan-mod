...
void
art_box_rep::display_image (renderer ren, url u, tree eff, SI xl, SI yb, SI xr,
                            SI yt) {
  // if (ren->is_screen) {
  ren->round (xl, yb);
  ren->round (xr, yt);
...
art_box_rep::display_one (renderer ren, tree prg) {
  if (prg[0] == "image" && is_atomic (prg[1])) {
    ...
    if (!is_func (grid, TUPLE, 3) || !is_func (grid[1], TUPLE) ||
        !is_func (grid[2], TUPLE))
      return;
    tree          eff= grid[0];
    ...
    for (int i= 0; i < nx; i++) {
      ...
      for (int j= 0; j < ny; j++) {
        ...
        if (nextx > curx && nexty > cury) {
          ...
          tree creff (EFF_CROP, eff, grid[1][ii], grid[2][jj], grid[1][ii + 1],
                      grid[2][jj + 1]);
...
static tree
minus (tree t) {
  if (is_atomic (t) && is_double (t->label)) return "-" * t->label;
  else if (is_func (t, TMLEN, 1)) return tree (TMLEN, minus (t[0]));
          ...
          if (format[(2 - row) * 4 + col] == 'x') {
            ...
            if (col != 1 || row != 1)
              if ((col == 1 && (lrep != "" && rrep != "")) ||
                  (row == 1 && (brep != "" && trep != ""))) {
                tree hor (TUPLE, crx1);
                if (col == 1 && (lrep != "" && rrep != "")) hor << lrep << rrep;
                hor << crx2;
                tree ver (TUPLE, cry1);
                if (row == 1 && (brep != "" && trep != "")) ver << brep << trep;
                ver << cry2;
...
art_box_rep::get_image_extents (tree prg, SI& xl, SI& xr, SI& yb, SI& yt) {
  ...
  for (int i= 2; i < N (prg); i+= 2) {
    ...
    else if (prg[i] == "hoffset") dxl= dxr= get_length (prg[i + 1], x2);
    else if (prg[i] == "voffset") dyb= dyt= get_length (prg[i + 1], y2);
    else if (prg[i] == "loffset") dxl= get_length (prg[i + 1], x2);
    else if (prg[i] == "roffset") dxr= get_length (prg[i + 1], x2);
    else if (prg[i] == "boffset") dyb= get_length (prg[i + 1], y2);
    else if (prg[i] == "toffset") dyt= get_length (prg[i + 1], y2);
  }
  if (xw_done && xl_done && !xr_done) {
    xr     = xl + xw;
    xr_done= true;
  }
  if (xw_done && xr_done && !xl_done) {
    xl     = xr - xw;
    xl_done= true;
  }
  if (yh_done && yb_done && !yt_done) {
    yt     = yb + yh;
    yt_done= true;
  }
  if (yh_done && yt_done && !yb_done) {
    yb     = yt - yh;
    yb_done= true;
  }
  xl+= dxl;
