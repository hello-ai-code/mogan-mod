...
anim_compose_box_rep::anim_compose_box_rep (path ip, array<box> b2, player pl)
    : anim_box_rep (ip, pl, 0.0), bs (b2), offsets (N (bs)) {
...
void
anim_compose_box_rep::loci (SI x, SI y, SI delta, list<string>& ids,
                            rectangles& rs) {
  bs[current]->loci (x, y, delta, ids, rs);
}

void
anim_compose_box_rep::collect_page_numbers (hashmap<string, tree>& h,
                                            tree                   page) {
...
anim_repeat_box_rep::anim_next () {
  ...
  else r= min (r, pl->get_refresh_time (t - current_it * duration));
  return r;
}
...
struct anim_translate_box_rep : public anim_effect_box_rep {
  ...
  anim_translate_box_rep (path ip, box b, player pl, int len, SI sx, SI sy,
                          SI ex, SI ey)
      : anim_effect_box_rep (ip, b, pl, len), start_x (sx), start_y (sy),
        end_x (ex), end_y (ey) {}
...
struct anim_progressive_box_rep : public anim_effect_box_rep {
  ...
  anim_progressive_box_rep (path ip, box b, player pl, int l, rectangle r1,
                            rectangle r2)
      : anim_effect_box_rep (ip, b, pl, l), start_r (r1), end_r (r2) {}
...
box
anim_translate_box (path ip, box b, player pl, int len, SI sx, SI sy, SI ex,
                    SI ey) {
  return tm_new<anim_translate_box_rep> (ip, b, pl, len, sx, sy, ex, ey);
}

box
anim_progressive_box (path ip, box b, player pl, int len, rectangle r1,
                      rectangle r2) {
...
box
video_box (path ip, player pl, url u, SI w, SI h, int alpha, int ms, bool rep,
           int px) {
