...
SI
box_rep::left_correction () {
  return (SI) (-min (0, y1) * left_slope ());
}
SI
box_rep::right_correction () {
  return (SI) (max (0, y2) * right_slope ());
}
SI
box_rep::lsub_correction () {
  return 0;
}
SI
box_rep::lsup_correction () {
  return 0;
}
SI
box_rep::rsub_correction () {
  return 0;
}
SI
box_rep::rsup_correction () {
  return 0;
}
SI
box_rep::sub_lo_base (int level) {
  (void) level;
  return y1;
}
SI
box_rep::sub_hi_lim (int level) {
  (void) level;
  return y1 + ((y2 - y1) / 3);
}
SI
box_rep::sup_lo_lim (int level) {
  (void) level;
  return (y1 + y2) >> 1;
}
SI
box_rep::sup_lo_base (int level) {
  (void) level;
  return y2 - ((y2 - y1) / 3);
}
SI
box_rep::sup_hi_lim (int level) {
  (void) level;
  return y2;
}
SI
box_rep::wide_correction (int mode) {
  (void) mode;
  return 0;
}
void
box_rep::get_bracket_extents (SI& lo, SI& hi) {
  lo= y1;
  hi= y2;
}
...
box_rep::find_selection (path lbp, path rbp) {
  if (lbp == rbp)
    return selection (rectangles (), find_tree_path (lbp),
                      find_tree_path (rbp));
  else
    return selection (rectangle (x1, y1, x2, y2), find_tree_path (path (0)),
...
box_rep::find_frame (path bp, bool last) {
  ...
  while (!is_nil (bp)) {
    ...
    if (!is_nil (g)) {
      if (last) f= g;
      else f= scaling (1.0, point (x, y)) * g;
    }
  }
  return f;
}
...
clear_pattern_rectangles (renderer ren, rectangle m, rectangles l) {
  while (!is_nil (l)) {
    rectangle r (l->item);
    ren->clear_pattern (m->x1 - ren->ox, m->y1 - ren->oy, m->x2 - ren->ox,
                        m->y2 - ren->oy, r->x1 - ren->ox, r->y1 - ren->oy,
...
cursor
copy (cursor cu) {
  return cursor (cu->ox, cu->oy, cu->delta, cu->y1, cu->y2, cu->slope,
                 cu->valid);
}

bool
operator== (cursor cu1, cursor cu2) {
  return (cu1->ox == cu2->ox) && (cu1->oy == cu2->oy) &&
         // (cu1->delta == cu2->delta) &&
...
operator<< (tm_ostream& out, gr_selection sel) {
  return out << "gr_selection (" << sel->type << ", " << sel->dist << ", "
             << sel->cp << ")";
}
      ...
      if (has_subtree (the_et, p)) {
        ...
        for (int i= 0; i < N (ids); i++) {
          ...
          for (int j= 0; j < N (lns); j++)
            if (is_compound (lns[j], "link", 4) && lns[j][0] == "hyperlink" &&
                is_compound (lns[j][3], "url", 1) && is_atomic (lns[j][3][0])) {
...
descend_decode (path ip, int side) {
  if (is_nil (ip)) return descend (ip, side);
    ...
    case DECORATION_LEFT:
      return descend (ip->next, 0);
    case DECORATION_MIDDLE:
      return descend (ip->next, side);
    case DECORATION_RIGHT:
      return descend (ip->next, 1);
    default:
      return descend (ip, side);
    }
}
