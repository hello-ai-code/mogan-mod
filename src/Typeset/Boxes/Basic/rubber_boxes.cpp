...
struct marker_box_rep : public box_rep {
  ...
  path find_lip () { return is_accessible (ip) ? descend (ip, pos) : ip; }
  path find_rip () { return is_accessible (ip) ? descend (ip, pos) : ip; }
  path find_right_box_path () { return path (0); }
...
bracket_box_rep::bracket_box_rep (path ip, int br_type2, pencil pen2, SI y1b,
                                  SI y2b)
    : box_rep (ip) {
  br_type= br_type2;
  pen    = pen2;
...
draw_bracket (renderer ren, int br_type, SI x, SI y, SI w, SI h, pencil pen) {
  ...
  case Rbracket: {
    ...
    ren->set_pencil (pen->set_width (ren->pixel));
    for (i= 0; i < lw; i+= ren->pixel)
      ren->arc (ox - ww + i, oy - hh, ox + ww - i, oy + hh, -(45 << 6),
                90 << 6);
  } break;
  case Lcrochet:
    ren->line (x, y, x, y + h);
