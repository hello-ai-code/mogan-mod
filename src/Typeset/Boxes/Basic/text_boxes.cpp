...
get_wide_stix (string s, font fn, SI width) {
  ...
  while (true) {
    string test= radical * as_string (n) * ">";
    fn->get_extents (test, ex);
    if (ex->x2 - ex->x1 > width || n >= 6) return radical * as_string (n) * ">";
    n++;
  }
}
