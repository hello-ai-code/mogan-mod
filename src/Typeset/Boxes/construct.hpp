...
box resize_box (path ip, box b, SI x1, SI y1, SI x2, SI y2, bool chf= false,
                bool adjust= false);
...
box clip_box (path ip, box b, SI x1, SI y1, SI x2, SI y2, tree xt, tree yt,
              SI scx, SI scy);
box vcorrect_box (path ip, box b, SI top_cor, SI bot_cor);
box page_box (path ip, tree page, int page_nr, brush bgc, SI w, SI h,
              array<box> bs, array<SI> bs_x, array<SI> bs_y, array<box> decs,
              array<SI> decs_x, array<SI> decs_y);
box page_border_box (path ip, box pb, color tmb, SI l, SI r, SI b, SI t,
                     SI pix);
...
box anim_translate_box (path ip, box b, player pl, int len, SI sx, SI sy, SI ex,
                        SI ey);
box anim_progressive_box (path ip, box b, player pl, int len, rectangle r1,
                          rectangle r2);
box sound_box (path ip, player pl, url u, SI h);
box video_box (path ip, player pl, url u, SI w, SI h, int a, int msecs,
               bool rep, int px);
