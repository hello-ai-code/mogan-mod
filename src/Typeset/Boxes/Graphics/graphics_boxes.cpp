...
graphics_box_rep::find_child (SI x, SI y, SI delta, bool force) {
  ...
  for (i= 0; i < n; i++)
    if (distance (i, x, y, delta) == 0) {
      tree ty= (tree) bs[i];
      if ((bs[i]->accessible () || force) && is_tuple (ty) &&
          ty[0] == "text-at")
        return i;
    }
  return m;
...
graphics_box_rep::graphical_select (SI x1, SI y1, SI x2, SI y2) {
  ...
  int           i, n= subnr ();
  for (i= n - 1; i >= 0; i--)
    res << bs[i]->graphical_select (x1 - sx (i), y1 - sy (i), x2 - sx (i),
                                    y2 - sy (i));
  return res;
}
...
struct point_box_rep : public box_rep {
  ...
  brush  br;
  string style;
  point_box_rep (path ip, point p, SI radius, pencil pen, brush br,
                 string style);
...
point_box_rep::point_box_rep (path ip2, point p2, SI r2, pencil pen2, brush br2,
                              string style2)
    : box_rep (ip2), p (p2), r (r2), pen (pen2), br (br2), style (style2) {
...
get_contour (string style) {
  array<point> a;
  if (style == "square")
    a << point (-1.0, -1.0) << point (1.0, -1.0) << point (1.0, 1.0)
      << point (-1.0, 1.0);
  else if (style == "diamond")
    a << point (-1.0, 0.0) << point (0.0, -1.0) << point (1.0, 0.0)
      << point (0.0, 1.0);
  else if (style == "triangle")
    a << point (-1.0, -1.0) << point (1.0, -1.0) << point (0.0, 1.0);
  else if (style == "star") {
    int n= 5;
    for (int i= 0; i < 2 * n; i++) {
    ...
    a << point (0.0, 0.0) << point (1.0, 0.0) << point (0.0, 0.0)
      << point (0.0, 1.0) << point (0.0, 0.0) << point (-1.0, 0.0)
      << point (0.0, 0.0) << point (0.0, -1.0);
  else if (style == "cross")
    a << point (0.0, 0.0) << point (1.0, 1.0) << point (0.0, 0.0)
...
curve_box_rep::curve_box_rep (path ip2, curve c2, pencil pen2,
  ...
  if (N (arrows2) > 0 && !is_nil (arrows2[0])) {
    ...
    if (!error) {
      frame fr = scaling (1.0, a[0]) * rotation_2D (point (0.0, 0.0), arg (tg));
...
curve_box_rep::transform (frame fr) {
  return curve_box (ip, fr (c), 1.0, pen, style, motif, style_unit, fill_br,
                    arrows);
}
...
curve_box_rep::apply_motif (array<box> arrows) {
  ...
  for (int i= 0; i < N (a); i++) {
    ...
    if (i == 0 && a[0] != a[N (a) - 1]) prev= cur - (next - cur);
    if (i == N (a) - 1 && a[0] != a[N (a) - 1]) next= cur + (cur - prev);
...
box
graphics_box (path ip, array<box> bs, frame f, grid g, point lim1, point lim2) {
