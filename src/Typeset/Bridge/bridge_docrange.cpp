...
bridge_docrange_rep::bridge_docrange_rep (typesetter ttt, tree st, path ip,
                                          array<bridge>& brs2, int begin2,
                                          int end2, bool divide2)
    : bridge_rep (ttt, st, ip), brs (brs2), begin (begin2), end (end2),
      divide (divide2) {
  if (divide) {
...
bridge_docrange_rep::my_typeset (int desired_status) {
  ...
  if (divide) {
    ...
    for (i= begin; i < end; i++) {
      int wanted=
          (i == end - 1 ? desired_status & WANTED_MASK : WANTED_PARAGRAPH);
      ttt->a= (i == 0 ? a : array<line_item> ());
