...
lazy_document_rep::query (lazy_type request, format fm) {
  if ((request == LAZY_BOX) && (fm->type == QUERY_VSTREAM_WIDTH)) {
    ...
    for (i= 0; i < n; i++) {
      format tmp_fm=
          make_query_vstream_width (i == 0 ? before : array<line_item> (),
...
lazy_document_rep::produce (lazy_type request, format fm) {
  ...
  if (request == LAZY_VSTREAM) {
    ...
    for (i= 0; i < n; i++) {
      format tmp_fm=
          make_format_vstream (width, i == 0 ? before : array<line_item> (),
                               i == n - 1 ? after : array<line_item> ());
...
lazy_hidden_rep::produce (lazy_type request, format fm) {
  ...
  if (request == LAZY_VSTREAM) {
    ...
    array<page_item> temp_l= typeset_stack (env, t, ip, fvs->width, fvs->before,
                                            fvs->after, temp_sb);
    int              i= 0, n= N (temp_l);
    for (i= 0; i < n; i++)
...
make_lazy_eval (edit_env env, tree t, path ip) {
  tree r;
  if (is_func (t, EVAL, 1)) r= env->exec (t[0]);
  else if (is_func (t, QUASI, 1)) r= env->exec (tree (QUASIQUOTE, t[0]));
  else r= env->exec (t);
  array<line_item> a  = typeset_marker (env, descend (ip, 0));
  array<line_item> b  = typeset_marker (env, descend (ip, 1));
  lazy             par= make_lazy (env, attach_right (r, ip));
