...
skeleton break_pages (array<page_item> l, space ph, int qual, space fn_sep,
                      space fnote_sep, space float_sep, font fn,
                      int first_page);
box      page_box (path ip, box b, tree page, int page_nr, brush bgc, SI width,
...
pager_rep::pages_make_page (pagelet pg) {
  ...
  box page=
      page_box (ip, lb, page_t, nr, bgc, width, height, left, top + dtop,
                top + dtop + text_height, header, footer, head_sep, foot_sep);
...
pager_rep::papyrus_make () {
  ...
  array<SI> bs_y (1);
  bs_y[0]= -top - dtop;
  box pb = page_box (ip, "?", 0, bgc, width, height, bs, bs_x, bs_y, 0, 0, 0);
  pages << pb;
}
