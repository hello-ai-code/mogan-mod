...
array<double>
curve_rep::find_closest_points (double t1, double t2, point p, double eps) {
...
intersection (curve f, curve g, point p0, double eps) {
  ...
  if (f == g) {
    ...
    if (res1 && res2) found= intersection (f, g, d1, d2);
  }
  if (found) res << f (d1);
  return res;
}
...
struct spline_rep : public curve_rep {
  ...
  inline double S (array<dpol> p1, array<dpol> p2, array<dpol> p3, int i,
                   double u);
...
// Creation
spline_rep::spline_rep (array<point> a2, array<path> cip2, bool close2,
                        bool interpol2)
  ...
  for (i= 0; i <= n; i++) {
    ...
    double di32= d (i + 3, 2);
    double di31= d (i + 3, 1);
    p1[i]      = dpol (square (U[i]) / di22 / di11, -2 * U[i] / di22 / di11,
                       1 / di22 / di11);
    p2[i]      = dpol (
        -U[i + 2] * U[i] / di22 / di21 - U[i + 3] * U[i + 1] / di32 / di21,
        (U[i + 2] + U[i]) / di22 / di21 + (U[i + 3] + U[i + 1]) / di32 / di21,
        -1 / di22 / di21 - 1 / di32 / di21);
    p3[i]= dpol (square (U[i + 3]) / di32 / di31, -2 * U[i + 3] / di32 / di31,
                 1 / di32 / di31);
    /*
    p1[i][2]= 1/di22/di11;
...
// Evaluation
double
spline_rep::S (array<dpol> p1, array<dpol> p2, array<dpol> p3, int i,
               double u) {
  if (i < 0 || i > n) return 0;
  ...
  else TM_FAILED ("we should **never** go here");
  return 0.0;
}
...
arc_rep::arc_rep (array<point> a2, array<path> cip2, bool close)
  ...
  if (n != 3 || linearly_dependent (o1, o2, o3) ||
      (N (center= intersection (midperp (o1, o2, o3), midperp (o2, o3, o1))) ==
       0)) {
    center= i= j= 0 * o1;
    r1= r2= 1;
    e1    = 0;
...
struct compound_curve_rep : public curve_rep {
  ...
  double curvature (double t1, double t2) {
    ...
    double m = 0.0;
    if (i1 == i2) m= cs[i1]->curvature (u1, u2);
    else
      for (int i= i1; i <= i2; i++) {
...
compound (array<curve> a) {
  ...
  if (N (a) == 0) TM_FAILED ("at least one curve expected");
  if (N (a) == 1) return a[0];
...
array<double>
partial_curve_rep::find_closest_points (double t1, double t2, point p,
                                        double eps) {
  if (t1 > t2) {
    double a= t1;
    t1      = t2;
...
struct recontrol_curve_rep : public curve_rep {
  ...
  int    get_control_points (array<double>& abs, array<point>& a2,
    ...
    abs= array<double> (n);
    for (i= 0; i < n; i++)
      abs[i]= i / (1.0 * max (1, n - 1));
    return n;
  }
  array<double> find_closest_points (double t1, double t2, point p,
                                     double eps) {
