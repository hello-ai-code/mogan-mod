...
struct math_font_rep : font_rep {
  ...
  math_font_rep (string name, scheme_tree t, font base, font error, double hzf,
                 double vzf);
...
math_font_rep::math_font_rep (string name, scheme_tree t, font base, font error,
                              double hzf2, double vzf2)
    : font_rep (name, base), def (t), base_fn (base), error_fn (error),
      ...
      italic (load_translator ("italic")), font_name (N (t[1]) - 1),
      font_table (N (t[1]) - 1), rubber_name (N (t[2]) - 1),
      rubber_table (N (t[2]) - 1), hzf (hzf2), vzf (vzf2) {
  int i;
  for (i= 1; i < N (t[1]); i++)
    font_name[i - 1]= t[1][i];
  for (i= 1; i < N (t[2]); i++)
...
math_font_rep::init_font (int fn_nr, font& fn) {
  ...
  else fn= find_magnified_font (t, hzf, vzf);
...
math_font_rep::magnify (double zoomx, double zoomy) {
  return math_font (def, base_fn->magnify (zoomx, zoomy),
                    error_fn->magnify (zoomx, zoomy), hzf * zoomx, vzf * zoomy);
}
...
font
math_font (scheme_tree t, font base_fn, font error_fn, double hzf, double vzf) {
  ...
  return make (
      font, full_name,
      tm_new<math_font_rep> (full_name, t, base_fn, error_fn, hzf, vzf));
}
