...
get_variant (array<string> v) {
  ...
  for (int i= 1; i < N (v); i++) {
    if (v[i] == "mono" && contains (string ("typewriter"), v))
      ;
    else if (v[i] == "mono" || v[i] == "typewriter") r << string ("tt");
    else if (v[i] == "sansserif") r << string ("ss");
    else if (v[i] == "digital" || v[i] == "pen" || v[i] == "artpen" ||
             v[i] == "chalk" || v[i] == "marker")
      r << v[i];
    else if (is_category (v[i])) r << v[i];
    else if (is_glyphs (v[i])) r << v[i];
    else if (is_other (v[i])) r << v[i];
  }
  if (N (r) == 0) return "rm";
  return recompose (r, "-");
}
...
get_shape (array<string> v) {
  array<string> r;
  for (int i= 1; i < N (v); i++)
    if (ends (v[i], "condensed") || ends (v[i], "unextended") ||
        ends (v[i], "wide") || v[i] == "proportional" ||
...
is_other_internal (string s) {
  return is_other (s) && s != "rm" && s != "ss" && s != "tt" &&
         s != "small-caps" && s != "right" && s != "slanted";
}
...
shape_features (string s) {
  ...
  for (int i= 0; i < N (v); i++)
    if (ends (v[i], "condensed") || ends (v[i], "unextended") ||
        ends (v[i], "wide") || v[i] == "mono" || v[i] == "proportional" ||
        v[i] == "italic" || v[i] == "mathitalic" || v[i] == "mathupright" ||
        v[i] == "mathshape" || v[i] == "smallcaps" || v[i] == "long" ||
        v[i] == "flat")
      r << v[i];
...
logical_font (string family, string variant, string series, string shape) {
  ...
  array<string> v;
  for (int i= 0; i < N (r); i++)
    if (r[i] != "medium" && r[i] != "upright") v << r[i];
  // cout << family << ", " << variant << ", "
  //      << series << ", " << shape << " -> " << v << "\n";
  return v;
}
...
find_closest (string& family, string& variant, string& series, string& shape,
  ...
  */
    ...
    if (contains (string ("bold"), lfn) && !contains (string ("bold"), gfn))
