...
void
set_retina_factor (int f) {
  retina_factor= f;
}
void
set_retina_zoom (int z) {
  retina_zoom= z;
}
void
set_retina_icons (int i) {
  retina_icons= i;
}
void
set_retina_scale (double s) {
  retina_scale= s;
}
...
renderer_rep::extra_clipping (SI x1, SI y1, SI x2, SI y2) {
  ...
  x1= max (x1, ox1);
  y1= max (y1, oy1);
  x2= max (x1, min (x2, ox2));
  y2= max (y1, min (y2, oy2));
...
renderer_rep::is_visible (SI x1, SI y1, SI x2, SI y2) {
  return (x2 >= (cx1 - ox)) && (y2 >= (cy1 - oy)) && (x1 < (cx2 - ox)) &&
         (y1 < (cy2 - oy));
}
...
void
renderer_rep::clear_pattern (SI mx1, SI my1, SI mx2, SI my2, SI x1, SI y1,
                             SI x2, SI y2) {
  brush      b   = get_background ();
  brush_kind kind= b->get_type ();
  if (kind == brush_none)
    ;
  else if (kind == brush_color) clear (x1, y1, x2, y2);
    ...
    SI dw= 0;
    if (pattern[1] == "100%") dw= pixel;
    SI dh= 0;
    if (pattern[2] == "100%") dh= pixel;
