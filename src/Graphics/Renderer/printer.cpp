...
printer_rep::printer_rep (url ps_file_name2, int dpi2, int nr_pages2,
                          string page_type2, bool landscape2, double paper_w2,
                          double paper_h2)
    : renderer_rep (false), ps_file_name (ps_file_name2), dpi (dpi2),
      nr_pages (nr_pages2), page_type (page_type2), landscape (landscape2),
      paper_w (paper_w2), paper_h (paper_h2),
      use_alpha (get_preference ("experimental alpha") == "on"), linelen (0),
      fg ((color) (-1)), bg ((color) (-1)), opacity (255), pen (black),
      bgb (white), ncols (0), lw (-1), nwidths (0), cfn (""), nfonts (0),
      xpos (0), ypos (0), tex_flag (false), toc (TUPLE), defs ("?"),
      tex_chars ("?"), tex_width ("?"), tex_fonts ("?"),
      tex_font_chars (array<int> (0)), metadata ("") {
  ...
  prologue << "%%BoundingBox: 0 0 " << as_string (psw) << " " << as_string (psh)
           << "\n";
  if (landscape) prologue << "%%Orientation: Landscape\n";
  ...
  define (PS_CLIP,
          string ("/pt4 X /pt3 X /pt2 X /pt1 X\n") *
  ...
  define (PS_FILL_ARC,
          string ("/a2 X /a1 X /r2 X /r1 X /pt2 X /pt1 X\n") *
...
printer_rep::~printer_rep () {
  ...
  generate_tex_fonts ();
  prologue << "end\n"
           << "systemdict /pdfmark known{userdict /?pdfmark systemdict /exec "
              "get put}{userdict /?pdfmark systemdict /pop get put userdict "
  ...
  if (page_type != "user") prologue << page_type << "\n";
  else {
    prologue << "/setpagedevice where\n";
    prologue << "{ pop << /PageSize [" << as_string (psw) << " "
             << as_string (psh) << "] >> setpagedevice }\n";
    prologue << "if\n";
  }
  prologue << "%%EndPaperSize\n";
  if (landscape) prologue << "@landscape\n";
  prologue << "%%EndSetup\n";
...
printer_rep::next_page () {
  ...
  cur_page++;
  body << "\n%%Page: " << as_string (cur_page) << " " << as_string (cur_page)
       << "\n"
       << as_string (cur_page) << " " << as_string (cur_page - 1) << " bop\n";
...
printer_rep::select_color (color c) {
  ...
  g        = 10000 + ((g * 1000) / 255);
  b        = 10000 + ((b * 1000) / 255);
  string rr= as_string (r);
  rr       = rr (1, 2) * "." * rr (2, 5);
  string gg= as_string (g);
  gg       = gg (1, 2) * "." * gg (2, 5);
  string bb= as_string (b);
  bb       = bb (1, 2) * "." * bb (2, 5);
...
printer_rep::make_tex_char (string name, unsigned char c, glyph gl) {
  ...
  if ((d1 < 256) && (d2 < 256) && (d3 < 256) && (d4 < 256) && (d5 < 256)) {
    ...
    hex_code= "[<" * hex_code * ">";
    hex_code << as_string (d1) << " " << as_string (d2) << " " << as_string (d3)
             << " " << as_string (d4) << " " << as_string (d5) << " ";
  }
...
static SI
parse_length (string pfb, int& pos) {
  QN c4= (QN) pfb[pos++];
  QN c3= (QN) pfb[pos++];
  QN c2= (QN) pfb[pos++];
  QI c1= (QI) pfb[pos++];
...
pfb_to_pfa (url file) {
  ...
  while ((pos < size) && (type != 3)) {
    ...
    default:
      TM_FAILED ("Unknown field type while reading PFB file");
      break;
    }
  }
  return pfa;
...
printer_rep::generate_tex_fonts () {
  ...
  while (it->busy ()) {
    ...
    if ((pos != -1) && ends (fn_name, "tt")) {
      ...
      if (suffix (u) == "pfb") ttf= pfb_to_pfa (u);
    }
#endif
    ...
    if (ttf != "") {
      ...
      int szpos= pos - 1;
      while ((szpos > 0) && is_numeric (fn_name[szpos - 1]))
        szpos--;
      ...
      prologue << "/" << tex_fonts[fn_name] << " " << as_string (N (a)) << " "
...
void
printer_rep::image (string name, string eps, SI x1, SI y1, SI x2, SI y2, SI w,
                    SI h, SI x, SI y, int alpha) {
...
printer_rep::draw_scalable (scalable im, SI x, SI y, int alpha) {
  if (im->get_type () != scalable_image)
    ...
    int       x1, y1, x2, y2;
    if (suffix (u) == "eps")
      ps_bounding_box (u, x1, y1, x2, y2); // get cached value
    // if original image is not eps at this point it was already converted
    // through a temp file
...
printer_rep::href (string label, SI x1, SI y1, SI x2, SI y2) {
  ...
  print ("]");
  if (preserve) print ("/Border [16 16 1 [3 10]] /Color [0.75 0.5 1.0]");
  else print ("/Border [16 16 0 [3 10]] /Color [0.75 0.5 1.0]");
  print ("/Subtype /Link");
  print ("/ANN pdfmark");
  cr ();
}
...
printer (url ps_file_name, int dpi, int nr_pages, string page_type,
  ...
  return tm_new<printer_rep> (ps_file_name, dpi, nr_pages, page_type, landscape,
                              paper_w, paper_h);
}
