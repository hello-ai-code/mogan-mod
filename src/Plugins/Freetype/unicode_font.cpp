...
#define CEIL(l)                                                                \
  (((((l * hdpi + (std_dpi - 1)) / std_dpi) + std_pixel - 1) / std_pixel) *    \
   std_pixel)
...
unicode_font_rep::unicode_font_rep (string name, string family2, double size2,
  ...
  // get space length
  get_extents (" ", ex);
  spc  = space ((3 * (ex->x2 - ex->x1)) >> 2, ex->x2 - ex->x1,
                (3 * (ex->x2 - ex->x1)) >> 1);
  extra= spc / 2;
  mspc = spc;
  sep  = wfn / 10;
  ...
  if (family == "Times New Roman") ligs= LIGATURE_FI + LIGATURE_FL;
  if (family == "Zapfino")
  ...
  if (starts (family, "STIX-")) {
    ...
    if (starts (family, "texgyrepagella-")) mspc= spc + 0.5 * space (spc->def);
  }
...
unicode_font_rep::get_extents (string s, metric& ex) {
  if (N (s) == 0) {
    ...
    while (i < n) {
      ...
      // cout << "Kerning " << ((char) pc) << ((char) uc) << " " << ROUND
      // (fnm->kerning (pc, uc)) << ", " << ROUND (next->x2) << "\n";
    }
  }
}
...
is_math_italic (string c) {
  ...
  if (code <= 0x213a) {
    if (code == 0x210a || code == 0x210b || code == 0x210e || code == 0x210f ||
        code == 0x2110 || code == 0x2112 || code == 0x2113 || code == 0x211b ||
        code == 0x212c || code == 0x212f || code == 0x2130 || code == 0x2131 ||
...
is_integral (string s) {
  ...
  while (pos + 1 < n && s[pos] == 'i' && s[pos + 1] == 'i')
    pos++;
  return test (s, pos, "int-") || test (s, pos, "int>") ||
         test (s, pos, "idotsint");
}
...
unicode_font_rep::get_left_slope (string s) {
  ...
  if (N (c) >= 3) {
    ...
    else if (math_type == MATH_TYPE_TEX_GYRE && is_integral (s)) return 0.1;
  }
  get_extents (c, ex);
...
unicode_font_rep::get_left_correction (string s) {
  ...
  else if (ex->x3 < ex->x1) return ex->x1 - ex->x3;
  return 0;
}
...
unicode_font_rep::get_right_correction (string s) {
  ...
  if (math_type == MATH_TYPE_TEX_GYRE && is_integral (s))
    return (ex->x2 - ex->x1) / 16;
  else if (ex->x4 > ex->x2) return ex->x4 - ex->x2;
  return 0;
}
