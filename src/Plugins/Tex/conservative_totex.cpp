...
void
texmacs_neighbours (tree t, hashmap<tree, tree>& prec,
                    hashmap<tree, tree>& succ) {
  if (is_atomic (t)) return;
  else {
    int i, n= N (t);
    for (i= 1; i < n; i++)
      if (is_compound (t[i - 1], "mlx", 2) && is_compound (t[i], "mlx", 2)) {
        prec (t[i][0])    = t[i - 1][0];
        succ (t[i - 1][0])= t[i][0];
      }
  }
...
int
common_len (tree id, tree p, int c, int delta, hashmap<tree, tree> corr,
...
tree
texmacs_best_match (tree ids, tree p, int c, hashmap<tree, tree> corr,
                    hashmap<tree, tree> pred, hashmap<tree, tree> succ) {
...
texmacs_invarianted (tree t, tree p, int c, string src,
                     hashmap<tree, tree> corr, hashmap<tree, tree> pred,
                     hashmap<tree, tree> succ) {
  if (corr->contains (t)) {
...
tree
texmacs_invarianted_merge (tree t, string src, tree org, tree u,
                           hashmap<tree, path> h) {
  if (is_atomic (t)) return t;
  else {
    if (true) {
    ...
    if (is_concat (t) || is_document (t)) {
      ...
      for (i= 0; i < n; i++) {
        ...
        if (N (r) > 0 && is_compound (r[N (r) - 1], "ilx", 1) &&
        ...
        int j= i;
        while (j < n && !is_compound (t[j], "ilx", 1))
          j++;
        if (j < n && j > i && N (r) > 0 &&
...
conservative_texmacs_to_latex (tree doc, object opts) {
  ...
  hashmap<string, tree> atts_map= tree_hashmap (UNINIT, atts);
