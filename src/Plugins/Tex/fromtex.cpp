...
is_var_compound (tree t, string s) {
  return is_compound (t, s) || (is_func (t, APPLY) && t[0] == s) ||
         (is_func (t, EXPAND) && t[0] == s);
}

bool
is_var_compound (tree t, string s, int n) {
  return is_compound (t, s, n) || (is_func (t, APPLY, n + 1) && t[0] == s) ||
...
is_block_environnement (tree t) {
  ...
  if (!is_begin (t) && !is_end (t)) return is_block_algorithm (t);
  string s= as_string (t[0]);
  s       = replace (s, "*", "");
  return is_enunciation (s) || ends (s, "abstract") || ends (s, "center") ||
         ends (s, "description") || ends (s, "document") ||
         ends (s, "enumerate") || ends (s, "figure") || ends (s, "flushleft") ||
         ends (s, "flushright") || ends (s, "itemize") || ends (s, "list") ||
         ends (s, "quotation") || ends (s, "quote") || ends (s, "sloppypar") ||
         ends (s, "table") || ends (s, "tabularx") ||
         ends (s, "teaserfigure") || ends (s, "thebibliography") ||
         ends (s, "theindex") || ends (s, "titlepage") ||
         ends (s, "trivlist") || ends (s, "verbatim") || ends (s, "verse") ||
         ends (s, "multicols") || ends (s, "part") || ends (s, "chapter") ||
         ends (s, "section") || ends (s, "indent") || ends (s, "tmparsep") ||
         ends (s, "tmparmod");
}
...
kill_space_invaders (tree t, char& status) {
  ...
  for (int i= 0; i < N (t); i++) {
    ...
    if (is_concat (u)) r << kill_space_invaders (u, status);
      ...
      i++;
      while (i < N (t) && (might_not_be_typesetted (t[i]) || t[i] == " " ||
                           t[i] == "\t" || t[i] == "\n")) {
      ...
      case 'N':
        if (u == " " || u == "\t" || u == "\n")
          ;
        else if (might_not_be_typesetted (u)) r << u;
        else {
          r << u;
          status= 'M';
        }
...
font_of_tree (tree t) {
  ...
  if (lang == "cyrillic") return "cyrillic";
  if (lang == "cjk") return default_chinese_font_name ();
  if (lang == "japanese") return default_japanese_font_name ();
  if (lang == "korean") return default_korean_font_name ();
  return "roman";
}
    ...
    for (i= 0; i < n; i++) {
      if (t[i] == " ") r << t[i];
        ...
        if (current_lang != charlang && charlang != lan) {
          if (current_lang != lan) r << tree (RESET, "font");
...
set_special_fonts (tree t, string lan) {
  ...
  // Korean or Taiwanese.
  if (lan == "chinese" || lan == "taiwanese" || lan == "japanese") lan= "cjk";
...
is_declaration (tree u) {
  return (is_tuple (u, "\\def") || is_tuple (u, "\\def*") ||
          is_tuple (u, "\\def**") || is_tuple (u, "\\newenvironment") ||
          is_tuple (u, "\\newenvironment*") ||
          is_tuple (u, "\\newenvironment**") || is_tuple (u, "\\newtheorem") ||
          is_tuple (u, "\\newtheorem*"));
}
...
filter_preamble (tree t) {
  ...
  for (i= 0; i < n; i++) {
    ...
    if (in_preamble) {
      if (u == tuple ("\\begin-document")) {
        ...
        preamble << tuple ("\\newtheorem", u[N (u) - 1], u[N (u) - 1]) << "\n"
                 << "\n";
      else if (is_tuple (u, "\\SetKw", 2) || is_tuple (u, "\\SetKwData", 2) ||
        ...
        preamble << u << "\n"
               ...
               is_tuple (u, "\\CopyrightYear") || is_tuple (u, "\\crdata")) {
        preamble << u << "\n"
                 << "\n";
      }
      else if (i + 6 < N (t) && is_tuple (t[i], "\\begingroup") &&
               is_tuple (t[i + 1], "\\blx") &&
               is_tuple (t[i + 2], "\\endgroup") && is_declaration (t[i + 3]) &&
        ...
        preamble << u << "\n"
               ...
               is_tuple (u, "\\footnotesep") || is_tuple (u, "\\footskip") ||
               is_tuple (u, "\\headheight") || is_tuple (u, "\\headsep") ||
               is_tuple (u, "\\jot") || is_tuple (u, "\\marginparwidth") ||
               is_tuple (u, "\\mathindent") ||
               is_tuple (u, "\\oddsidemargin") || is_tuple (u, "\\parindent") ||
               is_tuple (u, "\\textheight") || is_tuple (u, "\\textwidth") ||
               is_tuple (u, "\\columnwidth") || is_tuple (u, "\\linewidth") ||
               is_tuple (u, "\\topmargin") || is_tuple (u, "\\topskip")) {
        i++;
        ...
        while (i < n && is_atomic (t[i]) && N (t[i]->label) == 1 &&
               (is_alpha (t[i]->label[0]) || is_numeric (t[i]->label[0]) ||
                t[i]->label[0] == '-' || t[i]->label[0] == '.'))
          val << t[i++]->label;
    ...
    else if (is_custom_maketitle (u)) doc << tuple ("\\custom-maketitle");
    else if (is_metadata_env (u)) {
      string s= as_string (u[0]);
      s       = "\\end-" * s (7, N (s));
      while (i < n && !is_tuple (t[i], s))
        i++;
    }
    else if (!is_metadata (u)) doc << u;
  }
...
/******************************************************************************
 * Import macro as pictures (no-op: LaTeX_Preview plugin removed)
 ******************************************************************************/
...
latex_symbol_to_tree (string s) {
  ...
  if (s[0] == '\\') {
    ...
    if (latex_type ('\\' * s) == "command") {
      ...
      if (s == "strut")
        return tree (APPLY, "resize", "", "0pt", "-0.3bls", "0pt", "0.7bls");
      ...
      if (s == "twocolumn") return tree (SET, "par-columns", "2");
      if (s == "onecolumn") return tree (SET, "par-columns", "1");
      if (s == "bysame") return "---";
      if (s == "MR") return "MR ";
    }
    ...
    if ((latex_type (s) == "modifier") && (latex_arity (s) == 0)) {
      ...
      if (s == "normalfont")
        return concat (tree (SET, FONT_FAMILY, "rm"),
                       tree (SET, FONT_SHAPE, "right"),
                       tree (SET, FONT_SERIES, "medium"));
...
test_alpha_on_end (tree t) {
  ...
  if (is_concat (t) && (N (t) >= 1)) return test_alpha_on_end (t[N (t) - 1]);
  return false;
}
...
string_arg (tree t, bool url) {
  ...
  else if (is_func (t, RSUB, 1)) return "_" * string_arg (t[0], url);
  else if (is_func (t, RSUP, 1)) return "^" * string_arg (t[0], url);
  else if (is_func (t, APPLY, 1) && t[0] == "nbsp" && !url) return " ";
  else if (is_func (t, APPLY, 1) && t[0] == "nbsp" && url) return "~";
  else if (is_func (t, APPLY, 1) && t[0] == "emdash") return "---";
  else {
    // cout << "t= " << t << "\n";
    return "";
  }
...
latex_key_arg (tree t) {
  ...
  if (s == "") {
    ...
    if (is_concat (t) && N (t) == 3 && is_apply (t[1], "op", 1)) {
      if (t[1][1] == "<leftarrow>") return "left";
      else if (t[1][1] == "<rightarrow>") return "right";
      else if (t[1][1] == "<uparrow>") return "up";
      else if (t[1][1] == "<downarrow>") return "down";
      else return t;
    }
    else return t;
  }
  ...
  int    i, n= N (s);
  for (i= 0; i < n; i++) {
    if (test (s, i, "Shift+")) r << "S-", i+= 5;
    else if (test (s, i, "Ctrl+")) r << "C-", i+= 4;
    else if (test (s, i, "Alt+")) r << "A-", i+= 3;
    else if (test (s, i, "Meta+")) r << "M-", i+= 4;
    else if (test (s, i, "Hyper+")) r << "H-", i+= 5;
    else if (is_alpha (s[i])) r << locase (s[i]);
    else r << s[i];
  }
  return r;
}
...
latex_concat_to_tree (tree t, bool& new_flag) {
  ...
  for (i= 0; i < n; i++) {
    ...
    bool operator_flag= is_tuple (t[i]) && (N (t[i]) == 1) &&
    ...
    if (is_atomic (u)) {
        ...
        if (command_type["!mode"] == "math") {
          ...
          if ((t[i] != tree (TUPLE, "\\ "))) {
            ...
            if (i + 1 < N (t) && is_tuple (t[i + 1])) {
              string s= t[i + 1][0]->label;
              if (starts (s, "\\end-") && (command_type["!verbatim"] != "true"))
                continue;
            }
          }
        }
      ...
      if ((command_type["!mode"] == "math") && (!cc_flag) && old_flag &&
          (new_flag || operator_flag))
        s= "*" * s;
      if ((N (r) > 0) && is_atomic (r[N (r) - 1])) {
        if (s != " " || r[N (r) - 1]->label != " ") r[N (r) - 1]->label << s;
      }
      else r << s;
    }
    else {
...
m2e (tree t, string var, string val) {
  return tree (CONCAT, tree (SET, copy (var), copy (val)), l2e (t[1]),
               tree (RESET, copy (var)));
}

tree
var_m2e (tree t, string var, string val) {
  return tree (CONCAT, tree (SET, copy (var), copy (val)), t2e (t[1], false),
               tree (RESET, copy (var)));
}

tree
mod_m2e (tree t, string var, string val) {
  if (command_type["!mode"] != "math") return m2e (t, var, val);
  else
    return tree (CONCAT, tree (SET, "mode", "text"),
                 tree (SET, copy (var), copy (val)), t2e (t[1], false),
                 tree (RESET, copy (var)), tree (RESET, "mode"));
}
...
is_left_type (string s) {
  return (s == "(") || (s == "[") || (s == "\\{") || (s == "\\lvert") ||
         (s == "\\lVert") || (s == "\\lfloor") || (s == "\\lceil") ||
         (s == "\\langle");
}

static bool
is_right_type (string s) {
  return (s == ")") || (s == "]") || (s == "\\}") || (s == "\\rvert") ||
         (s == "\\rVert") || (s == "\\rfloor") || (s == "\\rceil") ||
         (s == "\\rangle");
}
...
is_mid_type (string s) {
  return (s == "|") || (s == "||") || (s == "\\|") || (s == "\\vert") ||
         (s == "\\Vert") || (s == "\\lvert") || (s == "\\rvert") ||
         (s == "\\lVert") || (s == "\\rVert") || (s == "/") ||
         (s == "\\arrowvert") || (s == "\\backslash") || (s == "\\Arrowvert") ||
         (s == "\\bracevert") || (s == "\\Uparrow") || (s == "\\downarrow") ||
         (s == "\\uparrow") || (s == "\\Downarrow") || (s == "\\updownarrow") ||
         (s == "\\Updownarrow");
}
...
latex_cref_to_tree (string s) {
  if (N (cref_table) == 0)
    cref_table << string ("fig") << string ("Figure") << string ("tab")
               << string ("Table") << string ("alg") << string ("Algorithm")
               << string ("subsec") << string ("Subsection") << string ("ssec")
               << string ("Subsection") << string ("sec") << string ("Section")
               << string ("thm") << string ("Theorem") << string ("prop")
               << string ("Proposition") << string ("lem") << string ("Lemma")
               << string ("cor") << string ("Corollary") << string ("note")
               << string ("Note") << string ("rem") << string ("Remark")
               << string ("def") << string ("Definition") << string ("dfn")
               << string ("Definition") << string ("conv")
               << string ("Convention") << string ("war") << string ("Warning")
               << string ("exa") << string ("Example") << string ("not")
               << string ("Notation") << string ("prob") << string ("Problem")
               << string ("prb") << string ("Problem") << string ("exe")
               << string ("Exercise") << string ("exc") << string ("Exercise")
               << string ("sol") << string ("Solution") << string ("eqn")
               << string ("Equation") << string ("ch") << string ("Chapter")
               << string ("th") << string ("Theorem") << string ("lm")
               << string ("Lemma") << string ("ax") << string ("Axiom")
               << string ("ex") << string ("Example") << string ("eq")
               << string ("Equation");
  tree   t (CONCAT);
  string type= "";
  for (int i= 0; i < N (cref_table); i+= 2)
    if (type == "" && starts (s, cref_table[i])) type= cref_table[i + 1];
  for (int i= 0; i < N (cref_table); i+= 2)
    if (type == "" && ends (s, cref_table[i])) type= cref_table[i + 1];
  for (int i= 0; i < N (cref_table); i+= 2)
    if (type == "" && occurs (cref_table[i], s)) type= cref_table[i + 1];
...
find_next_length (tree t) {
  if (is_atomic (t)) return t;
  else if (is_func (t, APPLY) && N (t) >= 2)
    if (t[0] == "tex-len") return find_next_length (t[1]);
  return "";
}
...
is_negative_length (tree t) {
  ...
  for (int i= 0; i < N (s); i++) {
    if (s[i] == '-') return true;
      ...
      ;
    else if (is_numeric (s[i]) || is_alpha (s[i]) || s[i] == '+' ||
             s[i] == '.' || s[i] == ',')
      return false;
  }
  return false;
...
translate_keys (array<tree> a, hashmap<tree, tree> dic) {
  ...
  for (i= 0; i < n - 1; i+= 2) {
    tree tmp= dic[a[i]];
    if (tmp == "") r << tree (SET, a[i], a[i + 1]);
    else if (is_tuple (tmp) && N (tmp) == 2) r << tree (SET, tmp[0], tmp[1]);
    else r << tree (SET, tmp, a[i + 1]);
  }
  return r;
}
...
latex_command_to_tree (tree t) {
  if (is_tuple (t) && N (t) > 1) {
    string s= as_string (t[0]);
    s       = s (1, N (s));
    if (latex_type (s) == "ignore") return tree ();
  }
  if (is_tuple (t, "\\def", 2)) {
    ...
    if (is_func (t, TUPLE, 3) && is_func (t[2], TUPLE, 1) &&
      ...
      l2edef= l2e (def);
      if (N (l2edef) != 0 && !is_concat (l2edef)) f << l2edef;
        ...
        for (int a= 1; a < N (def); a++)
          adef << def[a];
        f << adef;
      }
      else f << def;
      if (latex_arity (s) > 0) return tree (ASSIGN, var, f);
      else {
        tree r= concat ();
        r << tree (ASSIGN, var, f);
        def= tuple (s * "*");
  ...
  if (is_tuple (t, "\\part*", 2) || is_tuple (t, "\\chapter*", 2) ||
      is_tuple (t, "\\section*", 2) || is_tuple (t, "\\subsection*", 2) ||
      is_tuple (t, "\\subsubsection*", 2) || is_tuple (t, "\\paragraph*", 2) ||
  ...
  if (is_tuple (t, "\\begin-part") || is_tuple (t, "\\begin-part*") ||
      is_tuple (t, "\\begin-part**") || is_tuple (t, "\\begin-chapter") ||
      is_tuple (t, "\\begin-chapter*") || is_tuple (t, "\\begin-chapter**") ||
      is_tuple (t, "\\begin-section") || is_tuple (t, "\\begin-section*") ||
      is_tuple (t, "\\begin-section**") || is_tuple (t, "\\begin-subsection") ||
      is_tuple (t, "\\begin-subsection*") ||
      is_tuple (t, "\\begin-subsection**") ||
      is_tuple (t, "\\begin-subsubsection") ||
      is_tuple (t, "\\begin-subsubsection*") ||
      is_tuple (t, "\\begin-subsubsection**") ||
      is_tuple (t, "\\begin-paragraph") || is_tuple (t, "\\begin-paragraph*") ||
  ...
  if (is_tuple (t, "\\end-part") || is_tuple (t, "\\end-part*") ||
      is_tuple (t, "\\end-chapter") || is_tuple (t, "\\end-chapter*") ||
      is_tuple (t, "\\end-section") || is_tuple (t, "\\end-section*") ||
      is_tuple (t, "\\end-subsection") || is_tuple (t, "\\end-subsection*") ||
      is_tuple (t, "\\end-subsubsection") ||
      is_tuple (t, "\\end-subsubsection*") || is_tuple (t, "\\end-paragraph") ||
      is_tuple (t, "\\end-paragraph*") || is_tuple (t, "\\end-subparagraph") ||
  ...
  if (is_tuple (t, "\\geometry", 1)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      if (N (l[i]) == 1) {
        ...
        else if (type != "undefined") r << tree (ASSOCIATE, "paper-type", type);
      }
      else if (N (l[i]) == 2) {
  ...
  if (is_tuple (t, "\\marginpar*", 2)) {
    ...
    if (l == "" && r != "") return tree (APPLY, "marginal-right-note", "", r);
    if (r == "" && l != "") return tree (APPLY, "marginal-left-note", "", l);
    return (concat (tree (APPLY, "marginal-right-note", "", r),
  ...
  if (is_tuple (t, "\\equal", 2)) return tree (EQUAL, l2e (t[1]), l2e (t[2]));
  ...
  if (is_tuple (t, "\\\"", 1))
    return latex_accent (t[1], "<ddot>"); // diaeresis
  ...
  if (is_tuple (t, "\\textnormalfont", 1))
    return m2e (m2e (m2e (t, FONT_FAMILY, "rm"), FONT_SERIES, "medium"),
                FONT_SHAPE, "right");
  ...
  if (is_tuple (t, "\\IF*", 2))
    return tree (
        BEGIN, "algo-if-else-if",
        concat (l2e (t[2]), " ", tree (APPLY, "algo-comment", l2e (t[1]))));
  if (is_tuple (t, "\\KwData", 1)) return tree (APPLY, "algo-data", l2e (t[1]));
  ...
  if (is_tuple (t, "\\While*", 3))
    return tree (
        APPLY, "algo-while", l2e (t[2]),
        concat (tree (APPLY, "algo-comment", l2e (t[1])), "\n", l2e (t[2])));
  if (is_tuple (t, "\\Begin", 1)) return tree (APPLY, "algo-begin", l2e (t[1]));
  ...
  if (is_tuple (t, "\\LOOP*", 1))
    return concat (tree (BEGIN, "algo-loop"),
                   tree (APPLY, "algo-comment", l2e (t[1])), "\n");
  if (is_tuple (t, "\\KwIn", 1)) return tree (APPLY, "algo-inputs", l2e (t[1]));
  ...
  if (is_tuple (t, "\\centering", 1)) return tree (APPLY, "center", l2e (t[1]));
  ...
  if (is_tuple (t, "\\overline", 1)) return tree (WIDE, l2e (t[1]), "<bar>");
  ...
  if (is_tuple (t, "\\uhat", 1)) return tree (VAR_WIDE, l2e (t[1]), "^");
  if (is_tuple (t, "\\tilde", 1) || is_tuple (t, "\\Tilde", 1))
    return tree (WIDE, l2e (t[1]), "~");
  if (is_tuple (t, "\\utilde", 1)) return tree (VAR_WIDE, l2e (t[1]), "~");
  ...
  if (is_tuple (t, "\\uwidetilde", 1)) return tree (VAR_WIDE, l2e (t[1]), "~");
  if (is_tuple (t, "\\dot", 1) || is_tuple (t, "\\Dot", 1))
    return tree (WIDE, l2e (t[1]), "<dot>");
  if (is_tuple (t, "\\underdot", 1))
    return tree (VAR_WIDE, l2e (t[1]), "<dot>");
  ...
  if (is_tuple (t, "\\udddot", 1))
    return tree (VAR_WIDE, l2e (t[1]), "<dddot>");
  ...
  if (is_tuple (t, "\\ucheck", 1))
    return tree (VAR_WIDE, l2e (t[1]), "<check>");
  if (is_tuple (t, "\\grave", 1) || is_tuple (t, "\\Grave", 1))
    return tree (WIDE, l2e (t[1]), "<grave>");
  if (is_tuple (t, "\\ugrave", 1))
    return tree (VAR_WIDE, l2e (t[1]), "<grave>");
  if (is_tuple (t, "\\acute", 1) || is_tuple (t, "\\Acute", 1))
    return tree (WIDE, l2e (t[1]), "<acute>");
  if (is_tuple (t, "\\uacute", 1))
    return tree (VAR_WIDE, l2e (t[1]), "<acute>");
  ...
  if (is_tuple (t, "\\ubreve", 1))
    return tree (VAR_WIDE, l2e (t[1]), "<breve>");
  ...
  if (is_tuple (t, "\\abovering", 1) || is_tuple (t, "\\ring", 1) ||
      is_tuple (t, "\\mathring", 1))
  ...
  if (is_tuple (t, "\\hspace", 1) || is_tuple (t, "\\hspace*", 1) ||
      is_tuple (t, "\\mspace", 1)) {
    if (is_tuple (t[1], "\\tex-len", 3)) return tree (SPACE, l2e (t[1]));
    else {
      tree r= t2e (t[1]);
  ...
  if (is_tuple (t, "\\vspace", 1) || is_tuple (t, "\\vspace*", 1)) {
    if (is_tuple (t[1], "\\tex-len", 3)) return tree (VSPACE, l2e (t[1]));
  ...
  if (is_tuple (t, "\\value", 1)) return compound ("value-counter", v2e (t[1]));
  ...
  if (is_tuple (t, "\\setcounter", 2)) {
    if (v2e (t[2]) == "0") return compound ("reset-counter", v2e (t[1]));
    return tree (ASSIGN, v2e (t[1]) * "-nr", v2e (t[2]));
  }
  if (is_tuple (t, "\\addtocounter", 2)) {
    if (v2e (t[2]) == "1") return compound ("inc-counter", v2e (t[1]));
    return tree (ASSIGN, v2e (t[1]) * "-nr",
                 tree (PLUS, v2e (t[1]) * "-nr", v2e (t[2])));
  }
  if (is_tuple (t, "\\custombinding", 1)) return tree (SET_BINDING, v2e (t[1]));
  ...
  if (is_tuple (t, "\\@startsection") && N (t) >= 7) {
    ...
    if (is_negative_length (spa)) spa= tree (HSPACE, abs_length (spa));
    else spa= tree (VSPACE, spa);

    if (is_compound (style))
      for (int i= 0; i < N (style); i++) {
        if (is_func (style[i], RESET) && !inserted) {
          if (center)
            r << spb << compound ("center", tree (ARG, "name")) << spa
              << indentafter;
          else
            r << spb << tree (HSPACE, indent) << tree (ARG, "name") << spa
              << indentafter;
          inserted= true;
        }
        else r << style[i];
  ...
  if (is_tuple (t, "\\@setfontsize", 3)) {
    ...
    return tree (
        WITH, "font-size-base", fontsize, "par-sep",
  ...
  if (is_tuple (t, "\\mathchoice", 4))
    return compound ("math-choice", l2e (t[1]), l2e (t[2]), l2e (t[3]),
                     l2e (t[4]));
  if (is_tuple (t, "\\ensuremath", 1)) return var_m2e (t, MODE, "math");
  ...
  if (is_tuple (t, "\\citet", 1) || is_tuple (t, "\\citep", 1) ||
    ...
    if (ends (cite_type, "*")) {
      star     = "*";
      cite_type= cite_type (0, N (cite_type) - 1);
    }
  ...
  if (is_tuple (t, "\\onlinecite", 1)) return compound ("cite-arg", l2e (t[1]));
  ...
  if (is_tuple (t, "\\bibitem", 1)) return compound ("bibitem", v2e (t[1]));
  ...
  if (is_tuple (t, "\\hypertarget", 2))
    return tree (CONCAT, l2e (t[2]), tree (LABEL, t2e (t[1])));
  ...
  if (is_tuple (t, "\\xleftrightarrow*", 2))
    return tree (LONG_ARROW, "<rubber-leftrightarrow>", l2e (t[1]), l2e (t[2]));
  ...
  if (is_tuple (t, "\\tmmathbf", 1))
    return tree (CONCAT, tree (SET, MATH_FONT_SERIES, "bold"), l2e (t[1]),
  ...
  if (is_tuple (t, "\\tmhlink", 1)) return tree (HLINK, l2e (t[1]), l2e (t[2]));
  if (is_tuple (t, "\\tmaction", 1))
    return tree (ACTION, l2e (t[1]), l2e (t[2]));
  if (is_tuple (t, "\\tmconverterinput", 4))
    return compound ("converter-input", l2e (t[1]), l2e (t[3]), l2e (t[4]));
  if (is_tuple (t, "\\tmconverteroutput", 4))
    return compound ("converter-output", l2e (t[1]), l2e (t[3]), l2e (t[4]));
  if (is_tuple (t, "\\tmscriptinput", 4))
    return compound ("script-input", l2e (t[1]), "default", l2e (t[3]),
                     l2e (t[4]));
  if (is_tuple (t, "\\tmscriptoutput", 4))
    return compound ("script-output", l2e (t[1]), "default", l2e (t[3]),
                     l2e (t[4]));
