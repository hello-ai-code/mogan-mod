...
parse_matrix_params (tree t, string tr, string br, string hoff) {
  ...
  for (i= 0; i < n; i++) {
    ...
    case 'X': {
      ...
      char   how_c = s[i];
      string how   = (how_c == 'm') ? 'c' : ((how_c == 'b') ? 'b' : 't');
      tformat << tree (CWITH, tr, br, col_s, col_s, halign, how);
      if (how_c != 'X') {
        int start= ++i;
        if (i < n && s[start] == '{') {
  ...
  for (i++; i < N (t); i++) {
    ...
    if (v == tree (FORMAT, "line separator")) {
      ...
      continue;
             ...
             (v[0] == "array" || v[0] == "array*" || v[0] == "tabular" ||
              v[0] == "tabular*" || v[0] == "tabularx" ||
              v[0] == "tabularx*")) {
      ...
      tformat << tree (CWITH, row_t, row_t, col_s, col_s, CELL_ROW_SPAN,
                       height);
      if (as_string (v[2]) != "*") {
        ...
        tformat << tree (CWITH, row_t, row_b, col_s, col_s, CELL_HMODE,
                         "exact");
      }
    ...
    for (i= 0; i < n; i++)
      if (is_func (u[i], BEGIN)) {
        if (u[i][0] == "array" || u[i][0] == "array*")
          ...
          parse_pmatrix (r, u, i, "[", "]", "tabular*");
        else if (u[i][0] == "vmatrix") parse_pmatrix (r, u, i, "", "", "det");
...
admissible_env (tree t) {
  ...
  return type == "list" || type == "environment" || type == "enunciation" ||
  ...
  if (is_func (u, CONCAT)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      if ((is_func (v, BEGIN) || is_func (v, END)) && N (v) > 0 &&
          (v[0] == "tmpadded" || v[0] == "tmpadded*" || v[0] == "tmframed" ||
           v[0] == "tmframed*" || v[0] == "tmunderlined" ||
           v[0] == "tmunderlined*" || v[0] == "tmoverlined" ||
           v[0] == "tmoverlined*" || v[0] == "tmbothlined" ||
           v[0] == "tmbothlined*" || v[0] == "tmornamented" ||
           v[0] == "tmornamented*")) {
        string env= as_string (v[0]);
        if (env[N (env) - 1] == '*' && N (v) > 1) {
          if (is_concat (v[1])) r << A (v[1]);
          else r << v[1];
          env= env (2, N (env) - 1);
        }
        else env= env (2, N (env));
        r << tree (L (v), env);
        continue;
      }
      ...
      if (is_func (v, BEGIN, 2) &&
          (v[0] == "otherlanguage" || v[0] == "otherlanguage*")) {
      ...
      if (is_func (v, BEGIN, 1) && (v[0] == "picture")) {
        for (; i < n; i++)
          ...
          else if (is_func (u[i], END, 1) && (u[i][0] == "picture")) break;
        continue;
      }
      ...
      */
      ...
      if ((is_func (v, BEGIN, 1) || is_func (v, BEGIN, 2)) &&
        ...
        if (v == tree (BEGIN, "displaymath")) v= tree (BEGIN, "equation*");
      ...
      if (is_func (v, END, 1) && admissible_env (v)) {
        if (v == tree (END, "displaymath")) v= tree (END, "equation*");
        if (v == tree (END, "math")) {
          r << tree (RESET, MODE);
          spc_flag= item_flag= false;
          continue;
      ...
      if ((is_atomic (v)) && spc_flag && (N (v->label) > 0) &&
          (v->label[0] == ' ')) {
...
textm_algorithm_break_after (tree t) {
  ...
  return (
      (is_func (t, END) &&
       (t[0] == "algo-inputs" || t[0] == "algo-outputs" || t[0] == "algo-for" ||
        t[0] == "algo-while" || t[0] == "algo-function" ||
        t[0] == "algo-procedure" || t[0] == "algo-if-else-if" ||
        t[0] == "algo-loop" || t[0] == "algo-repeat" || t[0] == "algo-body")) ||
...
textm_algorithm_end_algo (tree t) {
  return (is_func (t, END) && (t[0] == "algorithmic" || t[0] == "algorithm2e" ||
                               t[0] == "algorithm"));
}
...
complete_algorithm_args (tree t, bool& in) {
  ...
  for (int i= 0, n= N (t); i < n; i++) {
    ...
    if (in && textm_algorithm_need_arg (t[i])) {
      ...
      r << t[i] << "\n";
    else if (in && is_func (t[i], SPACE, 1) && t[i][0] == "0.75spc") r << "\n";
...
textm_algorithm_end_if_arg (tree t) {
  return textm_algorithm_end_if (t) || is_apply (t, "algo-else") ||
         is_apply (t, "algo-else-if");
}
...
complete_algorithm_if_else_if (tree t, bool in) {
  ...
  for (int i= 0; i < N (t); i++) {
      ...
      while (i < N (t) && !textm_algorithm_end_algo (t[i]) &&
        ...
        while (i < N (t) && !textm_algorithm_end_algo (t[i]) &&
          ...
          if ((t[i] == " " || t[i] == "\n") &&
            ...
            i++;
          else arg << complete_algorithm_if_else_if (t[i++], in);
        }
        if (arg != concat ()) ifelse << arg;
        else i++;
    ...
    else r << complete_algorithm_if_else_if (t[i], in);
  }
  return r;
}
...
make_paragraphs (tree t, tree r) {
  ...
  if (s != concat ()) r << clean_paragraph_concat (s);
  return r;
}
...
is_preamble_command (tree t, tree& doc, string& style) {
  ...
  if (is_func (t, APPLY, 2)) {
    ...
    if ((t[0] == "documentstyle") || (t[0] == "documentclass")) {
...
is_bibliography_command (tree t, tree& doc, string& bib_style) {
  if (is_func (t, APPLY, 2)) {
    ...
    if (t[0] == "bibliography") {
      ...
      if (exists (bbl)) bib= bibtex_load_bbl ("bib", bbl);
      else bib= tree (DOCUMENT, compound ("bib-list", "[99]", ""));
      doc << begin << bib << end;
      return true;
    }
  }
...
env_hacks (tree t) {
  ...
  array<tree> ea;
  for (i= 0; i < N (beg); i++)
    if (is_func (beg[i], VSPACE)) ba << tree (VAR_VSPACE, A (beg[i]));
    ...
    else if (is_func (beg[i], NEW_PAGE)) ba << tree (VAR_NEW_PAGE, A (beg[i]));
    else ba << beg[i];
  for (i= 0; i < N (end); i++)
    if (is_func (end[i], NO_INDENT)) ea << tree (VAR_NO_INDENT, A (end[i]));
  ...
  for (int k= 0; k < N (beg) - i; k++) {
    ok= ok && is_func (beg[i - k - 1], SET, 2) &&
        beg[i - k - 1][0] == beg[i + k][0];
...
handle_improper_matches (tree t) {
  if (is_atomic (t)) return t;
    ...
    else if (is_func (r, ENV) && N (r) >= 2) return env_hacks (r);
    return r;
  }
}
...
finalize_floats (tree t) {
  if (is_atomic (t)) return t;
    ...
    tree capt= find_caption (t[N (t) - 1]);
    if (capt == "") return t;
    else return tree (make_tree_label ("specified-algorithm"), capt, body);
  }
  else {
    int  i, n= N (t);
    tree r (t, n);
...
is_hyper_link (string s) {
  return starts (s, "http://") || starts (s, "https://") ||
         starts (s, "ftp://");
}
...
finalize_misc (tree t) {
  ...
  // Fixme: to be improved when TeXmacs will allow easy personalisation
  else if (is_compound (t, "enumerate", 2)) return compound ("enumerate", t[1]);
  else if (is_compound (t, "verbatim", 1) && is_atomic (t[0]) &&
           is_hyper_link (t[0]->label)) {
  ...
  else if (is_compound (t, "flushleft", 1) || is_compound (t, "leftaligned", 1))
  ...
  else if (is_compound (t, "text", 1) && is_func (t[0], WITH, 3) &&
           t[0][0] == "font-family" && t[0][1] == "rm")
    return compound ("math-up", finalize_misc (t[0][2]));
  else if (is_var_compound (t, "algo-for", 4)) {
    return compound ("algo-for-all", finalize_misc (t[N (t) - 2]),
                     finalize_misc (t[N (t) - 1]));
  }
  else if (is_compound (t, "minipage", 3)) {
    if (is_atomic (t[1])) t[1]= textm_normalize_length (t[1]->label);
    if (is_document (t[2]) && N (t[2]) == 1) t[2]= t[2][0];
    return t;
  }
  else {
    int    i, n= N (t);
    ...
    if (latex_type ("\\begin-" * l) == "user" &&
        latex_arity ("\\begin-" * l) < 0 &&
...
flatten_documents (array<tree> a) {
  array<tree> r;
  for (int i= 0; i < N (a); i++) {
    if (is_document (a[i])) r << flatten_documents (A (a[i]));
    else r << a[i];
  }
  return r;
}
...
modernize_newlines (tree t, bool skip) {
  ...
  else if (is_document (r)) r= make_document (A (r));
  ...
  if (is_atomic (t)) {
    ...
    if (rev) {
      if (state == 'n' || state == 's' || ctrl) {
        if (is_beginning_by_space (s, rev)) s= remove_next_spaces (s, rev);
      }
    }
    else {
      if (state == 's')
...
remove_superfluous_newlines (tree t) {
  ...
  tree r (L (t));
  for (int i= 0; i < N (t); i++) {
    if (!is_document (t) || t[i] != "") r << remove_superfluous_newlines (t[i]);
  }
...
concat_document_correct (tree t) {
  ...
  else if (is_document (r) && contains_document (r)) r= make_document (A (r));
  return r;
}
...
associate_sections_and_labels (tree t, array<int>& path,
                               array<array<int>>& paths, array<tree>& labels,
                               array<int>& sec_path) {
...
insert_label (tree t, tree label, array<int> path) {
  ...
  else if (is_concat (t) || is_document (t)) t[i]= concat (t[i], label);
  return t;
}
...
merge_successive_withs (tree t, bool force_concat= false) {
  ...
  if (!is_concat (t) && !is_document (t)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      else r << merge_successive_withs (t[i]);
    }
  }
  return r;
}
...
remove_geometry (tree t) {
  ...
  tree r (L (t));
  for (i= 0; i < n; i++)
    if (!is_compound (t[i], "geometry")) r << remove_geometry (t[i]);
  return r;
}
...
auto_vspace (tree t) {
  ...
  if (starts (s, "equation") || starts (s, "eqnarray") ||
      starts (s, "gather") || starts (s, "multline") || starts (s, "align") ||
      starts (s, "alignat") || starts (s, "flalign") || starts (s, "itemize") ||
      starts (s, "enumerate") || starts (s, "description") ||
      starts (s, "solution"))
    return true;
  return latex_type ("\\begin-" * s) == "enunciation";
}
...
is_vspace (tree t, bool eat) {
  if (is_func (t, LINE_BREAK, 0) || is_func (t, NEW_LINE, 0) ||
      is_func (t, NEXT_LINE, 0))
    return true;
  if (!eat) return false;
  if (!is_func (t, VSPACE, 1) && !is_func (t, VAR_VSPACE, 1)) return false;
  return t[0] == "0.25fn" || t[0] == "0.5fn" || t[0] == "1fn" ||
...
clean_vspace (tree t) {
  if (is_atomic (t)) return t;
    ...
    for (i= 0; i < n; i++) {
      ...
      else if (is_concat (cur) && N (cur) > 1 && is_func (cur[0], VSPACE))
        cur[0]= tree (VAR_VSPACE, A (cur[0]));
      else if (is_concat (cur) && N (cur) > 2 && is_func (cur[0], NO_INDENT) &&
...
filter_geometry (tree t) {
  ...
  if (is_atomic (t))
    ...
    int i, n= N (t[0]);
    for (i= 0; i < n; i++)
      if (is_compound (t[0][i], "associate")) r << t[0][i];
  }
  else {
    int i, n= N (t);
    for (i= 0; i < n; i++)
...
static void
collect (hashmap<tree_label, bool>& def, hashmap<tree_label, bool>& use,
         tree t) {
  if (is_atomic (t)) return;
...
static void
add_missing (tree& doc, string name, tree val, hashmap<tree_label, bool> def,
             hashmap<tree_label, bool> use) {
  tree_label l= make_tree_label (name);
  if (use->contains (l) && !def->contains (l)) doc << tree (ASSIGN, name, val);
}
...
guess_missing (tree t) {
  ...
  if (is_func (t, DOCUMENT) && N (t) >= 1) {
    if (is_compound (t[0], "hide-preamble", 1) && is_func (t[0][0], DOCUMENT)) {
      tree r= copy (t);
      r[0][0] << A (d);
      return r;
    }
...
latex_to_tree (tree t0) {
  ...
  if (!exists (url ("$TEXMACS_STYLE_PATH", style * ".ts"))) style= "generic";
