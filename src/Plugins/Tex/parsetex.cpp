...
latex_parser::parse (string s, int& i, string stop, int change) {
  ...
  while ((i < n) && is_space (s[i]))
    i++;
  while ((i < n) && no_error && (s[i] != '\0' || N (stop) != 0) &&
         (N (stop) != 1 || s[i] != stop[0]) &&
         (s[i] != '$' || stop != "$$" || i + 1 >= n || s[i + 1] != '$') &&
         (stop != "denom" ||
          (s[i] != '$' && s[i] != '}' && (i + 2 > n || s (i, i + 2) != "\\]") &&
           (i + 2 > n || s (i, i + 2) != "\\)") &&
           (i + 4 > n || s (i, i + 4) != "\\end"))) &&
    ...
    case '\\':
      ...
      if ((i + 6) < n &&
          (s (i + 1, i + 6) == "hskip" || s (i + 1, i + 6) == "vskip")) {
        string skip= s (i + 1, i + 6);
        i+= 7;
        bool tmp_textm_class_flag= textm_class_flag;
        textm_class_flag         = true;
        if (can_parse_length (s, i)) {
          if (skip == "hskip") t << tuple ("\\hspace", parse_length (s, i));
          else t << tuple ("\\vspace", parse_length (s, i));
        }
      ...
      else if ((i + 5) < n && s (i, i + 3) == "\\sp" &&
               !is_tex_alpha (s[i + 3])) {
        i+= 3;
        t << parse_command (s, i, "\\<sup>");
      }
      else if ((i + 5) < n && s (i, i + 3) == "\\sb" &&
               !is_tex_alpha (s[i + 3])) {
        i+= 3;
      ...
      else if (can_parse_length (s, i)) t << parse_length (s, i);
      else {
    ...
    default:
      if (i + 2 < n && s[i] == '-' && s[i + 1] == '-' && s[i + 2] == '-') {
        ...
        if ((i >= start + 3) && (command_type["!mode"] == "math")) {
          while ((i < n) && (is_iso_alpha (s[i]) || is_punctuation (s[i]) ||
                             is_space (s[i])))
            i++;
          if (i >= start + 20) {
...
latex_parser::parse_backslash (string s, int& i, int change) {
  ...
  if (((i + 6) < n) && (s (i, i + 4) == "\\url") && s[i + 4] != '{' &&
      s[i + 4] != ' ') {
    i+= 5;
    return parse_verbatim (s, i, s (i - 1, i), "\\url");
  }
  if (((i + 7) < n) && (s (i, i + 5) == "\\path") && s[i + 5] != '{' &&
      s[i + 5] != ' ') {
    i+= 6;
    return parse_verbatim (s, i, s (i - 1, i), "\\verbatim");
  }
  ...
  if (((i + 27) < n) && (s (i, i + 14) == "\\begin{tmcode}")) {
    ...
    else return parse_alltt (s, i, "\\end{tmcode}", "tmcode");
  }
  ...
  if (!is_tex_alpha (s[i])) {
    ...
    if (s[i - 1] == '[')
      return parse_command (s, i, "\\begin-displaymath", change);
    if (s[i - 1] == ']')
      return parse_command (s, i, "\\end-displaymath", change);
    return parse_command (s, i, s (i - 2, i), change);
  }

  /************************* normal commands *********************************/
  int start= i - 1;
  while ((i < n) && is_tex_alpha (s[i]))
    i++;
  if ((i < n) && (s[i] == '*') && latex_type (s (start, i + 1)) != "undefined")
    i++;
  string r= s (start, i);
  while ((i < n) && s[i] == ' ')
    i++;
...
is_math_environment (tree t) {
  ...
  for (i= N (b) - 1; i >= 0; i--)
    if (is_tuple (b[i]) && N (b[i]) > 0 && is_atomic (b[i][0]))
      if (latex_type (b[i][0]->label) == "math-environment") break;
  for (j= 0; j < N (e); j++)
    if (is_tuple (e[j]) && N (e[j]) > 0 && is_atomic (e[j][0]))
      if (latex_type (e[j][0]->label) == "math-environment") break;
  if (i >= 0 && j < N (e)) {
    string bs= b[i][0]->label;
    string es= e[j][0]->label;
    bool   ok= starts (bs, "\\begin-") && starts (es, "\\end-") &&
             bs (7, N (bs)) == es (5, N (es));
    // cout << t[1] << " -> " << ok << "\n";
    return ok;
  }
...
latex_parser::is_substituable (tree t) {
  ...
  if (is_tuple (t) && N (t) > 0) {
    ...
    if (latex_type (cmd) == "replace" ||
        (cmd[N (cmd) - 1] == '*' &&
         latex_type (cmd (0, N (cmd) - 1)) == "replace"))
      return true;
  }
  return false;
}
...
array<string>
tree_to_str_array (tree t1, tree t2) {
  array<string> r;
  r << as_string (t1);
  r << as_string (t2);
  return r;
...
is_var_def (tree t) {
  return is_tuple (t, "\\declaretheorem", 1) ||
         ...
         is_tuple (t, "\\SetKwData", 2) || is_tuple (t, "\\SetKwFunction", 2) ||
         is_tuple (t, "\\SetKwInput", 2) || is_tuple (t, "\\SetKwInOut", 2) ||
         is_tuple (t, "\\SetKw") || is_tuple (t, "\\newdimen", 1) ||
         is_tuple (t, "\\newlength", 1);
}
...
static bool
is_def (tree t) {
  return is_tuple (t, "\\def", 2) || is_tuple (t, "\\def*", 3) ||
         is_tuple (t, "\\def**", 4) || is_env (t);
}
...
contains_side_effects (tree t, bool root= true) {
  ...
  int i, n= N (t);
  for (i= 0; i < n; i++)
    if (contains_side_effects (t[i], false)) return true;
  return false;
}
...
latex_parser::parse_command (string s, int& i, string cmd, int change) {
  ...
  if (latex_type (cmd) == "undefined") return parse_unknown (s, i, cmd, change);
  ...
  bool mbox_flag= (starts (cmd, "\\text") || cmd == "\\mbox") &&
                  (command_type["!mode"] == "math");
  ...
  if (delimdef) {
    ...
    while (i < n && arity >= 0 && (arity > 0 || option)) {
      ...
      if (option && (is_opening_option (s[j]) ||
        ...
        while ((j + 3 <= n) && is_numeric (s[j + 1]) && (s[j + 2] == '#'))
          j+= 2;
        if (j + 2 <= n) {
          t << s (j + 1, j + 2);
          u << s (j + 1, j + 2);
  ...
  if ((pic && latex_type (cmd) == "replace") ||
      latex_type (cmd) == "begin-end!" || latex_type (cmd) == "defined-env!" ||
    ...
    else if (N (body) > 0 && latex_type (cmd) == "begin-end!" && is_tuple (t) &&
             N (t) == 2)
...
latex_parser::parse_length (string s, int& i, int e) {
  ...
  tree r= tree (CONCAT);
  while (i < n) {
    if (is_numeric (s[i]) || s[i] == '.' || s[i] == '-') r << s (i, i + 1);
    else if (read (s, i, "plus") || read (s, i, "\\@plus")) {
      ...
      if (is_tuple (next, "\\tex-len", 3)) {
        ...
        return tuple ("\\tex-len", r, next[2], next[1]);
             ...
             (is_numeric (r[N (r) - 1]->label) || r[N (r) - 1] == "." ||
              r[N (r) - 1] == "-")) {
...
accented_to_Cork (tree t) {
  ...
  tree r (t, n);
  for (i= 0; i < n; i++)
    r[i]= accented_to_Cork (t[i]);
  if (is_compound (t[0])) return r;

  string s= t[0]->label;
  if ((N (s) == 2) && (s[0] == '\\') && (n == 2) && is_atomic (r[1]) &&
      (N (r[1]->label) <= 2)) {
    string v= r[1]->label;
    if (N (v) == 0) {
      if (s[1] == '`') {
...
skip_expansion (url u) {
  ...
  return s == "aaai1.sty" || s == "aixi.sty" || s == "algorithm.sty" ||
         s == "algorithm2e.sty" || s == "algorithmic.sty" ||
         s == "amssymb.sty" || s == "aopmath.sty" || s == "boxedeps.sty" ||
         s == "braids.sty" || s == "bux2ref.sty" || s == "buxmath.sty" ||
         s == "buxlayout.sty" || s == "chicagob.sty" || s == "chicagor.sty" ||
         s == "cleveref.sty" || s == "courier.sty" || s == "dfadobe.sty" ||
         s == "eepic.sty" || s == "fancyhdr.sty" || s == "geompsfi.sty" ||
         s == "helvet.sty" || s == "hyperref.sty" || s == "IEEEtrantools.sty" ||
         s == "jair.sty" || s == "jmlr2e.sty" || s == "latexsym.sty" ||
         s == "microtype.sty" || s == "natbib.sty" || s == "psfig.tex" ||
         s == "soul.sty" || s == "tcolorbox.sty" || s == "theapa.sty" ||
         s == "tikz.sty" || s == "times.sty";
}
    ...
    if (s[i] == '\n' || (s[i] == '\\' && test (s, i, "\\nextbib"))) {
      ...
      if (test (s, i, "%%%%%%%%%% Start TeXmacs macros\n")) {
        ...
        start= i;
               ...
               (count == 0 &&
                (test_env (s, i, "document") || test_env (s, i, "abstract") ||
                 test_macro (s, i, "\\part") ||
      ...
      else if (test_macro (s, i, "\\input") || test_macro (s, i, "\\include") ||
        ...
        while (i < N (s) && s[i] != '}')
          i++;
        array<string> names= trim_spaces (tokenize (s (start_name, i), ","));
...
get_latex_language (string s) {
  ...
  for (int i= 0; i < N (langs); i++)
    if (test (r, 0, as_string (langs[i]))) return as_string (langs[i]);
...
get_latex_encoding (string s) {
  ...
  if (occurs ("TeXmacs macros", s) && occurs ("\\catcode", s)) return "Cork";
...
parse_latex (string s, bool change, bool as_pic) {
  ...
  if (encoding != "Cork") s= convert (s, "LaTeX", "UTF-8");
