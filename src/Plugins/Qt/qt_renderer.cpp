...
qt_renderer_rep::get_extents (int& w2, int& h2) {
  if (painter->device ()) {
    w2= painter->device ()->width ();
    h2= painter->device ()->height ();
  }
  else {
    w2= w;
    h2= h;
...
qt_renderer_rep::lines (array<SI> x, array<SI> y) {
  ...
  {
    double px= double (pnt[i].rx ()); // explicit conversion needed for ARM
                                      // where qreal==float!=double
...
qt_renderer_rep::arc (SI x1, SI y1, SI x2, SI y2, int alpha, int delta) {
  ...
  decode (x2, y2, rx2, ry2);
  painter->setRenderHints (QPainter::Antialiasing);
  painter->drawArc (QRectF (rx1, ry2, rx2 - rx1, ry1 - ry2), alpha / 4,
                    delta / 4);
}
...
qt_renderer_rep::draw (int glyph_index, font_glyphs fng, SI x, SI y,
  ...
  //<< cx2/256 << ","  << cy2/256 << LF;
  draw_clipped (mi->img, mi->w, mi->h, x - mi->xo * std_shrinkf,
                y + mi->yo * std_shrinkf);
}
 ...
 * Instead we maintain an internal pixmap which represents the state of the
 * pixels according to texmacs. When we are asked to initialize a
 * qt_shadow_renderer_rep we simply read the pixels form this backing store. At
 * the Qt level then (in QTMWidget) we make sure that the state of the backing
 * store is in sync with the screen via paintEvent/repaint mechanism.
 *
 */
...
qt_renderer_rep::put_shadow (renderer ren, SI x1, SI y1, SI x2, SI y2) {
  ...
  if (x1 < x2 && y2 < y1) {
    ...
    else painter->drawPixmap (rect, *(shadow->px.QPixmap_ptr ()), rect);
...
qt_proxy_renderer_rep::get_shadow (renderer ren, SI x1, SI y1, SI x2, SI y2) {
  ...
  if (x1 < x2 && y2 < y1) {
    ...
    if (headless_mode) {
      ...
      if (_image) shadow->painter->drawImage (rect, *_image, rect);
    }
    else {
      QPixmap* _pixmap= static_cast<QPixmap*> (painter->device ());
      if (_pixmap) shadow->painter->drawPixmap (rect, *_pixmap, rect);
    }
    //    cout << "qt_shadow_renderer_rep::get_shadow "
    //         << rectangle(x1,y2,x2,y1) << LF;
    //  XCopyArea (dpy, win, shadow->win, gc, x1, y2, x2-x1, y1-y2, x1, y2);
...
qt_shadow_renderer_rep::get_shadow (renderer ren, SI x1, SI y1, SI x2, SI y2) {
  ...
  if (x1 < x2 && y2 < y1) {
    ...
    else shadow->painter->drawPixmap (rect, *(px.QPixmap_ptr ()), rect);
    //    cout << "qt_shadow_renderer_rep::get_shadow "
    //         << rectangle(x1,y2,x2,y1) << LF;
    //  XCopyArea (dpy, win, shadow->win, gc, x1, y2, x2-x1, y1-y2, x1, y2);
  }
