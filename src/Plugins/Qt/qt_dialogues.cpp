 ...
 * DESCRIPTION: Widgets for automatically created dialogues (questions in
 *popups) COPYRIGHT  : (C) 2008  Massimiliano Gubinelli
 *******************************************************************************
...
public:
  QTMFilterHack (ButtonList _buttons)
      : buttons (_buttons), current (1), N (_buttons.size ()) {}
  bool eventFilter (QObject* target, QEvent* event) {
    if (event->type () == QEvent::KeyPress) {
      ...
      if (keyEvent->key () == Qt::Key_Tab) {
        ...
        else current= current + 1 >= N ? 0 : current + 1;
...
qt_inputs_list_widget_rep::qt_inputs_list_widget_rep (command       _cmd,
  ...
  for (int i= 0; i < N (_prompts); i++)
    add_child (tm_new<qt_field_widget_rep> ((qt_inputs_list_widget_rep*) this,
                                            _prompts[i]));
}
  ...
  if ((N (children) == 1) && (field (0)->type == "question" ||
    ...
    if (choices > 0) {
      for (int i= 0; i < choices; ++i) {
        ...
        buttonlist[i]=
            msgBox.addButton (to_qstring (blabel), QMessageBox::ActionRole);
      }
...
qt_input_text_widget_rep::qt_input_text_widget_rep (command _cmd, string _type,
...
qt_input_text_widget_rep::as_qwidget () {
  ...
  bool can_autocommit=
      !(ends (type, "search") || ends (type, "replace") ||
        ends (type, "replace") || starts (type, "interactive"));
  ...
  if (ends (type, "file") || type == "directory") {
    ...
    fsModel->setRootPath (
        QDir::homePath ()); // This is NOT the starting location
    completer->setModel (fsModel);
    le->setCompleter (completer);
  }
  else if (type != "password" && N (proposals) > 0 &&
           !(N (proposals) == 1 && N (proposals[0]) == 0)) {
    // else if (N(proposals) > 0 && ! (N(proposals) == 1 && N(proposals[0]) ==
    // 0)){
