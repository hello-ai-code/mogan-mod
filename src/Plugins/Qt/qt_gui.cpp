...
qt_gui_rep::show_wait_indicator (widget w, string message, string arg) {
  ...
  if (waitDialogs.count ()) {
    ...
    waitWindow->updateGeometry ();
    {
      ...
      // HACK:
      //  processEvents is needed to let Qt update windows coordinates in the
      //  case
...
qt_gui_rep::process_queued_events (int max) {
  ...
  while (max < 0 || count < max) {
...
#ifdef QT_CPU_FIX
    if (ev.x1 != qp_type::QP_NULL && ev.x1 != qp_type::QP_SOCKET_NOTIFICATION &&
    ...
    case qp_type::QP_MOUSE: {
      ...
      T x= open_box<T> (ev.x2);
      if (!is_nil (x.x1))
        concrete_simple_widget (x.x1)->handle_mouse (x.x2.x1, x.x2.x2, x.x2.x3,
                                                     x.x2.x4, x.x2.x5, x.x2.x6);
    } break;
...
qt_gui_rep::process_keypress (qt_simple_widget_rep* wid, string key, time_t t) {
  typedef triple<widget, string, time_t> T;
  add_event (
      queued_event (qp_type::QP_KEYPRESS, close_box<T> (T (wid, key, t))));
}
...
qt_gui_rep::process_command (command _cmd, object _args) {
  typedef pair<command, object> T;
  add_event (
      queued_event (qp_type::QP_COMMAND_ARGS, close_box<T> (T (_cmd, _args))));
}
...
bool
set_selection (string key, tree t, string s, string sv, string sh,
               string format) {
...
qt_gui_rep::put_graphics_on_clipboard (url file) {
  ...
  if ((extension == "bmp") || (extension == "png") || (extension == "jpg") ||
    ...
    if (extension == "svg")
      mime= "image/svg+xml"; // this works with Inskcape version >= 0.47
...
queued_event
event_queue::next () {
  if (is_nil (q)) return queued_event ();
