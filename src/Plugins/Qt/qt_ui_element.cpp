...
/*! Ad-hoc command to be used with toggle widgets.
 ...
 The command associated with a qt_ui_element::toggle_widget has as a parameter
 the state of the QCheckBox. Since it is assumed everywhere else that commands
 injected into the gui's queue accept no parameters, and changes would be too
 big, we choose to encapsulate the original command in a new one which will
 execute the first with its argument. \sa qt_ui_element, ,
 qt_ui_element_rep::as_qwidget, qt_ui_element_rep::toggle_widget
 */
...
/*! Ad-hoc command to be used with enum widgets.
 ...
 The command associated with a qt_ui_element::enum_widget has one parameter. For
 the reason to be of this class, see \sa qt_toggle_command_rep . \sa
 qt_ui_element, , qt_ui_element_rep::as_qwidget, qt_ui_element_rep::enum_widget
 */
...
public:
  ...
  void apply () {
    if (qwid) cmd (list_object (object (from_qstring (qwid->currentText ()))));
  }
...
qt_glue_widget_rep::as_qaction () {
  ...
  col = old_col;
#else
  if (!headless_mode) icon.addPixmap (*(render ().QPixmap_ptr ()));
#endif
  a->setIcon (icon);
  a->setEnabled (false);
  return a;
...
qt_glue_widget_rep::as_qwidget () {
  ...
  if (!headless_mode) qw->setPixmap (*(render ().QPixmap_ptr ()));
...
qt_ui_element_rep::make_popup_widget () {
  if (type == qt_widget_rep::vertical_menu) return tm_new<qt_menu_rep> (this);
  else return qt_widget_rep::make_popup_widget ();
}
...
qt_ui_element_rep::as_qaction () {
  ...
  case aligned_widget:
    //  a table with two columns FIXME!!!!
    {
      ...
      if (N (lhs) != N (rhs)) TM_FAILED ("aligned_widget: N(lhs) != N(rhs) ");
...
qt_ui_element_rep::as_qlayoutitem () {
  ...
  case vertical_list: {
    ...
    else l->setContentsMargins (0, 0, 0, 0);
  ...
  case aligned_widget:
    //  a table with two columns (FIXME!)
    {
      ...
      for (int i= 0; i < N (lhs); i++) {
        ...
        if (lli)
          l->addItem (lli, i, 0, 1, 1, Qt::AlignRight | Qt::AlignVCenter);
  ...
  case menu_separator: {
    ...
    QSizePolicy::Policy hpolicy=
        vertical ? QSizePolicy::Fixed : QSizePolicy::Preferred;
  ...
  case glue_widget:
    // Used only for non-colored glue widgets (skips qt_glue_widget_rep)
    {
      ...
      QSize               sz= QSize (w, h);
      QSizePolicy::Policy hpolicy=
          x.x1 ? QSizePolicy::MinimumExpanding : QSizePolicy::Minimum;
      QSizePolicy::Policy vpolicy=
          x.x2 ? QSizePolicy::MinimumExpanding : QSizePolicy::Minimum;
...
qt_ui_element_rep::as_qwidget () {
  ...
  case resize_widget: {
    ...
    QSize minSize= qt_decode_length (
        widths.x1, heights.x1, qwid->minimumSizeHint (), qwid->fontMetrics ());
    QSize defSize= qt_decode_length (
        widths.x2, heights.x2, qwid->minimumSizeHint (), qwid->fontMetrics ());
    QSize maxSize= qt_decode_length (
        widths.x3, heights.x3, qwid->minimumSizeHint (), qwid->fontMetrics ());
  ...
  case enum_widget: {
    ...
    w->setEditable (value.isEmpty () ||
                    values.last ().isEmpty ()); // weird convention?!
    if (values.last ().isEmpty ()) values.removeLast ();

    w->addItemsAndResize (values, x.x5, "");
    int index=
        w->findText (value, Qt::MatchFixedString | Qt::MatchCaseSensitive);
    if (index != -1) w->setCurrentIndex (index);
    ...
    QObject::connect (w, SIGNAL (currentIndexChanged (int)), c,
                      SLOT (apply ()));
  ...
  case filtered_choice_widget: {
    ...
    T            x     = open_box<T> (load);
    string       filter= x.x4;
    QTMListView* choiceWidget=
        new QTMListView (x.x1, to_qstringlist (x.x2),
                         QStringList (to_qstring (x.x3)), false, true, true);
  ...
  case scrollable_widget: {
    ...
    //  you show() the scroll area. In this case, you can also not show() the
    //  widget later."
    qwid= w;
