...
QTMAction::QTMAction (QObject* parent) : QAction (parent) {
  QObject::connect (the_gui->gui_helper, SIGNAL (refresh ()), this,
                    SLOT (doRefresh ()));
...
QTMTileAction::createWidget (QWidget* parent) {
  if (DEBUG_QT_WIDGETS) debug_widgets << "QTMTileAction::createWidget\n";
  ...
  for (int i= 0; i < actions.count (); i++) {
    ...
    if (tm_style_sheet == "") tb->setStyle (qtmstyle ());
  ...
  for (int i= 0; i < actions.count (); i++) {
    ...
    if (QWidgetAction* wa= qobject_cast<QWidgetAction*> (sa)) {
      ...
      if (use_mini_bars) {
        QFont f= tb->font ();
        int   fs=
            as_int (get_preference ("gui:mini-fontsize", QTM_MINI_FONTSIZE));
...
QTMLazyMenu::attachTo (QAction* a) {
  QObject::connect (a, SIGNAL (destroyed (QObject*)), this,
                    SLOT (destroy (QObject*)));
  a->setMenu (this);
}
...
QTMInputTextWidgetHelper::QTMInputTextWidgetHelper (qt_widget _wid, bool _cac)
...
QTMLineEdit::QTMLineEdit (QWidget* parent, string _type, string _ww, int style,
                          command _cmd)
    : QLineEdit (parent), completing (false), type ("default"),
      name ("default"), serial ("default"), ww (_ww), cmd (_cmd), last_key (0) {
  set_type (_type);
...
QTMLineEdit::continuous () {
  return starts (type, "search") || starts (type, "replace-") ||
         starts (type, "spell") || starts (serial, "form-");
}
...
QTMLineEdit::keyPressEvent (QKeyEvent* ev) {
  ...
  if (continuous ()) {
    ...
    if ((last_key != Qt::Key_Tab || type == "replace-what") &&
        ...
        last_key != Qt::Key_Down && last_key != Qt::Key_Up &&
        last_key != Qt::Key_Enter && last_key != Qt::Key_Return &&
    ...
    case Qt::Key_Tab: {
      ...
      if (completing) {
        ...
        if (hasSelectedText ()) setCursorPosition (selectionStart ());
        ...
        QString prefix;
        if (hasSelectedText ()) prefix= text ().left (selectionStart ());
        else prefix= text ().left (cursorPosition ());
    ...
    case Qt::Key_Backtab: {
      if (!completing) {
        ...
        else c->setCompletionPrefix (text ().left (cursorPosition ()));
        // If there are no completions, go to the end of the line or
        // send the key up for tab navigation
        if (c->completionCount () == 0 ||
            (c->completionCount () == 1 &&
        ...
        if (c->currentCompletion () != "") {
          int pos;
          if (hasSelectedText ()) pos= selectionStart ();
          else pos= cursorPosition ();
          setText (c->currentCompletion ());
          setSelection (pos, text ().length ());
        }
      }
...
QTMTabWidget::QTMTabWidget (QWidget* p) : QTabWidget (p) {
  QObject::connect (this, SIGNAL (currentChanged (int)), this,
                    SLOT (resizeOthers (int)));
}
 ...
 */
void
QTMComboBox::addItemsAndResize (const QStringList& texts, string ww,
                                string hh) {
...
QTMComboBox::event (QEvent* ev) {
  ...
  if (ev->type () == QEvent::KeyPress && isEditable ()) { // Handle ALL keys
    ...
    if (k->key () == Qt::Key_Up || k->key () == Qt::Key_Down) showPopup ();
...
QTMListView::QTMListView (const command& cmd, const QStringList& strings,
                          const QStringList& selections, bool multiple,
                          bool scroll, bool filtered, QWidget* parent)
    : QListView (parent) {
...
/*! Reimplemented from QListView.
 ...
 We simply emit another signal, mainly to notify QTMScrollArea that we have a
 new selection for those cases were we don't have our own scrollbars.
 */
void
QTMListView::selectionChanged (const QItemSelection& c,
...
QTMTreeView::QTMTreeView (command cmd, tree data, const tree& roles, QWidget* p)
  ...
  QObject::connect (this, SIGNAL (pressed (const QModelIndex&)), this,
                    SLOT (callOnChange (const QModelIndex&)));
}

void
QTMTreeView::currentChanged (const QModelIndex& curr, const QModelIndex& prev) {
  (void) prev;
  if (selectedIndexes ().contains (curr)) callOnChange (curr, false);
}
