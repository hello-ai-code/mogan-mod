...
int
tex_font_metric_rep::h (QN c) {
  if ((c < bc) || (c > ec)) return 0;
  return height[byte1a (char_info[c - bc])];
}
int
tex_font_metric_rep::d (QN c) {
  if ((c < bc) || (c > ec)) return 0;
  return depth[byte1b (char_info[c - bc])];
}
int
tex_font_metric_rep::i (QN c) {
  if ((c < bc) || (c > ec)) return 0;
  return italic[byte2x (char_info[c - bc])];
}
int
tex_font_metric_rep::tag (QN c) {
  return (char_info[c - bc] >> 8) & 3;
}
int
tex_font_metric_rep::rem (QN c) {
...
int
tex_font_metric_rep::design_size () {
  return header[1];
}
int
tex_font_metric_rep::parameter (int i) {
  return (i < np) ? param[i] : 0;
}
int
tex_font_metric_rep::spc () {
  return parameter (1);
}
int
tex_font_metric_rep::spc_stretch () {
  return parameter (2);
}
int
tex_font_metric_rep::spc_shrink () {
  return parameter (3);
}
int
tex_font_metric_rep::x_height () {
  return parameter (4);
}
int
tex_font_metric_rep::spc_quad () {
  return parameter (5);
}
int
tex_font_metric_rep::spc_extra () {
  return parameter (6);
}
...
tex_font_metric_rep::execute (int* s, int n, int* buf, int* ker, int& m) {
  ...
  while (sp >= 0) {
    ...
    if ((bp >= m - 2) || (sp >= m - 2)) {
      failed_error << "\nString is ";
      for (i= 0; i < n; i++)
        failed_error << (char) s[i];
      failed_error << "\n";
      TM_FAILED ("string too complex for ligature kerning");
    }
  }
...
print (tex_font_metric tfm) {
  ...
  for (i= tfm->bc; i <= tfm->ec; i++) {
    ...
    case 2:
      ...
      break;
    case 3:
      cout << " [ext " << (int) tfm->top (i) << ", " << (int) tfm->mid (i)
           << ", " << (int) tfm->bot (i) << ", " << (int) tfm->rep (i) << "]";
      break;
    }
    cout << "\n";
  }
  ...
  if ((tfm->left == -1) && (tfm->right == -1) && (tfm->left_prog == -1) &&
      (tfm->right_prog == -1))
...
load_tfm (url file_name, string family, int size) {
  ...
  if ((tfm->lf - 6) !=
      (tfm->lh + (tfm->ec + 1 - tfm->bc) + tfm->nw + tfm->nh + tfm->nd +
       tfm->ni + tfm->nl + tfm->nk + tfm->ne + tfm->np))
    TM_FAILED ("invalid tfm file");
  ...
  if (starts (family, "helvetica-ti") || starts (family, "helvetica-bi"))
    tfm->param[0]= (int) (0.212557 * ((double) (1 << 20)));
  if (starts (family, "nc-schoolbook-ti") ||
      starts (family, "nc-schoolbook-bi"))
