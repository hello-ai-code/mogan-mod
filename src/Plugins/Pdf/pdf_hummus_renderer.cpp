...
pdf_hummus_renderer_rep::begin_page () {
  ...
  if (NULL == contentContext) {
    ...
    contentContext->cm ((double) default_dpi / dpi, 0, 0,
...
pdf_hummus_renderer_rep::end_page () {
  ...
  EStatusCodeAndObjectIDType res=
      pdfWriter.GetDocumentContext ().WritePageAndRelease (page);
  status= res.first;
  if (status != PDFHummus::eSuccess) {
...
void
pdf_hummus_renderer_rep::set_clipping (SI x1, SI y1, SI x2, SI y2,
                                       bool restore) {
...
public:
  ...
  pdf_pattern_rep (pdf_image _im, SI _w, SI _h, SI _sx, SI _sy, double _scale_x,
                   double _scale_y, ObjectIDType _id)
      : im (_im), w (_w), h (_h), sx (_sx), sy (_sy), scale_x (_scale_x),
  ...
  void flush (PDFWriter& pdfw) {
    ...
    if (st != PDFHummus::eSuccess)
      convert_error << "Cannot flush tiled pattern " << im->u << "\n";
  }
};
...
pdf_hummus_renderer_rep::register_pattern_image (brush br, SI pixel) {
  ...
  pdf_pattern p_pdf (image_pdf, w, h, width + to_x (0), height, // FIXME ???
...
pdf_hummus_renderer_rep::select_alpha (int a) {
  if (!alpha_id->contains (a)) {
    ObjectIDType temp= pdfWriter.GetObjectsContext ()
                           .GetInDirectObjectsRegistry ()
                           .AllocateNewObjectID ();
    alpha_id (a)= temp;
  }
  std::string name=
      page->GetResourcesDictionary ().AddExtGStateMapping (alpha_id (a));
...
pdf_hummus_renderer_rep::set_pencil (pencil pen2) {
  ...
  if (pen->get_cap () == cap_round) contentContext->J (1); // round cap
...
void
pdf_hummus_renderer_rep::draw_bitmap_glyph (int ch, font_glyphs fn, SI x,
                                            SI y) {
  (void) x;
  (void) y;
  ...
  if (!pdf_glyphs->contains (char_name)) {
    glyph gl= fn->get (ch);
    // debug_convert << "draw bitmap glyph " << (double)gl->width / 8 << " " <<
    // (double)gl->height / 8 << "\n";
    if (is_nil (gl)) return;
    string       buf           = load_virtual_glyph (gl);
    ObjectIDType imageXObjectID= pdfWriter.GetObjectsContext ()
                                     .GetInDirectObjectsRegistry ()
                                     .AllocateNewObjectID ();
    pdf_glyphs (char_name)=
        pdf_raw_image (buf, gl->width, gl->height, imageXObjectID);
  }
}
...
t3font_get_local_glyph (int global_glyph, int font_chunk, string font_name) {
  ...
  if (chunk != font_chunk)
    convert_error << "pdf_hummus_renderer, in t3font support, glyph "
                  ...
                  << as_string (font_chunk) << " of font " << font_name << LF;
  if (global_glyph <= 255) return global_glyph;
...
t3font_get_global_glyph (int local_glyph, int font_chunk, string font_name) {
  ...
  if (font_chunk == 0) return local_glyph;
...
t3font_rep::write_char (glyph gl, ObjectIDType inCharID) {
  ...
  if (is_nil (gl)) {
    ...
    data << as_string (llx) << " " << as_string (lly) << " " << as_string (urx)
         << " " << as_string (ury) << " d1\r\n";
    data << "q\r\n";
    data << as_string ((double) (cwidth)) << " 0 0 "
         << as_string ((double) (cheight)) << " " << as_string ((double) (llx))
...
t3font_rep::write_definition (int& registry_id) {
  ...
  while (it->busy ())
    glyph_list << t3font_get_local_glyph (it->next (), font_chunk,
                                          fn->res_name);
  ...
  for (int i= 0; i < N (glyph_list); ++i) {
    int   ch= t3font_get_global_glyph (glyph_list[i], font_chunk, fn->res_name);
    glyph gl= fn->get (ch);
    ObjectIDType temp=
  ...
  dict << "\t/Subtype /Type3\r\n\t/FontBBox [ " << as_string (b0) << " "
       << as_string (b1) << " " << as_string (b2) << " " << as_string (b3)
       << "]\r\n";
  ...
  for (int i= 0; i < N (glyph_list); ++i) {
    ...
    dict << "\t\t/ch" << as_string (ch) << " " << as_string (temp)
         << " 0 R\r\n";
  }
  if (N (glyph_list) > 0)
  ...
  cmap << "1 begincodespacerange <" << as_hexadecimal (firstchar, 2) << "> <"
       << as_hexadecimal (lastchar, 2) << "> endcodespacerange\r\n";
#if 1
  cmap << "1 beginbfrange\r\n";
  ...
  cmap << as_string (N (glyph_list)) << " beginbfchar\r\n";
  for (int i= 0; i < N (glyph_list); i++) {
    int lch= glyph_list[i];
    int ch = t3font_get_global_glyph (lch, font_chunk, fn->res_name);
    cmap << "\t<" << as_hexadecimal (lch, 2) << "> <" << as_hexadecimal (ch, 4)
         << ">\r\n";
  }
  cmap << "endbfchar\r\n";
#endif
  ...
  if (exists (u) && !load_string (u, s, false)) {
    ...
    for (int i= 0; i < N (t); i++)
      if (is_func (t[i], TUPLE) && (N (t[i]) >= 1)) {
        tree key= t[i][0];
        if (is_string (key)) set->insert (as_string (key));
      }
  }
  loaded= true;
  return set;
...
pdf_hummus_renderer_rep::make_pdf_font (string fontname) {
  ...
  if (!is_none (u)) {
    ...
    if (font != NULL && no_font_issues (u)) {
      ...
      convert_warning << "pdf_hummus_renderer, font: " << fname << " in file "
                      << u << " cannot be loaded. "
...
match_font_base_name (string fontname, string basename) {
  if (!starts (fontname, basename)) return false;
  int i= N (basename);
  while (i < N (fontname)) {
    if (fontname[i] == ':') break;
    if (fontname[i] < '0' || fontname[i] > '9') return false;
    i++;
  }
  return true;
}
...
requires_hack_notdef_for_tex_font (string fontname) {
  ...
  if (remember->contains (fontname)) return remember[fontname];
  bool r= match_font_base_name (fontname, "cmbsy") ||
...
pdf_hummus_renderer_rep::draw (int glyph_index, font_glyphs fn, SI x, SI y,
  ...
  else cfid= native_fonts (fontname);
  begin_text ();
...
void
pdf_hummus_renderer_rep::bezier_arc (SI x1, SI y1, SI x2, SI y2, int alpha,
  ...
  // Control points for an arc of radius 1, centered on the x-axis and spanning
  // phi degrees from: http://www.tinaja.com/glib/bezcirc2.pdf
  ...
  for (k= 0; k < i; k++) {
    ...
    if (k == 0)
      contentContext->m (rx * (bx0 * cphi - by0 * sphi),
                         ry * (+bx0 * sphi + by0 * cphi)); // start point
    contentContext->c (
  ...
  // paint
  if (filled) {
    // contentContext->l(0.0, 0.0); // for a filled "pie" with vertex at the
    // center
    contentContext->f ();
  }
  else
    ((abs (delta) == 360 * 64) ? contentContext->s () : contentContext->S ());
  // here we close the path if it's a full circle
  contentContext->Q (); // restore the graphics state (undoes centering only)
}

void
pdf_hummus_renderer_rep::arc (SI x1, SI y1, SI x2, SI y2, int alpha,
                              int delta) {
  // debug_convert << "arc\n";
  end_text ();
...
void
pdf_hummus_renderer_rep::fill_arc (SI x1, SI y1, SI x2, SI y2, int alpha,
                                   int delta) {
...
void
pdf_hummus_renderer_rep::image (url u, double w, double h, SI x, SI y,
                                int alpha) {
  ...
  if (is_nil (im)) {
    im                 = pdf_image (u, pdfWriter.GetObjectsContext ()
                                           .GetInDirectObjectsRegistry ()
                                           .AllocateNewObjectID ());
  ...
  contentContext->q ();
  std::string initial_GState_name=
      page->GetResourcesDictionary ().AddExtGStateMapping (initial_GState_id);
  contentContext->gs (initial_GState_name);
  contentContext->cm (((double) w) / ((double) im->w), 0, 0,
                      ((double) h) / ((double) im->h), to_x (x), to_y (y));
  std::string pdfFormName=
      page->GetResourcesDictionary ().AddFormXObjectMapping (im->id);
...
pdf_hummus_renderer_rep::draw_scalable (scalable im, SI x, SI y, int alpha) {
  ...
  if (im->get_type () != scalable_image ||
    ...
    int       ox= r->x1, oy= r->y1;
    image (u, ((double) w) / pixel, ((double) h) / pixel, x - ox, y - oy,
           alpha);
  }
}
...
int
pdf_hummus_renderer_rep::get_label_id (string label) {
...
pdf_hummus_renderer_rep::href (string label, SI x1, SI y1, SI x2, SI y2) {
  ...
  ObjectIDType annotId = pdfWriter.GetObjectsContext ()
                            .GetInDirectObjectsRegistry ()
                            .AllocateNewObjectID ();
  ...
  if (preserve) dict << "\t/Border [16 16 1 [3 10]] /Color [0.75 0.5 1.0]\r\n";
  else dict << "\t/Border [16 16 0 [3 10]] /Color [0.75 0.5 1.0]\r\n";
  dict << "\t/Rect ["
       << as_string (((double) default_dpi / dpi) * to_x (x1 - 5 * pixel))
       << " ";
...
pdf_hummus_renderer_rep::flush_dests () {
  ...
  while (!is_nil (it)) {
    ...
    {
      dict << "\t\t/label" << as_string (get_label_id (label)) << " [ "
           << as_string (page_id (dest_page)) << " 0 R /XYZ "
...
pdf_hummus_renderer_rep::toc_entry (string kind, string title, SI x, SI y) {
  ...
  outlines << outline_data (escape_string (title), page_num, to_x (x),
                            to_y (y + 20 * pixel), ls);
}
...
pdf_hummus_renderer_rep::recurse (ObjectsContext& objectsContext, int parent_ls,
  ...
  while (curId) {
    ...
    if (!is_nil (it) && ((it->item).x5 > oitem.x5)) {
      // go down in level
      recurse (objectsContext, oitem.x5, it, curId, subFirstId, subLastId,
               subCount);
    }

    // continue at this level or above
    if (!is_nil (it) && ((it->item).x5 <= oitem.x5) &&
        ((it->item).x5 > parent_ls)) {
      nextId=
          objectsContext.GetInDirectObjectsRegistry ().AllocateNewObjectID ();
    }
    else {
...

    {
      ...
      dict << "\t/Dest [ " << as_string (page_id ((oitem).x2)) << " 0 R /XYZ "
           ...
           << as_string (((double) default_dpi / dpi) * ((oitem).x4))
           << " null ]\r\n"
           << ">>\r\n";
...
void
pdf_hummus_renderer_rep::fetch (SI x1, SI y1, SI x2, SI y2, renderer ren, SI x,
                                SI y) {
  // debug_convert << "fetch\n";
  (void) x1;
  (void) y1;
...
renderer
pdf_hummus_renderer (url pdf_file_name, int dpi, int nr_pages, string page_type,
  ...
  return tm_new<pdf_hummus_renderer_rep> (
