...
edit_table_rep::search_row (path fp, int row) {
  ...
  tree st= subtree (et, fp);
  if (!is_func (st, ROW)) return search_row (fp, N (st) - 1);
  return fp;
}
...
edit_table_rep::search_cell (path p, int col) {
  ...
  if (is_func (st, TFORMAT)) return search_cell (p, N (st) - 1);
...
void
edit_table_rep::with_decode (int nr_rows, int nr_cols, int& i1, int& j1,
                             int& i2, int& j2) {
...
void
edit_table_rep::with_decode (int nr_rows, int nr_cols, int& I1, int& J1,
                             int& I2, int& J2, int& i1, int& j1, int& i2,
                             int& j2) {
  i1= I1;
  j1= J1;
  i2= I2;
...
void
edit_table_rep::with_read (tree with, int nr_rows, int nr_cols, int& i1,
...
void
edit_table_rep::with_read (tree with, int nr_rows, int nr_cols, int& I1,
                           int& J1, int& I2, int& J2, int& i1, int& j1, int& i2,
                           int& j2) {
  with_raw_read (with, I1, J1, I2, J2);
  i1= I1;
  j1= J1;
...
edit_table_rep::table_get_format (path fp, string var) {
  ...
  for (k= 0; k < n; k++)
    if (is_func (st[k], TWITH, 2) && (st[k][0] == var)) val= st[k][1];
  return val;
}
...
edit_table_rep::table_del_format (path fp, string var) {
  ...
  for (k= n - 2; k >= 0; k--)
    if (is_func (st[k], TWITH, 2))
      if ((var == "") || (var == st[k][0])) remove (fp * k, 1);
}

void
edit_table_rep::table_set_format (path fp, int I1, int J1, int I2, int J2,
                                  string var, tree val) {
...
tree
edit_table_rep::table_get_format (path fp, int I1, int J1, int I2, int J2,
                                  string var) {
  int  i1, j1, i2, j2;
  int  nr_rows, nr_cols;
  tree st= table_get_format (fp);
...
void
edit_table_rep::table_del_format (path fp, int I1, int J1, int I2, int J2,
                                  string var) {
  int  i1, j1, i2, j2;
  int  nr_rows, nr_cols;
  tree st= subtree (et, fp);
...
void
edit_table_rep::table_get_format (path fp, string var, tree** val, int nr_rows,
                                  int nr_cols) {
  int  i, j;
  ...
  for (k= n - 2; k >= 0; k--)
    if (is_func (st[k], CWITH, 6))
      if ((var == "") || (var == st[k][4])) {
        ...
        row1= max (row1, 0);
        row2= min (row2, nr_rows - 1);
        col1= max (col1, 0);
        col2= min (col2, nr_cols - 1);
...
edit_table_rep::table_set_extents (path fp, int nr_rows, int nr_cols) {
  ...
  if (nr_rows > old_rows || nr_cols > old_cols)
    table_insert (fp, old_rows, old_cols, max (0, nr_rows - old_rows),
                  max (0, nr_cols - old_cols));
  if (nr_rows < old_rows || nr_cols < old_cols)
    table_remove (fp, nr_rows, nr_cols, max (0, old_rows - nr_rows),
                  max (0, old_cols - nr_cols));
}

void
edit_table_rep::table_get_limits (path fp, int& i1, int& j1, int& i2, int& j2) {
  ...
  for (k= n - 2; k >= 0; k--)
    if (is_func (st[k], CWITH, 6)) {
      ...
      if (delr > 0) {
        ...
        if ((I1 < 0) && (i1 < row + delr))
          assign (fp * path (k, 0),
                  as_string (I1 + delr + min (0, row - 1 - i1)));
        if ((I2 > 0) && (i2 >= row))
          assign (fp * path (k, 1), as_string (I2 - min (delr, i2 - row)));
        if ((I2 < 0) && (i2 < row + delr))
          assign (fp * path (k, 1),
                  as_string (I2 + delr + min (0, row - 1 - i2)));
      }
      if (delc > 0) {
        if ((col <= j1) && (j2 < col + delc)) {
          remove (fp * k, 1);
          continue;
        }
        if ((J1 > 0) && (j1 >= col))
        ...
        if ((J1 < 0) && (j1 < col + delc))
          assign (fp * path (k, 2),
                  as_string (J1 + delc + min (0, col - 1 - j1)));
        if ((J2 > 0) && (j2 >= col))
          assign (fp * path (k, 3), as_string (J2 - min (delc, j2 - col)));
        if ((J2 < 0) && (j2 < col + delc))
          assign (fp * path (k, 3),
                  as_string (J2 + delc + min (0, col - 1 - j2)));
      }
    }
}
...
void
edit_table_rep::table_bound (path fp, int& row1, int& col1, int& row2,
                             int& col2) {
  fp= search_format (fp);
...
edit_table_rep::table_go_to_border (path fp, bool right) {
  while ((rp < fp) && (is_func (subtree (et, path_up (fp)), TFORMAT)))
    fp= path_up (fp);
  if ((rp < fp) && is_document (subtree (et, path_up (fp))) &&
      (rp < path_up (fp)) && is_extension (subtree (et, path_up (fp, 2)), 1))
    fp= path_up (fp);
...
edit_table_rep::back_in_table (tree t, path p, bool forward) {
  ...
  if (forward) {
    if (col < (nr_cols - 1)) {
      table_go_to (p, row, col + 1, true);
      return;
    }
    if (row < (nr_rows - 1)) {
      table_go_to (p, row + 1, 0, true);
      return;
    }
  }
  else {
    if (col > 0) {
      table_go_to (p, row, col - 1, false);
      return;
    }
    if (row > 0) {
      table_go_to (p, row - 1, nr_cols - 1, false);
      return;
    }
  }
...
tree
edit_table_rep::table_get_subtable (path fp, int row1, int col1, int row2,
                                    int col2) {
...
void
edit_table_rep::table_write_subtable (path fp, int row, int col, tree subt) {
  ...
  tp            = old_tp;
  if (calc_flag)
    subt= as_tree (call ("calc-table-renumber", object (subt), object (row + 1),
                         object (col + 1)));

  if (is_func (subtree (et, fp), TFORMAT) && is_func (subt, TFORMAT)) {
  ...
  for (i= 0; i < sub_rows; i++) {
    ...
    tree subr= subt[i];
    while (is_func (subr, TFORMAT))
      subr= subr[N (subr) - 1];
    for (j= 0; j < sub_cols; j++) {
      path cp  = search_cell (rp, j + col);
      tree subc= subr[j];
      while (is_func (subc, TFORMAT))
        subc= subc[N (subr) - 1];
...
search_decoration (tree T, int& row, int& col) {
  ...
  for (row= 0; row < N (T); row++) {
    tree R= T[row];
    while (is_func (R, TFORMAT))
      R= R[N (R) - 1];
    for (col= 0; col < N (R); col++) {
      tree C= R[col];
      while (is_func (C, TFORMAT))
        C= C[N (C) - 1];
...
edit_table_rep::table_correct_block_content () {
  ...
  for (row= 0; row < nr_rows; row++)
    for (col= 0; col < nr_cols; col++) {
      ...
      tree t1=
          table_get_format (fp, row + 1, col + 1, row + 1, col + 1, CELL_BLOCK);
      tree t2= table_get_format (fp, row + 1, col + 1, row + 1, col + 1,
                                 CELL_HYPHEN);
      bool f1= (t1 == "no" || (t1 == "auto" && t2 == "n"));
      bool f2= (t1 == "yes" || (t1 == "auto" && is_atomic (t2) && t2 != "n"));
      if (f1 && is_document (st) && N (st) == 1) remove_node (cp * 0);
      else if (f2 && !is_document (st)) insert_node (cp * 0, DOCUMENT);
    }
}
...
edit_table_rep::cell_set_format (string var, tree val) {
  if (selection_active_table (false)) {
    ...
    col++;
    if (is_nil (fp)) return;
    if (cell_mode == "row") table_set_format (fp, row, 1, row, -1, var, val);
