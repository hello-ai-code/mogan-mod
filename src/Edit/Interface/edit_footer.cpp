...
edit_interface_rep::set_left_footer (tree l) {
  SERVER (
      set_left_footer (serialize (tree_translate (l, "english", "english"))));
}
...
edit_interface_rep::set_right_footer (tree r) {
  SERVER (
      set_right_footer (serialize (tree_translate (r, "english", "english"))));
}
  ...
  for (i= 0; i < k; i++)
    if (is_atomic (t[2 * i]) && (t[2 * i] != "") && is_atomic (t[2 * i + 1])) {
      ...
      if ((var == MODE) || (var == COLOR) || (var == PAR_MODE) ||
          (var == LANGUAGE) || (var == FONT_FAMILY) || (var == FONT_SHAPE) ||
          (var == FONT_SERIES) || (var == MATH_LANGUAGE) ||
          (var == MATH_FONT_FAMILY) || (var == MATH_FONT_SHAPE) ||
          (var == MATH_FONT_SERIES) || (var == PROG_LANGUAGE) ||
          (var == PROG_FONT_FAMILY) || (var == PROG_FONT_SHAPE) ||
          (var == PROG_FONT_SERIES) || (var == PROG_SESSION))
        s << t[2 * i + 1]->label;
      else if ((var == FONT) || (var == MATH_FONT) || (var == PROG_FONT))
        s << verbatim (t[2 * i + 1]->label);
      else s << (var * "=" * t[2 * i + 1]->label);
    }
  return s;
}
...
edit_interface_rep::compute_operation_footer (tree st) {
  ...
  if (N (st) >= 2) {
    switch (L (st)) {
    case VAR_WIDE:
      r= concat ("under ", get_accent_type (as_string (st[1])));
      break;
    default:;
    }
  }
  if (r == "" && N (st) >= 1) {
    ...
    case FLOAT:
      ...
      break;
    case MID:
      r= concat ("middle ", as_symbol (st[0]));
      break;
    case RIGHT:
      r= concat ("close ", as_symbol (st[0]));
      break;
    case BIG:
      r= concat ("big ", as_symbol (st[0]));
      break;
    case LPRIME:
      r= concat ("left prime ", as_string (st[0]));
      break;
    case LONG_ARROW:
      r= concat ("long arrow ", as_string (st[0]));
      break;
    case RPRIME:
    ...
    case WIDE:
      ...
      break;
    case ASSIGN:
      r= concat ("assign ", as_string (st[0]));
      break;
    case PROVIDE:
      r= concat ("provide ", as_string (st[0]));
      break;
    case WITH:
      r= concat ("with ", get_with_text (st));
      break;
    case PROVIDES:
      r= concat ("provides ", as_string (st[0]));
      break;
    case VALUE:
      r= concat ("value ", as_string (st[0]));
      break;
    case QUOTE_VALUE:
      r= concat ("quoted value ", as_string (st[0]));
      break;
    case OR_VALUE:
      r= concat ("or-value ", as_string (st[0]));
      break;
    case ARG:
      r= concat ("argument ", as_string (st[0]));
      break;
    case QUOTE_ARG:
      r= concat ("quoted argument ", as_string (st[0]));
      break;
    case COMPOUND:
    ...
    case VAR_INCLUDE:
    case INCLUDE:
      r= concat ("include ", verbatim (as_string (st[0])));
      break;
    case INACTIVE:
      r= concat ("inactive ", drd->get_name (L (st[0])));
      break;
    case VAR_INACTIVE:
      r= concat ("inactive ", drd->get_name (L (st[0])));
      break;
    case LABEL:
      r= concat ("label: ", verbatim (as_string (st[0])));
      break;
    case REFERENCE:
      r= concat ("reference: ", verbatim (as_string (st[0])));
      break;
    case PAGEREF:
      r= concat ("page reference: ", verbatim (as_string (st[0])));
      break;
    case GET_ATTACHMENT:
      r= concat ("get attachment: ", verbatim (as_string (st[0])));
      break;
    case WRITE:
      r= concat ("write to ", verbatim (as_string (st[0])));
      break;
    case TOC_NOTIFY:
      r= concat ("toc notify: ", verbatim (as_string (st[1])));
      break;
    case SPECIFIC:
      r= concat ("specific ", as_string (st[0]));
      break;
    case HYPHENATE_AS:
      r= concat ("hyphenate as ", verbatim (as_string (st[0])));
      break;
    case IMAGE:
      r= concat ("image");
      break;
...
edit_interface_rep::compute_compound_footer (tree t, path p) {
  ...
  case WIDE:
    ...
    else return concat (up, "wide ");
  case VAR_WIDE:
    if (N (st) >= 1)
      return concat (up, "under " * get_accent_type (as_string (st[1])) * " ");
    else return concat (up, "var-wide ");
  case TREE:
    if (l == 0) return concat (up, "root ");
  ...
  case DRD_PROPS:
    if (l == 0) return concat (up, "drd property(variable) ");
    if ((l & 1) == 1)
  ...
  case COMPOUND:
    ...
    else return concat (up, "compound ");
  case HLINK:
    if (N (st) >= 2) {
      if (l == 0) {
  ...
  case ATTR:
    ...
    else return concat (up, "value(" * as_string (l / 2 + 1) * ") ");
  case SPECIFIC:
  ...
  case ANIM_TRANSLATE:
    if (l == 0) return concat (up, "anim-translate ");
    else return up;
  case ANIM_PROGRESSIVE:
    if (l == 0) return concat (up, "anim-progressive ");
    else return up;
  default:
...
TM_DEBUG (int concrete_count= 0; int abstract_count= 0; int box_count= 0;
          int event_count= 0; int widget_count= 0; int line_item_count= 0;
          int list_count= 0; int command_count= 0; int observer_count= 0;
          int iterator_count= 0; int function_count= 0; int instance_count= 0;)
