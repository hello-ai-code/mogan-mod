...
find_bib_file (url base, string fname, string suffix= ".bib",
  ...
  url bibf (fname);
  if (exists (bibf) && (!rooted || is_rooted (bibf))) return bibf;
  if (exists (relative (base, bibf))) return relative (base, bibf);
...
tree
arrange_bib (tree t) {
  if (is_atomic (t)) return replace (t->label, "--", "\25");
  else if (is_func (t, DOCUMENT, 0)) return tree (DOCUMENT, "");
  else {
    int  i, n= N (t);
    tree r (t, n);
    for (i= 0; i < n; i++)
...
void
edit_process_rep::generate_bibliography (string bib, string style,
                                         string fname) {
#ifdef USE_PLUGIN_BIBTEX
  ...
  if (is_none (bib_file)) {
    ...
    if (is_none (bbl_file)) {
      if (supports_db ()) {
        t= as_tree (
            call (string ("bib-compile"), bib, style, bib_t, xbib_file));
...
index_name_sub (tree t, bool all) {
  ...
  else if (all && is_func (t, WITH)) return index_name_sub (t[N (t) - 1], all);
  else return "";
}
...
make_entry (tree& D, tree t, hashmap<string, tree> refs, bool rec) {
  ...
  for (i= 0; i < n; i++)
    if (is_func (t[i], TUPLE, 1)) {
      bool flag= true;
      for (j= 0; j < n; j++)
        if (is_func (t[j], TUPLE, 2) && (t[i][0] == t[j][0])) flag= false;
      if (flag) D << t[i][0];
    }
  ...
  for (i= 0; i < n; i++)
    if (is_func (t[i], TUPLE, 2)) {
      ...
      string prev= "", next= "";
      if (is_func (r, PAGEREF, 1) && is_atomic (r[0]) &&
...
edit_process_rep::generate_aux_recursively (string which, tree st, path p) {
  ...
  for (i= 0; i < n; i++)
    if (!is_aux (st[i])) {
      if (is_compound (st[i])) generate_aux_recursively (which, st[i], p * i);
    }
    else {
      tree t    = st[i];
