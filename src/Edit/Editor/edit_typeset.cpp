...
edit_typeset_rep::set_data (new_data data) {
  ...
  while (it->busy ()) {
    string key= it->next ();
    (void) call (string ("notify-set-attachment"), buf->buf->name, key,
                 data->att[key]);
  }
}
...
typesetter
edit_typeset_rep::get_typesetter () {
  return ttt;
}
tree
edit_typeset_rep::get_style () {
  return the_style;
}
void
...
hashmap<string, tree>
edit_typeset_rep::get_init () {
  return init;
}
hashmap<string, tree>
edit_typeset_rep::get_fin () {
  return fin;
}
hashmap<string, tree>
edit_typeset_rep::get_ref () {
  return buf->data->ref;
}
hashmap<string, tree>
edit_typeset_rep::get_aux () {
  return buf->data->aux;
}
hashmap<string, tree>
edit_typeset_rep::get_att () {
  return buf->data->att;
}
void
...
void
edit_typeset_rep::set_aux (hashmap<string, tree> H) {
  buf->data->aux= H;
}
void
...
edit_typeset_rep::init_update () {
  ...
  else if (buf->data->init["part-flag"] == "true") grefs= copy (buf->data->ref);
}
...
restricted_exec (edit_env env, tree t, int end) {
  if (is_func (t, ASSIGN, 2) && end == 2) env->exec (t);
...
filter_format (tree fm, int i, int n) {
  ...
  for (int k= 0; k < N (fm); k++)
    if (is_func (fm[k], CWITH) && N (fm[k]) >= 4 && is_int (fm[k][0]) &&
        is_int (fm[k][1])) {
  ...
  while (!is_nil (p)) {
    if (L (t) == TFORMAT && p->item == N (t) - 1) {
      ...
      for (int k= 0; k < N (t) - 1; k++)
        if (is_func (t[k], CWITH, 6) && is_atomic (t[k][4]) &&
    ...
    else if ((L (t) == TABLE || L (t) == ROW) && p->item >= 0 &&
             p->item < N (t)) {
...
edit_typeset_rep::typeset_exec_until (path p) {
  ...
  if (enable_fastenv) {
    ...
    if (env->read (PREAMBLE) == "true") env->write (MODE, "src");
  }
...
edit_typeset_rep::get_env_language () {
  ...
  if (mode == "text") return text_language (get_env_string (LANGUAGE));
  else if (mode == "math")
...
simplify_execed (tree t) {
  ...
  if (is_func (r, QUOTE, 1) && is_atomic (r[0])) return r[0];
  else return r;
}
...
edit_typeset_rep::exec (tree t, hashmap<string, tree> H, bool expand_refs) {
  ...
  env->write_env (H);
  t= env->exec (t);
  if (expand_refs) t= expand_references (t, buf->data->ref);
  t= simplify_execed (t);
...
edit_typeset_rep::exec_latex (tree t, path p) {
  t= convert_OTS1_symbols_to_universal_encoding (t);
  bool expand_unknown_macros=
      "on" ==
      as_string (call ("get-preference", "texmacs->latex:expand-macros"));
  bool expand_user_macro=
      "on" ==
      as_string (call ("get-preference", "texmacs->latex:expand-user-macros"));
  if (!expand_unknown_macros && !expand_user_macro) return t;
  if (p == (rp * 0)) typeset_preamble ();
  typeset_exec_until (p);
  hashmap<string, tree> H = copy (cur[p]);
...
report_redefined (array<tree> redefined) {
  for (int i= 0; i < N (redefined); i++) {
    tree t= redefined[i];
    if (t[1]->label == "") typeset_warning << "Redefined " << t[0]->label << LF;
    else
      typeset_warning << "Redefined " << t[0]->label << " as " << t[1]->label
                      << LF;
  }
}
...
edit_typeset_rep::typeset (SI& x1, SI& y1, SI& x2, SI& y2) {
  ...
  while (true) {
    ...
    if ((N (env->missing) == missing_nr &&
         N (env->redefined) == redefined_nr) ||
