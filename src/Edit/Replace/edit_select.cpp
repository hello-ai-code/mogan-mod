...
selection_encode (string lan, string s) {
  ...
  else if (lan == "spanish") return spanish_to_ispanish (s);
  else if (lan == "german") return german_to_igerman (s);
  else return s;
}
...
selection_decode (string lan, string s) {
  ...
  else if (lan == "spanish") return ispanish_to_spanish (s);
  else if (lan == "german") return igerman_to_german (s);
  else return s;
}
...
void
edit_select_rep::get_selection (path& p1, path& p2) {
  p1= start (cur_sel);
  p2= end (cur_sel);
}
void
edit_select_rep::set_selection (path p1, path p2) {
  select (p1, p2);
}
...
edit_select_rep::select_enlarge_text () {
  ...
  if (mode == "text" || mode == "src") {
    ...
    if (i2 < N (s)) f= min (f, breaking_force (s[i2]));
...
incomplete_script_selection (tree t, path lp, path rp) {
  ...
  if (l > 0 && (is_func (t[l - 1], LSUB) || is_func (t[l - 1], LSUP)))
    return true;
  if (r + 1 < N (t) && (is_func (t[r + 1], RSUB) || is_func (t[r + 1], RSUP)))
    return true;
  return false;
}
...
edit_select_rep::select_enlarge () {
  ...
  if (semantic_select (pp, p1, p2, 1)) select (p1, p2);
  else {
  ...
  path p = common (cur_sel);
  tree st= subtree (et, p);
  if (drd->var_without_border (L (st)) || is_func (st, TFORMAT) ||
      is_func (st, DOCUMENT, 1) || is_script (st) ||
...
stop_enlarge_environmental (tree t) {
  ...
  return (s == "part") || (s == "chapter") || (s == "section") ||
         (s == "subsection") || (s == "subsubsection") || (s == "paragraph") ||
         (s == "subparagraph");
}
...
path
edit_select_rep::selection_get_subtable (int& row1, int& col1, int& row2,
                                         int& col2) {
  path fp= find_subtable_selection (et, start (cur_sel), end (cur_sel), row1,
                                    col1, row2, col2);
...
edit_select_rep::selection_get_path () {
  path p1, p2;
  selection_get (p1, p2);
  if (p2 == p1 && !is_empty (cur_sel)) return path_up (p1);
  return common (p1, p2);
}
  ...
  if (is_tuple (t, "texmacs", 3)) {
    ...
    if (mode == "math" && t[2] == "text")
      ...
      if ((t[2] != mode) && (t[2] != "src") && (mode != "src") &&
          ((t[2] == "math") || (mode == "math"))) {
        if (t[2] == "math") insert_tree (compound ("math", ""), path (0, 0));
        else if (t[2] == "text")
...
edit_select_rep::cut (path p1, path p2) {
  ...
  tree st= subtree (et, p);
  raw_cut (p1, p2);
  if (!is_func (st, TFORMAT) && !is_func (st, TABLE) && !is_func (st, ROW) &&
      !is_document (subtree (et, p)) && is_concat (subtree (et, path_up (p))))
...
edit_select_rep::focus_search (path p, bool skip_flag, bool up_flag) {
  ...
  if (none_accessible (st) && p == path_up (tp) && last_item (tp) != 0)
    return p;
  if (is_atomic (st) || is_func (st, DOCUMENT) || is_func (st, CONCAT) ||
      is_func (st, TFORMAT) || is_func (st, TABLE) || is_func (st, ROW) ||
      is_func (st, CELL) || is_compound (st, "shown") || is_func (st, HIDDEN) ||
      is_compound (st, "shared") || is_compound (st, "slide") ||
      is_compound (st, "with-screen-color") || is_compound (st, "mc-field") ||
      is_compound (st, "live-io*") || up_flag)
...
edit_select_rep::focus_get (bool skip_flag) {
  ...
  if (!is_nil (focus_p)) return focus_search (focus_p, skip_flag, false);
  if (selection_active_any ())
